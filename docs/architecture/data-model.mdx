...
The Switchyard database contains **80 tables** organized into focused domains:
...
| Store API - Cart | 7 | Shopping carts, line items, addresses |
| Store API - Customer | 4 | Customers, groups, addresses |
| Store API - Payments | 5 | Payment collections, sessions, captures |
| Store API - Promotions | 9 | Promotions, rules, targeting |
| Store API - Locations | 3 | Physical locations, tax rates |
| Equipment Monitoring | 5 | Sensors, alerts, temperature readings |
| Other | 9 | Portals, migrations, archives |

<Note>
The schema was consolidated from 170+ tables to 80 focused tables by removing redundant Medusa framework tables that conflicted with our custom architecture.
</Note>
...
| stock_location_id | uuid | Reference to stock_location |
...
Historical price data from retailers - used for cost analysis and sourcing decisions.

| Field | Type | Description |
|-------|------|-------------|
| product_id | uuid | Reference to scraped_products |
| store_name | string | Retailer name |
| price | decimal | Price at time of scrape |
| unit_price | decimal | Per-unit price |
| created_at | datetime | When recorded |

### retailer_selling_prices
...
| product_id | uuid | Reference to scraped_products |
| store_name | string | Retailer name |
| selling_price | decimal | Retailer's customer price |
| updated_at | datetime | Last updated |
