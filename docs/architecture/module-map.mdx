---
title: Module Map
description: Medusa modules used in Switchyard and their purposes
---

# Module Map

Switchyard is built on Medusa, a modular commerce platform. This document maps which Medusa modules we use, which we've customized, and which are not needed for our use case.
...
Extends Medusa's stock locations with a hierarchical structure for warehouse organization.
...
These Medusa modules are not actively used in Switchyard operations, but are kept because they are tightly integrated with Medusa's core functionality. Removing them would risk breaking internal workflows.
...
Modules are configured in `apps/goods-backend/medusa-config.ts`. To disable a module, remove it from the modules array or set `enabled: false`.
