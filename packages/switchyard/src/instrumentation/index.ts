...
import { SpanStatusCode } from "@switchyard/framework/opentelemetry/api"
import type { NodeSDKConfiguration } from "@switchyard/framework/opentelemetry/sdk-node"
import type { SpanExporter } from "@switchyard/framework/opentelemetry/sdk-trace-node"
import { TransactionOrchestrator } from "@switchyard/framework/orchestration"
import { Tracer } from "@switchyard/framework/telemetry"
import { ICachingModuleService } from "@switchyard/framework/types"
import { camelToSnakeCase, FeatureFlag } from "@switchyard/framework/utils"
...
export function instrumentHttpLayer() {
  ...
  const HTTPTracer = new Tracer("@switchyard/http", "2.0.0")
...
export function instrumentRemoteQuery() {
  const QueryTracer = new Tracer("@switchyard/query", "2.0.0")
...
export function instrumentWorkflows() {
  const WorkflowsTracer = new Tracer(
    "@switchyard/framework/workflows-sdk",
    "2.0.0"
  )
...
export function instrumentCache() {
  ...
  const CacheTracer = new Tracer("@switchyard/caching", "2.0.0")
...
export function registerOtel(
  ...
  } = require("@switchyard/framework/opentelemetry/resources")
  const { NodeSDK } = require("@switchyard/framework/opentelemetry/sdk-node")
  const {
    SimpleSpanProcessor,
  } = require("@switchyard/framework/opentelemetry/sdk-trace-node")

  if (instrument.db) {
    const {
      PgInstrumentation,
    } = require("@switchyard/framework/opentelemetry/instrumentation-pg")
