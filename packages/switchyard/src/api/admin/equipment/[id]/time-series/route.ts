...
export const GET = async (
  ...
  const startTime = req.query.startTime ? parseInt(req.query.startTime as string, 10) : Date.now() - 24 * 60 * 60 * 1000
  const endTime = req.query.endTime ? parseInt(req.query.endTime as string, 10) : Date.now()
