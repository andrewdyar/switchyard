...
export const useEquipmentList = (filters?: { type?: string; is_active?: boolean }) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { equipment } = await sdk.client.fetch(`/admin/equipment?${params.toString()}`)
...
export const useEquipment = (id: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { equipment } = await sdk.client.fetch(`/admin/equipment/${id}`)
      return equipment as Equipment
    },
    enabled: !!id,
  })
...
export const useEquipmentAlerts = (filters?: {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { alerts } = await sdk.client.fetch(`/admin/equipment/alerts?${params.toString()}`)
      return alerts as EquipmentAlert[]
    },
...
export const useResolveAlert = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, reason }: { id: string; reason?: string }) => {
      const { alert } = await sdk.client.fetch(`/admin/equipment/alerts/${id}/resolve`, {
        method: "PATCH",
        body: JSON.stringify({ reason }),
      })
      return alert as EquipmentAlert
    },
    onSuccess: () => {
...
export const useAcknowledgeAlert = () => {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const { alert } = await sdk.client.fetch(`/admin/equipment/alerts/${id}/acknowledge`, {
        method: "PATCH",
      })
      return alert as EquipmentAlert
    },
...
export const useEquipmentTimeSeries = (
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { readings } = await sdk.client.fetch(
        `/admin/equipment/${equipmentId}/time-series?${params.toString()}`
      )
      return readings as TemperatureReading[]
    },
...
export const useEquipmentCurrentReadings = (equipmentId: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const allReadings = readings as TemperatureReading[]
