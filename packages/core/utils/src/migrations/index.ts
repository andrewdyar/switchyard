import { MikroORM, MikroORMOptions } from "@switchyard/deps/mikro-orm/core"
import {
  MigrateOptions,
  MigrationResult,
  UmzugMigration,
} from "@switchyard/deps/mikro-orm/migrations"
import {
  defineConfig,
  PostgreSqlDriver,
...
const replacements = [
  // MikroORM imports - replace mikro-orm/{subpath} with @switchyard/framework/mikro-orm/{subpath}
  {
    pattern: /from\s+['"]@?mikro-orm\/([^'"]+)['"]/g,
    // eslint-disable-next-line quotes
    replacement: 'from "@switchyard/framework/mikro-orm/$1"',
  },
  // PG imports - replace pg with @switchyard/framework/pg
  {
    pattern: /from\s+['"]pg['"]/g,
    // eslint-disable-next-line quotes
    replacement: 'from "@switchyard/framework/pg"',
  },
]
