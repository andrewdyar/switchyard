# switchyard-oas-cli - 0.1.0 - experimental

A command-line tool for all OpenAPI Specifications (OAS) related tooling.

## Install

`yarn add --dev @switchyard/switchyard-oas-cli`

Install in the global namespace is not yet supported.
~~`npm install -g @switchyard/switchyard-oas-cli`~~
...
This command will scan the `@switchyard/medusa` package in order to extract JSDoc OAS into a json file.
