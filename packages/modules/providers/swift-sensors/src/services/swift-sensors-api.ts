/**
 * Swift Sensors API Client
 * 
 * Provides methods for authenticating and fetching data from the Swift Sensors API.
 */
...
export class SwiftSensorsAPIClient {
  ...
  protected logger_?: { info: (...args: any[]) => void; error: (...args: any[]) => void; warn: (...args: any[]) => void }
  ...
  constructor(
    ...
    if (!options.apiKey) {
      throw new Error("Swift Sensors API key is required")
    }
    if (!options.email || !options.password) {
      throw new Error("Swift Sensors email and password are required")
    }
   ...
   */
  async getSensorUpdates(accountId: string): Promise<SwiftSensorsSensorUpdates> {
    const headers = await this.getAuthHeaders()
