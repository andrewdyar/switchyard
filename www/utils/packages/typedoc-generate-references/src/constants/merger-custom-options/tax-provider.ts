...
A Tax Module Provider is used to retrieve the tax lines in a provided context. The Tax Module provides a default \`system\` provider. You can create your own Tax Module Provider, either in a plugin, in a module provider, or directly in your Switchyard application's codebase, then use it in any tax region.`,
...
If you're creating the module provider in a Switchyard application, create it under the \`src/modules\` directory. For example, \`src/modules/my-tax\`.
...
Create the file \`src/modules/my-tax/service.ts\` that holds the module provider's main service. It must implement the \`ITaxProvider\` interface imported from \`@switchyard/framework/types\`:

\`\`\`ts title="src/modules/my-tax/service.ts"
import { ITaxProvider } from "@switchyard/framework/types"
...
To use your Tax Module Provider, add it to the \`providers\` array of the Tax Module in \`switchyard.config.ts\`:
