...
<Prerequisites items={[
  ...
  {
    text: "Deployed Switchyard application with at least one region.",
...
```bash
MEDUSA_BACKEND_URL= # URL of Switchyard application
...
1. `MEDUSA_BACKEND_URL` is the URL of your deployed Switchyard application
...
3. `NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY` is the publishable API key of the storefront. You can create one or find the default one in the Switchyard Admin dashboard.
...
Make sure to add other relevant environment variables. For example, if you’re using the Stripe Payment Module Provider in the Switchyard application, add the `NEXT_PUBLIC_STRIPE_KEY` environment variable.
...
### Set STORE_CORS and AUTH_CORS in Deployed Switchyard Application

You must also set the `STORE_CORS` environment variable in your Switchyard application to the storefront’s URL:
...
Once the environment variables changes are deployed in the Switchyard application, do the following to redeploy the storefront:
