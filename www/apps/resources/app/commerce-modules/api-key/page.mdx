...
Refer to the [Switchyard Admin User Guide](!user-guide!/settings/developer) to learn how to manage publishable and secret API keys using the dashboard.

</Note>

Switchyard has API-key related features available out-of-the-box through the API Key Module. A [module](!docs!/learn/fundamentals/modules) is a standalone package that provides features for a single domain. Each of Switchyard's commerce features are placed in Commerce Modules, such as this API Key Module.
