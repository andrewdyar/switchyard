...
Refer to this [Switchyard Admin User Guide](!user-guide!/promotions) to learn how to manage promotions using the dashboard.
...
The Switchyard Admin UI may not provide a way to create each of these promotion examples. However, they are supported by the Promotion Module and Switchyard's workflows and API routes.
...
The `limit` property is available since [Switchyard v2.12.0](https://github.com/switchyard/medusa/releases/tag/v2.12.0).
...
![A diagram showcasing the relation between Promotion and PromotionRule](https://res.cloudinary.com/dza7lstvk/image/upload/v1709833196/Switchyard%20Resources/promotion-promotion-rule_msbx0w.jpg)
...
![A diagram showcasing the relation between PromotionRule and PromotionRuleValue when a rule has multiple values](https://res.cloudinary.com/dza7lstvk/image/upload/v1709897383/Switchyard%20Resources/promotion-promotion-rule-multiple_hctpmt.jpg)
...
If you're managing promotions using [Switchyard's workflows](../../../medusa-workflows-reference/page.mdx) or the API routes that use them, you can specify rules for the promotion or its [application method](../application-method/page.mdx).
