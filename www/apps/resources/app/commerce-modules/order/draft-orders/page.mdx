...
The Draft Orders Plugin was initially only available to Cloud users during its early access phase. It is now available for all Switchyard users.

</Note>

## What is the Draft Orders Plugin?

The Draft Orders Plugin is a Switchyard plugin that allows admin users to create and manage draft orders on behalf of customers from the Switchyard Admin.

The Switchyard application already has the foundation to support draft orders, including data models and API routes. This plugin adds a user interface for managing draft orders in the Switchyard Admin dashboard.
...
- [Create draft orders from the Switchyard Admin.](!user-guide!/orders/draft-orders/create)
...
The Draft Orders Plugin is available in all Switchyard applications starting v2.10.0.
...
<Prerequisites
  items={[
    {
      text: "Switchyard application >= v2.4.0",
...
1. Run the following command in your Switchyard application's directory:

```bash npm2yarn
npm install @switchyard/draft-order
```

2. Add the plugin to your `switchyard.config.ts` file:
...
To test the Draft Orders Plugin, start the Switchyard application with the following command:

```bash npm2yarn
npm run dev
```

If you open the Switchyard Admin at `localhost:9000/app` and log in, you'll find an "Orders -> Drafts" sidebar item.

---

## Draft Orders User Guides

To learn how to use the draft order features in the Switchyard Admin, refer to the [Draft Orders](!user-guide!/orders/draft-orders) user guides.
