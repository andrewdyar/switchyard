...
![A diagram showcasing the relation between the price set and price](https://res.cloudinary.com/dza7lstvk/image/upload/v1709648650/Switchyard%20Resources/price-set-money-amount_xeees0.jpg)
...
For example, Switchyard links a product variant from the [Product Module](../../product/page.mdx) to a price set that contains prices in multiple currencies. A variant's price set would be similar to the following:
...
When the customer views and purchases a product, Switchyard selects the price based on the customer's currency.
...
For example, Switchyard allows you to specify prices for a product variant that are applied only when the customer is in a specific region. The price would be similar to the following:
...
When the customer views and purchases a product, Switchyard selects the price based on the customer's region.