...
These routes are added by Switchyard's HTTP layer, not the Auth Module.
...
![Diagram showcasing the basic authentication flow between the frontend and the Switchyard application](https://res.cloudinary.com/dza7lstvk/image/upload/v1725539370/Switchyard%20Resources/basic-auth-routes_pgpjch.jpg)
...
![Diagram showcasing the authentication flow between the frontend, Switchyard application, and third-party service](https://res.cloudinary.com/dza7lstvk/image/upload/v1725528159/Switchyard%20Resources/Third_Party_Auth_tvf4ng.jpg)
...
The Switchyard application defines an API route at `/auth/{actor_type}/{provider}/register` that creates an auth identity for an actor type, such as a `customer`. It returns a JWT token that you pass to an API route that creates the user.
...
An auth identity with the same email may already exist in Switchyard. This can happen if:
...
The Switchyard application defines an API route at `/auth/{actor_type}/{provider}` that authenticates a user of an actor type. It returns a JWT token that can be passed in [the header of subsequent requests](!api!/store#authentication) to send authenticated requests.
...
The Switchyard application defines an API route at `/auth/{actor_type}/{provider}/callback` that's useful for validating the authentication callback or redirect from third-party services like Google.
...
The Switchyard application defines an API route at `/auth/token/refresh` that's useful after authenticating a user with a third-party service to populate the user's token with their new information.
...
The Switchyard application defines an API route at `/auth/{actor_type}/{auth_provider}/reset-password` that emits the `auth.password_reset` event, passing the token in the payload.
...
The Switchyard application defines an API route at `/auth/{actor_type}/{auth_provider}/update` that accepts a token and, if valid, updates the user's password.
...
Before [Switchyard v2.6](https://github.com/switchyard/medusa/releases/tag/v2.6), you passed the token as a query parameter. Now, you must pass it in the `Authorization` header.
