...
<Prerequisites
  items={[
    {
      text: "Register a GitHub App. When setting the Callback URL, set it to a URL in your frontend that later uses Switchyard's callback route to validate the authentication.",
...
To use the GitHub Auth Module Provider, add the module to the array of providers passed to the Auth Module in your `switchyard.config.ts`:
  ...
  modules: [
    {
      ...
      options: {
        providers: [
          // other providers...
          {
            resolve: "@switchyard/medusa/auth-github",
            id: "github",
            options: {
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      At this URL, the frontend will receive a `code` query parameter. It then sends that  `code` query parameter to the Switchyard application's `/auth/{actor_type}/github/callback` route.
