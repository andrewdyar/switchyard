...
2. Set the `app_metadata` property of the associated auth identity using the `setAuthAppMetadataStep` from Switchyard's core workflows. You specify the actor type `manager` in the `actorType` property of the step’s input.
...
export const createRouteHighlights = [
  ["15", "AuthenticatedSwitchyardRequest", "The request must be authenticated to associate the manager with an auth identity."],
