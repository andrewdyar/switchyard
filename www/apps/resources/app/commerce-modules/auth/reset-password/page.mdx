...
Refer to this [Switchyard Admin User Guide](!user-guide!/reset-password) to learn how to reset your user admin password using the dashboard.

</Note>

## Reset Password Flow Overview

![Diagram showcasing the reset password flow detailed below](https://res.cloudinary.com/dza7lstvk/image/upload/v1754050032/Switchyard%20Resources/reset-password_qheixj.jpg)
...
1. The user requests to reset their password either through the frontend (for example, [Switchyard Admin](!user-guide!/reset-password)) or the [Generate Reset Password Token API route](../authentication-route/page.mdx#generate-reset-password-token-route).
2. The Switchyard application generates a password reset token and emits the `auth.password_reset` event.
...
3. The user receives the notification and clicks on the link to reset their password.
    - The user can reset their password either through the frontend (for example, [Switchyard Admin](!user-guide!/reset-password)) or the [Reset Password API route](../authentication-route/page.mdx#reset-password-route).
...
After adding the subscriber, you will have a complete reset password flow you can utilize using the Switchyard Admin, storefront, or API routes.
...
Switchyard provides providers like [SendGrid](../../../infrastructure-modules/notification/sendgrid/page.mdx) and [Resend](../../../integrations/guides/resend/page.mdx), and you can also [create your own custom provider](/references/notification-provider-module).
...
For testing purposes, you can use the [Local Notification Module Provider](../../../infrastructure-modules/notification/local/page.mdx) by adding this to your `switchyard.config.ts`:
...
This event's payload previously had an `actorType` field. It was renamed to `actor_type` after [Switchyard v2.0.7](https://github.com/switchyard/medusa/releases/tag/v2.0.7).
...
Note that the Switchyard Admin has a reset password form at `/reset-password?token={token}&email={email}`.
...
Then, open the Switchyard Admin (locally at `http://localhost:9000/app`) and click on "Reset" in the login form. Then, enter the email of the user you want to reset the password for.
...
The Switchyard Admin already has a reset password page at `/reset-password?token={token}&email={email}`. So, you only need to implement this page in your storefront or custom admin dashboard.
