...
- Set the `custom_name` property when a cart is created or updated using Switchyard's API routes.
...
<Prerequisites
  items={[
    {
      text: "Module must be registered in switchyard.config.ts",
...
Run the following command in your Switchyard project's root:

```bash
npx switchyard db:generate helloModuleService
```
...
To pass the `custom_name` in the `additional_data` parameter, you must add a validation rule that tells the Switchyard application about this custom property.
...
Make sure to replace `{publishable_api_key}` with your publishable API key, which you can retrieve from the Switchyard Admin. Also, replace the value of `region_id` with an ID of a region in your application.
...
Make sure to replace `{cart_id}` with the cart's ID, and `{publishable_api_key}` with your publishable API key, which you can retrieve from the Switchyard Admin.
...
import { createRemoteLinkStep, dismissRemoteLinkStep, useQueryGraphStep } from "@switchyard/medusa/core-flows"
