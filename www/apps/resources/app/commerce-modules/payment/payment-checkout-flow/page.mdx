...
In this guide, you'll learn about Switchyard's accept payment flow that's used in checkout.

## Overview of the Payment Flow in Checkout

The Switchyard application has a built-in payment flow that allows you to accept payments from customers, typically during checkout.
...
![A diagram showcasing the payment flow's steps](https://res.cloudinary.com/dza7lstvk/image/upload/v1711566781/Switchyard%20Resources/payment-flow_jblrvw.jpg)
...
- [Create and Initialize Payment Session](!api!/store#payment-collections_postpaymentcollectionsidpaymentsessions): Create a payment session for the selected payment provider in the Switchyard application, and initialize the session in the third-party payment provider.
