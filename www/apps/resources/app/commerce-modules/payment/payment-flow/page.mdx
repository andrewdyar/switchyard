...
Switchyard already provides a built-in payment flow that allows you to accept payments from customers, which you can learn about in the [Accept Payment Flow in Checkout](../payment-checkout-flow/page.mdx) guide.

You may need to implement a custom payment flow if you have a different use case, or you're using the Payment Module separately from the Switchyard application.

This guide will help you understand how to implement a payment flow using the Payment Module's main service or workflows.

You can also follow this guide to get a general understanding of how the payment flow works in the Switchyard application.
...
It's highly recommended to use Switchyard's workflows to implement this flow. Use the Payment Module's main service for more complex cases.
...
In the Switchyard application, you associate the payment collection with a cart, which is the resource that the customer is trying to pay for.
...
```ts
import { createPaymentCollectionForCartWorkflow } from "@switchyard/medusa/core-flows"
...
In the Switchyard application, you need to use [Query](!docs!/learn/fundamentals/module-links/query) to retrieve the available payment providers in a region.
...
```ts
import { createPaymentSessionsWorkflow } from "@switchyard/medusa/core-flows"
...
```ts
import { authorizePaymentSessionStep } from "@switchyard/medusa/core-flows"
...
- Complete the cart using the [completeCartWorkflow](/references/medusa-workflows/completeCartWorkflow) if you're using the Switchyard application.
