...
![A diagram showcasing how a payment's multiple captures are stored](https://res.cloudinary.com/dza7lstvk/image/upload/v1711565445/Switchyard%20Resources/payment-capture_f5fve1.jpg)
...
![A diagram showcasing how a payment's multiple refunds are stored](https://res.cloudinary.com/dza7lstvk/image/upload/v1711565555/Switchyard%20Resources/payment-refund_lgfvyy.jpg)
...
The `Payment` data model has a `data` property used to store that data. The first time it's set is when the [payment provider in Switchyard](../payment-provider/page.mdx) authorizes the payment.

Then, the `data` property is passed to the Switchyard payment provider when the payment is captured or refunded, allowing the payment provider to utilize the data to process the payment with the third-party provider.
