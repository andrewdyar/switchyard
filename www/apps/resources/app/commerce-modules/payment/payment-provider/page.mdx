...
Refer to this [Switchyard Admin User Guide](!user-guide!/settings/regions) to learn how to manage the payment providers available in a region using the dashboard.
...
The Payment Module Provider handles payment processing in the Switchyard application. It integrates third-party payment services, such as [Stripe](./stripe/page.mdx).
...
![Diagram showcasing the communication between Switchyard, the Payment Module Provider, and the third-party payment provider.](https://res.cloudinary.com/dza7lstvk/image/upload/v1746791374/Switchyard%20Resources/payment-provider-service_l4zi6m.jpg)
...
A payment provider is a module whose main service extends the `AbstractPaymentProvider` imported from `@switchyard/framework/utils`.
...
After you create a payment provider, you can enable it as a payment provider in a region using the [Switchyard Admin dashboard](!user-guide!/settings/regions).
...
When the Switchyard application starts, it registers the Payment Module Providers defined in the `providers` option of the Payment Module.

For each Payment Module Provider, the Switchyard application finds all payment provider services defined in them to register.

### PaymentProvider Data Model

A registered payment provider is represented by the [PaymentProvider data model](/references/payment/models/PaymentProvider) in the Switchyard application.

![Diagram showcasing the PaymentProvider data model](https://res.cloudinary.com/dza7lstvk/image/upload/v1746791364/Switchyard%20Resources/payment-provider-model_lx91oa.jpg)
...
- `id`: The unique identifier of the Payment Module Provider. The ID's format is `pp_{identifier}_{id}`, where:
    ...
    - `id` is the value of the `id` property of the Payment Module Provider in `switchyard.config.ts`.
- `is_enabled`: A boolean indicating whether the payment provider is enabled.

### How to Remove a Payment Provider?

If you remove a payment provider from the `providers` option, the Switchyard application will not remove the associated `PaymentProvider` data model record.

Instead, the Switchyard application will set the `is_enabled` property of the `PaymentProvider`'s record to `false`. This allows you to re-enable the payment provider later if needed by adding it back to the `providers` option.
