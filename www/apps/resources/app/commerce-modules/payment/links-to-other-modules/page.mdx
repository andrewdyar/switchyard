...
Switchyard defines a link between the `Customer` and `AccountHolder` data models, allowing payment providers to save payment methods for a customer, if the payment provider supports it.

<Note>

This link is available starting from Switchyard `v2.5.0`.
...
You can specify for each region which payment providers are available. The Switchyard application defines a link between the `PaymentProvider` and the `Region` data models.

![A diagram showcasing an example of how resources from the Payment and Region modules are linked](https://res.cloudinary.com/dza7lstvk/image/upload/v1711569520/Switchyard%20Resources/payment-region_jyo2dz.jpg)
