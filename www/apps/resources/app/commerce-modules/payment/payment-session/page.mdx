...
![Diagram showcasing how every payment session has a different payment provider](https://res.cloudinary.com/dza7lstvk/image/upload/v1711565056/Switchyard%20Resources/payment-session-provider_guxzqt.jpg)
...
The `PaymentSession` data model has a `data` property used to store that data. It's set by the [payment provider in Switchyard](../payment-provider/page.mdx) when the payment is initialized.

Then, when the payment session is authorized, the `data` property is used by the payment provider in Switchyard to process the payment with the third-party provider.
