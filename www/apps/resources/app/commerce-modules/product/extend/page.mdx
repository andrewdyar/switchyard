...
- Set the `custom_name` property when a product is created or updated using Switchyard's API routes.
...
<Prerequisites
  items={[
    {
      text: "Module must be registered in switchyard.config.js",
...
Also, make sure to replace `{shipping_profile_id}` with an existing shipping profile's ID. You can retrieve one either from the Switchyard Admin dashboard or using the [List Shipping Profiles API Route](!api!/admin#shipping-profiles_getshippingprofiles).
...
import { updateProductsWorkflow } from "@switchyard/medusa/core-flows"
import { 
