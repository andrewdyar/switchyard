...
In this guide, you'll learn how to retrieve the available inventory quantity of a product variant in your Switchyard application customizations. That includes API routes, workflows, subscribers, scheduled jobs, and any resource that can access the [Switchyard container](!docs!/learn/fundamentals/medusa-container).
...
Product variants have a `manage_inventory` boolean field that indicates whether the Switchyard application manages the inventory of the product variant. 

When `manage_inventory` is disabled, the Switchyard application always considers the product variant to be in stock. So, you can't retrieve the inventory quantity for those products.
...
To retrieve the inventory quantity of a product variant, use the `getVariantAvailability` utility function imported from `@switchyard/framework/utils`. It returns the available quantity of the product variant.
...
import { getVariantAvailability } from "@switchyard/framework/utils"
...
import { SwitchyardStoreRequest, SwitchyardResponse } from "@switchyard/framework/http"
import { getVariantAvailability } from "@switchyard/framework/utils"
...
Notice that the request object's type is `SwitchyardStoreRequest` instead of `SwitchyardRequest` to ensure the availability of the `publishable_key_context` property.
