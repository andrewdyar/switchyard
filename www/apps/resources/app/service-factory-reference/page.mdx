...
This section of the documentation provides a reference to the methods generated for services extending the service factory (`SwitchyardService`) and how to use them.
...
When your module's main service extends the service factory, Switchyard will:

1. Generate [data-management methods in your main service](#main-service-methods) for each of the data models passed to the `SwitchyardService` function.
2. Generate [internal services](#internal-generated-service-methods) for each of the data models passed to the `SwitchyardService` function, which can be resolved in loaders.
...
- `{DataModelName}` is the pascal-case version of the data model's key that's passed in the object parameter of `SwitchyardService`. The name is pluralized for all operations except the `retrieve` operation.
