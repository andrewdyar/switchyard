...
export const ogImage = "https://res.cloudinary.com/dza7lstvk/image/upload/v1738682676/Switchyard%20Resources/guide-custom-item-pricing_lrilmb.jpg"
...
In this guide, you'll learn how to add line items with custom prices to a cart in Switchyard.
...
![Diagram showcasing overview of implementation for adding an item to cart from storefront.](https://res.cloudinary.com/dza7lstvk/image/upload/v1738920014/Switchyard%20Resources/custom-line-item-3_zu3qh2.jpg)

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/custom-item-price",
...
A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/metal-prices`.

![Diagram showcasing the module directory to create](https://res.cloudinary.com/dza7lstvk/image/upload/v1738247192/Switchyard%20Resources/custom-item-price-1_q16evr.jpg)
...
![Diagram showcasing the service file to create](https://res.cloudinary.com/dza7lstvk/image/upload/v1738247303/Switchyard%20Resources/custom-item-price-2_eaefis.jpg)
...
A module has a container of Switchyard Framework tools and local resources in the module that you can access in the service constructor's first parameter. Learn more in [this documentation](!docs!/learn/fundamentals/modules/container).
...
If the response contains an error, you throw a `SwitchyardError` with the error message. Otherwise, you return the response, which is of type `PriceResponse`.
...
![The directory structure of the Metal Prices Module after adding the definition file.](https://res.cloudinary.com/dza7lstvk/image/upload/v1738248049/Switchyard%20Resources/custom-item-price-3_imtbuw.jpg)
...
![The access token is below the "API Token" header of your GoldAPI.io dashboard.](https://res.cloudinary.com/dza7lstvk/image/upload/v1738248335/Switchyard%20Resources/Screenshot_2025-01-30_at_4.44.07_PM_xht3j4.png)
...
You'll only implement the `getVariantMetalPricesStep`. Switchyard provides the other steps out-of-the-box.
...
![The directory structure after adding the step file.](https://res.cloudinary.com/dza7lstvk/image/upload/v1738249036/Switchyard%20Resources/custom-item-price-4_kumzdc.jpg)
...
2. An async function that receives two parameters:
    - An input object with the variant, currency code, and quantity. The variant has a `calculated_price` property that holds the variant's fixed price in the Switchyard application. This is useful when you want to add a fixed price to the real-time custom price, such as handling fees.
    - The [Switchyard container](!docs!/learn/fundamentals/medusa-container), which is a registry of Framework and commerce tools that you can access in the step.

In the step function, so far you only resolve the Metal Prices Module's service from the Switchyard container.
...
![The directory structure after adding the workflow file.](https://res.cloudinary.com/dza7lstvk/image/upload/v1738251380/Switchyard%20Resources/custom-item-price-5_zorahv.jpg)
...
After that, you use Switchyard's `acquireLockStep` to acquire a lock on the cart, and `addToCartWorkflow` to add the item with the custom price to the cart.
...
```ts title="src/workflows/add-custom-to-cart.ts"
import { WorkflowResponse } from "@switchyard/framework/workflows-sdk"
import { 
  releaseLockStep,
} from "@switchyard/medusa/core-flows"
```
...
![The directory structure after adding the API route file.](https://res.cloudinary.com/dza7lstvk/image/upload/v1738252712/Switchyard%20Resources/custom-item-price-6_deecbu.jpg)
...
export const POST = async (
  req: SwitchyardRequest<HttpTypes.StoreAddCartLineItem>, 
...
In the function, you retrieve the cart's ID from the path parameter, and the item's details from the request body. This API route will accept the same request body parameters as Switchyard's [Add Item to Cart API Route](!api!/store#carts_postcartsidlineitems).

Then, you execute the `addCustomToCartWorkflow` by invoking it, passing it the Switchyard container, which is available in the request's `scope` property, then executing its `run` method. You pass the workflow's input object with the cart's ID and the item to add to the cart.
...
A middleware is a function executed before the API route when a request is sent to it. You define middlewares in Switchyard in the `src/api/middlewares.ts` directory.
...
![The directory structure after adding the middleware file.](https://res.cloudinary.com/dza7lstvk/image/upload/v1738253099/Switchyard%20Resources/custom-item-price-7_l7iw2a.jpg)
...
} from "@switchyard/medusa/api/store/carts/validators"
...
- `middlewares`: An array of the middlewares to apply. You apply the `validateAndTransformBody` middleware, which validates the request body based on the `StoreAddCartLineItem` schema. This validation schema is the same schema used for Switchyard's [Add Item to Cart API Route](!api!/store#carts_postcartsidlineitems).
...
Then, open the Switchyard Admin dashboard at `localhost:9000/app` and log in with the email and password you created when you installed the Switchyard application in the first step.

Once you log in, click on Products in the sidebar, then click the Create button at the top right.

![Click on Products in the sidebar at the left, then click on the Create button at the top right of the content](https://res.cloudinary.com/dza7lstvk/image/upload/v1738253415/Switchyard%20Resources/Screenshot_2025-01-30_at_6.09.36_PM_ee0jr2.png)
...
![Fill in the product details, enable the "Yes, this is a product with variants" toggle, and add the "Metal" option with "Gold" value](https://res.cloudinary.com/dza7lstvk/image/upload/v1738253520/Switchyard%20Resources/Screenshot_2025-01-30_at_6.11.29_PM_lqxth9.png)
...
![Find the Gold variant in the Variants section, click on the three-dots icon, and choose "Edit"](https://res.cloudinary.com/dza7lstvk/image/upload/v1738254038/Switchyard%20Resources/Screenshot_2025-01-30_at_6.19.52_PM_j3hjcx.png)

In the side window that opens, find the Weight field, enter the weight, and click the Save button.

![Enter the weight in the Weight field, then click the Save button](https://res.cloudinary.com/dza7lstvk/image/upload/v1738254165/Switchyard%20Resources/Screenshot_2025-01-30_at_6.22.15_PM_yplzdp.png)
...
![Click on the three-dots icon at the top right of the Variants section, then choose "Edit Prices"](https://res.cloudinary.com/dza7lstvk/image/upload/v1738255203/Switchyard%20Resources/Screenshot_2025-01-30_at_6.39.35_PM_s3jpxh.png)
...
![Enter fixed prices for the variant in the table, then click the Save button](https://res.cloudinary.com/dza7lstvk/image/upload/v1738255272/Switchyard%20Resources/Screenshot_2025-01-30_at_6.40.45_PM_zw1l59.png)
...
![Click on the icon at the right of the JSON section to copy the variant's ID](https://res.cloudinary.com/dza7lstvk/image/upload/v1738254314/Switchyard%20Resources/Screenshot_2025-01-30_at_6.24.49_PM_ka7xew.png)
...
To retrieve the publishable API key, on the Switchyard Admin:
...
![Click on publishable API keys in the Settings sidebar, then click on a publishable API key in the list](https://res.cloudinary.com/dza7lstvk/image/upload/v1738254523/Switchyard%20Resources/Screenshot_2025-01-30_at_6.28.17_PM_mldscc.png)

3. Click on the publishable API key to copy it.

![Click on the publishable API key to copy it](https://res.cloudinary.com/dza7lstvk/image/upload/v1738254601/Switchyard%20Resources/Screenshot_2025-01-30_at_6.29.26_PM_vvatki.png)
...
You've now implemented custom item pricing in Switchyard. You can also customize the [storefront](../../../nextjs-starter/page.mdx) to use the new API route to add custom-priced items to the cart.
