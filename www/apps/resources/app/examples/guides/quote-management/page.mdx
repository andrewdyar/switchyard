...
import { Github, PlaySolid } from "@switchyard/icons"
import { Prerequisites, WorkflowDiagram } from "docs-ui"

export const ogImage = "https://res.cloudinary.com/dza7lstvk/image/upload/v1741682814/Switchyard%20Resources/quote_ldmqaq.jpg"

export const metadata = {
  title: `Implement Quote Management in Switchyard`,
  openGraph: {
    images: [
      {
        url: ogImage,
...
In this guide, you'll learn how to implement quote management in Switchyard.

When you install a Switchyard application, you get a fully-fledged commerce platform with a Framework for customization. The Switchyard application's commerce features are built around [Commerce Modules](../../../commerce-modules/page.mdx) which are available out-of-the-box.

By default, the Switchyard application provides standard commerce features for orders and carts. However, Switchyard's customization capabilities facilitate extending existing features to implement quote-management features.
...
This guide is based on the [B2B starter](https://github.com/switchyard/b2b-starter-medusa) explaining how to implement some of its quote management features. You can refer to the B2B starter for other features not covered in this guide.

</Note>

## Summary

By following this guide, you'll add the following features to Switchyard:

1. Customers can request a quote for a set of products.
2. Merchants can manage quotes in the Switchyard Admin dashboard. They can reject a quote or send a counter-offer, and they can make edits to item prices and quantities.
3. Customers can accept or reject a quote once it's been sent by the merchant.
4. Once the customer accepts a quote, it's converted to an order in Switchyard.

![Quote management system workflow diagram illustrating the complete quote lifecycle: customers request quotes from cart items, merchants review and create detailed quotes with pricing, quotes are sent to customers for approval, and upon acceptance, quotes are automatically converted into processable orders within the Switchyard e-commerce platform](https://res.cloudinary.com/dza7lstvk/image/upload/v1741173690/Switchyard%20Resources/quote-management-summary_xd319j.jpg)

To implement these features, you'll be customizing the Switchyard server and the Switchyard Admin dashboard.

You can follow this guide whether you're new to Switchyard or an advanced Switchyard developer.

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/quote-management",
...
## Step 1: Install a Switchyard Application
...
Start by installing the Switchyard application on your machine with the following command:
...
Afterwards, the installation process will start, which will install the Switchyard application in a directory with your project's name. If you chose to install the Next.js starter, it'll be installed in a separate directory with the `{project-name}-storefront` name.

<Note title="Why is the storefront installed separately">

The Switchyard application is composed of a headless Node.js server and an admin dashboard. The storefront is installed or custom-built separately and connects to the Switchyard application through its REST endpoints, called [API routes](!docs!/learn/fundamentals/api-routes). Learn more about Switchyard's architecture in [this documentation](!docs!/learn/introduction/architecture).

</Note>

Once the installation finishes successfully, the Switchyard Admin dashboard will open with a form to create a new user. Enter the user's credentials and submit the form. Afterwards, you can log in with the new user and explore the dashboard.

<Note title="Ran into Errors">

Check out the [troubleshooting guides](../../../troubleshooting/create-switchyard-app-errors/page.mdx) for help.
...
In Switchyard, you can build custom features in a [module](!docs!/learn/fundamentals/modules). A module is a reusable package with functionalities related to a single feature or domain. Switchyard integrates the module into your application without implications or side effects on your setup.
...
A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/quote`.

![Diagram showcasing the directory structure after adding the Quote Module's directory](https://res.cloudinary.com/dza7lstvk/image/upload/v1741074268/Switchyard%20Resources/quote-1_lxgyyg.jpg)
...
![Diagram showcasing the directory structure after adding the quote model](https://res.cloudinary.com/dza7lstvk/image/upload/v1741074453/Switchyard%20Resources/quote-2_lh012l.jpg)
...
![Directory structure after adding the service](https://res.cloudinary.com/dza7lstvk/image/upload/v1741075946/Switchyard%20Resources/quote-4_hg4bnr.jpg)

```ts title="src/modules/quote/service.ts"
import { SwitchyardService } from "@switchyard/framework/utils"
import { Quote } from "./models/quote"

class QuoteModuleService extends SwitchyardService({ 
  Quote, 
}) {}
...
The `QuoteModuleService` extends `SwitchyardService` from the Modules SDK which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.

So, the `QuoteModuleService` class now has methods like `createQuotes` and `retrieveQuote`.

<Note>

Find all methods generated by the `SwitchyardService` in [this reference](../../../service-factory-reference/page.mdx).
...
The final piece to a module is its definition, which you export in an `index.ts` file at its root directory. This definition tells Switchyard the name of the module and its service.

So, create the file `src/modules/quote/index.ts` with the following content:

![Directory structure after adding the module definition](https://res.cloudinary.com/dza7lstvk/image/upload/v1741076106/Switchyard%20Resources/quote-5_ngitn1.jpg)
...
### Add Module to Switchyard's Configurations

Once you finish building the module, add it to Switchyard's configurations to start using it.

In `switchyard.config.ts`, add a `modules` property and pass an array with your custom module:
...
Switchyard's CLI tool generates the migrations for you. To generate a migration for the Quote Module, run the following command in your Switchyard application's directory:

```bash
npx switchyard db:generate quote
```

The `db:generate` command of the Switchyard CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/quote` that holds the generated migration.

![The directory structure of the Quote Module after generating the migration](https://res.cloudinary.com/dza7lstvk/image/upload/v1741076301/Switchyard%20Resources/quote-6_adzf76.jpg)
...
Switchyard integrates modules into your application without implications or side effects by isolating modules from one another. This means you can't directly create relationships between data models in your module and data models in other modules.

Instead, Switchyard provides the mechanism to define links between data models, and retrieve and manage linked records while maintaining module isolation. Links are useful to define associations between data models in different modules, or extend a model in another module to associate custom properties with it.
...
![Directory structure after adding the quote-cart link](https://res.cloudinary.com/dza7lstvk/image/upload/v1741077395/Switchyard%20Resources/quote-7_xrvodi.jpg)
...
import CartModule from "@switchyard/medusa/cart"
...
3. An optional object with additional configurations for the link. By default, Switchyard creates a table in the database to represent the link you define. However, when you only want to retrieve the linked records without managing and storing the links, you can set the `readOnly` option to `true`.
...
![Directory structure after adding the quote-customer link](https://res.cloudinary.com/dza7lstvk/image/upload/v1741078047/Switchyard%20Resources/quote-8_bbngmh.jpg)
...
![Directory structure after adding the quote-order-change link](https://res.cloudinary.com/dza7lstvk/image/upload/v1741078511/Switchyard%20Resources/quote-11_faac5m.jpg)

```ts title="src/links/quote-order-change.ts"
import { defineLink } from "@switchyard/framework/utils"
import QuoteModule from "../modules/quote"
import OrderModule from "@switchyard/medusa/order"
...
![Directory structure after adding the quote-order link](https://res.cloudinary.com/dza7lstvk/image/upload/v1741078607/Switchyard%20Resources/quote-12_ixr2f7.jpg)
...
The first four steps are provided by Switchyard in its `@switchyard/medusa/core-flows` package. So, you only need to implement the `createQuotesStep` step.
...
![Directory structure after adding the create-quotes step](https://res.cloudinary.com/dza7lstvk/image/upload/v1741085446/Switchyard%20Resources/quote-13_tv9i23.jpg)

export const createQuotesStepHighlights = [
  ["15", "container", "Resolve the Quote Module's service from the Switchyard container."],
...
2. An async function that receives two parameters:
    ...
    - An object that has properties including the [Switchyard container](!docs!/learn/fundamentals/medusa-container), which is a registry of Framework and commerce tools that you can access in the step.

In the step function, you resolve the Quote Module's service from the Switchyard container using the `resolve` method of the container, passing it the module's name as a parameter.

Then, you create the quotes using the `createQuotes` method. As you remember, the Quote Module's service extends the `SwitchyardService` which generates data-management methods for you.
...
2. An object that has properties including the [Switchyard container](!docs!/learn/fundamentals/medusa-container).

In the compensation function, you resolve the Quote Module's service from the Switchyard container and call the `deleteQuotes` method to delete the quotes created in the step.

### createRequestForQuoteWorkflow

You can now create the workflow using the steps provided by Switchyard and your custom step.
...
import { CreateOrderLineItemDTO } from "@switchyard/framework/types"
...
You first prepare the order's details using `transform` from the Workflows SDK. Since Switchyard creates an internal representation of the workflow's constructor before any data actually has a value, you can't manipulate data directly in the function. So, Switchyard provides utilities like `transform` to manipulate data instead. You can learn more in the [transform variables](!docs!/learn/fundamentals/workflows/variable-manipulation) documentation.

Then, you create the draft order using the `createOrderWorkflow` workflow which you imported from `@switchyard/medusa/core-flows`. The workflow creates and returns the created order.
...
You prepare the order change's details using `transform` and then create the order change using the `beginOrderEditOrderWorkflow` workflow which is provided by Switchyard.
...
![Directory structure after adding the store/quotes route](https://res.cloudinary.com/dza7lstvk/image/upload/v1741086995/Switchyard%20Resources/quote-14_meo0yo.jpg)
...
export const POST = async (
  req: AuthenticatedSwitchyardRequest<CreateQuoteType>,
...
`AuthenticatedSwitchyardRequest` accepts the request body's type as a type argument.

</Note>

In the route handler function, you create the quote using the [createRequestForQuoteWorkflow](#createrequestforquoteworkflow) from the previous step. Then, you resolve Query from the Switchyard container, which is available in the request object's `req.scope` property.
...
In Switchyard, you create validation schemas using [Zod](https://zod.dev/) in a TypeScript file under the `src/api` directory. So, create the file `src/api/store/validators.ts` with the following content:

![Directory structure after adding the validators file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741089363/Switchyard%20Resources/quote-15_iy6jem.jpg)
...
![Directory structure after adding the store middlewares file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741089625/Switchyard%20Resources/quote-16_oryolz.jpg)
...
![Directory structure after adding the query-config file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741090067/Switchyard%20Resources/quote-17_n6xsdb.jpg)
...
```ts title="src/api/store/validators.ts"
// other imports...
import { createFindParams } from "@switchyard/medusa/api/utils/validators"
...
You create a `GetQuoteParams` schema using the `createFindParams` utility from Switchyard. This utility creates a schema that allows clients to specify query parameters such as:
...
You apply the `validateAndTransformQuery` middleware on all routes starting with `/store/customers/me/quotes`. The `validateAndTransformQuery` middleware that Switchyard provides accepts two parameters:
...
To test out the API route, start the Switchyard application:

```bash npm2yarn
npm run dev
```

Then, open the Switchyard Admin dashboard at `http://localhost:9000/app` and login using the credentials you set up earlier.
...
To retrieve the publishable API key from the Switchyard Admin, refer to [this user guide](!user-guide!/settings/developer/publishable-api-keys).
...
- `{your_publishable_api_key}` with the publishable API key you retrieved from the Switchyard Admin.
...
Make sure to replace the `{your_publishable_api_key}` with the publishable API key you retrieved from the Switchyard Admin.
...
After the customer creates a quote, the admin user needs to view these quotes to manage them. In this step, you'll create the API route to list quotes for the admin user. Then, in the next step, you'll customize the Switchyard Admin dashboard to display these quotes.
...
![Directory structure after adding the admin quotes route](https://res.cloudinary.com/dza7lstvk/image/upload/v1741094735/Switchyard%20Resources/quote-18_uvwqt6.jpg)
...
In the route handler function, you resolve Query from the Switchyard container and use it to retrieve the list of quotes. Similar to before, you use `req.queryConfig` to specify the fields to retrieve in the response.
...
![Directory structure after adding the query-config file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741095492/Switchyard%20Resources/quote-19_xca6aq.jpg)
...
![Directory structure after adding the admin validators file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741095771/Switchyard%20Resources/quote-20_iygrip.jpg)

```ts title="src/api/admin/validators.ts"
import {
  createFindParams,
} from "@switchyard/medusa/api/utils/validators"
...
You define the `AdminGetQuoteParams` schema using the `createFindParams` utility from Switchyard. The schema allows clients to specify query parameters such as:
...
Your API route is now ready for use. You'll test it in the next step by customizing the Switchyard Admin dashboard to display the quotes.

---

## Step 7: List Quotes Route in Switchyard Admin

Now that you have the API route to retrieve the list of quotes, you want to show these quotes to the admin user in the Switchyard Admin dashboard. The Switchyard Admin is customizable, allowing you to add new pages as UI routes.

A UI route is a React component that specifies the content to be shown in a new page in the Switchyard Admin dashboard. You'll create a UI route to display the list of quotes in the Switchyard Admin.
...
Switchyard provides a [JS SDK](../../../js-sdk/page.mdx) that you can use to send requests to the Switchyard server from any client application, including your Switchyard Admin customizations.

The JS SDK is installed by default in your Switchyard application. To configure it, create the file `src/admin/lib/sdk.ts` with the following content:

![Directory structure after adding the sdk file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741098137/Switchyard%20Resources/quote-23_plm90s.jpg)
...
You create an instance of the JS SDK using the `Switchyard` class from the `@switchyard/js-sdk` package. You pass it an object having the following properties:

- `baseUrl`: The base URL of the Switchyard server.
...
In your development, you'll need types that represents the data you'll retrieve from the Switchyard server. So, create the file `src/admin/types.ts` with the following content:

![Directory structure after adding the admin type](https://res.cloudinary.com/dza7lstvk/image/upload/v1741098478/Switchyard%20Resources/quote-25_jr79pa.jpg)
...
When sending requests to the Switchyard server from your admin customizations, it's recommended to use [Tanstack Query](https://tanstack.com/query/latest), allowing you to benefit from its caching and data fetching capabilities.

So, you'll create a `useQuotes` hook that uses Tanstack Query and the JS SDK to fetch the list of quotes from the Switchyard server.

Create the file `src/admin/hooks/quotes.tsx` with the following content:

![Directory structure after adding the hooks quotes file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741098244/Switchyard%20Resources/quote-24_apdpem.jpg)

```ts title="src/admin/hooks/quotes.tsx"
import { ClientHeaders, FetchError } from "@switchyard/js-sdk"
import {
  QuoteQueryParams,
  AdminQuotesResponse,
} from "../types"
...
You return the fetched data from the Switchyard server. You'll use this hook in the UI route.

### Create Quotes UI Route

You can now create the UI route that will show a new page in the Switchyard Admin with the list of quotes.
...
So, to add the UI route at `/quotes` in the Switchyard Admin, create the file `src/admin/routes/quotes/page.tsx` with the following content:

![Directory structure after adding the Quotes UI route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741099122/Switchyard%20Resources/quote-26_qrqzut.jpg)

```tsx title="src/admin/routes/quotes/page.tsx"
import { defineRouteConfig } from "@switchyard/admin-sdk"
import { DocumentText } from "@switchyard/icons"
import { 
...
The route file must export a React component that implements the content of the page. To show a link to the route in the sidebar, you can also export a configuration object created with `defineRouteConfig` that specifies the label and icon of the route in the Switchyard Admin sidebar.

In the `Quotes` component, you'll show a table of quotes using the [DataTable component](!ui!/components/data-table) from Switchyard UI. This component requires you first define the columns of the table.
...
In the component, you use the `useQuotes` hook to fetch the quotes from the Switchyard server. You pass the following query parameters in the request:
...
You can now test out the UI route and the route added in the previous section from the Switchyard Admin.

First, start the Switchyard application:
...
![Switchyard Admin dashboard displaying the Quotes management interface with a data table showing quote entries including columns for quote ID, customer information, status, and creation date](https://res.cloudinary.com/dza7lstvk/image/upload/v1741099952/Switchyard%20Resources/Screenshot_2025-03-04_at_4.52.17_PM_nqxyfq.png)
...
![Directory structure after adding the single quote route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741100686/Switchyard%20Resources/quote-27_ugvhbb.jpg)
...
![Preview of the quote details page in Switchyard Admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1741158359/Switchyard%20Resources/Screenshot_2025-03-05_at_9.05.45_AM_wfmb5w.png)

Before you create the UI route, you need to create the hooks necessary to retrieve data from the Switchyard server, and some components that will show the different elements of the page.
...
The hook returns the fetched data from the Switchyard server. You'll use this hook later in the UI route.

In addition, you'll need a hook to retrieve a preview of the quote's draft order. An order preview includes changes or edits to be applied on an order's items, such as changes in prices and quantities. Switchyard already provides a [Get Order Preview API route](!api!/admin#orders_getordersidpreview) that you can use to retrieve the preview.

To create the hook, create the file `src/admin/hooks/order-preview.tsx` with the following content:

![Directory structure after adding the order preview hook file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741157692/Switchyard%20Resources/quote-32_tb1tqw.jpg)
...
![Directory structure after adding the format amount utility file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741157986/Switchyard%20Resources/quote-33_k5sa9q.jpg)
...
![Diagram showcasing where this component will be in the page](https://res.cloudinary.com/dza7lstvk/image/upload/v1741183186/Switchyard%20Resources/amount-highlighted_havznm.png)
...
![Directory structure after adding the amount component](https://res.cloudinary.com/dza7lstvk/image/upload/v1741101819/Switchyard%20Resources/quote-28_iwukg2.jpg)
...
![Screenshot showcasing where this component will be in the page](https://res.cloudinary.com/dza7lstvk/image/upload/v1741183303/Switchyard%20Resources/item-highlighted-cropped_ddyikt.png)

Create the file `src/admin/components/quote-items.tsx` with the following content:

![Directory structure after adding the quote items component](https://res.cloudinary.com/dza7lstvk/image/upload/v1741102170/Switchyard%20Resources/quote-29_r5ljph.jpg)
...
import { Badge, Text } from "@switchyard/ui"
...
![Screenshot showcasing where this component will be in the page](https://res.cloudinary.com/dza7lstvk/image/upload/v1741183481/Switchyard%20Resources/totals-highlighted_hpxier.png)

Create the file `src/admin/components/totals-breakdown.tsx` with the following content:

![Directory structure after adding the totals breakdown component](https://res.cloudinary.com/dza7lstvk/image/upload/v1741155757/Switchyard%20Resources/quote-30_de0kjq.jpg)

```tsx title="src/admin/components/totals-breakdown.tsx"
import { AdminOrder } from "@switchyard/framework/types"
...
You can now create the UI route that will show a quote's details in the Switchyard Admin.

Create the file `src/admin/routes/quote/[id]/page.tsx` with the following content:

![Diagram showcasing the directory structure after adding the quote details UI route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741157385/Switchyard%20Resources/quote-31_grwlon.jpg)

export const quotesDetailsHighlights = [
  ["21", "useQuote", "Retrieve the quote's details from the Switchyard server."],
...
To test the quote details UI route, start the Switchyard application:
...
![Quote details page in Switchyard Admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1741158359/Switchyard%20Resources/Screenshot_2025-03-05_at_9.05.45_AM_wfmb5w.png)
...
As mentioned before, the `useQueryGraphStep` is provided by Switchyard's `@switchyard/medusa/core-flows` package. So, you'll only implement the remaining steps.
...
![Diagram showcasing the directory structure after adding the validate quote rejection step file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741159537/Switchyard%20Resources/quote-34_mtcxwa.jpg)
...
import { InferTypeOf } from "@switchyard/framework/types"
...
![Diagram showcasing the directory structure after adding the update quotes step file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741159754/Switchyard%20Resources/quote-35_moaulz.jpg)
...
![Diagram showcasing the directory structure after adding the merchant reject quote workflow file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741159969/Switchyard%20Resources/quote-36_l1ffxm.jpg)
...
import { useQueryGraphStep } from "@switchyard/core-flows"
import { createWorkflow } from "@switchyard/workflows-sdk"
import { QuoteStatus } from "../modules/quote/models/quote"
...
![Diagram showcasing the directory structure after adding the reject quote API route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741160251/Switchyard%20Resources/quote-37_jwlfcw.jpg)
...
1. `prompt`: A function that you'll use to show a confirmation pop-up when the merchant tries to reject the quote. The `usePrompt` hook is available from the Switchyard UI package.
...
To test the reject quote feature, start the Switchyard application:
...
![Quote details page with reject quote button in Switchyard Admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1741161544/Switchyard%20Resources/Screenshot_2025-03-05_at_9.58.41_AM_xzdv6k.png)
...
![Directory structure after adding the merchant send quote workflow file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741162342/Switchyard%20Resources/quote-38_n4ksr0.jpg)
...
![Directory structure after adding the send quote API route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741162497/Switchyard%20Resources/quote-39_us1jbh.jpg)
...
To test the send quote feature, start the Switchyard application:
...
![Quote details page with send quote button in Switchyard Admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1741162950/Switchyard%20Resources/Screenshot_2025-03-05_at_10.22.11_AM_sjuipg.png)
...
![Directory structure after adding the customer preview order API route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741163145/Switchyard%20Resources/quote-40_lmcgve.jpg)
...
To test the customer preview order API route, start the Switchyard application:

```bash npm2yarn
npm run dev
```

Then, grab the ID of a quote placed by a customer that you have their [authentication token](#retrieve-customer-authentication-token). You can find the quote ID in the URL when viewing the quote's details page in the Switchyard Admin dashboard.
...
![Directory structure after adding the customer reject quote workflow file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741164371/Switchyard%20Resources/quote-41_fgpqhz.jpg)
...
![Directory structure after adding the customer reject quote API route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741164538/Switchyard%20Resources/quote-42_bryo2z.jpg)
...
To test the customer reject quote feature, start the Switchyard application:
...
After sending the request, the quote will be rejected, and the updated quote will be returned in the response. You can also view the quote from the Switchyard Admin dashboard, where you'll find its status has changed.
...
![Directory structure after adding the validate quote can accept step file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741165829/Switchyard%20Resources/quote-43_cxc3qi.jpg)
...
![Directory structure after adding the customer accept quote workflow file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741166025/Switchyard%20Resources/quote-44_c09ts9.jpg)
...
![Directory structure after adding the customer accept quote API route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741166543/Switchyard%20Resources/quote-45_y8zprn.jpg)
...
To test the customer accept quote feature, start the Switchyard application:
...
You can also view the quote from the Switchyard Admin dashboard, where you'll find its status has changed. The quote will also have an order, which you can view in the Orders page or using the "View Order" button on the quote's details page.

![View order button on quote's details page in Switchyard Admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1741166844/Switchyard%20Resources/Screenshot_2025-03-05_at_11.27.02_AM_s90rqh.png)
...
![Edit quote items page in Switchyard Admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1741169659/Switchyard%20Resources/Screenshot_2025-03-05_at_12.14.05_PM_ufvkqb.png)

In this step, you'll add a new UI route to manage the quote's items. This will include:

1. Adding hooks to send requests to [Switchyard's Order Edits API routes](!api!/admin#order-edits).
2. Implement the components you'll use within the UI route.
3. Add the new UI route to the Switchyard Admin.

### Intermission: Order Editing Overview

Before you start implementing the customizations, here's a quick overview of how order editing works in Switchyard.

When the admin wants to edit an order's items, Switchyard creates an order change. You've already implemented this part on quote creation.

Then, when the admin makes an edit to an item, Switchyard saves that edit but without applying it to the order or finalizing the edit. This allows the admin to make multiple edits before finalizing the changes.
...
![Screenshot of the manage item component in Switchyard Admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1741186495/Switchyard%20Resources/manage-item-highlight_ouffnu.png)

Create the file `src/admin/components/manage-item.tsx` with the following content:

![Directory structure after adding the manage item component file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741168152/Switchyard%20Resources/quote-46_yxanj7.jpg)

```tsx
import { AdminOrder, AdminOrderPreview } from "@switchyard/framework/types"
import {
  Badge,
  CurrencyInput,
  Hint,
...
- `updateItem`: The `mutateAsync` function returned by the `useUpdateQuoteItem` hook. This function updates the item's quantity and price using Switchyard's Order Edits API routes.
...
![Screenshot of the manage quote form in the Switchyard Admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1741186643/Switchyard%20Resources/manage-quote-form-highlight_pfyee5.png)

Create the file `src/admin/components/manage-quote-form.tsx` with the following content:

![Directory structure after adding the manage quote form component file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741168581/Switchyard%20Resources/quote-47_f5kamq.jpg)
...
![Directory structure after adding the edit quote items UI route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1741168993/Switchyard%20Resources/quote-48_roangs.jpg)

```tsx title="src/admin/routes/quotes/[id]/manage/page.tsx"
import { useParams } from "react-router-dom"
import { useQuote } from "../../../../hooks/quotes"
import { Container, Heading, Toaster } from "@switchyard/ui"
...
You define a `QuoteManage` component that will show the form to manage the quote's items in the Switchyard Admin dashboard.
...
To test the edit quote items UI route, start the Switchyard application:

```bash npm2yarn
npm run dev
```

Then, open the Switchyard Admin dashboard at `http://localhost:9000/admin`. Open a quote's details page whose status is either `pending_merchant`, `merchant_rejected` or `customer_rejected`. You'll find a new "Manage Quote" button.

![Manage Quote button on quote's details page in Switchyard Admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1741169567/Switchyard%20Resources/Screenshot_2025-03-05_at_12.12.21_PM_c5fhsp.png)
...
You've now implemented quote management features in Switchyard. There's still more that you can implement to enhance the quote management experience:

- Refer to the [B2B starter](https://github.com/switchyard/b2b-starter-medusa) for more quote-management related features, including how to add or remove items from a quote, and how to allow messages between the customer and the merchant.
...
If you're new to Switchyard, check out the [main documentation](!docs!/learn), where you'll get a more in-depth learning of all the concepts you've used in this guide and more.

To learn more about the commerce features that Switchyard provides, check out Switchyard's [Commerce Modules](../../../commerce-modules/page.mdx).
