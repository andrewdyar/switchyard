...
export const metadata = {
  title: `Switchyard Examples`,
}

# {metadata.title}

This documentation page has examples of customizations useful for your custom development in the Switchyard application.
...
To resolve resources from the Switchyard container in an API route:

```ts highlights={[["8", "resolve", "Resolve the Product Module's\nmain service from the Switchyard container."]]}
...
export const POST = async (
  req: SwitchyardRequest<HelloWorldReq>,
  res: SwitchyardResponse
) => {
  res.json({
...
To change it to another content type, use the `writeHead` method of `SwitchyardResponse`:
...
export const POST = async (
  req: SwitchyardRequest<PostStoreCustomSchemaType>,
  res: SwitchyardResponse
) => {
  res.json({
...
To throw errors in an API route, use `SwitchyardError` from the Switchyard Framework:

```ts highlights={[["9", "SwitchyardError"]]}
import { SwitchyardRequest, SwitchyardResponse } from "@switchyard/framework/http"
...
By default, Switchyard configures CORS for all routes starting with `/admin`, `/store`, and `/auth`.
...
import { parseCorsOrigins } from "@switchyard/framework/utils"
...
By default, the Switchyard application parses a request's body using JSON.
...
5. Add the module to the configurations in `switchyard.config.ts`:
...
1. Pass options to the module in `switchyard.config.ts`:

```ts title="switchyard.config.ts" highlights={[["6", "options"]]}
module.exports = defineConfig({
  // ...
  modules: [
    {
...
export default class BlogModuleService extends SwitchyardService({
  Post,
}){
...
A data model represents a table in the database. Switchyard provides a data model language to intuitively create data models.
...
The service factory `SwitchyardService` generates data-management methods for your data models.

To extend the service factory in your module's service:

```ts highlights={[["4", "SwitchyardService"]]}
import { SwitchyardService } from "@switchyard/framework/utils"
import Post from "./models/post"
...
Query fetches data across modules. It’s a set of methods registered in the Switchyard container under the `query` key.
...
To resolve resources from the Switchyard container in a subscriber, use the `container` property of its parameter:
...
A scheduled job is a function executed at a specified interval of time in the background of your Switchyard application.
...
To limit the scheduled job's execution to a number of times during the Switchyard application's runtime, use the `numberOfExecutions` configuration:
...
A loader is a function defined in a module that's executed when the Switchyard application starts.
...
export default async function helloWorldLoader() {
  console.log(
    "[HELLO MODULE] Just started the Switchyard application!"
  )
}
```
...
You can customize the Switchyard Admin to inject widgets in existing pages, or create new pages using UI routes.
...
Switchyard provides a `@switchyard/test-utils` package with utility tools to create integration tests for your custom API routes, modules, or other Switchyard customizations.
...
Switchyard provides all its commerce features as separate Commerce Modules, such as the Product or Order modules.
...
To create an actor type that can authenticate to the Switchyard application, such as a `manager`:
