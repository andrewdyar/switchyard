...
export const ogImage = "https://res.cloudinary.com/dza7lstvk/image/upload/v1737564127/Switchyard%20Resources/plugins-wishlist_ktut0d.jpg"
...
In this guide, you'll learn how to build a wishlist [plugin](!docs!/learn/fundamentals/plugins) in Switchyard.
...
- Install and set up a Switchyard application project.
- Install and set up a Switchyard plugin.
...
- Test and use the wishlist plugin in your Switchyard application.

You can follow this guide whether you're new to Switchyard or an advanced Switchyard developer.

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/wishlist-plugin",
...
You'll first install a Switchyard application that exposes core commerce features through REST APIs. You'll later install the wishlist plugin in this application to test it out.
...
Afterward, the installation process will start, which will install the Switchyard application in a directory with your project's name. If you chose to install the Next.js starter, it'll be installed in a separate directory with the `{project-name}-storefront` name.
...
## Step 2: Install a Switchyard Plugin Project

A plugin is a package of reusable Switchyard customizations that you can install in any Switchyard application. You can add in the plugin [API Routes](!docs!/learn/fundamentals/api-routes), [Workflows](!docs!/learn/fundamentals/workflows), and other customizations, as you'll see in this guide. Afterward, you can test it out locally in a Switchyard application, then publish it to npm to install and use it in any Switchyard application.
...
A Switchyard plugin is set up in a different project, giving you the flexibility in building and publishing it, while providing you with the tools to test it out locally in a Switchyard application.

To create a new Switchyard plugin project, run the following command in a directory different than that of the Switchyard application:

```bash npm2yarn
npx create-switchyard-app@latest medusa-plugin-wishlist --plugin
```
...
![Directory structure of a plugin project](https://res.cloudinary.com/dza7lstvk/image/upload/v1737019441/Switchyard%20Book/project-dir_q4xtri.jpg)

---

## Step 3: Set up Plugin in Switchyard Application

Before you start your development, you'll set up the plugin in the Switchyard application you installed in the first step. This will allow you to test the plugin during your development process.
...
Next, you'll install the plugin in the Switchyard application from the local registry.

<Note>

If you've installed your Switchyard project before v2.3.1, you must install [yalc](https://github.com/wclr/yalc) as a development dependency first.

</Note>

Run the following command in the Switchyard application's directory to install the plugin:

```bash title="Switchyard application"
npx switchyard plugin:add medusa-plugin-wishlist
```

This command installs the plugin in the Switchyard application from the local package registry.

Next, register the plugin in the `switchyard.config.ts` file of the Switchyard application:
...
To add custom tables to the database, which are called data models, you create a module. A module is a package with functionalities related to a single feature or domain. Switchyard integrates the module into your application without implications or side effects on your setup.

While you can create modules outside of a plugin and install them in the Switchyard application, plugins allow you to bundle modules with other customizations, such as API routes and workflows.
...
![Diagram showcasing the module directory to create](https://res.cloudinary.com/dza7lstvk/image/upload/v1737461182/Switchyard%20Resources/wishlist-1_z3kzfv.jpg)
...
![Directory structure after adding the Wishlist model](https://res.cloudinary.com/dza7lstvk/image/upload/v1737461304/Switchyard%20Resources/wishlist-2_co2lht.jpg)
...
- `sales_channel_id`: The ID of the sales channel where the wishlist is created. In Switchyard, product availability can differ between sales channels. This ensures only products available in the customer's sales channel are added to the wishlist.
...
![Directory structure after adding the WishlistItem model](https://res.cloudinary.com/dza7lstvk/image/upload/v1737461521/Switchyard%20Resources/wishlist-3_fxcjxy.jpg)
...
![Directory structure after adding the service file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737461698/Switchyard%20Resources/wishlist-4_j5ka26.jpg)
...
export default class WishlistModuleService extends SwitchyardService({
  Wishlist,
  WishlistItem,
}) {}
```

The `WishlistModuleService` extends `SwitchyardService` from the Modules SDK which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.
...
![Directory structure after adding the module definition file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737461829/Switchyard%20Resources/wishlist-5_mb4tjf.jpg)
...
Switchyard's CLI tool generates the migrations for you. To generate a migration for the Wishlist Module, run the following command in the plugin project:
...
Then, to reflect these migrations on the database of the Switchyard application using this module, run the following command:

<Note>

Make sure that `npx switchyard plugin:develop` is running in the plugin project to publish the changes to the local registry.
...
The Wishlist Module's data models store IDs of records in data models implemented in Switchyard's core Commerce Modules, such as the ID of a customer or a product variant.

However, modules are [isolated](!docs!/learn/fundamentals/modules/isolation) to ensure they're re-usable and don't have side effects when integrated into the Switchyard application. So, to build associations between modules, you define [module links](!docs!/learn/fundamentals/module-links). A Module link associates two modules' data models while maintaining module isolation.
...
![Directory structure after adding the link file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737462649/Switchyard%20Resources/wishlist-6_xqytog.jpg)
...
3. An object of configurations for the module link. By default, Switchyard creates a table in the database to represent the link you define. However, in this guide, you only want this link to retrieve the customer associated with a wishlist. So, you enable `readOnly` telling Switchyard not to create a table for this link.
...
![Directory structure after adding the link file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737462829/Switchyard%20Resources/wishlist-7_ddiwxy.jpg)

```ts title="src/modules/wishlist/links/wishlist-sales-channel.ts"
import { defineLink } from "@switchyard/framework/utils"
import WishlistModule from "../modules/wishlist"
import SalesChannelModule from "@switchyard/medusa/sales-channel"
...
![Directory structure after adding the link file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737467010/Switchyard%20Resources/wishlist-8_hgoaby.jpg)
...
![Directory structure after adding the step file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737467678/Switchyard%20Resources/wishlist-9_lzlifn.jpg)
...
1. An async function that executes the step's logic. The function receives two parameters:
   ...
   - An object holding the workflow's context, including the [Switchyard Container](!docs!/learn/fundamentals/medusa-container) that allows you to resolve Framework and commerce tools.
...
![Directory structure after adding the step file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737467998/Switchyard%20Resources/wishlist-10_xex4d0.jpg)
...
![Directory structure after adding the workflow file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737468333/Switchyard%20Resources/wishlist-11_absftb.jpg)
...
![Directory structure after adding the route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737468859/Switchyard%20Resources/wishlist-12_gvvb9z.jpg)

```ts title="src/api/store/customers/me/wishlists/route.ts"
import { AuthenticatedSwitchyardRequest, SwitchyardResponse } from "@switchyard/framework/http"
...
You'll now test that this API route defined in the plugin is working as expected using the Switchyard application you installed in the first step.
...
In the Switchyard application's directory, run the following command to start the development server:
...
Before sending the request, you need to obtain a publishable API key. So, open the Switchyard Admin at `http://localhost:9000/app` and log in with the user you created earlier.
...
![In the admin, click on Publishable API key in the sidebar. A table will show your API keys and allow you to create one.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733230421/Switchyard%20Resources/Screenshot_2024-12-03_at_2.53.07_PM_gau9jy.png)

### Retrieve Authenticated Customer Token

Then, you need an authentication token of a registered customer. To create a customer, first, send the following request to the Switchyard application:
...
To test the API route, start the Switchyard application.
...
The `useQueryGraphStep` is from Switchyard's workflows package. So, you'll only implement the other steps.
...
![Directory structure after adding the step file](https://res.cloudinary.com/dza7lstvk/image/upload/v1740071251/Switchyard%20Resources/wishlist-29_bq6kcn.jpg)
...
![Directory structure after adding the step file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737470093/Switchyard%20Resources/wishlist-13_nn924e.jpg)
...
![Directory structure after adding the step file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737470156/Switchyard%20Resources/wishlist-14_ckoesz.jpg)
...
![Directory structure after adding the step file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737470302/Switchyard%20Resources/wishlist-15_oc696x.jpg)
...
![Directory structure after adding the workflow file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737470660/Switchyard%20Resources/wishlist-16_ovujwp.jpg)
...
![Directory structure after adding the route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737470985/Switchyard%20Resources/wishlist-17_zmqk6c.jpg)
...
export async function POST(
  req: AuthenticatedSwitchyardRequest<PostStoreCreateWishlistItemType>,
  res: SwitchyardResponse
) {
  if (!req.publishable_key_context?.sales_channel_ids.length) {
    throw new SwitchyardError(
...
In Switchyard, you create validation schemas using [Zod](https://zod.dev/) in a TypeScript file under the `src/api` directory. So, create the file `src/api/store/customers/me/wishlists/items/validators.ts` with the following content:

![Directory structure after adding the validation schema file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737471383/Switchyard%20Resources/wishlist-18_hj9iom.jpg)
...
The `validateAndTransformBody` middleware is available out-of-the-box in Switchyard, allowing you to validate and transform the request body using a Zod schema.
...
![Directory structure after adding the middleware file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737471615/Switchyard%20Resources/wishlist-19_ryyzdk.jpg)
...
In this file, you export the middlewares definition using `defineMiddlewares` from the Switchyard Framework. This function accepts an object having a `routes` property, which is an array of middleware configurations to apply on routes.
...
Start the Switchyard application to test out the API route.
...
The `useQueryGraphStep` is from Switchyard's workflows package, and you implemented the `validateWishlistExistsStep` [previously](#validatewishlistexistsstep) . So, you'll only implement the other steps.
...
![Directory structure after adding the step file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737474621/Switchyard%20Resources/wishlist-20_jcwrtf.jpg)
...
![Directory structure after adding the step file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737474703/Switchyard%20Resources/wishlist-21_e50lrg.jpg)
...
![Directory structure after adding the workflow file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737474872/Switchyard%20Resources/wishlist-22_wt1g36.jpg)
...
![Directory structure after adding the route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737475074/Switchyard%20Resources/wishlist-23_qatcia.jpg)
...
![Directory structure after adding the route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737475331/Switchyard%20Resources/wishlist-24_tiwjpr.jpg)
...
You first retrieve the [http Switchyard configuration](!docs!/learn/configurations/switchyard.config#http) which holds configurations related to JWT secrets and expiration times. You then use the `jsonwebtoken` package to sign a token containing the wishlist ID. You return the token in the response.
...
![Directory structure after adding the route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737475795/Switchyard%20Resources/wishlist-25_sodzsr.jpg)

```ts title="src/api/store/wishlists/[token]/route.ts"
import { SwitchyardResponse, SwitchyardStoreRequest } from "@switchyard/framework"
import { SwitchyardError } from "@switchyard/framework/utils"
import { decode, JwtPayload } from "jsonwebtoken"

export async function GET(
  req: SwitchyardStoreRequest,
  res: SwitchyardResponse
) {
  if (!req.publishable_key_context?.sales_channel_ids.length) {
    throw new SwitchyardError(
...
## Step 13: Show Wishlist Count in Switchyard Admin

In this step, you'll customize the Switchyard Admin dashboard to show for each product the number of wishlists it's in.

The Switchyard Admin dashboard's pages are customizable to insert widgets of custom content in pre-defined injection zones. You create these widgets as React components that allow admin users to perform custom actions.
...
```ts title="src/modules/wishlist/service.ts"
// other imports...
import { InjectManager } from "@switchyard/framework/utils"
...
export default class WishlistModuleService extends SwitchyardService({
  ...
  async getWishlistsOfVariants(
    variantIds: string[],
    @SwitchyardContext() context: Context<EntityManager> = {}
  ): Promise<number> {
...
Methods with the `@InjectManager` decorator accept as a last parameter a context object that has the `@SwitchyardContext` decorator. The entity manager is injected into the `manager` property of this parameter.
...
![Directory structure after adding the route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737630927/Switchyard%20Resources/wishlist-26_ervnfg.jpg)
...
Then, you resolve the `WishlistModuleService` from the Switchyard Container and use its `getWishlistsOfVariants` method to retrieve the number of wishlists the product's variants are in. You return the count in the response.

You'll use this API route in the widget next.

### Create Wishlist Count Widget

You'll now create the widget that will be shown on a product's page in the Switchyard Admin.

In the widget, you'll send a request to the API route you created to retrieve the wishlist count for the product. To send the request, you'll use the [JS SDK](../../../js-sdk/page.mdx), which is a JavaScript library that simplifies sending requests to Switchyard's API routes.

To initialize the JS SDK, create the file `src/admin/lib/sdk.ts` with the following content:

![Directory structure after adding the SDK file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737631853/Switchyard%20Resources/wishlist-27_pkzeaj.jpg)
...
![Directory structure after adding the widget file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737631988/Switchyard%20Resources/wishlist-28_fx8rw7.jpg)
...
Finally, you display the widget's content using components from [Switchyard UI](!ui!), allowing you to align the design of your widget with the Switchyard Admin's design system.

### Test it Out

To test it out, start the Switchyard application.
...
1. open the Switchyard Admin at `localhost:9000/app` and log in.

2. Click on Products in the sidebar, then choose a product from the table.

![Click on the "Products" in the sidebar on the right, then choose a product from the table shown in the middle](https://res.cloudinary.com/dza7lstvk/image/upload/v1737632826/Switchyard%20Resources/Screenshot_2025-01-23_at_1.46.29_PM_xjsn8s.png)

3. You should see the widget you created showing the number of wishlists the product is in at the top of the page.

![The widget is shown at the top of the product page before other sections](https://res.cloudinary.com/dza7lstvk/image/upload/v1737632826/Switchyard%20Resources/Screenshot_2025-01-23_at_1.46.05_PM_hfyz7u.png)

---

## Next Steps

You've now implemented the wishlist functionality in a Switchyard plugin. You can publish that plugin as explained in [this documentation](!docs!/learn/fundamentals/plugins/create#5-publish-plugin-to-npm) to NPM and install it in any Switchyard application. This will allow you to re-use your plugin or share it with the community.
