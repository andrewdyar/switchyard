...
In this tutorial, you'll learn how to integrate Avalara with Switchyard to handle tax calculations.

When you install a Switchyard application, you get a fully-fledged commerce platform with a Framework for customization. Switchyard's architecture supports integrating third-party services, such as tax providers, allowing you to build custom features around core commerce flows.

[Avalara](https://www.avalara.com/) is a leading provider of tax compliance solutions, including sales tax calculation, filing, and remittance. By integrating Avalara with Switchyard, you can calculate taxes during checkout with accurate rates based on customer location.
...
![Diagram showing Avalara integration with Switchyard for tax calculation during checkout](https://res.cloudinary.com/dza7lstvk/image/upload/v1760969087/Switchyard%20Resources/avalara-summary_rqrkpf.jpg)

<Card
  title="Example Repository"
  text="Find the full code of the guide in this repository."
  href="https://github.com/switchyard/examples/tree/main/avalara-integration"
  icon={Github}
/>
...
The Switchyard application consists of a headless Node.js server and an admin dashboard. The storefront is installed or custom-built separately and connects to the Switchyard application through its REST endpoints, called [API routes](!docs!/learn/fundamentals/api-routes). Learn more in [Switchyard's Architecture documentation](!docs!/learn/introduction/architecture).
...
Switchyard's [Tax Module](../../../commerce-modules/tax/page.mdx) implements concepts and functionalities related to taxes, but delegates tax calculations to external services through Tax Module Providers.

In this step, you'll integrate Avalara as a Tax Module Provider. Later, you'll use it to calculate taxes in your Switchyard application.
...
First, install the AvaTax SDK package to interact with Avalara's API. Run the following command in your Switchyard application's directory:
...
A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/avalara`.

### c. Define Module Options

Next, define a TypeScript type for the Avalara Module options. These options configure the module when it's registered in the Switchyard application.
...
Next, you'll implement the `getTaxLines` method required by the `ITaxProvider` interface. This method calculates the tax lines for line items and shipping methods. Switchyard uses this method during checkout to calculate taxes.
...
The final piece of a module is its definition, which you export in an `index.ts` file at the module's root directory. This definition tells Switchyard the module's details, including its service.
...
### f. Add Module Provider to Switchyard's Configuration

After finishing the module, add it to Switchyard's configuration to start using it.
...
To pass a Tax Module Provider to the Tax Module, add the `modules` property to the Switchyard configuration and pass the Tax Module in its value.
...
![Avalara Account ID in the account dropdown at the top right of the Avalara dashboard](https://res.cloudinary.com/dza7lstvk/image/upload/v1760967142/Switchyard%20Resources/CleanShot_2025-10-20_at_16.30.55_2x_gpqi9i.png)
...
![The Avalara license key tab with the "Generate new key" button](https://res.cloudinary.com/dza7lstvk/image/upload/v1760967334/Switchyard%20Resources/CleanShot_2025-10-20_at_16.35.06_2x_fe7cym.png)
...
First, run the following command in your Switchyard application's directory to start the server:
...
1. Go to `http://localhost:9000/admin` and log in to the Switchyard Admin dashboard.
...
![Setting Avalara as the tax provider for a region in the Switchyard Admin dashboard](https://res.cloudinary.com/dza7lstvk/image/upload/v1760968076/Switchyard%20Resources/CleanShot_2025-10-20_at_16.47.45_2x_g96acs.png)
...
The Next.js Starter Storefront was installed in a separate directory from Switchyard. The directory name is `{your-project}-storefront`.

If your Switchyard application's directory is `medusa-avalara`, find the storefront by going back to the parent directory and changing to the `medusa-avalara-storefront` directory:
...
While the Switchyard server is running, open another terminal window in the storefront's directory and run the following command to start the storefront:
...
![Taxes calculated during checkout in the Next.js Starter Storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1760968357/Switchyard%20Resources/CleanShot_2025-10-20_at_16.52.14_2x_bgozfz.png)

7. In the Delivery step, select a shipping method. This will update the tax amount based on the shipping method's price.

![Taxes updated based on the selected shipping method during checkout in the Next.js Starter Storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1760968412/Switchyard%20Resources/CleanShot_2025-10-20_at_16.53.14_2x_wxcuyn.png)
...
In this step, you'll implement the logic to create an Avalara transaction when an order is placed in Switchyard. You will:
...
Switchyard provides the first and last step out of the box. You only need to create the `createTransactionStep`.
...
To test the order placement with Avalara transaction creation, make sure both the Switchyard server and the Next.js Starter Storefront are running.
...
![The transaction's details in the Avalara dashboard showing the calculated taxes for the order](https://res.cloudinary.com/dza7lstvk/image/upload/v1761122370/Switchyard%20Resources/CleanShot_2025-10-22_at_11.38.09_2x_bu0jwl.png)
...
In this step, you'll sync Switchyard's products with Avalara items. This way, you can manage tax codes and other settings for your products directly from Avalara.
...
Switchyard provides the first and last step out of the box. You only need to create the `createItemStep`.
...
To test the product creation with Avalara item creation, make sure the Switchyard server is running.

Then:

1. Open the Switchyard Admin dashboard at `http://localhost:9000/admin`.
...
![The list of items in the Avalara dashboard showing the item created for the product](https://res.cloudinary.com/dza7lstvk/image/upload/v1761123470/Switchyard%20Resources/CleanShot_2025-10-22_at_11.57.33_2x_jbb4km.png)
...
Switchyard provides the first step out of the box, and you have already created the `createProductItemWorkflow`. You only need to create the `updateItemStep`.
...
import { createWorkflow, WorkflowResponse, transform, when } from "@switchyard/framework/workflows-sdk"
...
To test the product update with Avalara item update, make sure the Switchyard server is running.
...
Switchyard provides the first step out of the box. You only need to create the `deleteItemStep`.
...
import { createWorkflow, when, WorkflowResponse } from "@switchyard/framework/workflows-sdk"
...
To test the product deletion with Avalara item deletion, make sure the Switchyard server is running.
...
You've successfully integrated Avalara with Switchyard to handle tax calculations during checkout, create transactions when orders are placed, and sync products with Avalara items.
...
If you're new to Switchyard, check out the [main documentation](!docs!/learn) for a more in-depth understanding of the concepts used in this guide.
