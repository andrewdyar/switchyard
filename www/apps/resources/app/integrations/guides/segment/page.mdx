...
export const metadata = {
  title: `Integrate Segment (Analytics) with Switchyard`,
}

# {metadata.title}

In this tutorial, you'll learn how to integrate Segment with Switchyard to track events and analytics.
...
To track analytics in your Switchyard application, you can integrate [Segment](https://segment.com/), a service that collects analytics from multiple sources and sends them to various destinations. This tutorial will help you set up Segment in your Switchyard application and track common events.
...
- Integrate Segment with your Switchyard application.
- Handle Switchyard's `order.placed` event to track order placements.
- Track custom events in your Switchyard application with Segment.

You can follow this tutorial whether you're new to Switchyard or an advanced Switchyard developer.

![Diagram showcasing the integration of Segment with Switchyard](https://res.cloudinary.com/dza7lstvk/image/upload/v1748264333/Switchyard%20Book/segment-overview_apkrtp.jpg)

<Card
  title="Example Repository"
  text="Find the full code of the guide in this repository."
  href="https://github.com/switchyard/examples/tree/main/segment-integration"
  icon={Github}
/>
...
To integrate third-party services into Switchyard, you create a custom module. A module is a reusable package with functionalities related to a single feature or domain.

Switchyard's [Analytics Module](../../../infrastructure-modules/analytics/page.mdx) provides an interface to track events in your Switchyard application. It delegates the actual tracking to the configured Analytics Module Provider.

In this step, you'll integrate Segment as an Analytics Module Provider. Later, you'll use it to track events in your Switchyard application.
...
Run the following command in your Switchyard application's directory:
...
A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/segment`.
...
- `options`: Options that are passed to the module provider when it's registered in Switchyard's configurations. You define the following option:
    - `writeKey`: The Segment write key. You'll learn how to retrieve and set this option in the [Add Module Provider to Switchyard's Configurations](#h-add-module-provider-to-medusas-configurations) section.
...
// other imports...
import { ProviderIdentifyAnalyticsEventDTO } from "@switchyard/types"
...
```ts title="src/modules/segment/service.ts"
// other imports...
import { ProviderTrackAnalyticsEventDTO } from "@switchyard/types"
...
The `shutdown` method is used to gracefully shut down the Segment client when the Switchyard application is stopped. It allows you to send all pending events to Segment before the application exits.
...
To pass an Analytics Module Provider to the Analytics Module, you add the `modules` property to the Switchyard configuration and pass the Analytics Module in its value.

The Analytics Module accepts a `providers` option, which is an array of Analytics Module Providers to register. However, you can only register one analytics provider in your Switchyard application.
...
![Add more button in the Connections page](https://res.cloudinary.com/dza7lstvk/image/upload/v1748254988/Switchyard%20Book/CleanShot_2025-05-26_at_10.47.24_2x_qqlkwk.png)

3. In the "Choose a Source" step, select "Node.js" and click the "Next" button.

![Select Node.js as the Source](https://res.cloudinary.com/dza7lstvk/image/upload/v1748255028/Switchyard%20Book/CleanShot_2025-05-26_at_10.47.57_2x_zy9g8k.png)
...
![Copy the write key](https://res.cloudinary.com/dza7lstvk/image/upload/v1748255065/Switchyard%20Book/CleanShot_2025-05-26_at_10.48.41_2x_lpgsbb.png)
...
You'll first track the order-placement event, which is triggered natively in the Switchyard application.

Switchyard's events system allows you to listen to events triggered by the Switchyard application and execute custom logic asynchronously in a [subscriber](!docs!/learn/fundamentals/events-and-subscribers).
...
export const stepHighlights = [
  ...
  ["13", "analyticsModuleService", "Resolve the Analytics Module's service from the Switchyard container."],
...
In the step function, you resolve the Analytics Module's service from the Switchyard container. This service is the interface to track events with the configured Analytics Module Provider, which is Segment in this case.
...
1. Retrieve the Switchyard order using the `useQueryGraphStep` helper step. This step uses Switchyard's [Query](!docs!/learn/fundamentals/module-links/query) tool to retrieve data across modules. You pass it the order ID to retrieve.
...
In the subscriber function, you execute the `trackOrderPlacedWorkflow` by invoking it, passing the Switchyard container as a parameter. Then, you chain a `run` method, passing it the order ID from the event's data payload as input.
...
So, if your Switchyard application's directory is `medusa-segment`, you can find the storefront by going back to the parent directory and changing to the `medusa-segment-storefront` directory:
...
First, run the following command in your Switchyard application's directory to start the Switchyard server:
...
![Order Placed Event in Segment Debugger](https://res.cloudinary.com/dza7lstvk/image/upload/v1748259137/Switchyard%20Book/CleanShot_2025-05-26_at_14.31.25_2x_xi5p9h.png)

---

## Track Custom Event

In your Switchyard application, you often need to track custom events that are relevant to your business use case. For example, a B2B business may want to track whenever a user requests a quote.

In Switchyard, you can emit custom events in your workflows when an action occurs. Then, you can create a subscriber that listens to the custom event and executes a workflow to track it in Segment.

For example, if you have a `createQuoteWorkflow`, you can use Switchyard's [emitEventStep](!docs!/learn/fundamentals/events-and-subscribers/emit-event#emit-event-in-a-workflow) to emit a custom event after the quote is created:
...
You've now integrated Segment with your Switchyard application and tracked common events like order placement. You can expand on the features in this tutorial to:

- Track more events in your Switchyard application, such as user sign-ups, cart additions, and more. You can refer to the [Events Reference](/references/events) for a full list of events emitted by Switchyard.
