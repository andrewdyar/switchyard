...
export const ogImage = "https://res.cloudinary.com/dza7lstvk/image/upload/v1743097088/Switchyard%20Resources/integrations-algolia_qwpguu.jpg"

export const metadata = {
  title: `Integrate Algolia (Search) with Switchyard`,
  openGraph: {
    images: [
      {
        url: ogImage,
...
In this tutorial, you'll learn how to integrate Switchyard with Algolia.

When you install a Switchyard application, you get a fully-fledged commerce platform with a Framework for customization. Switchyard's architecture supports integrating third-party services, such as a search engine, allowing you to build your unique requirements around core commerce flows.

[Algolia](https://www.algolia.com/doc/) is a search engine that enables you to build and manage an intuitive search experience for your customers. By integrating Algolia with Switchyard, you can index e-commerce data, such as products, and allow clients to search through them.
...
- Integrate Algolia into Switchyard.
...
![Diagram illustrating the integration of Algolia with Switchyard](https://res.cloudinary.com/dza7lstvk/image/upload/v1742889842/Switchyard%20Resources/algolia-summary_lhegrr.jpg)

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/algolia-integration",
...
To integrate third-party services into Switchyard, you create a custom module. A module is a reusable package with functionalities related to a single feature or domain. Switchyard integrates the module into your application without implications or side effects on your setup.

In this step, you'll create a custom module that provides the necessary functionalities to integrate Algolia with Switchyard.
...
Before building the module, you need to install Algolia's JavaScript client. Run the following command in your Switchyard application's root directory:
...
A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/algolia`.
...
The next methods you'll add to the service are methods to retrieve and delete data from the Algolia index. You'll see their use later as you keep the Algolia index in sync with Switchyard.
...
![Find the Algolia App ID by clicking on the application name in the Algolia dashboard, then copying the ID below the name in the pop-up](https://res.cloudinary.com/dza7lstvk/image/upload/v1742815360/Switchyard%20Resources/Screenshot_2025-03-24_at_1.19.30_PM_kdp3y5.png)
...
![In the settings page, find the Team and Access section at the right of the page and choose API Keys](https://res.cloudinary.com/dza7lstvk/image/upload/v1742815534/Switchyard%20Resources/Screenshot_2025-03-24_at_1.25.09_PM_hwsiba.png)
...
![In the Algolia dashboard, go to Search -> Index and copy the index name at the top of the page](https://res.cloudinary.com/dza7lstvk/image/upload/v1742815790/Switchyard%20Resources/Screenshot_2025-03-24_at_1.28.58_PM_yq10sf.png)
...
To keep the Algolia index in sync with Switchyard, you need to trigger indexing when products are created, updated, or deleted in Switchyard. You can also allow the admin to manually trigger a reindex.
...
Switchyard provides the `useQueryGraphStep` in its `@switchyard/medusa/core-flows` package. So, you only need to implement the other steps.
...
If you get a type error on resolving the Algolia Module, run the Switchyard application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](!docs!/learn/fundamentals/generated-types).
...
In the step function, you resolve the Algolia Module's service from the Switchyard container using the name you exported in the module definition's file.
...
2. A context object similar to the step function that holds the Switchyard container.
...
1. Execute `useQueryGraphStep` to retrieve products from Switchyard's database. This step uses Switchyard's [Query](!docs!/learn/fundamentals/module-links/query) tool to retrieve data across modules. You pass it the pagination and filter parameters you received in the input.
...
In this step, you'll add the functionality to trigger the `syncProductsWorkflow` manually from the Switchyard Admin dashboard. This requires:

1. Creating a subscriber that listens to a custom `algolia.sync` event to trigger syncing products to Algolia.
2. Creating an API route that the Switchyard Admin dashboard can call to emit the `algolia.sync` event, which triggers the subscriber.
3. Add a new page or UI route to the Switchyard Admin dashboard to allow the admin to trigger the reindex.
...
The subscriber function receives an object as a parameter that has a `container` property, which is the Switchyard container.
...
To allow the Switchyard Admin dashboard to trigger the `algolia.sync` event, you need to create an API route that emits the event.
...
In the route handler, you use the Switchyard container that is available in the request object to resolve the [Event Module](../../../infrastructure-modules/event/page.mdx). This module manages events and their subscribers.
...
A UI route is a React component that specifies the content to be shown in a new page in the Switchyard Admin dashboard. You'll create a UI route to display a button that triggers the reindex when clicked.
...
Before creating the UI route, you'll configure Switchyard's [JS SDK](../../../js-sdk/page.mdx) that you can use to send requests to the Switchyard server from any client application, including your Switchyard Admin customizations.
...
So, to create a new page under the Settings section of the Switchyard Admin, create the file `src/admin/routes/settings/algolia/page.tsx` with the following content:

```tsx title="src/admin/routes/settings/algolia/page.tsx"
import { Container, Heading, Button, toast } from "@switchyard/ui"
...
You'll now test out the entire flow, starting from triggering the reindex manually from the Switchyard Admin dashboard, to checking the Algolia dashboard for the indexed products.

Run the following command to start the Switchyard application:

```bash npm2yarn
npm run dev
```

Then, open the Switchyard Admin at `http://localhost:9000/app` and log in with the credentials you set up in the first step.
...
![The Algolia Sync page in the Switchyard Admin dashboard with a button to sync products to Algolia](https://res.cloudinary.com/dza7lstvk/image/upload/v1742820813/Switchyard%20Resources/Screenshot_2025-03-24_at_2.52.31_PM_eiegzb.png)

You can check that the sync ran and was completed by checking the Switchyard logs in the terminal where you started the Switchyard application. You should find the following messages:
...
![The Algolia dashboard showing the indexed products](https://res.cloudinary.com/dza7lstvk/image/upload/v1742821034/Switchyard%20Resources/Screenshot_2025-03-24_at_2.56.38_PM_mtojrv.png)
...
Then, either create a product or update an existing one using the Switchyard Admin dashboard. If you check the Algolia dashboard, you'll find that the product was created or updated.
...
To test product deletion, start the Switchyard application:

```bash npm2yarn
npm run dev
```

Then, delete a product from the Switchyard Admin dashboard. If you check the Algolia dashboard, you'll find that the product was deleted there as well.

---

## Step 6: Add Search API Route

Before customizing the storefront to show the search UI, you'll create an API route in your Switchyard application that allows storefronts to search products in Algolia.

<Note title="Tip">

While you can implement the search functionality directly in the storefront to interact with Algolia, this approach centralizes your search integration in Switchyard, allowing you to change or modify the integration as necessary. You can also rely on the same behavior and results across different storefronts.
...
export async function POST(
  req: SwitchyardRequest<SearchRequest>,
  res: SwitchyardResponse
) {
...
The last step is to provide the search functionalities to customers on your storefront. In the first step, you installed the [Next.js Starter Storefront](../../../nextjs-starter/page.mdx) along with the Switchyard application.
...
So, if your Switchyard application's directory is `medusa-search`, you can find the storefront by going back to the parent directory and changing to the `medusa-search-storefront` directory:
...
Next, you need to configure the search client. Not only do you need to initialize Algolia, but you also need to change the searching mechanism to use your custom API route in the Switchyard application instead of Algolia's API directly.
...
You also define a `search` method that sends a `POST` request to the search API route you created in the Switchyard application. You use the JS SDK (which is initialized in this same file) to send the request.
...
Do not expose your Admin API key in the storefront. The Admin API key should only be used in the Switchyard application to interact with Algolia, as it has full access to your Algolia account.
...
To test out the storefront changes and the search API route, start the Switchyard application:
...
![The Next.js Starter Storefront showing the search modal with search results](https://res.cloudinary.com/dza7lstvk/image/upload/v1742827777/Switchyard%20Resources/Screenshot_2025-03-24_at_4.49.23_PM_kzhldx.png)

---

## Next Steps

You've now integrated Algolia with Switchyard and added search functionality to your storefront. You can expand on these features to:

- Add filters to the search results. You can do that using Algolia's [widgets](https://www.algolia.com/doc/guides/building-search-ui/widgets/showcase/react/) and customizing the search API route in Switchyard to accept filter parameters.
