...
In this tutorial, you'll learn how to build a [plugin](!docs!/learn/fundamentals/plugins) that migrates data, such as products, from Magento to Switchyard.
...
Switchyard's modular architecture allows you to build a custom digital commerce platform that meets your business requirements without the limitations of a monolithic system. By migrating from Magento to Switchyard, you can take advantage of Switchyard's modern technology stack to build a scalable and flexible commerce platform that grows with your business.

By following this tutorial, you'll create a Switchyard plugin that migrates data from a Magento server to a Switchyard application in minimal time. You can re-use this plugin across multiple Switchyard applications, allowing you to adopt Switchyard across your projects.
...
- Implement a Magento Module in the plugin to connect to Magento's APIs and retrieve products.
    - This guide will only focus on migrating product data from Magento to Switchyard. You can extend the implementation to migrate other data, such as customers, orders, and more.
- Trigger data migration from Magento to Switchyard in a scheduled job.

You can follow this tutorial whether you're new to Switchyard or an advanced Switchyard developer.

![Diagram showcasing the flow of migrating data from Magento to Switchyard](https://res.cloudinary.com/dza7lstvk/image/upload/v1739360550/Switchyard%20Resources/magento-summary_hsewci.jpg)

<Card
  title="Example Repository"
  text="Find the full code of the guide in this repository. The repository also includes additional features, such as triggering migrations from the Switchyard Admin dashboard."
  href="https://github.com/switchyard/examples/tree/main/migrate-from-magento"
  icon={Github}
/>
...
You'll first install a Switchyard application that exposes core commerce features through REST APIs. You'll later install the Magento plugin in this application to test it out.
...
The Switchyard application is composed of a headless Node.js server and an admin dashboard. The storefront is installed or custom-built separately and connects to the Switchyard application through its REST endpoints, called [API routes](!docs!/learn/fundamentals/api-routes). Refer to the [Switchyard Architecture](!docs!/learn/introduction/architecture) documentation to learn more.
...
```bash npm2yarn
npx create-switchyard-app@latest medusa-plugin-magento --plugin
```
...
npx switchyard plugin:add medusa-plugin-magento
```
...
To connect to external applications in Switchyard, you create a custom module. A module is a reusable package with functionalities related to a single feature or domain. Switchyard integrates the module into your application without implications or side effects on your setup.
...
export default class MagentoModuleService {
  ...
  async authenticate() {
    ...
    if (!response.ok) {
      throw new SwitchyardError(SwitchyardError.Types.UNAUTHORIZED, `Failed to authenticate with Magento: ${token}`)
    }
...
If the request is successful, you store the access token and its expiration date in the `accessToken` property. If the request fails, you throw a `SwitchyardError` with the error message returned by Magento.
...
![Diagram showcasing the types file to create](https://res.cloudinary.com/dza7lstvk/image/upload/v1739346287/Switchyard%20Resources/magento-3_fpghog.jpg)
...
![Diagram showcasing the module definition file to create](https://res.cloudinary.com/dza7lstvk/image/upload/v1739348316/Switchyard%20Resources/magento-4_bmepvh.jpg)
...
As mentioned earlier when you registered the plugin in the Switchyard Application's `switchyard.config.ts` file, you can pass options to the plugin. These options are then passed to the modules in the plugin.

So, add the following options to the plugin's registration in the `switchyard.config.ts` file of the Switchyard application:
...
- `MAGENTO_IMAGE_BASE_URL`: The base URL to use for product images. Magento stores product images in the `pub/media/catalog/product` directory, so you can reference them directly or use a CDN URL. If the URLs of product images in the Switchyard server already have a different base URL, you can omit this option.

<Note title="Tip">

Switchyard supports integrating third-party services, such as [S3](../../../infrastructure-modules/file/s3/page.mdx), in a File Module Provider. Refer to the [File Module](../../../infrastructure-modules/file/page.mdx) documentation to find other module providers and how to create a custom provider.
...
In this section, you'll add the feature to migrate products from Magento to Switchyard. To implement this feature, you'll use a workflow.
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        ...
        description: "Retrieve Switchyard store details, which you'll need when creating the products.",
        depth: 1,
      ...
      },
      {
        type: "step",
        name: "useQueryGraphStep",
        description: "Retrieve Magento products that are already in Switchyard to update them, instead of creating them.",
        depth: 1,
        link: "/references/helper-steps/useQueryGraphStep"
      },
      {
        ...
        description: "Create products in the Switchyard application.",
        depth: 1,
      ...
      },
      {
        ...
        description: "Update existing products in the Switchyard application.",
        depth: 1,
...
You only need to implement the `getMagentoProductsStep` step, which retrieves the products from Magento. The other steps and workflows are provided by Switchyard's `@switchyard/medusa/core-flows` package.
...
If you get a type error on resolving the Magento Module, run the Switchyard application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](!docs!/learn/fundamentals/generated-types).

</Note>

![Diagram showcasing the get-magento-products file to create](https://res.cloudinary.com/dza7lstvk/image/upload/v1739349590/Switchyard%20Resources/magento-5_ueb4wn.jpg)
...
You'll now create the workflow that migrates products from Magento using the step you created and steps from Switchyard's `@switchyard/medusa/core-flows` package.

In your plugin, create the file `src/workflows/migrate-products-from-magento.ts` with the following content:

![Diagram showcasing the migrate-products-from-magento file to create](https://res.cloudinary.com/dza7lstvk/image/upload/v1739349820/Switchyard%20Resources/magento-6_jjdaxj.jpg)
...
1. An object with the workflow's configuration, including the name and whether to store the workflow's executions. You enable storing the workflow execution so that you can view it later in the Switchyard Admin dashboard.
...
Next, you'll retrieve the Switchyard store details and shipping profiles. These are necessary to prepare the data of the products to create or update.
...
You use the `useQueryGraphStep` step to retrieve the store details and shipping profiles. `useQueryGraphStep` is a Switchyard step that wraps [Query](!docs!/learn/fundamentals/module-links/query), allowing you to use it in a workflow. Query is a tool that retrieves data across modules.
...
Since the Switchyard application creates an internal representation of the workflow's constructor function, you can't manipulate data directly, as variables have no value while creating the internal representation.
...
In the transformation function, you return the IDs of the Magento products. Then, you use the `useQueryGraphStep` to retrieve products in the Switchyard application that have an `external_id` property matching the IDs of the Magento products. You'll use this property to store the IDs of the products in Magento.
...
export const prepareHighlights = [
  ...
  ["24", "external_id", "Set the Magento product's ID in the Switchyard product's `external_id` property."],
  ...
  ["33", "existingProduct", "Find the existing product in Switchyard that matches the Magento product's ID."],
  ["39", "options", "Map the Magento product's configurable product options to Switchyard product options."],
  ["49", "variants", "Map the Magento product's children to Switchyard product variants."],
  ["62", "existingVariant", "Find the existing variant in the existing product, if set."],
  ["68", "prices", "Set the variant's prices based on the Magento child's price for every supported currency in the Switchyard store."],
  ["77", "id", "Set the ID of the existing variant, if available."],
  ["81", "images", "Map the Magento product's media gallery entries to Switchyard product images."],
...
You use `transform` again to prepare the data to create and update the products in the Switchyard application. For each Magento product, you map its equivalent Switchyard product's data:
...
- You map the Magento product's configurable product options to Switchyard product options. In Switchyard, a product's option has a label, such as "Color", and values, such as "Red". To map the option values, you use the attributes retrieved from Magento.
- You map the Magento product's children to Switchyard product variants. For the variant options, you pass an object whose keys is the option's label, such as "Color", and values is the option's value, such as "Red". For the prices, you set the variant's price based on the Magento child's price for every supported currency in the Switchyard store. Also, you set the Magento child product's ID in the Switchyard variant's `metadata.external_id` property.
- You map the Magento product's media gallery entries to Switchyard product images. You filter out the thumbnail image and set the URL and the Magento image's ID in the Switchyard image's `metadata.external_id` property.
...
You use the `createProductsWorkflow` and `updateProductsWorkflow` workflows from Switchyard's `@switchyard/medusa/core-flows` package to create and update the products in the Switchyard application.
...
You can now use this workflow to migrate products from Magento to Switchyard. You'll learn how to use it in the next steps.

---

## Step 6: Schedule Product Migration

There are many ways to execute tasks asynchronously in Switchyard, such as [scheduling a job](!docs!/learn/fundamentals/scheduled-jobs) or [handling emitted events](!docs!/learn/fundamentals/events-and-subscribers). 

In this guide, you'll learn how to schedule the product migration at a specified interval using a scheduled job. A scheduled job is an asynchronous function that the Switchyard application runs at the interval you specify during the Switchyard application's runtime.
...
![Diagram showcasing the migrate-magento file to create](https://res.cloudinary.com/dza7lstvk/image/upload/v1739358924/Switchyard%20Resources/magento-7_rqoodo.jpg)
...
- An asynchronous function that executes the job's logic. The function receives the [Switchyard container](!docs!/learn/fundamentals/medusa-container) as a parameter.
...
Based on the job's configurations, the Switchyard application will run the job at midnight every day.
...
This ensures that the plugin's latest changes are reflected in the Switchyard application.

Finally, start the Switchyard application that the plugin is installed in:
...
To confirm that the products were migrated, open the Switchyard Admin dashboard at `http://localhost:9000/app` and log in. Then, click on Products in the sidebar. You'll see your magento products in the list of products.

![Click on products at the sidebar on the right, then view the products in the table in the middle.](https://res.cloudinary.com/dza7lstvk/image/upload/v1739359394/Switchyard%20Resources/Screenshot_2025-02-12_at_1.22.44_PM_uva98i.png)

---

## Next Steps

You've now implemented the logic to migrate products from Magento to Switchyard. You can re-use the plugin across Switchyard applications. You can also expand on the plugin to:
...
- Allow triggering migrations from the Switchyard Admin dashboard using [Admin Customizations](!docs!/learn/fundamentals/admin). This feature is available in the [Example Repository](https://github.com/switchyard/example-repository/tree/main/src/admin).
