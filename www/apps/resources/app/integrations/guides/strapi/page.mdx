...
export const metadata = {
  title: `Integrate Strapi (CMS) with Switchyard`,
}

# {metadata.title}

In this tutorial, you'll learn how to integrate [Strapi](https://strapi.io/) with Switchyard.
...
This guide was built with Strapi v5.30.1. If you're using a different version and you run into issues, consider [opening an issue](https://github.com/switchyard/medusa/issues/new?template=docs.yml).
...
- Integrate Strapi with Switchyard to interact with Strapi's API.
- Implement two-way synchronization of product data between Switchyard and Strapi:
    - Handle product events to sync data from Switchyard to Strapi.
    - Handle Strapi webhooks to sync data from Strapi to Switchyard.
- Display product data from Strapi in the Next.js Starter Storefront.

You can follow this tutorial whether you're new to Switchyard or an advanced Switchyard developer, but you're expected to have knowledge in Strapi, as its concepts are not explained in the tutorial.

![Diagram illustrating the flow of data between Switchyard, Strapi, admin, and customer (storefront)](https://res.cloudinary.com/dza7lstvk/image/upload/v1763375209/Switchyard%20Resources/strapi-summary_pioikw.jpg)

<Card
  title="Full Code"
  text="Find the full code of the guide in this repository."
  href="https://github.com/switchyard/examples/tree/main/strapi-integration"
  icon={Github}
/>
...
In a separate directory from your Switchyard application, run the following command to create a new Strapi project:
...
In this section, you'll define a content type for products in Strapi. These products will be synced from Switchyard, allowing you to manage their content using Strapi's CMS features.
...
```json title="src/api/product/content-types/product/schema.json" badgeLabel="Strapi" badgeColor="orange"
{
  ...
  "info": {
    ...
    "pluralName": "products",
    "displayName": "Product",
    "description": "Products from Switchyard"
  },
  "options": {
...
1. `medusaId`: A unique identifier that maps to the Switchyard product ID.
...
```json title="src/api/product-variant/content-types/product-variant/schema.json" badgeLabel="Strapi" badgeColor="orange"
{
  ...
  "info": {
    ...
    "description": "Product variants from Switchyard"
  },
  "options": {
    "draftAndPublish": false
  },
...
1. `medusaId`: A unique identifier that maps to the Switchyard product variant ID.
...
```json title="src/api/product-option/content-types/product-option/schema.json" badgeLabel="Strapi" badgeColor="orange"
{
  ...
  "info": {
    ...
    "description": "Product options from Switchyard"
  },
  "options": {
    "draftAndPublish": false
  },
...
1. `medusaId`: A unique identifier that maps to the Switchyard product option ID.
...
```json title="src/api/product-option-value/content-types/product-option-value/schema.json" badgeLabel="Strapi" badgeColor="orange"
{
  ...
  "info": {
    ...
    "description": "Product option values from Switchyard"
  },
  "options": {
    "draftAndPublish": false
  },
...
1. `medusaId`: A unique identifier that maps to the Switchyard product option value ID.
...
You now have all the customizations in Strapi ready. You'll return to Strapi later after you set up the integration with Switchyard.

---

## Step 3: Integrate Strapi with Switchyard

In this step, you'll integrate Strapi with Switchyard by creating a Strapi Module.

A [module](!docs!/learn/fundamentals/modules) is a reusable package that provides functionalities related to a single feature or domain. Switchyard integrates the module into your application without implications or side effects on your setup.
...
First, you'll install the Strapi client in your Switchyard application to interact with Strapi's API.

In your Switchyard application directory, run the following command to install the Strapi client:

```bash badgeLabel="Switchyard application" badgeColor="green"
npm install @strapi/client
```

### b. Create Module Directory

A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/strapi`.

### c. Create Strapi Client Loader

Next, you'll create the Strapi client when the Switchyard server starts by creating a loader.

A [loader](!docs!/learn/fundamentals/modules/loaders) is an asynchronous function that runs when the Switchyard server starts. Loaders are useful for setting up connections to third-party services and reusing those connections throughout your module.
...
```ts title="src/modules/strapi/loaders/init-client.ts" badgeLabel="Switchyard application" badgeColor="green"
...
```ts title="src/modules/strapi/service.ts" badgeLabel="Switchyard application" badgeColor="green"
...
This method takes an image ID as a parameter and deletes the corresponding image from Strapi using the Strapi client's files API. If the deletion fails, it throws a `SwitchyardError` with a formatted error message.
...
Next, you'll add a method to update a document of a content type in Strapi. This will be useful to implement two-way synching between Switchyard and Strapi.
...
Next, you'll add a method to delete a document of a content type from Strapi. You'll use this method when a document is deleted in Switchyard, or when reverting document creation in case of failures.
...
#### Retrieve Document by Switchyard ID Method

Next, you'll add a method to retrieve a document of a content type from Strapi by its Switchyard ID. This will be useful to retrieve a document in case you need to revert changes.
...
1. `collection`: The collection (content type) in which the document exists. It uses the `Collection` enum.
2. `medusaId`: The Switchyard ID of the document to be retrieved.
...
The final piece of a module is its definition, which you export in an `index.ts` file at the module's root directory. This definition tells Switchyard the name of the module, its service, and optionally its loaders.

To create the module's definition, create the file `src/modules/strapi/index.ts` with the following content:

```ts title="src/modules/strapi/index.ts" badgeLabel="Switchyard application" badgeColor="green"
...
### f. Add Module to Switchyard's Configurations
...
![Strapi dashboard with the API Token settings opened](https://res.cloudinary.com/dza7lstvk/image/upload/v1763136134/Switchyard%20Resources/CleanShot_2025-11-14_at_18.00.44_2x_ec8lds.png)

4. In the API token form:
    - Set a name for the token, such as "Switchyard".
...
![Strapi dashboard with the Create API Token form filled](https://res.cloudinary.com/dza7lstvk/image/upload/v1763136134/Switchyard%20Resources/CleanShot_2025-11-14_at_18.01.25_2x_hdex6b.png)

Then, copy the generated API token.

Finally, set the following environment variables in your Switchyard project's `.env` file:
...
In this step, you'll define a virtual read-only link between the product content type in Strapi and the `Product` model in Switchyard. Later, you'll be able to retrieve products from Strapi when retrieving products in Switchyard.

### a. Define the Link

To define a virtual read-only link, create the file `src/links/product-strapi.ts` with the following content:

```ts title="src/links/product-strapi.ts" badgeLabel="Switchyard application" badgeColor="green"
...
- An object of the second data model that is part of the link. In this case, it's the product content type from the Strapi Module. You set the following properties:
    ...
    - `primaryKey`: the primary key of the linked data model, which is `product_id`. Switchyard will look for this field in the retrieved `Products` from Strapi to match it with the `id` field of the `Product` model.
...
When you retrieve products from Switchyard with their `strapi_product` link, Switchyard will call the `list` method of the Strapi Module's service to retrieve the linked products from Strapi.
...
The `list` method receives a `filter` object with a `product_id` property, which contains the Switchyard product ID(s) to retrieve their corresponding data from Strapi.
...
You can now retrieve product data from Strapi when retrieving products in Switchyard. You'll learn how to do this in the upcoming steps.

---

## Step 5: Handle Product Creation

In this step, you'll implement the logic to listen to product creation events in Switchyard and create the corresponding product data in Strapi.
...
2. A [subscriber](!docs!/learn/fundamentals/events-and-subscribers) that listens to the product creation event in Switchyard and triggers the workflow.
...
The first step is available out-of-the-box in Switchyard. You need to create the rest of the steps.
...
```ts title="src/workflows/steps/create-options-in-strapi.ts" badgeLabel="Switchyard application" badgeColor="green"
...
In the step function, you resolve the Strapi Module's service from the Switchyard container. Then, you loop through the product options and create them in Strapi using the service's `create` method.
...
```ts title="src/workflows/steps/create-option-values-in-strapi.ts" badgeLabel="Switchyard application" badgeColor="green"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
import { STRAPI_MODULE } from "../../modules/strapi"
import StrapiModuleService, { Collection } from "../../modules/strapi/service"
...
The `updateProductOptionValuesMetadataStep` stores the Strapi IDs of the created product option values in the `metadata` property of the corresponding product option values in Switchyard. This allows you to reference the Strapi option values later, such as when updating or deleting them.
...
```ts title="src/workflows/steps/update-product-option-values-metadata.ts" badgeLabel="Switchyard application" badgeColor="green"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
import { Modules } from "@switchyard/framework/utils"
import { ProductOptionValueDTO } from "@switchyard/framework/types"
...
```ts title="src/workflows/create-options-in-strapi.ts" badgeLabel="Switchyard application" badgeColor="green" collapsibleLines="1-15" expandButtonLabel="Show Imports"
import { 
  createWorkflow, 
  WorkflowResponse,
  transform,
...
1. Retrieve the product options in Switchyard using the `useQueryGraphStep`.
    - This step uses [Query](!docs!/learn/fundamentals/module-links/query) to retrieve data in Switchyard across modules.
...
In a workflow, you can't manipulate data because Switchyard stores an internal representation of the workflow on application startup. Learn more in the [Data Manipulation](!docs!/learn/fundamentals/workflows/variable-manipulation) documentation.
...
The first, second, and last steps are available out-of-the-box in Switchyard. You need to create the rest of the steps.
...
```ts title="src/workflows/steps/upload-images-to-strapi.ts" badgeLabel="Switchyard application" badgeColor="green"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
import { STRAPI_MODULE } from "../../modules/strapi"
...
```ts title="src/workflows/steps/create-variants-in-strapi.ts" badgeLabel="Switchyard application" badgeColor="green"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
import { STRAPI_MODULE } from "../../modules/strapi"
import StrapiModuleService, { Collection } from "../../modules/strapi/service"
...
The `updateProductVariantsMetadataStep` stores the Strapi IDs of the created product variants in the `metadata` property of the corresponding product variants in Switchyard. This allows you to reference the Strapi variants later, such as when updating or deleting them.

To create the step, create the file `src/workflows/steps/update-product-variants-metadata.ts` with the following content:

```ts title="src/workflows/steps/update-product-variants-metadata.ts" badgeLabel="Switchyard application" badgeColor="green"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
import { Modules } from "@switchyard/framework/utils"
import { ProductVariantDTO } from "@switchyard/framework/types"
...
```ts title="src/workflows/create-variants-in-strapi.ts" badgeLabel="Switchyard application" badgeColor="green" collapsibleLines="1-12" expandButtonLabel="Show Imports"
import { 
  createWorkflow, 
  WorkflowResponse,
  transform,
...
import { acquireLockStep, releaseLockStep, useQueryGraphStep } from "@switchyard/medusa/core-flows"
...
The workflow receives the IDs of the product variants to create in Strapi and the Switchyard product ID they belong to.
...
2. Retrieve the product variants in Switchyard using the `useQueryGraphStep`.
...
In a workflow, you can't perform steps based on conditions because Switchyard stores an internal representation of the workflow on application startup. Learn more in the [Conditions in Workflows](!docs!/learn/fundamentals/workflows/conditions) documentation.
...
You only need to create the `createProductInStrapiStep` step. The rest of the steps and workflows are either available out-of-the-box in Switchyard or you have already created them.
...
```ts title="src/workflows/steps/create-product-in-strapi.ts" badgeLabel="Switchyard application" badgeColor="green"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
import { STRAPI_MODULE } from "../../modules/strapi"
import StrapiModuleService, { Collection } from "../../modules/strapi/service"
...
```ts title="src/workflows/create-product-in-strapi.ts" badgeLabel="Switchyard application" badgeColor="green"
import { 
  createWorkflow, 
  WorkflowResponse,
  transform,
...
2. Retrieve the product in Switchyard using the `useQueryGraphStep`.
...
Finally, you need to create a subscriber that listens to the product creation event in Switchyard and triggers the `createProductInStrapiWorkflow`.
...
```ts title="src/subscribers/product-created-strapi-sync.ts" badgeLabel="Switchyard application" badgeColor="green"
import {
...
Then, run the following command in the Switchyard application's directory to start the Switchyard server:

```bash npm2yarn
npm run dev
```

Next, open the Switchyard Admin dashboard and [create a new product](!user-guide!/products/create). Once you create the product, you'll see the following in the Switchyard server logs:
...
Next, you'll handle product updates in Strapi and synchronize the changes back to Switchyard. You'll create a workflow to update the relevant product data in Switchyard based on the data received from Strapi.

Then, you'll create an API route webhook that Strapi can call whenever product data is updated. With this setup, you'll have two-way synchronization between Switchyard and Strapi for product data.
        ...
        steps: [
          {
            type: "workflow",
            name: "updateProductsWorkflow",
            description: "If the data updated in Strapi is product data, update the product in Switchyard.",
            link: "/references/medusa-workflows/updateProductsWorkflow",
            depth: 1
          },
          {
        ...
        steps: [
          {
            ...
            description: "If the data updated in Strapi is product variant data, update the product variant in Switchyard.",
        ...
        steps: [
          {
            ...
            description: "If the data updated in Strapi is product option data, update the product option in Switchyard.",
        ...
        steps: [
          {
            ...
            description: "If the data updated in Strapi is product option value data, update the product option value in Switchyard.",
            depth: 1
          },
          {
            type: "step",
...
You only need to create the `prepareStrapiUpdateDataStep`, `clearProductCacheStep`, and `updateProductOptionValueStep` steps. The rest of the steps and workflows are available out-of-the-box in Switchyard.
...
```ts title="src/workflows/steps/prepare-strapi-update-data.ts" badgeLabel="Switchyard application" badgeColor="green"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
...
The `clearProductCacheStep` clears the product cache in Switchyard to ensure that updated data is served to clients. This is necessary as you'll enable [caching](../../../infrastructure-modules/caching/page.mdx) later, which may cause stale data to be served to the storefront.

To create the step, create the file `src/workflows/steps/clear-product-cache.ts` with the following content:

```ts title="src/workflows/steps/clear-product-cache.ts" badgeLabel="Switchyard application" badgeColor="green"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
import { Modules } from "@switchyard/framework/utils"
...
The `updateProductOptionValueStep` updates product option values in Switchyard based on the data received from Strapi.

To create the step, create the file `src/workflows/steps/update-product-option-value.ts` with the following content:

```ts title="src/workflows/steps/update-product-option-value.ts" badgeLabel="Switchyard application" badgeColor="green"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
import { Modules } from "@switchyard/framework/utils"
...
In the step, you resolve the Product Module's service and update the option value in Switchyard.
...
```ts title="src/workflows/handle-strapi-webhook.ts" badgeLabel="Switchyard application" badgeColor="green" collapsibleLines="1-19" expandButtonLabel="Show Imports"
import { 
  createWorkflow, 
  when,
  transform,
...
2. Check if the updated model is a product using `when`. If so, you:
    - Update the product in Switchyard using the `updateProductsWorkflow`.
...
3. Check if the updated model is a product variant using `when`. If so, you
    - Update the product variant in Switchyard using the `updateProductVariantsWorkflow`.
    - Clear the product cache using the `clearProductCacheStep`.
4. Check if the updated model is a product option using `when`. If so, you:
    - Update the product option in Switchyard using the `updateProductOptionsWorkflow`.
    - Clear the product cache using the `clearProductCacheStep`.
5. Check if the updated model is a product option value using `when`. If so, you:
    - Update the product option value in Switchyard using the `updateProductOptionValueStep`.
...
```ts title="src/api/webhooks/strapi/route.ts" badgeLabel="Switchyard application" badgeColor="green"
import { SwitchyardRequest, SwitchyardResponse } from "@switchyard/framework/http"
import { simpleHash, Modules } from "@switchyard/framework/utils"
import { 
...
3. Generate a hash of the webhook payload to detect duplicate webhook calls. This is necessary since you've implemented two-way synchronization between Switchyard and Strapi, which may lead to infinite loops of updates.
...
```ts title="src/api/middlewares.ts" badgeLabel="Switchyard application" badgeColor="green"
import { 
  defineMiddlewares, 
  ...
  routes: [
    {
      ...
      middlewares: [
        async (
          ...
          try {
            // Validate the API key using Switchyard's API Key Module
...
### d. Enable Caching in Switchyard
...
The [Caching Module](../../../infrastructure-modules/caching/page.mdx) is currently guarded by a feature flag. To enable it, add the feature flag and module in your `switchyard.config.ts` file:
...
You can now use the Caching Module's service in your workflows and API routes. Switchyard will also cache product and cart data automatically to improve performance.

### e. Webhook Handling Preparation

Before you test the webhook handling, you need to create a secret API key in Switchyard, then configure webhooks in Strapi.

Make sure to start both the Switchyard and Strapi servers if they are not already running.

#### Create Secret API Key in Switchyard

To create the secret API key in Switchyard:

1. Open the Switchyard Admin dashboard.
...
![Switchyard Admin dashboard with the Secret API Keys page showing the Strapi API key](https://res.cloudinary.com/dza7lstvk/image/upload/v1763368257/Switchyard%20Resources/CleanShot_2025-11-17_at_10.20.28_2x_b6cilm.png)

#### Configure Webhook in Strapi

Next, you need to configure a webhook in Strapi to call the Switchyard webhook API route whenever product data is updated.
...
4. In the webhook creation form:
   - **Name**: Enter a name for the webhook. For example, "Switchyard".
   - **URL**: Enter the URL of the Switchyard webhook API route. It should be `http://localhost:9000/webhooks/strapi` if you're running Switchyard locally.
   - **Headers**: Add a new header with the key `Authorization` and the value `Bearer YOUR_SECRET_API_KEY`. Replace `YOUR_SECRET_API_KEY` with the API key you created in Switchyard.
...
![Strapi Webhook Creation form](https://res.cloudinary.com/dza7lstvk/image/upload/v1763368257/Switchyard%20Resources/CleanShot_2025-11-17_at_10.30.22_2x_zohl1q.png)

### Test Strapi Webhook Handling

To test out the webhook handling:

1. Make sure both the Switchyard and Strapi servers are running.
...
Once you save the changes, Strapi will send a webhook to Switchyard. You should see the following in the Switchyard server logs:
...
You can also check the product in the Switchyard Admin dashboard to verify that the changes made in Strapi are reflected in Switchyard.

---

## Step 7: Show Strapi Data in Storefront

Now that you've integrated Strapi with Switchyard, you can customize the Next.js Starter Storefront to display product content from Strapi, allowing you to show product content and assets optimized for the storefront.
...
The Next.js Starter Storefront was installed in a separate directory from Switchyard. The directory's name is `{your-project}-storefront`.

So, if your Switchyard application's directory is `medusa-strapi`, you can find the storefront by going back to the parent directory and changing to the `medusa-strapi-storefront` directory:
...
Since you've created a [virtual read-only link](#step-4-create-virtual-read-only-link-to-strapi-products) to Strapi products in Switchyard, you can retrieve Strapi product data when retrieving Switchyard products.
...
The Strapi product data will now be included in the `strapi_product` property of each Switchyard product.
...
/**
 * Get Strapi product data from a Switchyard product
 */
...
/**
 * Get product title from Strapi, fallback to Switchyard
 */
...
/**
 * Get product description from Strapi, fallback to Switchyard
 */
...
/**
 * Get product thumbnail from Strapi, fallback to Switchyard
 */
...
/**
 * Get product images from Strapi, fallback to Switchyard
 */
export function getProductImages(
  ...
  if (strapiProduct?.images && strapiProduct.images.length > 0) {
    // Convert Strapi media to Switchyard product image format
...
/**
 * Get variant title from Strapi, fallback to Switchyard
 */
...
/**
 * Get option title from Strapi, fallback to Switchyard
 */
...
/**
 * Get option value text from Strapi, fallback to Switchyard
 */
...
export function getVariantImages(
  ...
  // Fall back to Switchyard variant images
...
- `getStrapiProduct`: Retrieves the Strapi product data from a Switchyard product.
- `getProductTitle`: Retrieves the product title from Strapi, falling back to Switchyard if not available.
- `getProductSubtitle`: Retrieves the product subtitle from Strapi.
- `getProductDescription`: Retrieves the product description from Strapi, falling back to Switchyard if not available.
- `getProductThumbnail`: Retrieves the product thumbnail from Strapi, falling back to Switchyard if not available.
- `getProductImages`: Retrieves the product images from Strapi, falling back to Switchyard if not available.
- `getVariantTitle`: Retrieves the variant title from Strapi, falling back to Switchyard if not available.
- `getOptionTitle`: Retrieves the option title from Strapi, falling back to Switchyard if not available.
- `getOptionValueText`: Retrieves the option value text from Strapi, falling back to Switchyard if not available.
- `getVariantOptionValues`: Retrieves all option values for a variant with Strapi labels.
- `getVariantImages`: Retrieves images for a specific variant from Strapi, falling back to Switchyard if not available.
...
function getImagesForVariant(
  ...
  // Get Strapi images or fallback to Switchyard images
  ...
  // Get variant images from Strapi or fallback to Switchyard
...
To test the storefront customizations, make sure both the Switchyard and Strapi servers are running.
...
Your setup now supports creating products in Strapi when they're created in Switchyard. However, you should also support updating and deleting products and their related models to keep data in sync between systems.
...
You can find all workflows and subscribers for product events in the [Strapi Integration Repository](https://github.com/switchyard/examples/tree/main/strapi-integration/medusa).

---

## Next Steps

You've successfully integrated Switchyard with Strapi to manage content related to products, variants, and options. You can expand this integration by adding more features, such as:
...
2. Enable [internationalization](https://docs.strapi.io/cms/features/internationalization) in Strapi to support multiple languages:
    - You only need to manage the localized content in Strapi. Only the default locale will be synced with Switchyard.
