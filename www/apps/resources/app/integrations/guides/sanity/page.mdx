...
export const ogImage = "https://res.cloudinary.com/dza7lstvk/image/upload/v1732200494/Switchyard%20Resources/opengraph-image_kstzrn.jpg"

export const metadata = {
  title: `Integrate Switchyard with Sanity (CMS)`,
  openGraph: {
    images: [
      {
        url: ogImage,
...
In this guide, you'll learn how to integrate Switchyard with Sanity.

When you install a Switchyard application, you get a fully-fledged commerce platform with support for customizations. While Switchyard allows you to manage basic content, such as product description and images, you might need rich content-management features, such as localized content. The Switchyard Framework supports you in integrating a CMS with these features.

Sanity is a CMS that simplifies managing content from third-party sources into a single interface. By integrating it with Switchyard, you can manage your storefront and commerce-related content, such as product details, from a single interface. You also benefit from advanced content-management features, such as live-preview editing.

This guide will teach you how to:

- Install and set up Switchyard.
- Install and set up Sanity with Switchyard's Next.js Starter storefront.
- Sync product data from Switchyard to Sanity when a product is created or updated.
- Customize the Switchyard Admin dashboard to check the sync status and trigger syncing products to Sanity.

You can follow this guide whether you're new to Switchyard or an advanced Switchyard developer. This guide also assumes you're familiar with Sanity concepts, which you can learn about in [their documentation](https://www.sanity.io/docs).

<Card
  title="Example Repository"
  text="Find the full code of the guide in this repository."
  href="https://github.com/switchyard/examples/tree/main/sanity-integration"
  icon={Github}
/>
...
Afterwards, the installation process will start, which will install the Switchyard application in a directory with your project's name, and the Next.js Starter Storefront in a directory with the `{project-name}-storefront` name.
...
Once the installation finishes successfully, the Switchyard Admin dashboard will open with a form to create a new user. Enter the user's credential and submit the form.
...
In this step, you'll install [Sanity's JavaScript client SDK](https://www.sanity.io/docs/js-client) in the Switchyard application, which you'll use later in your code when sending requests to Sanity.

In your terminal, move to the Switchyard application's directory and run the following command:
...
When the Switchyard application connects to Sanity, it must connect to a project in Sanity.

So, before building the integration in Switchyard, create a project in Sanity using their website:
...
![The Create new project button is at the top of the dashboard page.](https://res.cloudinary.com/dza7lstvk/image/upload/v1732091565/Switchyard%20Resources/Screenshot_2024-11-20_at_10.31.10_AM_vvq7y6.png)

3. Enter a project name and click "Create Project"

![A pop-up form will open where you can choose project name and organization.](https://res.cloudinary.com/dza7lstvk/image/upload/v1732091565/Switchyard%20Resources/Screenshot_2024-11-20_at_10.32.33_AM_xb0rsn.png)
...
To integrate third-party services into Switchyard, you create a custom module. A module is a re-usable package with functionalities related to a single feature or domain. Switchyard integrates the module into your application without implications or side effects on your setup.
...
A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/sanity`.
...
Switchyard registers the module's service in the [Switchyard container](!docs!/learn/fundamentals/medusa-container), allowing you to easily resolve the service from other customizations and use its methods.

<Note title="What is the Switchyard Container?">

The Switchyard application registers resources, such as a module's service or the [logging tool](!docs!/learn/debugging-and-testing/logging), in the Switchyard container so that you can resolve them from other customizations, as you'll see in later sections. Learn more about it in [this documentation](!docs!/learn/fundamentals/medusa-container).
...
- `logger` property, which is an instance of Switchyard's [Logger](!docs!/learn/debugging-and-testing/logging), to log messages.
...
- `type_map`: The types to sync from Switchyard to Sanity. For simplicity, this guide only covers syncing products, but you can support other data types like product categories, too.
- `studio_url`: The URL to the Sanity studio. This is used to show the studio URL later in the Switchyard Admin dashboard.
...
1. Resources to resolve from the Module's container. A module has a different container than the Switchyard application, which you can learn more about it in [this documentation](!docs!/learn/fundamentals/modules/container).
...
So, you'll add methods to the service that transform a Switchyard product to a Sanity document object.
...
- `createTransformationMap`: Pair of `SyncDocumentTypes` values (for example, `product`) and the method used to transform a Switchyard product to a Sanity document data to be created.
- `updateTransformationMap`: Pair of `SyncDocumentTypes` values (for example, `product`) and the method used to transform a Switchyard product to a Sanity update operation.
...
You initialize the `typeMap` property to map the `product` type in Switchyard to the `product` schema type in Sanity. You also initialize the `createTransformationMap` and `updateTransformationMap` to map the methods to transform a product for creation or update.
...
In this section, you'll add the methods that accept data from Switchyard and create or update them as documents in Sanity.

Add the following methods to the `SanityModuleService` class:

export const syncMethodsHighlights = [
  ["9", "upsertSyncDocument", "Creates or updates a document in Sanity for a data type in Switchyard."],
  ["21", "createSyncDocument", "Creates a document in Sanity for a data type in Switchyard."],
  ["30", "updateSyncDocument", "Updates a document in Sanity for a data type in Switchyard."]
]
...
- `upsertSyncDocument`: Creates or updates a document in Sanity for a data type in Switchyard.
- `createSyncDocument`: Creates a document in Sanity for a data type in Switchyard. It uses the `createTransformationMap` property to use the transform method of the specified Switchyard data type (for example, a product's data).
- `updateSyncDocument`: Updates a document in Sanity for a data type in Switchyard. It uses the `updateTransformationMap` property to use the transform method of the specified Switchyard data type (for example, a product's data).
...
Next, you must export the Module definition, which lets Switchyard know what the Module's name is and what is its service.
...
In the file, you export the `SANITY_MODULE` which is the Module's name. You'll use it later when you resolve the module from the Switchyard container.
...
Finally, to register a module in Switchyard, you must add it to Switchyard's configurations.

Switchyard's configurations are set in the `switchyard.config.ts` file, which is at the root directory of your Switchyard application. The configuration object accepts a `modules` array, whose value is an array of modules to add to the application.

Add the `modules` property to the exported configurations in `switchyard.config.ts`:
...
![The API tab is at the top of the project dashboard next to Settings.](https://res.cloudinary.com/dza7lstvk/image/upload/v1732091810/Switchyard%20Resources/Screenshot_2024-11-20_at_10.35.29_AM_ltj7cd.png)

    - Scroll down to Tokens and click on the "Add API Token" button.

![The Add API token button is at the top right of the Tokens section.](https://res.cloudinary.com/dza7lstvk/image/upload/v1732091810/Switchyard%20Resources/Screenshot_2024-11-20_at_10.35.52_AM_ccgsum.png)

    - Enter a name for the API token, choose "Editor" for the permissions, then click Save.

![In the Token form, enter the name and choose "Editor" for permissions.](https://res.cloudinary.com/dza7lstvk/image/upload/v1732091811/Switchyard%20Resources/Screenshot_2024-11-20_at_10.36.25_AM_nqxa5y.png)

- `SANITY_PROJECT_ID`: The ID of the project, which you can find at the top section of your Sanity project's dashboard.

![The project ID is in the top information of the project.](https://res.cloudinary.com/dza7lstvk/image/upload/v1732091988/Switchyard%20Resources/Screenshot_2024-11-20_at_10.39.24_AM_cscir8.png)
...
To test that the module is working, you'll start the Switchyard application and see if the "Connected to Sanity" message is logged in the console.

To start the Switchyard application, run the following command in the application's directory:
...
That means your Sanity credentials were correct, and Switchyard was able to connect to Sanity.
...
However, modules are [isolated](!docs!/learn/fundamentals/modules/isolation) to ensure they're re-usable and don't have side effects when integrated into the Switchyard application. So, to build associations between modules, you define [module links](!docs!/learn/fundamentals/module-links).
...
2. The second data model part of the link. Since the Sanity Module doesn't have a Switchyard data model, you specify the configurations in a `linkable` object that has the following properties:
    - `serviceName`: The registration name in the Switchyard container of the service managing the data model, which in this case is the Sanity Module's name (since the module's service is registered under that name).
...
3. An object of configurations for the module link. By default, Switchyard creates a table in the database to represent the link you define. Since the module link isn't created between two Switchyard data models, you enable the `readOnly` configuration, which will tell Switchyard not to create a table in the database for this link.
...
After integrating Sanity with a custom module, you now want to sync product data from Switchyard to Sanity, automatically and manually. To implement the sync logic, you need a workflow.
...
In this section, you'll create a workflow that syncs products from Switchyard to Sanity. Later, you'll execute this workflow when a product is created or updated, or when an admin user triggers the syncing manually.
...
If you get a type error on resolving the Sanity Module, run the Switchyard application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](!docs!/learn/fundamentals/generated-types).

</Note>

export const syncStepHighlights = [
  ["13", "createStep", "Create a step."],
  ["15", "container", "The Switchyard container to resolve resources."],
...
The step function accepts the step's input as a first parameter, and an object of options as a second. The object of options has a `container` property, which is an instance of the Switchyard container that you can use to resolve resources.

In the step, you resolve from the Switchyard container Sanity Module's service and [Query](!docs!/learn/fundamentals/module-links/query), which is a tool that allows you to retrieve data across modules and links.
...
In the `while` loop, you loop over the array of products to sync them to Sanity. You use the `promiseAll` Switchyard utility that loops over an array of promises and ensures that all transactions within these promises are rolled back in case an error occurs.
...
## Step 7: Handle Product Changes in Switchyard

You've defined the workflow to sync the products. Now, you want to execute it when a product is created or updated.

Switchyard emits events when certain actions occur, such as when a product is created. Then, you can listen to those events in a subscriber.
...
- `container`: An instance of the Switchyard container to resolve resources.
...
To test it out, run the Switchyard application, then open the Switchyard Admin in your browser at `http://localhost:9000/app`. Try creating or updating a product. You'll see the following message in the console:
...
In this step, you'll install Sanity in the Next.js Starter and configure it. You'll then have a Sanity studio in your Next.js Starter Storefront, where you'll later view the product documents being synced from Switchyard, and update their content that you'll display in the storefront on the product details page.

Sanity has a CLI tool that helps you with the setup. First, change to the Next.js Starter's directory (it's outside the Switchyard application's directory and its name is `{project-name}-storefront`, where `{project-name}` is the name of the Switchyard application's directory).
...
![Find the API tab at the top of the dashboard.](https://res.cloudinary.com/dza7lstvk/image/upload/v1732096643/Switchyard%20Resources/Screenshot_2024-11-20_at_10.35.29_AM_ltj7cd.png)

2. Scroll down to CORS origins and click the "Add CORS origin" button.

![Find the CORS origins section and click the Add CORS origin button at its top right.](https://res.cloudinary.com/dza7lstvk/image/upload/v1732096997/Switchyard%20Resources/Screenshot_2024-11-20_at_12.02.50_PM_ahsthb.png)

3. Enter `http://localhost:8000` in the Origin field.
4. Enable the "Allow credentials" checkbox.

![After filling out the Origin field, click on the Allow credentials checkbox to enable it.](https://res.cloudinary.com/dza7lstvk/image/upload/v1732097074/Switchyard%20Resources/Screenshot_2024-11-20_at_12.04.09_PM_nxdvwh.png)
...
When you sync the products from Switchyard, you only sync the title. You manage the `specs` and `addons` fields within Sanity.
...
You add the product schema to the list of exported schemas, but also disable creating a new product. You can only create the products in Switchyard.
...
You'll find "Product Page" under Content. If you click on it, you'll find any product you've synced from Switchyard.

If you haven't synced any products yet or you want to see the live update, try now creating or updating a product in Switchyard. You'll find it added in the Sanity studio.
...
To test this out, first, run both the Next.js Starter storefront and the Switchyard application, and open the Sanity studio. Try editing the content of the first spec of a product.
...
You can now manage the product's content in Sanity, add more fields, and customize how you show them in the storefront. The Switchyard application will also automatically create documents in Sanity for new products you add or update, ensuring your products are always synced across systems.
...
The Switchyard Admin dashboard is customizable, allowing you to either inject components, called [widgets](!docs!/learn/fundamentals/admin/widgets), into existing pages, or adding new pages, called [UI routes](!docs!/learn/fundamentals/admin/ui-routes). In these customizations, you can send requests to the Switchyard application to perform custom operations.
...
![The widget in the product details page.](https://res.cloudinary.com/dza7lstvk/image/upload/v1732093722/Switchyard%20Resources/Screenshot_2024-11-20_at_11.08.23_AM_wzftfv.png)

Before you do that, however, you need two new API routes in your Switchyard application: one to retrieve a document from Sanity, and one to trigger syncing the product data.
...
To send requests from admin customizations to the Switchyard server, you need to use Switchyard's [JS SDK](../../../js-sdk/page.mdx). You'll also use [Tanstack Query](https://tanstack.com/query/latest) to benefit from features like data caching and invalidation.
...
import { AdminProduct, DetailWidgetProps } from "@switchyard/types"
import { ArrowUpRightOnBox } from "@switchyard/icons"
import { Button, CodeBlock, Container, StatusBadge, toast } from "@switchyard/ui"
...
To render a widget that matches the rest of the admin dashboard's design, you use components from the [Switchyard UI package](!ui!), such as the `CodeBlock` or `Container` components.
...
To test these customizations out, start the Switchyard application and open the admin dashboard. Then, choose a product and scroll down to the end of the page.
...
## Step 12: Add Track Syncs Page to Switchyard Admin
...
![A screenshot of the page to track and trigger syncs.](https://res.cloudinary.com/dza7lstvk/image/upload/v1732095185/Switchyard%20Resources/Screenshot_2024-11-20_at_11.09.42_AM_te8xic.png)

### Retrieve Sync Executions API Route

Switchyard has a [workflow engine](../../../infrastructure-modules/workflow-engine/page.mdx) that manages workflow executions, roll-backs, and other functionalities under the hood.
...
You add a `GET` API route at `/admin/sanity/syncs`. In the API route handler, you resolve the Workflow Engine Module's service from the Switchyard container. You use the `listAndCountWorkflowExecutions` method to retrieve the executions of the `sanitySyncProductsWorkflow` workflow, filtering by its name.
...
You return the transaction ID of the workflow, which you can use to track the execution's progress since the workflow will run in the background. This is not implemented in this guide, but Switchyard has a [Get Execution API route](!api!/admin#workflows-executions_getworkflowsexecutionsworkflow_idtransaction_id) that you can use to get the details of a workflow's execution.

### Add Sanity UI Route

In this section, you'll add a UI route in the admin dashboard, which is a new page, that shows the list of `sanitySyncProductsWorkflow` executions and allows triggering sync of all products in Switchyard.

A UI route is React component exported in a file under the `src/admin/routes` directory. Similar to a widget, a UI route can also send requests to the Switchyard application to perform actions using your custom API routes.
...
import { Sanity } from "@switchyard/icons"
import {
  Badge,
  Button,
  Container,
...
In the UI route, you use the `useSanitySyncs` hook to retrieve the list of sync executions and display them with their status. You also show a "Trigger Sync" button that, when clicked, uses the mutation from the `useTriggerSanitySync` hook to send a request to the Switchyard application and trigger the sync.

To display components that match the design of the Switchyard Admin, you use components from the [Switchyard UI package](!ui!).
...
To test it out, start the Switchyard application and open the admin dashboard. After logging in, you'll find a new "Sanity" item in the sidebar.
...
You've now integrated Switchyard with Sanity and can benefit from powerful commerce and CMS features.
