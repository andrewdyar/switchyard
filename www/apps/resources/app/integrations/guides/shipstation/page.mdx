...
export const ogImage = "https://res.cloudinary.com/dza7lstvk/image/upload/v1734607836/Switchyard%20Resources/integrations-shipstation_nlabgy.jpg"

export const metadata = {
  title: `Integrate Switchyard with ShipStation (Fulfillment)`,
  openGraph: {
    images: [
      {
        url: ogImage,
...
In this guide, you'll learn how to integrate Switchyard with ShipStation.

<Note title="Not a technical user?">

Refer your technical team to this guide to integrate ShipStation with your Switchyard application. You can then enable it using the Switchyard Admin as explained in [this user guide](!user-guide!/settings/locations-and-shipping/locations#manage-fulfillment-providers).

</Note>

When you install a Switchyard application, you get a fully-fledged commerce platform with support for customizations. Switchyard's [Fulfillment Module](../../../commerce-modules/fulfillment/page.mdx) provides fulfillment-related resources and functionalities in your store, but it delegates the processing and shipment of order fulfillments to providers that you can integrate.

[ShipStation](https://shipstation.com/) is a shipping toolbox that connects all your shipping providers within one platform. By integrating it with Switchyard, you can allow customers to choose from different providers like DHL and FedEx and view price rates retrieved from ShipStation. Admin users will also process the order fulfillment using the ShipStation integration.

This guide will teach you how to:

- Install and set up Switchyard.
- Set up a ShipStation account.
- Integrate ShipStation as a fulfillment provider in Switchyard.
...
<Card
  ...
  href="https://github.com/switchyard/examples/tree/main/shipstation-integration"
  icon={Github}
/>
...
In this step, you'll prepare your ShipStation account before integrating it into Switchyard. If you don't have an account, create one [here](https://www.shipstation.com/start-a-free-trial).
...
![Scroll down to the Enable carriers & see rates section, and find the "Enable Carriers" button.](https://res.cloudinary.com/dza7lstvk/image/upload/v1734523873/Switchyard%20Resources/Screenshot_2024-12-18_at_2.10.54_PM_pmvcfr.png)

2. In the pop-up that opens, click on Continue Setup.

![Click on the green Continue Setup button](https://res.cloudinary.com/dza7lstvk/image/upload/v1734524261/Switchyard%20Resources/Screenshot_2024-12-18_at_2.11.47_PM_wsl98i.png)
...
![Enable the two checkboxes, then click on Finish Setup at the bottom right](https://res.cloudinary.com/dza7lstvk/image/upload/v1734524486/Switchyard%20Resources/Screenshot_2024-12-18_at_2.20.12_PM_pkixma.png)
...
You must have at least one carrier (shipping provider) added in your ShipStation account. You'll later provide shipping options for each of these carriers in your Switchyard application.

To add carriers:

1. On the Onboard page, in the "Enable carriers & see rates" section, click on the "Add your carrier accounts" link.

![Scroll down to the Enable carriers & see rates section, and find the "Add your carrier accounts" link under the "Enable Carriers" button](https://res.cloudinary.com/dza7lstvk/image/upload/v1734336612/Switchyard%20Resources/Screenshot_2024-12-16_at_10.09.08_AM_nqshhg.png)

2. Click on a provider from the pop-up window.

![Click on the provider tiles in the pop-up window](https://res.cloudinary.com/dza7lstvk/image/upload/v1734336826/Switchyard%20Resources/Screenshot_2024-12-16_at_10.13.37_AM_og4sdq.png)
...
Switchyard's Fulfillment Module delegates processing fulfillments and shipments to other modules, called module providers. In this step, you'll create a ShipStation Module Provider that implements all functionalities required for fulfillment. In later steps, you'll add into Switchyard shipping options for ShipStation, and allow customers to choose it during checkout.
...
A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/shipstation`.

![The directory structure of the Switchyard application after adding the module's directory](https://res.cloudinary.com/dza7lstvk/image/upload/v1734338950/Switchyard%20Resources/shipstation-dir-overview-1_dlsrbv.jpg)
...
![The directory structure of the Switchyard application after adding the service](https://res.cloudinary.com/dza7lstvk/image/upload/v1734339042/Switchyard%20Resources/shipstation-dir-overview-2_cmgvcj.jpg)
...
A module can receive options that are set when you later add the module to Switchyard's configurations. These options allow you to safely store secret values outside of your code.
...
![The directory structure of the Switchyard application after adding the client file](https://res.cloudinary.com/dza7lstvk/image/upload/v1734339519/Switchyard%20Resources/shipstation-dir-overview-3_b8im2d.jpg)
...
The `getFulfillmentOptions` method returns the options that this fulfillment provider supports. When admin users add shipping options later in the Switchyard Admin, they'll select one of these options.
...
![The directory structure of the Switchyard application after adding the types file](https://res.cloudinary.com/dza7lstvk/image/upload/v1734340402/Switchyard%20Resources/shipstation-dir-overview-4_fwsle0.jpg)
...
- You can pass other data, such as `carrier_id` and `carrier_service_code`, and Switchyard will store the fulfillment option in the `data` property of shipping options created later.
...
If the user chooses calculated, Switchyard validates that your fulfillment provider supports calculated prices using the `canCalculate` method of your provider's service.
...
When the customer views available shipping options during checkout, the Switchyard application requests the calculated price from your fulfillment provider using its `calculatePrice` method.
...
When a customer chooses a shipping option during checkout, Switchyard creates a shipping method from that option. A shipping method has a `data` property to store data relevant for later processing of the method and its fulfillments.
...
After the customer places the order, the admin user can manage its fulfillments. When the admin user creates a fulfillment for the order, Switchyard uses the `createFulfillment` method of the associated provider to handle any processing in the third-party provider.
...
The last method you'll implement is the `cancelFulfillment` method. When an admin user cancels a fulfillment, Switchyard uses the associated provider's `cancelFulfillment` method to perform any necessary actions in the third-party provider.
...
Next, you must export the module provider's definition, which lets Switchyard know what module this provider belongs to and its service.

Create the file `src/modules/shipstation/index.ts` with the following content:

![The directory structure of the Switchyard application after adding the index file](https://res.cloudinary.com/dza7lstvk/image/upload/v1734350125/Switchyard%20Resources/shipstation-dir-overview-5_zs6beg.jpg)
...
Finally, to register modules and module providers in Switchyard, you must add them to Switchyard's configurations.
...
- `options`: An object of options to pass to the module. It has a `providers` property which is an array of module providers to register. Each module provider object has the following properties:
    ...
    - `id`: A unique ID, which Switchyard will use along with the `identifier` static property that you set earlier in the class to identify this module provider.
...
![The cog icon is at the top right of the navigation bar. It's the third icon from the right.](https://res.cloudinary.com/dza7lstvk/image/upload/v1734352047/Switchyard%20Resources/Screenshot_2024-12-16_at_2.27.02_PM_nnmwzo.png)

- In the sidebar, expand Account and click on API Settings

![The sidebar has an Account expandable. When you click on it, more items will show. Click on the API Settings](https://res.cloudinary.com/dza7lstvk/image/upload/v1734352145/Switchyard%20Resources/Screenshot_2024-12-16_at_2.28.32_PM_wwfc1s.png).
...
![Make sure V2 API is selected in the Select API Version dropdown, then click on the "Generate API Key" button.](https://res.cloudinary.com/dza7lstvk/image/upload/v1734352261/Switchyard%20Resources/Screenshot_2024-12-16_at_2.30.31_PM_vbkz4i.png)
...
1. Open the Switchyard Admin at `http://localhost:9000/app` and log in.
2. Go to Settings -> Locations & Shipping

![After clicking on settings in the main dashboard, a new sidebar will be shown where you can click on Location & Shipping](https://res.cloudinary.com/dza7lstvk/image/upload/v1733923761/Switchyard%20Resources/Screenshot_2024-12-11_at_2.41.25_PM_wjbq5f.png)
...
![A location's card has the "View details" link at the top-right.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733923793/Switchyard%20Resources/Screenshot_2024-12-11_at_2.41.50_PM_idglsu.png)
...
![The location's page has as a "Fulfillment Providers" section in the side column at the right. Click on the three-dots icon in that section and choose Edit from the dropdown](https://res.cloudinary.com/dza7lstvk/image/upload/v1733923832/Switchyard%20Resources/Screenshot_2024-12-11_at_2.42.47_PM_rzbjbf.png)
...
![Choose the fulfillment provider from the list and click on the Save button](https://res.cloudinary.com/dza7lstvk/image/upload/v1734517015/Switchyard%20Resources/Screenshot_2024-12-18_at_12.06.05_PM_nkmljy.png)

6. Under the Shipping section, click on the "Create option" link.

![The Create Option link is in the Shipping section next to shipping options](https://res.cloudinary.com/dza7lstvk/image/upload/v1734517185/Switchyard%20Resources/Screenshot_2024-12-18_at_12.19.26_PM_o3yz4n.png)
...
![Select Calculated for price type, and select the correct fulfillment provider and option](https://res.cloudinary.com/dza7lstvk/image/upload/v1734517536/Switchyard%20Resources/Screenshot_2024-12-18_at_12.24.44_PM_yk7s3z.png)
...
To test out the integration, you'll place an order using the Next.js Starter Storefront you installed with the Switchyard application. You'll then create a fulfillment for the order's items from the Switchyard Admin dashboard.

### Place Order in Storefront

Open the terminal in the Next.js Starter Storefront's directory. It's a sibling directory of the Switchyard application with the name `{project-name}-storefront`, where `{project-name}` is the name of the Switchyard application's project.

Then, while the Switchyard application is running, run the following command in the storefront's directory:
...
![After you click on Menu, choose Store from the side menu.](https://res.cloudinary.com/dza7lstvk/image/upload/v1734518126/Switchyard%20Resources/Screenshot_2024-12-18_at_12.35.10_PM_knk46m.png)

2. Click on a product and add it to the cart.

![On a product's page, choose and option then click "add to cart"](https://res.cloudinary.com/dza7lstvk/image/upload/v1734518298/Switchyard%20Resources/Screenshot_2024-12-18_at_12.36.18_PM_lrqnsj.png)

3. Click on "Cart" at the top right to go to the cart's page.

![Click on Cart at the top right of the navigation bar.](https://res.cloudinary.com/dza7lstvk/image/upload/v1734518298/Switchyard%20Resources/Screenshot_2024-12-18_at_12.36.59_PM_hmvpgb.png)

4. From the cart's page, click on "Go to checkout".

![The "Go to checkout" button is at the bottom right of the page.](https://res.cloudinary.com/dza7lstvk/image/upload/v1734518298/Switchyard%20Resources/Screenshot_2024-12-18_at_12.37.52_PM_ma4dij.png)
...
![Price is shown next to the ShipStation shipping option](https://res.cloudinary.com/dza7lstvk/image/upload/v1734528106/Switchyard%20Resources/Screenshot_2024-12-18_at_3.19.14_PM_smrzae.png)
...
![In the Unfulfilled Items section, click on the three dots, then Fulfill items from the dropdown](https://res.cloudinary.com/dza7lstvk/image/upload/v1734530779/Switchyard%20Resources/Screenshot_2024-12-18_at_4.05.29_PM_z9lwsk.png)

4. In the form that opens, choose the Location to fulfill the item(s) from, then click Create Fulfillment.

![Choose from the dropdown the location to fulfill the items from, then click Create Fulfillment at the bottom right.](https://res.cloudinary.com/dza7lstvk/image/upload/v1734530888/Switchyard%20Resources/Screenshot_2024-12-18_at_4.07.35_PM_f4h5o1.png)

5. The created fulfillment will be showing on the order's details page now.

![Created fulfillment with details](https://res.cloudinary.com/dza7lstvk/image/upload/v1734535715/Switchyard%20Resources/Screenshot_2024-12-18_at_5.28.06_PM_tsuk4a.png)
...
You've now integrated Switchyard with ShipStation. You can fulfill orders with many carriers and providers, all from a single integration and platform.
