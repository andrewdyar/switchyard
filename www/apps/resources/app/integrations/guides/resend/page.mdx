...
export const ogImage = "https://res.cloudinary.com/dza7lstvk/image/upload/v1732704590/Switchyard%20Resources/Integrations_Resend_pam0nf.jpg"

export const metadata = {
  title: `Integrate Switchyard with Resend (Email Notifications)`,
  openGraph: {
    images: [
      {
        url: ogImage,
...
In this guide, you'll learn how to integrate Switchyard with Resend.

When you install a Switchyard application, you get a fully-fledged commerce platform with a Framework for customization. Switchyard's architecture supports integrating third-party services, such as an email service, that allow you to build your unique requirements around core commerce flows.

[Resend](https://resend.com/docs/introduction) is an email service with an intuitive developer experience to send emails from any application type, including Node.js servers. By integrating Resend with Switchyard, you can build flows to send an email when a commerce operation is performed, such as when an order is placed.

This guide will teach you how to:

- Install and set up Switchyard.
- Integrate Resend into Switchyard for sending emails.
...
<Card
  ...
  href="https://github.com/switchyard/examples/tree/main/resend-integration"
  icon={Github}
/>
...
![Resend dashboard interface showing the Domains section in the left sidebar navigation and the prominent "Add Domain" button in the main content area, which initiates the process of configuring a custom domain for email sending through the Resend service](https://res.cloudinary.com/dza7lstvk/image/upload/v1732523238/Switchyard%20Resources/Screenshot_2024-11-25_at_10.18.11_AM_pmqgtv.png)

3\. In the form that opens, enter your domain name and select a region close to your users, then click Add.

![Domain configuration modal in Resend showing form fields for entering the custom domain name and selecting the appropriate geographical region for optimal email delivery performance, with an "Add" button to confirm the domain setup](https://res.cloudinary.com/dza7lstvk/image/upload/v1732523280/Switchyard%20Resources/Screenshot_2024-11-25_at_10.18.52_AM_sw2pr4.png)
...
![The DNS records to add are in a table under the DNS Records section. Once added, click the Verify DNS Records button at the top right.](https://res.cloudinary.com/dza7lstvk/image/upload/v1732523394/Switchyard%20Resources/Screenshot_2024-11-25_at_10.20.56_AM_ktvbse.png)

You also need an API key to connect to your Resend account from Switchyard, but you'll create that one in a later section.
...
A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/resend`.
...
export const serviceHighlights1 = [
  ...
  ["21", "identifier", "Unique identifier that Switchyard will use when registering the provider in the database."],
...
- `options` of type `ResendOptions`. Modules accept options through Switchyard's configurations. This ensures that the module is reusable across applications and you don't use sensitive variables like API keys directly in your code. The options that the Resend Module Provider accepts are:
    - `api_key`: The Resend API key.
    - `from`: The email address to send the emails from.
    - `html_templates`: An optional object to replace the default subject and template that the Resend Module uses. This is also useful to support custom emails in different Switchyard application setups.
...
1. Dependencies resolved from the [Module's container](!docs!/learn/fundamentals/modules/container), which is the module's local registry that the Switchyard application adds Framework tools to. In this service, you resolve the [Logger utility](!docs!/learn/debugging-and-testing/logging) from the module's container.
2. The module's options that are passed to the module in Switchyard's configuration as you'll see in a later section.
...
A Notification Module Provider's service can implement a static `validateOptions` method that ensures the options passed to the module through Switchyard's configurations are valid.
...
In the `validateOptions` method, you throw an error if the `api_key` or `from` options aren't passed to the module. To throw errors, you use `SwitchyardError` from the Modules SDK. This ensures errors follow Switchyard's conventions and are displayed similar to Switchyard's errors.
...
Finally, you use the `emails.send` method of the Resend client to send the email. If an error occurs you log it in the terminal. Otherwise, you return the ID of the send email as received from Resend. Switchyard uses this ID when creating the notification in its database.
...
![Click on the API keys in the sidebar, then click on the Create API Key button at the top right](https://res.cloudinary.com/dza7lstvk/image/upload/v1732535399/Switchyard%20Resources/Screenshot_2024-11-25_at_10.22.25_AM_v4d09s.png)

    - In the form that opens, enter a name for the API key (for example, Switchyard). You can keep its permissions to Full Access or change it to Sending Access. Once you're done, click Add.

![The form to create an API key with fields for the API key's name, permissions, and domain](https://res.cloudinary.com/dza7lstvk/image/upload/v1732535464/Switchyard%20Resources/Screenshot_2024-11-25_at_10.23.26_AM_g7gcuc.png)
...
![Click the copy icon to copy the API key](https://res.cloudinary.com/dza7lstvk/image/upload/v1732535791/Switchyard%20Resources/Screenshot_2024-11-25_at_10.23.43_AM_divins.png)
...
import { BigNumberValue, CustomerDTO, OrderDTO } from "@switchyard/framework/types"
  ...
  return (
    <Tailwind>
      <Html className="font-sans bg-gray-100">
        <Head />
        <Preview>Thank you for your order from Switchyard</Preview>
        <Body className="bg-white my-10 mx-auto w-full max-w-2xl">
          ...
          <Section className="bg-gray-50 p-6 mt-10">
            <Text className="text-center text-gray-500 text-sm">
              If you have any questions, reply to this email or contact our support team at support@switchyard.com.
            </Text>
            <Text className="text-center text-gray-500 text-sm">
              Order Token: {order.id}
            </Text>
            <Text className="text-center text-gray-400 text-xs mt-4">
              © {new Date().getFullYear()} Switchyardjs, Inc. All rights reserved.
            </Text>
          </Section>
        </Body>
      </Html>
...
First, install the CLI tool in your Switchyard application:
    ...
    "items": [
      {
        ...
        "title": "L",
        "subtitle": "Switchyard Sweatshirt",
        ...
        "product_title": "Switchyard Sweatshirt",
...
![The email template rendered in the browser](https://res.cloudinary.com/dza7lstvk/image/upload/v1745568201/Switchyard%20Resources/Screenshot_2025-04-25_at_10.41.26_AM_u86abc.png)

---

## Step 6: Send Email when Order is Placed

Switchyard has an event system that emits an event when a commerce operation is performed. You can then listen and handle that event in an asynchronous function called a subscriber.

So, to send a confirmation email when a customer places an order, which is a commerce operation that Switchyard already implements, you don't need to extend or hack your way into Switchyard's implementation as you would do with other commerce platforms.

Instead, you'll create a subscriber that listens to the `order.placed` event and sends an email when the event is emitted.

<Note>

Learn more about Switchyard's event system in [this documentation](!docs!/learn/fundamentals/events-and-subscribers).
...
import { CreateNotificationDTO } from "@switchyard/framework/types"
...
The `container` property in the second parameter is an instance of the [Switchyard container](!docs!/learn/fundamentals/medusa-container), which is a registry of Framework and commerce tools, such as a module's service, that you can resolve to utilize their functionalities.

<Note title="Tip">

The Switchyard container is accessible by all customizations, such as workflows and subscribers, except for modules. Each module has its own container with Framework tools like the Logger utility.
...
1. `useQueryGraphStep`, which is a step implemented by Switchyard that uses [Query](!docs!/learn/fundamentals/module-links/query), a tool that allows you to retrieve data across modules. You use it to retrieve the order's details.
...
export const subscriberHighlights = [
  ...
  ["9", "container", "The Switchyard container."],
...
The subscriber function accepts the event's details as a first parameter which has a `data` property that holds the data payload of the event. For example, Switchyard emits the `order.placed` event with the order's ID in the data payload. The function also accepts as a second parameter the [Switchyard container](!docs!/learn/fundamentals/medusa-container).
...
To test out the Resend integration, you'll place an order using the [Next.js Starter Storefront](../../../nextjs-starter/page.mdx) that you installed as part of installing Switchyard.

Start your Switchyard application first:

```bash npm2yarn
npm run dev
```

Then, in the Next.js Starter Storefront's directory (which was installed in a directory outside of the Switchyard application's directory with the name `{project-name}-storefront`, where `{project-name}` is the name of the Switchyard application's directory), run the following command to start the storefront:
...
![Choose Store from Menu](https://res.cloudinary.com/dza7lstvk/image/upload/v1732539139/Switchyard%20Resources/Screenshot_2024-11-25_at_2.51.59_PM_fubiwj.png)

2\. Click on a product, select its options, and add it to the cart.

![Choose an option, such as size, then click on the Add to cart button](https://res.cloudinary.com/dza7lstvk/image/upload/v1732539227/Switchyard%20Resources/Screenshot_2024-11-25_at_2.53.11_PM_iswcjy.png)

3\. Click on Cart at the top right, then click Go to Cart.

![Cart is at the top right. It opens a dropdown with a Go to Cart button](https://res.cloudinary.com/dza7lstvk/image/upload/v1732539354/Switchyard%20Resources/Screenshot_2024-11-25_at_2.54.44_PM_b1pnlu.png)

4\. On the cart's page, click on the "Go to checkout" button.

![The Go to checkout button is at the right side of the page](https://res.cloudinary.com/dza7lstvk/image/upload/v1732539443/Switchyard%20Resources/Screenshot_2024-11-25_at_2.56.27_PM_cvqshj.png)
...
![Enter your Resend account email if you didn't set up a custom domain](https://res.cloudinary.com/dza7lstvk/image/upload/v1732539536/Switchyard%20Resources/Screenshot_2024-11-25_at_2.58.31_PM_wmlh60.png)

6\. After entering the shipping address, choose a delivery and payment methods, then click the Place Order button.

Once the order is placed, you'll find the following message logged in the Switchyard application's terminal:
...
![Example of order-confirmation email](https://res.cloudinary.com/dza7lstvk/image/upload/v1732551372/Switchyard%20Resources/Screenshot_2024-11-25_at_6.15.59_PM_efyuoj.png)

---

## Next Steps

You've now integrated Switchyard with Resend. You can add more templates for other emails, such as customer registration confirmation, user invites, and more. Check out the [Events Reference](/references/events) for a list of all events that the Switchyard application emits.
