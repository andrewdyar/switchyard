...
import { Github } from "@switchyard/icons"

export const metadata = {
  title: `Integrate Payload CMS with Switchyard`,
}

# {metadata.title}

In this tutorial, you'll learn how to integrate [Payload](https://payloadcms.com/) with Switchyard.

When you install a Switchyard application, you get a fully-fledged commerce platform with a Framework for customization. Switchyard also facilitates integrating third-party services that enrich your application with features specific to your unique business use case.
...
This guide was built with Payload v3.54.0. If you're using a different version and you run into issues, consider [opening an issue](https://github.com/switchyard/medusa/issues/new?template=docs.yml).
...
- Install and set up Switchyard.
- Set up Payload in the Next.js Starter Storefront.
- Integrate Payload with Switchyard to sync product data.
    - You'll sync product data when triggered manually by admin users, or as a result of product events in Switchyard.
- Display product data from Payload in the Next.js Starter Storefront.

You can follow this tutorial whether you're new to Switchyard or an advanced Switchyard developer.

![Diagram showcasing a flowchart of interactions between customer, admin, Switchyard, and Payload](https://res.cloudinary.com/dza7lstvk/image/upload/v1754551568/Switchyard%20Resources/payload-summary_ndeiw0.jpg)

<Card
  title="Full Code"
  text="Find the full code of the guide in this repository."
  href="https://github.com/switchyard/examples/tree/main/payload-integration"
  icon={Github}
/>
...
Afterwards, the installation process will start, which will install the Switchyard application in a directory with your project's name and the Next.js Starter Storefront in a separate directory named `{project-name}-storefront`.

<Note title="Why is the storefront installed separately?">

The Switchyard application is composed of a headless Node.js server and an admin dashboard. The storefront is installed or custom-built separately and connects to the Switchyard application through its REST endpoints, called [API routes](!docs!/learn/fundamentals/api-routes). Learn more in [Switchyard's Architecture documentation](!docs!/learn/introduction/architecture).
...
You can find the files in the [examples GitHub repository](https://github.com/switchyard/examples/tree/main/payload-integration/storefront/src/app/(payload)). Copy these files into a new `src/app/(payload)` directory in the Next.js Starter Storefront.
...
![Overview of the Next.js Starter Storefront directory structure of the src directory](https://res.cloudinary.com/dza7lstvk/image/upload/v1754474683/Switchyard%20Resources/payload-storefront-dir_lt7yyw.jpg)
...
- `User`: A Payload user with API key authentication, allowing you later to sync product data from Switchyard to Payload.
- `Media`: A collection for media files, allowing you to manage product images and other media.
- `Product`: A collection for products, which will be synced with Switchyard's product data.
...
Note that you allow pasting URLs from specific sources, such as the Switchyard public images S3 bucket. This allows you to paste Switchyard's stock image URLs in the Payload admin.

#### Product Collection

Finally, you'll add the `Product` collection, which will be synced with Switchyard's product data.

Create the file `src/collections/Products.ts` with the following content:

export const productCollectionHighlights = [
  ["21", "update", "Only allow updating from Switchyard"],
  ["144", "update", "Only allow updating from Switchyard"],
  ["173", "update", "Only allow updating from Switchyard"],
  ["190", "update", "Only allow updating from Switchyard"],
  ["203", "update", "Only allow updating from Switchyard"],
  ["224", "create", "Only allow creating from Switchyard"],
  ["225", "delete", "Only allow deleting from Switchyard"]
]
  ...
  fields: [
    {
      name: "medusa_id",
      type: "text",
      label: "Switchyard Product ID",
      required: true,
      unique: true,
      admin: {
        description: "The unique identifier from Switchyard",
    ...
    },
    {
      ...
      fields: [
        ...
        },
        {
          name: "medusa_id",
          type: "text",
          label: "Switchyard Option ID",
          required: true,
          admin: {
            description: "The unique identifier for the option from Switchyard",
            hidden: true, // Hide this field in the admin UI
          },
          access: {
            update: ({ req }) => !!req.query.is_from_medusa,
    ...
    },
    {
      ...
      fields: [
        ...
        },
        {
          name: "medusa_id",
          type: "text",
          label: "Switchyard Variant ID",
          required: true,
          admin: {
            description: "The unique identifier for the variant from Switchyard",
            hidden: true, // Hide this field in the admin UI
          },
          access: {
            update: ({ req }) => !!req.query.is_from_medusa,
          ...
          fields: [
            {
              name: "medusa_id",
              type: "text",
              label: "Switchyard Option Value ID",
              required: true,
              admin: {
                description: "The unique identifier for the option value from Switchyard",
                hidden: true, // Hide this field in the admin UI
              },
              access: {
                update: ({ req }) => !!req.query.is_from_medusa,
...
- `medusa_id`: The product's ID in Switchyard, which is useful when syncing data between Payload and Switchyard.
...
All of these fields will be filled from Switchyard.

In addition, you also add the following [access-control](https://payloadcms.com/docs/access-control/overview) configurations:

- You disallow creating or deleting products from the Payload admin, as these actions should only be performed from Switchyard.
- You disallow updating the `medusa_id` fields from the Payload admin, as these fields are managed by Switchyard.
    ...
    // other fields...
    {
      ...
      validate: (value: any, { req, previousValue }) => {
        if (req.query.is_from_medusa) {
          return true // Skip validation if the request is from Switchyard
        }
...
If the request is from Switchyard (which is indicated by the `is_from_medusa` query parameter), the validation is skipped.
...
If the request is from Switchyard, the validation is skipped.
...
This hook checks if the `description` field is a string and converts it to rich-text format. This ensures that a description coming from Switchyard is properly formatted when stored in Payload.
...
![Payload Admin Dashboard](https://res.cloudinary.com/dza7lstvk/image/upload/v1754477731/Switchyard%20Resources/CleanShot_2025-08-06_at_13.55.06_2x_bhdkkd.png)

---

## Step 3: Integrate Payload with Switchyard

Now that Payload is set up in the Next.js Starter Storefront, you'll create a Payload [Module](!docs!/learn/fundamentals/modules) to integrate it with Switchyard.

A module is a reusable package that provides functionalities related to a single feature or domain. Switchyard integrates the module into your application without implications or side effects on your setup.
...
A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/payload`.
...
```ts title="src/modules/payload/types.ts" badgeLabel="Switchyard application" badgeColor="green"
...
```ts title="src/modules/payload/service.ts" badgeLabel="Switchyard application" badgeColor="green"
import {
...
2. The module options, which you'll [pass to the module when you register it later](#e-add-module-to-medusas-configurations) in the Switchyard application.
...
If the request fails, it throws a `SwitchyardError` with the error message.
...
The final piece to a module is its definition, which you export in an `index.ts` file at the module's root directory. This definition tells Switchyard the name of the module, its service, and optionally its loaders.

To create the module's definition, create the file `src/modules/payload/index.ts` with the following content:

```ts title="src/modules/payload/index.ts" badgeLabel="Switchyard application" badgeColor="green"
...
### e. Add Module to Switchyard's Configurations
...
```ts title="switchyard.config.ts" badgeLabel="Switchyard application" badgeColor="green"
...
To use the Payload Module, you need to set the module options in the environment variables of your Switchyard application.
...
![The user form with the API key enabled](https://res.cloudinary.com/dza7lstvk/image/upload/v1754479684/Switchyard%20Resources/CleanShot_2025-08-06_at_14.27.25_2x_gasihl.png)

Next, add the following environment variables to your Switchyard application's `.env` file:

```shell title=".env" badgeLabel="Switchyard application" badgeColor="green"
...
The Payload Module is now ready for use. You'll add customizations next to sync product data between Switchyard and Payload.

---

## Step 4: Create Virtual Read-Only Link to Products

Switchyard's [Module Links](!docs!/learn/fundamentals/module-links) feature allows you to virtually link data models from external services to modules in your Switchyard application. Then, when you retrieve data from Switchyard, you can also retrieve the linked data from the third-party service automatically.

In this step, you'll define a virtual read-only link between the `Products` collection in Payload and the `Product` model in Switchyard. Later, you'll be able to retrieve products from Payload while retrieving products in Switchyard.

### a. Define the Link

To define a virtual read-only link, create the file `src/links/product-payload.ts` with the following content:

```ts title="src/links/product-payload.ts" badgeLabel="Switchyard application" badgeColor="green"
...
- An object of the first data model that is part of the link. In this case, it's the `Product` model from Switchyard's Product Module.
- An object of the second data model that is part of the link. In this case, it's the `Products` collection from the Payload Module. You set the following properties:
    ...
    - `primaryKey`: the primary key of the linked data model, which is `product_id`. Switchyard will look for this field in the retrieved `Products` from payload to match it with the `id` field of the `Product` model.
...
When you retrieve products from Switchyard with their `payload_product` link, Switchyard will call the `list` method of the Payload Module's service to retrieve the linked products from Payload.
...
The `list` method receives a `filter` object with an `product_id` property, which is the Switchyard product ID(s) to retrieve their corresponding data from Payload.
...
Finally, you return an array of the payload products. You set the `product_id` field to the value of the `medusa_id` field, which is used to match the linked data in Switchyard.

You can now retrieve products from Payload while retrieving products in Switchyard. You'll learn how to do this in the upcoming steps.
...
In this step, you'll create the functionality to create a Switchyard product in Payload. You'll later execute that functionality either when triggered by an admin user, or automatically when a product is created in Switchyard.
    ...
    steps: [
      {
        ...
        description: "Retrieve the product data from Switchyard",
      ...
      },
      {
        ...
        description: "Store the Payload product ID in Switchyard",
...
You only need to create the `createPayloadItemsStep`, as the other two steps are already available in Switchyard.
...
If you get a type error on resolving the Payload Module, run the Switchyard application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](!docs!/learn/fundamentals/generated-types).

</Note>

export const createPayloadItemsStepHighlights = [
  ["13", "payloadModuleService", "Resolve the Payload Module's service from the Switchyard container."],
...
```ts title="src/workflows/steps/create-payload-items.ts" badgeLabel="Switchyard application" badgeColor="green" highlights={createPayloadItemsStepHighlights}
...
In the compensation function, you again resolve the Payload Module's service from the Switchyard container, then delete the created items from Payload.
...
export const createPayloadProductsWorkflowHighlights = [
  ["12", "useQueryGraphStep", "Retrieve the products from Switchyard using Query."],
  ["36", "createData", "Prepare the data to create the products in Payload."],
  ["66", "createPayloadItemsStep", "Create the products in Payload."],
  ["81", "updateProductsWorkflow", "Update the products in Switchyard with the Payload product IDs."],
]

```ts title="src/workflows/create-payload-products.ts" badgeLabel="Switchyard application" badgeColor="green" highlights={createPayloadProductsWorkflowHighlights}
...
import { updateProductsWorkflow, useQueryGraphStep } from "@switchyard/medusa/core-flows"
...
1. Retrieve the products from Switchyard using the `useQueryGraphStep`.
...
4. Prepare the data to update the products in Switchyard with the Payload product IDs.
    - You store the payload ID in the `metadata` field of the Switchyard product.
5. Update the products in Switchyard using the `updateProductsWorkflow`.

A workflow must return an instance of `WorkflowResponse` that accepts the data to return to the workflow's executor.

You'll use this workflow in the next steps to create Switchyard products in Payload.

---

## Step 6: Trigger Product Creation in Payload

In this step, you'll allow Switchyard Admin users to trigger the creation of Switchyard products in Payload. To implement this, you'll create:
...
- A [setting page](!docs!/learn/fundamentals/admin/ui-routes) in the Switchyard Admin that allows admin users to trigger the product creation in Payload.

### a. Trigger Product Sync API Route

An API route is a REST endpoint that exposes functionalities to clients, such as storefronts and the Switchyard Admin.
...
```ts title="src/api/admin/payload/sync/[collection]/route.ts" badgeLabel="Switchyard application" badgeColor="green"
...
export const productsSyncPayloadSubscriberHighlights = [
  ["15", "products", "Retrieve paginated products from Switchyard."],
...
```ts title="src/subscribers/products-sync-payload.ts" badgeLabel="Switchyard application" badgeColor="green" highlights={productsSyncPayloadSubscriberHighlights}
...
In the subscriber, you use [Query](!docs!/learn/fundamentals/module-links/query) to retrieve all products from Switchyard.
...
### c. Create Setting Page in Switchyard Admin

Next, you'll create a setting page in the Switchyard Admin that allows admin users to trigger syncing products with Payload.

#### Initialize JS SDK

To send requests from your Switchyard Admin customizations to the Switchyard server, you need to initialize the [JS SDK](../../../js-sdk/page.mdx).

Create the file `src/admin/lib/sdk.ts` with the following content:

```ts title="src/admin/lib/sdk.ts" badgeLabel="Switchyard application" badgeColor="green"
...
A setting page is a [UI route](!docs!/learn/fundamentals/admin/ui-routes) that adds a custom page to the Switchyard Admin under the Settings section. The UI route is a React component that renders the page's content.
...
```tsx title="src/admin/routes/settings/payload/page.tsx" badgeLabel="Switchyard application" badgeColor="green"
import { defineRouteConfig } from "@switchyard/admin-sdk"
import { Button, Container, Heading, toast } from "@switchyard/ui"
  ...
  return (
    <Container className="divide-y p-0">
      ...
      <div className="flex flex-col gap-4 px-6 py-4">
        <p>
          This page allows you to trigger syncing your Switchyard data with Payload. It
...
You can now test syncing products from Switchyard to Payload. To do that:

1. Start your Switchyard application with the following command:

```bash npm2yarn badgeLabel="Switchyard application" badgeColor="green"
npm run dev
```
...
3. Open the Switchyard Admin at `localhost:9000/app` and log in with your admin user.
4. Go to Settings -> Payload.
5. On the setting page, click the "Sync Products to Payload" button.

![The Payload Settings page with the Sync Products button](https://res.cloudinary.com/dza7lstvk/image/upload/v1754486648/Switchyard%20Resources/CleanShot_2025-08-06_at_16.23.45_2x_ubug2y.png)

You'll see a success message indicating that the products are being synced with Payload. You can also confirm that the event was triggered by checking the Switchyard server logs for the following message:

```bash
info:    Processing products.sync-payload which has 1 subscribers
```

To check that the products were created in Payload, open the Payload admin at `localhost:8000/admin` and go to "Products" from the sidebar. You should see your Switchyard products listed there.

![The Products collection in Payload with Switchyard products](https://res.cloudinary.com/dza7lstvk/image/upload/v1754486747/Switchyard%20Resources/CleanShot_2025-08-06_at_16.25.31_2x_h874oa.png)
...
In this step, you'll handle the `product.created` event to automatically create a product in Payload whenever a product is created in Switchyard.
...
```ts title="src/subscribers/product-created.ts" badgeLabel="Switchyard application" badgeColor="green"
import { SubscriberArgs, type SubscriberConfig } from "@switchyard/framework"
import { createPayloadProductsWorkflow } from "../workflows/create-payload-products"
...
To test out the automatic product creation in Payload, make sure that both the Switchyard application and the Next.js Starter Storefront are running.

Then, create a product in Switchyard using the Switchyard Admin. If you check the Products collection in the Payload admin, you should see the newly created product there as well.

---

## Step 8: Customize Storefront to Display Payload Products

Now that you've integrated Payload with Switchyard, you can customize the Next.js Starter Storefront to display product content from Payload. By doing so, you can show product content and assets that are optimized for the storefront.
...
When you fetch product data in the Next.js Starter Storefront from the Switchyard server, you can also retrieve the linked product data from Payload.

To do this, go to `src/lib/data/products.ts` in your Next.js Starter Storefront. You'll find a `listProducts` function that uses the JS SDK to fetch products from the Switchyard server.
...
Passing this field is possible because you [defined the virtual read-only link](#step-4-create-virtual-read-only-link-to-products) between the `Product` model in Switchyard and the `Products` collection in Payload.

Switchyard will now return the payload data of a product from Payload and include it in the `payload_product` field of the product object.

### b. Define Payload Product Type

Next, you'll define a TypeScript type that adds the `payload_product` property to Switchyard's `StoreProduct` type.
...
import { StoreProduct } from "@switchyard/types"
// @ts-ignore
...
The `StoreProductWithPayload` type extends the `StoreProduct` type from Switchyard and adds the `payload_product` property. This property contains the product data from Payload, including its title, description, images, options, and variants.
...
If the product has a description in Payload, it will be displayed using Payload's `RichText` component, which renders the rich text content. Otherwise, it will display the product description from Switchyard.
...
- `getProductImages`: This function accepts a product and returns either the images from Payload or the images from Switchyard if the product doesn't have images in Payload.
...
Next, you'll update the type of the `ImageGallery` component's props to receive an array of objects rather than an array of Switchyard images. This ensures the component can accept images from Payload.
...
The images on the product's details page will now be the images from Payload if available, or the images from Switchyard if not.
...
The thumbnail shown in the product listing will now use the thumbnail from Payload if available, or the thumbnail from Switchyard if not.
...
The product title in the product preview will now be the title from Payload if available, or the title from Switchyard if not.
...
You update the function to receive a `payloadData` parameter, which is the product data from Payload. This allows you to retrieve the option values from Payload instead of Switchyard.
...
export const productActionsComponentHighlights = [
  ...
  ["14", "title", "Use the Payload option title if available, otherwise use the Switchyard option title."]
]
...
You change the `title` prop of the `OptionSelect` component to use the title from Payload if available, or the Switchyard option title if not.
...
To test out the storefront customization, make sure that both the Switchyard application and the Next.js Starter Storefront are running.
...
![Product listing page in the storefront showing details from Payload](https://res.cloudinary.com/dza7lstvk/image/upload/v1754491849/Switchyard%20Resources/CleanShot_2025-08-06_at_17.50.34_2x_pj0hjs.png)
...
![Product details page in the storefront showing details from Payload](https://res.cloudinary.com/dza7lstvk/image/upload/v1754491898/Switchyard%20Resources/CleanShot_2025-08-06_at_17.51.28_2x_eokeyg.png)

---

## Step 9: Handle Switchyard Product Events

In this step, you'll create subscribers and workflows to handle the following Switchyard product events:

- [product.deleted](#a-handle-product-deletions): Delete the product in Payload when a product is deleted in Switchyard.
- [product-variant.created](#b-handle-product-variant-creation): Add a product variant to a product in Payload when a product variant is created in Switchyard.
- [product-variant.updated](#c-handle-product-variant-updates): Update a product variant's option values in Payload when a product variant is updated in Switchyard.
- [product-variant.deleted](#d-handle-product-variant-deletions): Remove a product's variant in Payload when a product variant is deleted in Switchyard.
- [product-option.created](#e-handle-product-option-creation): Add a product option to a product in Payload when a product option is created in Switchyard.
- [product-option.deleted](#f-handle-product-option-deletions): Remove a product's option in Payload when a product option is deleted in Switchyard.
...
```ts title="src/workflows/steps/delete-payload-items.ts" badgeLabel="Switchyard application" badgeColor="green"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
import { PAYLOAD_MODULE } from "../../modules/payload"
...
```ts title="src/workflows/delete-payload-products.ts" badgeLabel="Switchyard application" badgeColor="green"
import { createWorkflow, transform, WorkflowResponse } from "@switchyard/framework/workflows-sdk"
...
```ts title="src/subscribers/product-deleted.ts" badgeLabel="Switchyard application" badgeColor="green"
import { SubscriberArgs, type SubscriberConfig } from "@switchyard/framework"
...
To test the product deletion handling, make sure that both the Switchyard application and the Next.js Starter Storefront are running.

Then, open the Switchyard Admin at `localhost:9000/app` and go to the products list. Delete a product that exists in Payload.
    ...
    steps: [
      {
        type: "step",
        name: "useQueryGraphStep",
        description: "Retrieve the product variant from Switchyard",
        link: "/references/helper-steps/useQueryGraphStep",
        depth: 1
      },
      {
...
```ts title="src/workflows/steps/update-payload-items.ts" badgeLabel="Switchyard application" badgeColor="green"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
...
```ts title="src/workflows/create-payload-product-variant.ts" badgeLabel="Switchyard application" badgeColor="green"
import { createWorkflow, transform, when, WorkflowResponse } from "@switchyard/framework/workflows-sdk"
import { useQueryGraphStep } from "@switchyard/medusa/core-flows"
...
1. Retrieve the product variant details from Switchyard using the `useQueryGraphStep`, including the linked product data from Payload.
...
```ts title="src/subscribers/variant-created.ts" badgeLabel="Switchyard application" badgeColor="green"
import { SubscriberArgs, type SubscriberConfig } from "@switchyard/framework"
...
To test the product variant creation handling, make sure that both the Switchyard application and the Next.js Starter Storefront are running.

Then, open the Switchyard Admin at `localhost:9000/app` and open a product's details page. Add a new variant to the product and save the changes.
...
```ts title="src/workflows/update-payload-product-variants.ts" badgeLabel="Switchyard application" badgeColor="green"
import { createWorkflow, transform, when, WorkflowResponse } from "@switchyard/framework/workflows-sdk"
import { useQueryGraphStep } from "@switchyard/medusa/core-flows"
import { PayloadCollectionItem, PayloadUpsertData } from "../modules/payload/types"
import { updatePayloadItemsStep } from "./steps/update-payload-items"
...
```ts title="src/subscribers/variant-updated.ts" badgeLabel="Switchyard application" badgeColor="green"
import { SubscriberArgs, type SubscriberConfig } from "@switchyard/framework"
...
To test the product variant update handling, make sure that both the Switchyard application and the Next.js Starter Storefront are running.

Then, open the Switchyard Admin at `localhost:9000/app` and open a product's details page. Edit an existing variant's title and save the changes.
...
Since the `deletePayloadProductVariantsWorkflow` is executed after a product variant is deleted, you can't retrieve the product variant data from Switchyard.
...
```ts title="src/workflows/steps/retrieve-payload-items.ts" badgeLabel="Switchyard application" badgeColor="green"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
import { PAYLOAD_MODULE } from "../../modules/payload"
...
```ts title="src/workflows/delete-payload-product-variants.ts" badgeLabel="Switchyard application" badgeColor="green"
import { createWorkflow, transform, when, WorkflowResponse } from "@switchyard/framework/workflows-sdk"
import { updatePayloadItemsStep } from "./steps/update-payload-items"
...
```ts title="src/subscribers/variant-deleted.ts" badgeLabel="Switchyard application" badgeColor="green"
import { SubscriberArgs, type SubscriberConfig } from "@switchyard/framework"
...
To test the product variant deletion handling, make sure that both the Switchyard application and the Next.js Starter Storefront are running.

Then, open the Switchyard Admin at `localhost:9000/app` and open a product's details page. Delete an existing variant from the product.
    ...
    steps: [
      {
        type: "step",
        name: "useQueryGraphStep",
        description: "Retrieve the product option from Switchyard",
        link: "/references/helper-steps/useQueryGraphStep",
        depth: 1
      },
      {
...
```ts title="src/workflows/create-payload-product-options.ts" badgeLabel="Switchyard application" badgeColor="green"
import { createWorkflow, transform, when, WorkflowResponse } from "@switchyard/framework/workflows-sdk"
import { useQueryGraphStep } from "@switchyard/medusa/core-flows"
import { PayloadCollectionItem, PayloadUpsertData } from "../modules/payload/types"
import { updatePayloadItemsStep } from "./steps/update-payload-items"
...
1. Retrieve the product option details from Switchyard using the `useQueryGraphStep`, including the linked product data from Payload.
...
```ts title="src/subscribers/option-created.ts" badgeLabel="Switchyard application" badgeColor="green"
import { SubscriberArgs, type SubscriberConfig } from "@switchyard/framework"
...
To test the product option creation handling, make sure that both the Switchyard application and the Next.js Starter Storefront are running.

Then, open the Switchyard Admin at `localhost:9000/app` and open a product's details page. Add a new option to the product and save the changes.
...
```ts title="src/workflows/delete-payload-product-options.ts" badgeLabel="Switchyard application" badgeColor="green"
import { createWorkflow, transform, when, WorkflowResponse } from "@switchyard/framework/workflows-sdk"
import { updatePayloadItemsStep } from "./steps/update-payload-items"
import { retrievePayloadItemsStep } from "./steps/retrieve-payload-items"
...
```ts title="src/subscribers/option-deleted.ts" badgeLabel="Switchyard application" badgeColor="green"
import { SubscriberArgs, type SubscriberConfig } from "@switchyard/framework"
...
To test the product option deletion handling, make sure that both the Switchyard application and the Next.js Starter Storefront are running.

Then, open the Switchyard Admin at `localhost:9000/app` and open a product's details page. Delete an existing option from the product.
...
You've successfully integrated Switchyard with Payload to manage content related to products, variants, and options. You can expand on this integration by adding more features, such as:
...
1. Managing the content of other entities, like categories or collections. The process is similar to what you've done for products:
    ...
    2. Create Switchyard workflows and subscribers to handle the creation, update, and deletion of the entity.
...
2. Enable [localization](https://payloadcms.com/docs/configuration/localization) in Payload to support multiple languages.
    - You only need to manage the localized content in Payload. Only the default locale will be synced with Switchyard.
...
### Learn More about Switchyard

If you're new to Switchyard, check out the [main documentation](!docs!/learn), where you'll get a more in-depth understanding of all the concepts you've used in this guide and more.
...
1. Visit the [Switchyard GitHub repository](https://github.com/switchyard/medusa) to report issues or ask questions.
2. Join the [Switchyard Discord community](https://discord.gg/switchyard) for real-time support from community members.
