...
export const metadata = {
  title: `Add Newsletter Subscriptions with Mailchimp in Switchyard`,
}

# {metadata.title}

In this tutorial, you'll learn how to integrate Mailchimp with Switchyard to manage newsletter subscribers and automate newsletters.

When you install a Switchyard application, you get a fully-fledged commerce platform with a Framework for customization. Switchyard's architecture facilitates integrating third-party services to customize Switchyard's infrastructure for your business needs.

Switchyard's [Notification Module](../../../infrastructure-modules/notification/page.mdx) allows you to customize Switchyard's infrastructure to send notifications using the third-party provider that fits your business needs, such as [Mailchimp](https://mailchimp.com/).

In this tutorial, you'll integrate Mailchimp with Switchyard to allow customers to subscribe to your newsletter and automate sending newsletters.
...
- Integrate Mailchimp with Switchyard.
...
![Diagram showcasing the flow of the Mailchimp integration with Switchyard](https://res.cloudinary.com/dza7lstvk/image/upload/v1750679344/Switchyard%20Resources/mailchimp-overview_itmiul.jpg)

<Card
  title="Example Repository"
  text="Find the full code of the guide in this repository."
  href="https://github.com/switchyard/examples/tree/main/mailchimp-integration"
  icon={Github}
/>
...
To integrate third-party services into Switchyard, you create a custom [module](!docs!/learn/fundamentals/modules). A module is a reusable package with functionalities related to a single feature or domain.

Switchyard's [Notification Module](../../../infrastructure-modules/notification/page.mdx) provides an interface to send notifications in your Switchyard application. It delegates the actual sending of notifications to the underlying provider, such as Mailchimp.
...
Refer to the [Modules](!docs!/learn/fundamentals/modules) documentation to learn more about modules in Switchyard.
...
Run the following command to install the SDK with its types package in your Switchyard application:
...
A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/mailchimp`.
...
The service must also have an `identifier` static property, which is a unique identifier for the module. This identifier is used when registering the module in the Switchyard application.
...
- `options`: Options that are passed to the module provider when it's registered in Switchyard's configurations. You define the following options:
...
You'll learn how to set these options in the [Add Module Provider to Switchyard's Configurations](#g-add-module-provider-to-medusas-configurations) section.
...
The `validateOptions` method is used to validate the options passed to the module provider. If the method throws an error, the Switchyard application won't start.
...
When the Switchyard application needs to send a notification through a channel (such as `email`), it calls the `send` method of the channel's module provider.
...
If the subscription is successful, the method returns an object with the `id` of the subscribed email. If it fails, it throws a `SwitchyardError` with the error message from Mailchimp.
...
You also throw a `SwitchyardError` if the method fails at any point, providing the error message from Mailchimp.
...
- If the template is not recognized, you throw a `SwitchyardError` indicating that the template is invalid.
...
The final piece to a module is its definition, which you export in an `index.ts` file at the module's root directory. This definition tells Switchyard the module's details, including its service.
...
### g. Add Module Provider to Switchyard's Configurations

Once you finish building the module, add it to Switchyard's configurations to start using it.

In `switchyard.config.ts`, add a `modules` property to the configurations:
...
```ts title="switchyard.config.ts" highlights={configurationsHighlight}
...
To pass a Module Provider to the Notification Module, you add the `modules` property to the Switchyard configuration and pass the Notification Module in its value.
...
![Mailchimp dashboard with the profile menu opened](https://res.cloudinary.com/dza7lstvk/image/upload/v1750682692/Switchyard%20Resources/CleanShot_2025-06-23_at_11.46.07_2x_xo5a5l.png)

3. Click on the "Extras" tab and select "API keys" from the dropdown.

![Mailchimp account page with the Extras dropdown opened](https://res.cloudinary.com/dza7lstvk/image/upload/v1750682741/Switchyard%20Resources/CleanShot_2025-06-23_at_11.48.12_2x_l86ein.png)

4. Scroll down to the "Your API keys" section and click on the "Create A Key" button.

![Mailchimp API keys page with the Create A Key button highlighted](https://res.cloudinary.com/dza7lstvk/image/upload/v1750682771/Switchyard%20Resources/CleanShot_2025-06-23_at_11.48.53_2x_vf3ftl.png)

5. In the API key form, enter a name for the API key.
6. Click the "Generate Key" button to create the API key.

![Mailchimp API key creation form with a name field](https://res.cloudinary.com/dza7lstvk/image/upload/v1750682817/Switchyard%20Resources/CleanShot_2025-06-23_at_11.49.12_2x_xwwmi6.png)

Copy the generated API key and add it to the `.env` file in your Switchyard application:
...
Then, add the server prefix to the `.env` file in your Switchyard application:
...
![Mailchimp audience management page with the Manage Audiences option highlighted](https://res.cloudinary.com/dza7lstvk/image/upload/v1750683058/Switchyard%20Resources/CleanShot_2025-06-23_at_11.50.54_2x_ayyyuq.png)

3. Click on the "Create Audience" button at the top right.

![Mailchimp audience creation page with the Create Audience button highlighted](https://res.cloudinary.com/dza7lstvk/image/upload/v1750683095/Switchyard%20Resources/CleanShot_2025-06-23_at_11.51.51_2x_d0nxvc.png)
...
![Mailchimp audience creation form with fields for audience name, default from email address, and default from name](https://res.cloudinary.com/dza7lstvk/image/upload/v1750683517/Switchyard%20Resources/CleanShot_2025-06-23_at_11.54.25_2x_ihgiyt.png)
...
![Mailchimp audience contacts page with the More options button highlighted](https://res.cloudinary.com/dza7lstvk/image/upload/v1750683642/Switchyard%20Resources/CleanShot_2025-06-23_at_16.00.19_2x_zjetg6.png)

7. In the Audience settings page, copy the value of "Audience ID" from the first section.

![Mailchimp audience settings page with the Audience ID highlighted](https://res.cloudinary.com/dza7lstvk/image/upload/v1750683763/Switchyard%20Resources/CleanShot_2025-06-23_at_16.01.51_2x_a8grlw.png)

Add the copied ID to the `.env` file in your Switchyard application as the list ID:
...
![Mailchimp automations page with the Build from scratch button highlighted](https://res.cloudinary.com/dza7lstvk/image/upload/v1750684308/Switchyard%20Resources/CleanShot_2025-06-23_at_12.18.15_2x_ymj8ar.png)
...
![Mailchimp automation flow creation form with fields for flow name and audience selection](https://res.cloudinary.com/dza7lstvk/image/upload/v1750684445/Switchyard%20Resources/CleanShot_2025-06-23_at_12.19.17_2x_yso5wu.png)

5. A pop-up will open in the flow editor to choose a template to start from. Choose the "Signs up for Email" template.

![Mailchimp automation flow editor with the Signs up for Email template highlighted](https://res.cloudinary.com/dza7lstvk/image/upload/v1750685997/Switchyard%20Resources/CleanShot_2025-06-23_at_12.19.32_2x_x14tcv.png)
...
![Mailchimp automation flow editor with the Send Email action highlighted](https://res.cloudinary.com/dza7lstvk/image/upload/v1750686062/Switchyard%20Resources/CleanShot_2025-06-23_at_12.20.56_2x_iksdox.png)
...
<body>
  <div class="email-container">
    <h1 mc:edit="welcome_heading">Thank you for signing up to the Switchyard newsletter 🎉</h1>
    ...
    <div class="footer" mc:edit="footer_text">
      You’re receiving this email because you signed up for updates from Switchyard.<br>
...
![Mailchimp automation flow editor with the Continue button highlighted](https://res.cloudinary.com/dza7lstvk/image/upload/v1750686278/Switchyard%20Resources/CleanShot_2025-06-23_at_12.24.15_2x_v1mcas.png)

11. In the review pop-up, click the "Turn flow on" button to activate the automation flow.

![Mailchimp automation flow review pop-up with the Turn flow on button highlighted](https://res.cloudinary.com/dza7lstvk/image/upload/v1750686313/Switchyard%20Resources/CleanShot_2025-06-23_at_12.24.36_2x_wji8xx.png)
...
Now that you've integrated Mailchimp with Switchyard, you need to allow customers to subscribe to the newsletter.
...
export const apiRouteHighlights = [
  ...
  ["14", "eventModuleService", "Resolve the Event Module service from the Switchyard container."],
...
export async function POST(
  req: SwitchyardRequest<z.infer<typeof newsletterSignupSchema>>,
...
In the route handler, you use the [Switchyard container](!docs!/learn/fundamentals/medusa-container) to resolve the [Event Module](../../../infrastructure-modules/event/page.mdx)'s service.
...
You define middlewares using the `defineMiddlewares` function from the Switchyard Framework. It accepts an object having a `routes` property, whose value is an array of middleware route objects. Each middleware route object has the following properties:
...
The subscriber function receives an object as a parameter that has a `container` property, which is the Switchyard container. The Switchyard container holds Framework and commerce tools that you can resolve and use in your customizations.

In the subscriber function, you resolve the [Notification Module](../../../infrastructure-modules/notification/page.mdx)'s service from the Switchyard container. Then, you call the `createNotifications` method to create a notification with the following properties:
...
So, if your Switchyard application's directory is `medusa-newsletter`, you can find the storefront by going back to the parent directory and changing to the `medusa-newsletter-storefront` directory:
...
The `toast` function is imported from [Switchyard UI](!ui!), which requires adding `Toaster` component in the application's tree. So, import the `Toaster` component in the file `src/app/[countryCode]/(main)/layout.tsx`:

```tsx title="src/app/[countryCode]/(main)/layout.tsx" badgeLabel="Storefront" badgeColor="blue"
import { Toaster } from "@switchyard/ui"
```
...
![Next.js Starter Storefront with the newsletter subscription form in the footer](https://res.cloudinary.com/dza7lstvk/image/upload/v1750688006/Switchyard%20Resources/CleanShot_2025-06-23_at_17.12.48_2x_bjrylk.png)
...
![Next.js Starter Storefront with the newsletter subscription form success message](https://res.cloudinary.com/dza7lstvk/image/upload/v1750688118/Switchyard%20Resources/CleanShot_2025-06-23_at_17.15.01_2x_wetvkx.png)

If you set up the welcome email automation flow in Mailchimp, you'll receive a welcome email.

![Welcome email received from Mailchimp](https://res.cloudinary.com/dza7lstvk/image/upload/v1750688154/Switchyard%20Resources/CleanShot_2025-06-23_at_12.33.09_2x_zdteda.png)

---

## Step 5: Create Automated Product Newsletter

Next, you'll schedule sending a newsletter for new products once a week. Customers subscribed to the newsletter will receive an email with the latest products added to your Switchyard store.
...
A [workflow](!docs!/learn/fundamentals/workflows) is a series of actions, called steps, that complete a task with rollback and retry mechanisms. In Switchyard, you build commerce features in workflows, then execute them in other customizations, such as subscribers, scheduled jobs, and API routes.

You'll create a workflow that retrieves the latest products added to your Switchyard store, then creates a notification with the product data useful to send the newsletter.
...
1. Resolve the [Logger](!docs!/learn/debugging-and-testing/logging) utility from the Switchyard container to log messages.
2. Log a message indicating that the newsletter is being sent.
3. Execute the `sendNewProductsNewsletter` workflow by invoking it, passing the Switchyard container as a parameter, then calling its `run` method.
...
Also, if you didn't create any products in the past week, make sure to [create a few products in the Switchyard Admin](!user-guide!/products/create).
...
![Mailchimp campaigns page with the New Products campaign highlighted](https://res.cloudinary.com/dza7lstvk/image/upload/v1750689882/Switchyard%20Resources/CleanShot_2025-06-23_at_17.44.27_2x_tkdtwm.png)

If you also subscribed to the newsletter, you should receive an email with the latest products added to your Switchyard store.

![Mailchimp New Products newsletter email](https://res.cloudinary.com/dza7lstvk/image/upload/v1750689914/Switchyard%20Resources/CleanShot_2025-06-23_at_13.29.07_2x_m5xpht.png)
...
You've now integrated Mailchimp with Switchyard to allow customers to subscribe to a newsletter and to send automated newsletters about new products.
