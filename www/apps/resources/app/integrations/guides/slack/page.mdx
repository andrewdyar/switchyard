...
export const metadata = {
  title: `Integrate Slack (Notification) with Switchyard`,
}

# {metadata.title}

In this tutorial, you'll learn how to integrate Slack with Switchyard to receive notifications about created orders.
...
Switchyard's [Notification Module](../../../infrastructure-modules/notification/page.mdx) allows you to customize Switchyard's infrastructure to send notifications using the third-party provider that fits your business needs, such as [Slack](https://slack.com/).

In this tutorial, you'll integrate Slack with Switchyard to receive notifications about created orders.
...
Switchyard's [Notification Module](../../../infrastructure-modules/notification/page.mdx) provides an interface to send notifications in your Switchyard application. It delegates the actual sending of notifications to the underlying provider, such as Slack.
...
To send requests to Slack, you'll use the [Axios](https://axios-http.com/) library. So, run the following command to install it in your Switchyard application:
...
A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/slack`.
...
export const serviceHighlights = [
  ...
  ["7", "admin_url", "The URL of the Switchyard Admin dashboard, used to add links in the notifications."],
...
The service must also have an `identifier` static property, which is a unique identifier for the module. This identifier is used when registering the module in the [Switchyard container](!docs!/learn/fundamentals/medusa-container).
...
- `options`: Options that are passed to the module provider when it's registered in Switchyard's configurations. You define the following options:
    ...
    - `admin_url`: The URL of the Switchyard Admin dashboard, which you'll use to add links in the notifications.
...
When the Switchyard application needs to send a notification through a channel (such as Slack), it calls the `send` method of the channel's module provider.
...
In the method, you format the Slack message to show the order's ID, customer information, shipping address, order items, and total amounts. You also add a link to the order's details page in the Switchyard Admin dashboard.

Finally, you send the formatted message to Slack using the `axios.post` method with the configured webhook URL.

#### send Method

You can now implement the `send` method, which is the method that Switchyard calls to send notifications using the provider. Add the `send` method to the `SlackNotificationProviderService` class:
...
export const configHighlights = [
  ...
  ["14", "admin_url", "The URL of the Switchyard Admin dashboard, used to add links in the notifications."]
]

```ts title="switchyard.config.ts" highlights={configHighlights}
...
To pass a Notification Module Provider to the Notification Module, you add the `modules` property to the Switchyard configuration and pass the Notification Module in its value.
...
![Slack Apps page interface showing the prominent green "Create New App" button in the top navigation area, which initiates the process of creating a new Slack application for integrating with external services like Switchyard](https://res.cloudinary.com/dza7lstvk/image/upload/v1748940543/Switchyard%20Resources/CleanShot_2025-06-03_at_10.43.01_2x_zadmwn.png)
...
![Slack webhook URL generation interface showing the newly created webhook URL with a copy button, which provides the unique endpoint URL needed to configure Switchyard for sending automated notifications to the selected Slack channel](https://res.cloudinary.com/dza7lstvk/image/upload/v1748940718/Switchyard%20Resources/CleanShot_2025-06-03_at_10.45.51_2x_xet5u5.png)

Then, in the `.env` file of your Switchyard application, set the `SLACK_WEBHOOK_URL` and `SLACK_ADMIN_URL` environment variables:
...
In development, the Switchyard Admin dashboard is running on `http://localhost:9000/app`, so you can use that URL for the `SLACK_ADMIN_URL` environment variable.
...
Now that you've integrated Slack with Switchyard, you can send notifications to Slack at any point, including when an order is created.
...
Switchyard provides both steps in its `@switchyard/medusa/core-flows` package.
...
import { sendNotificationsStep, useQueryGraphStep } from "@switchyard/medusa/core-flows"
...
- Retrieve the order's details using the `useQueryGraphStep`. This step uses Switchyard's [Query](!docs!/learn/fundamentals/module-links/query) tool to retrieve data across modules. You pass it the order ID to retrieve.
...
- `container`: The [Switchyard container](!docs!/learn/fundamentals/medusa-container) to access the Framework and commerce tools.

In the subscriber function, you execute the `orderPlacedNotificationWorkflow` by invoking it, passing the Switchyard container as a parameter. Then, you chain a `run` method, passing it the order ID from the event's data payload as input.
...
So, if your Switchyard application's directory is `medusa-slack`, you can find the storefront by going back to the parent directory and changing to the `medusa-slack-storefront` directory:
...
You can also click on the order's ID in the notification to open the order's details page in the Switchyard Admin dashboard.

![Slack Notification Example](https://res.cloudinary.com/dza7lstvk/image/upload/v1748942706/Switchyard%20Resources/CleanShot_2025-06-03_at_11.24.16_2x_umecps.png)

---

## Next Steps

You've now integrated Slack with Switchyard to receive notifications about created orders. You can expand on this integration to send other types of notifications, such as order updates. Refer to the [Events Reference](/references/events) for a list of events you can listen to in your subscribers.
