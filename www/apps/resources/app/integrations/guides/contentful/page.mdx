...
export const ogImage = "https://res.cloudinary.com/dza7lstvk/image/upload/v1746006829/Switchyard%20Resources/localization_dtiqtb.jpg"

export const metadata = {
  title: `Implement Localization in Switchyard by Integrating Contentful`,
  openGraph: {
    images: [
      {
        url: ogImage,
...
In this tutorial, you'll learn how to localize your Switchyard store's data with Contentful.

When you install a Switchyard application, you get a fully-fledged commerce platform with a Framework for customization. While Switchyard provides features essential for internationalization, such as support for multiple [regions](../../../commerce-modules/region/page.mdx) and [currencies](../../../commerce-modules/currency/page.mdx), it doesn't provide content localization.

However, Switchyard's architecture supports the integration of third-party services to provide additional features, such as data localization. One service you can integrate is [Contentful](https://www.contentful.com/), a headless content management system (CMS) that allows you to manage and deliver content across multiple channels.
...
- Integrate Contentful with Switchyard.
- Create content types in Contentful for Switchyard models.
- Trigger syncing products and related data to Contentful when:
  - A product is created.
  - The admin user triggers syncing the products.
- Customize the [Next.js Starter Storefront](../../../nextjs-starter/page.mdx) to fetch localized data from Contentful through Switchyard.
- Listen to webhook events in Contentful to update Switchyard's data accordingly.

You can follow this tutorial whether you're new to Switchyard or an advanced Switchyard developer.

![Diagram illustrating the integration of Contentful with Switchyard](https://res.cloudinary.com/dza7lstvk/image/upload/v1744791908/Switchyard%20Resources/contentful-summary_j5cpdx.jpg)

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/localization-contentful",
...
To integrate third-party services into Switchyard, you create a module. A [module](!docs!/learn/fundamentals/modules) is a reusable package that provides functionalities related to a single feature or domain. Switchyard integrates the module into your application without implications or side effects on your setup.

In this step, you'll create a module that provides the necessary functionalities to integrate Contentful with Switchyard.
...
Before building the module, you need to install Contentful's management and delivery JS SDKs. So, run the following command in the Switchyard application's directory:
...
A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/contentful`.

### Create Loader

When the Switchyard application starts, you want to establish a connection to Contentful, then create the necessary content types if they don't exist in Contentful.

A module can specify a task to run on the Switchyard application's startup using [loaders](!docs!/learn/fundamentals/modules/loaders). A loader is an asynchronous function that a module exports. Then, when the Switchyard application starts, it runs the loader. The loader can be used to perform one-time tasks such as connecting to a database, creating content types, or initializing data.
...
try {
  ...
  const productContentType = await managementClient.contentType.createWithId({
    ...
    name: "Product",
    description: "Product content type synced from Switchyard",
    displayField: "title",
    fields: [
      {
        id: "title", 
...
- `medusaId`: The product's ID in Switchyard, which is a non-localized field. You'll store in this field the ID of the product in Switchyard.
...
try {
  ...
  description: "Product variant content type synced from Switchyard",
  displayField: "title",
  fields: [
    {
      id: "title",
...
- `medusaId`: The product variant's ID in Switchyard, which is a non-localized field. You'll store in this field the ID of the variant in Switchyard.
...
try {
  ...
  const productOptionContentType = await managementClient.contentType.createWithId({
    ...
    name: "Product Option",
    description: "Product option content type synced from Switchyard",
    displayField: "title",
    fields: [
    {
      id: "title",
...
- `medusaId`: The product option's ID in Switchyard, which is a non-localized field. You'll store in this field the ID of the option in Switchyard.
...
try {
  ...
  const productOptionValueContentType = await managementClient.contentType.createWithId({
    ...
    description: "Product option value content type synced from Switchyard",
    displayField: "value",
    fields: [
    {
      id: "value",
...
- `medusaId`: The product option value's ID in Switchyard, which is a non-localized field. You'll store in this field the ID of the option value in Switchyard.
...
![Example of the locale's settings in the Contentful dashboard](https://res.cloudinary.com/dza7lstvk/image/upload/v1744716537/Switchyard%20Resources/Screenshot_2025-04-15_at_2.28.46_PM_v50lrm.png)
...
![The cog icon is at the top right next to the user avatar. If you click on it, a dropdown will show where you can click on "CMA tokens".](https://res.cloudinary.com/dza7lstvk/image/upload/v1744714244/Switchyard%20Resources/Screenshot_2025-04-15_at_1.48.35_PM_ct2tfk.png)
...
![Click on the copy button to copy the key](https://res.cloudinary.com/dza7lstvk/image/upload/v1744714749/Switchyard%20Resources/Screenshot_2025-04-15_at_1.58.12_PM_tuugam.png)
...
![The cog icon is at the top right next to the user avatar. If you click on it, a dropdown will show where you can click on "API keys".](https://res.cloudinary.com/dza7lstvk/image/upload/v1744714971/Switchyard%20Resources/Screenshot_2025-04-15_at_2.02.31_PM_qfsn1h.png)
...
![Copy the API key from the "Content Delivery API - access token" field](https://res.cloudinary.com/dza7lstvk/image/upload/v1744715228/Switchyard%20Resources/Screenshot_2025-04-15_at_2.06.44_PM_ifu1mx.png)
...
To test out the module, you'll start the Switchyard application, which will run the module's loader.

To start the Switchyard application, run the following command:
...
![In the Contentful dashboard, go to the Content Model page](https://res.cloudinary.com/dza7lstvk/image/upload/v1744715783/Switchyard%20Resources/Screenshot_2025-04-15_at_2.16.09_PM_w7oszm.png)
...
- A product is created in Switchyard.
...
// other imports...
import { ProductOptionDTO } from "@switchyard/framework/types"
...
// other imports...
import { ProductDTO } from "@switchyard/framework/types"
...
To implement the logic that's triggered when a product is created in Switchyard, or when the admin user triggers a sync manually, you need to create a workflow.
...
In this section, you'll create a workflow that creates Switchyard products in Contentful using the Contentful Module.
...
Switchyard provides the `useQueryGraphStep` in its `@switchyard/medusa/core-flows` package. So, you only need to implement the second step.
...
If you get a type error on resolving the Contentful Module, run the Switchyard application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](!docs!/learn/fundamentals/generated-types).
...
export const createProductsContentfulStepHighlights = [
  ...
  ["15", "container", "The Switchyard container, useful to resolve Framework and commerce tools."],
  ["15", "resolve", "Resolve the Contentful Module's service from the Switchyard container."],
...
The Switchyard container is different from the module's container. Since modules are isolated, they each have a container with their resources. Refer to the [Module Container](!docs!/learn/fundamentals/modules/container) documentation for more information.

</Note>

In the step function, you resolve the Contentful Module's service from the Switchyard container using the name you exported in the module definition's file.
...
The compensation function accepts as a parameter the data passed from the step, and an object containing the Switchyard container.
...
1. Retrieve the Switchyard products using the `useQueryGraphStep` helper step. This step uses Switchyard's [Query](!docs!/learn/fundamentals/module-links/query) tool to retrieve data across modules. You pass it the product IDs to retrieve.
...
You now have the workflow that you can execute when a product is created in Switchyard, or when the admin user triggers a sync manually.

---

## Step 4: Trigger Sync on Product Creation

Switchyard has an event system that allows you to listen for events, such as `product.created`, and perform an asynchronous action when the event is emitted.
...
- `container`: The Switchyard container to access the Framework and commerce tools.

In the subscriber function, you execute the `createProductsContentfulWorkflow` by invoking it, passing the Switchyard container as a parameter. Then, you chain a `run` method, passing it the product ID from the event's data payload as input.

Finally, you log a message to the console to indicate that the product was created in Contentful.

### Test the Subscriber

To test out the subscriber, start the Switchyard application:

```bash npm2yarn
npm run dev
```

Then, open the Switchyard Admin dashboard and login.

<Note>

Can't remember the credentials? Learn how to create a user in the [Switchyard CLI reference](../../../switchyard-cli/commands/user/page.mdx).
...
The other way to sync products is when the admin user triggers a sync manually. This is useful when you already have products in Switchyard and you want to sync them to Contentful.
...
3. A UI route in the Switchyard Admin that displays a button to trigger the sync.

### Create Manual Sync Subscriber

You'll start by creating the subscriber that listens to a custom event to sync the Switchyard products to Contentful.
...
export const syncProductsSubscriberHighlights = [
  ["13", "query", "Resolve Query from the Switchyard container."],
  ["24", "graph", "Retrieve the Switchyard products with pagination."],
...
In the subscriber function, you use [Query](!docs!/learn/fundamentals/module-links/query) to retrieve all the products in Switchyard with pagination. Then, for each batch of products, you execute the `createProductsContentfulWorkflow` workflow, passing the product IDs to the workflow.
...
export const syncProductsRouteHighlights = [
  ...
  ["10", "eventService", "Resolve the Event Module's service from the Switchyard container."],
...
In the route handler, you resolve the [Event Module](../../../infrastructure-modules/event/page.mdx)'s service from the Switchyard container and emit the `products.sync` event.

### Create UI Route to Trigger Sync

Finally, you'll add a new page to the Switchyard Admin dashboard that displays a button to trigger the sync. To add a page, you need to create a UI route.

A [UI route](!docs!/learn/fundamentals/admin/ui-routes) is a React component that specifies the content to be shown in a new page of the Switchyard Admin dashboard. You'll create a UI route to display a button that triggers product syncing to Contentful when clicked.
...
Before creating the UI route, you'll configure Switchyard's [JS SDK](../../../js-sdk/page.mdx) so that you can use it to send requests to the Switchyard server.
...
You create an instance of the JS SDK using the `Switchyard` class from the JS SDK. You pass it an object having the following properties:
...
To test out the sync, start the Switchyard application:

```bash npm2yarn
npm run dev
```

Then, open the Switchyard Admin dashboard and login. In the sidebar, you'll find a new "Contentful" item. If you click on it, you'll see the page you created with the button to trigger the sync.

![The Contentful page in the Switchyard Admin dashboard with a button to trigger the sync](https://res.cloudinary.com/dza7lstvk/image/upload/v1744718825/Switchyard%20Resources/Screenshot_2025-04-15_at_3.06.53_PM_va8e20.png)
...
Assuming you have `4` products in Switchyard, the message indicates that the sync was successful.

You can also see the products in the Contentful dashboard.

![The Contentful dashboard showing the synced products](https://res.cloudinary.com/dza7lstvk/image/upload/v1744718896/Switchyard%20Resources/Screenshot_2025-04-15_at_3.08.02_PM_qexr0x.png)
...
export const getLocalesRouteHighlights = [
  ["12", "contentfulModuleService", "Resolve the Contentful Module's service from the Switchyard container."],
...
In the route handler, you resolve the Contentful Module's service from the Switchyard container to retrieve the locales and the default locale code.
...
In the first step of this tutorial, you installed the [Next.js Starter Storefront](../../../nextjs-starter/page.mdx) along with the Switchyard application. This storefront provides ecommerce features like a product catalog, a cart, and a checkout.
...
So, if your Switchyard application's directory is `medusa-contentful`, you can find the storefront by going back to the parent directory and changing to the `medusa-contentful-storefront` directory:
...
export const getLocalesHighlights = [
  ["13", "getLocales", "Retrieve the locales from the Switchyard server."],
...
1. `getLocales`: Retrieves the locales from the Switchyard server using the API route you created earlier.
...
export const localeSelectHighlights = [
  ...
  ["10", "locales", "The list of locales retrieved from the Switchyard server."],
...
import { ArrowRightMini } from "@switchyard/icons"
...
1. `locales`: The list of locales retrieved from the Switchyard server.
...
To test out all the changes made in this step, start the Switchyard application by running the following command in the Switchyard application's directory:
...
![The locale selector in the side menu](https://res.cloudinary.com/dza7lstvk/image/upload/v1744720332/Switchyard%20Resources/Screenshot_2025-04-15_at_3.31.51_PM_vdqou5.png)
...
1. Linking Switchyard's product to Contentful's product.
...
### Link Switchyard's Product to Contentful's Product

Switchyard facilitates retrieving data across systems using [module links](!docs!/learn/fundamentals/module-links). A module link forms an association between data models of two modules while maintaining module isolation.

Not only do module links support Switchyard data models, but they also support virtual data models that are not persisted in Switchyard's database. In that case, you create a [read-only module link](!docs!/learn/fundamentals/module-links/read-only) that allows you to retrieve data across systems.

In this section, you'll define a read-only module link between Switchyard's product and Contentful's product, allowing you to later retrieve a product's entry in Contentful within a single query.
...
export const productContentfulLinkHighlights = [
  ...
  ["7", "linkable", "The Switchyard data model to link."],
  ["8", "field", "The field in the Switchyard data model that holds the ID of the product."],
...
1. An object with the linkable configuration of the data model in Switchyard, and the field that will be passed as a filter to the Contentful Module's service.
2. An object with the linkable configuration of the virtual data model in Contentful. This object must have the following properties:
   - `serviceName`: The name of the service, which is the Contentful Module's name. Switchyard uses this name to resolve the module's service from the Switchyard container.
...
1. `id`: The ID of the product(s) in Switchyard to retrieve their entries in Contentful.
...
- `fields.medusaId`: Filter the products by their `medusaId` field, which holds the ID of the product in Switchyard.
...
- Pass the product's ID in the `product_id` property. This is essential to map a product in Switchyard to its entry in Contentful.
...
import { QueryContext } from "@switchyard/framework/utils"
...
In the route handler, you retrieve the `locale` and `id` path parameters from the request. Then, you resolve [Query](!docs!/learn/fundamentals/module-links/query) from the Switchyard container.
...
By specifying `contentful_product.*` in the `fields` property, Switchyard will retrieve the product's entry from Contentful using the `list` method you added to the Contentful Module's service.

Switchyard passes the filters and context to the `list` method, and attaches the returned data to the Switchyard product if its `product_id` matches the product's ID.
...
To do that, start the Switchyard application by running the following command in the Switchyard application's directory:
...
![The product titles are shown in the selected locale](https://res.cloudinary.com/dza7lstvk/image/upload/v1744723796/Switchyard%20Resources/Screenshot_2025-04-15_at_4.29.39_PM_izxt6j.png)

Then, if you click on a product, you'll see the product's title and description in the selected locale.

![The product's title and description are shown in the selected locale](https://res.cloudinary.com/dza7lstvk/image/upload/v1744723844/Switchyard%20Resources/Screenshot_2025-04-15_at_4.30.31_PM_pt6ngz.png)

---

## Step 8: Sync Changes from Contentful to Switchyard

The last feature you'll implement is syncing changes from Contentful to Switchyard.

Contentful's webhooks allow you to listen to changes in your Contentful entries. You can then set up a webhook listener API route in Switchyard that updates the product's data.

In this step, you'll set up a webhook listener that updates Switchyard's product data when a Contentful entry is published.

### Prerequisites: Public Server

Webhooks can only trigger deployed listeners. So, you must either [deploy your Switchyard application](../../../deployment/page.mdx), or use tools like [ngrok](https://ngrok.com/) to publicly expose your local application.
...
![The cog icon is at the top right next to the avatar icon. When you click on it, you see a dropdown with the "Webhooks" option](https://res.cloudinary.com/dza7lstvk/image/upload/v1744733023/Switchyard%20Resources/Screenshot_2025-04-15_at_7.03.21_PM_ji7ohw.png)

2. On the Webhooks page, click on the "Add Webhook" button.
3. In the webhook form:
    - In the Name fields, enter a name, such as "Switchyard".
    - In the URL field, enter `{your_app}/hooks/contentful`, where `{your_app}` is the public URL of your Switchyard application. You'll create the `/hooks/contentful` API route in a bit.
    - In the Triggers section, select the "Published" trigger for "Entry".

![The webhook form with the URL and triggers highlighted](https://res.cloudinary.com/dza7lstvk/image/upload/v1744733332/Switchyard%20Resources/Screenshot_2025-04-15_at_7.08.24_PM_coq6oo.png)

    - Scroll down to the "Headers" section, and choose "application/json" for the "Content type" field.

![The webhook form](https://res.cloudinary.com/dza7lstvk/image/upload/v1744733411/Switchyard%20Resources/Screenshot_2025-04-15_at_7.09.53_PM_kvlppv.png)
...
![The "Enable request verification" button in the "Settings" tab](https://res.cloudinary.com/dza7lstvk/image/upload/v1744786362/Switchyard%20Resources/Screenshot_2025-04-16_at_9.51.39_AM_byv3tt.png)

4. Copy the secret that shows up. You can update it later but you can't see the same secret again.

You'll use the secret to verify the webhook request in Switchyard next.

### Update Contentful Module Options

First, add the webhook secret as an environment variable in the Switchyard application's `.env` file:

```plain
CONTENTFUL_WEBHOOK_SECRET=aEl7...
```

Next, add the webhook secret to the Contentful Module options in the Switchyard application's `switchyard.config.ts` file:
...
You only need to implement the first step, as Switchyard provides the other workflows (running as steps) in the `@switchyard/medusa/core-flows` package.
...
export const prepareUpdateDataStepHighlights = [
  ...
  ["13", "contentfulModuleService", "Resolve the Contentful Module's service from the Switchyard container."],
...
In the step, you resolve the Contentful Module's service and use it to retrieve the default locale code. You need it to find the value to update the fields in Switchyard.
...
import { createWorkflow, when } from "@switchyard/framework/workflows-sdk"
import { EntryProps } from "contentful-management"
...
export const contentfulHookRouteHighlights = [
  ["15", "contentfulModuleService", "Resolve the Contentful Module's service from the Switchyard container."],
...
To test out the webhook listener, start the Switchyard application:

```bash npm2yarn
npm run dev
```

Then, try updating a product's title (in the default locale) in Contentful. You should see the product's title updated in Switchyard.

---

## Next Steps

You've now integrated Contentful with Switchyard and supported localized product details. You can expand on the features in this tutorial to:
...
2. Listen to other product events and update the Contentful entries accordingly.
    - Refer to the [Events Reference](/references/events) for details on all events emitted in Switchyard.
...
3. Contact the [sales team](https://switchyard.com/contact/) to get help from the Switchyard team.
