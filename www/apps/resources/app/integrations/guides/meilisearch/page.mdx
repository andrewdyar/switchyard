...
export const metadata = {
  title: `Integrate Meilisearch with Switchyard`,
}

# {metadata.title}

In this tutorial, you'll learn how to integrate Meilisearch with Switchyard to enable advanced search capabilities in your storefront.

When you install a Switchyard application, you get a fully-fledged commerce platform with a Framework for customization. Switchyard's architecture supports integrating third-party services, such as search engines, allowing you to build custom features around core commerce flows.

[Meilisearch](https://www.meilisearch.com/) is an open-source, fast, and relevant search engine that you can integrate with Switchyard to enhance your storefront's search functionality.
...
- Integrate Meilisearch into Switchyard.
...
![Diagram illustrating the integration of Meilisearch with Switchyard](https://res.cloudinary.com/dza7lstvk/image/upload/v1758091098/Switchyard%20Resources/meilisearch-summary_bdig6e.jpg)

<Card
  title="Example Repository"
  text="Find the full code of the guide in this repository."
  href="https://github.com/switchyard/examples/tree/main/meilisearch-integration"
  icon={Github}
/>
...
To integrate third-party services into Switchyard, you create a custom module. A module is a reusable package with functionalities related to a single feature or domain. Switchyard integrates the module into your application without side effects on your setup.

In this step, you'll create a custom module that provides the necessary functionalities to integrate Meilisearch with Switchyard.
...
Before building the module, you need to install Meilisearch's JavaScript client. Run the following command in your Switchyard application's root directory:
...
A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/meilisearch`.
...
The next methods you'll add to the service are methods to retrieve and delete data from the Meilisearch index. You'll use these later to keep the Meilisearch index in sync with Switchyard.
...
The last method you'll implement is a method to search through the Meilisearch index. This method lets you expose search functionality to clients through Switchyard's API routes.
...
To keep the Meilisearch index in sync with Switchyard, you need to trigger indexing when products are created, updated, or deleted in Switchyard. You can also allow admins to manually trigger a reindex.
...
Switchyard provides the `useQueryGraphStep` in its `@switchyard/medusa/core-flows` package. You only need to implement the other steps.
...
If you get a type error on resolving the Meilisearch Module, run the Switchyard application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](!docs!/learn/fundamentals/generated-types).
...
In the step function, you resolve the Meilisearch Module's service from the Switchyard container using the name you exported in the module definition's file.
...
The `deleteProductsFromMeilisearchStep` step deletes products from Meilisearch when they are unpublished or deleted in Switchyard.
...
1. Retrieve products from Switchyard's database using `useQueryGraphStep`. This step uses Switchyard's [Query](!docs!/learn/fundamentals/module-links/query) tool to retrieve data across modules. You pass it the pagination and filter parameters you received in the input.
...
2. Creating an API route that the Switchyard Admin dashboard can call to emit the `meilisearch.sync` event, which triggers the subscriber.
3. Adding a new page or UI route to the Switchyard Admin dashboard to allow admins to trigger the reindex.
...
To allow the Switchyard Admin dashboard to trigger the `meilisearch.sync` event, you need to create an API route that emits the event.
...
In the route handler, you use the Switchyard container that is available in the request object. You resolve the [Event Module](../../../infrastructure-modules/event/page.mdx). This module manages events and their subscribers.
...
Before creating the UI route, you'll configure Switchyard's [JS SDK](../../../js-sdk/page.mdx). You can use it to send requests to the Switchyard server from any client application, including your Switchyard Admin customizations.
...
So, to create a new page under the Settings section of the Switchyard Admin, create the file `src/admin/routes/settings/meilisearch/page.tsx` with the following content:
...
You'll now test out the entire flow. Start by triggering the reindex manually from the Switchyard Admin dashboard, then check the Meilisearch dashboard for the indexed products.
...
![The Meilisearch Sync page in the Switchyard Admin dashboard with a button to sync products to Meilisearch](https://res.cloudinary.com/dza7lstvk/image/upload/v1758094934/Switchyard%20Resources/CleanShot_2025-09-17_at_10.41.51_2x_wph4pp.png)
...
Finally, you can check the Meilisearch dashboard to see the indexed products. Open the Meilisearch dashboard, either Cloud or local, and choose the index you specified for products in the environment variable `MEILISEARCH_PRODUCT_INDEX_NAME`. You'll find your Switchyard products indexed there.

![The Meilisearch dashboard showing the indexed products](https://res.cloudinary.com/dza7lstvk/image/upload/v1758095145/Switchyard%20Resources/CleanShot_2025-09-17_at_10.45.29_2x_y8ytzf.png)
...
Then, either create a product or update an existing one using the Switchyard Admin dashboard. If you check the Meilisearch dashboard, you'll find that the product's index was created or updated.
...
Then, delete a product from the Switchyard Admin dashboard. If you check the Meilisearch dashboard, you'll find that the product index was deleted there as well.

---

## Step 6: Search Products in Next.js Starter Storefront

The last step is to provide search functionalities to customers on your storefront. In the first step, you installed the [Next.js Starter Storefront](../../../nextjs-starter/page.mdx) along with the Switchyard application.
...
![The Next.js Starter Storefront showing the search modal with search results](https://res.cloudinary.com/dza7lstvk/image/upload/v1758096281/Switchyard%20Resources/CleanShot_2025-09-17_at_11.04.26_2x_bhb9k7.png)

---

## Next Steps

You've now integrated Meilisearch with Switchyard and added search functionality to your storefront. You can expand on these features to:
