...
In this guide, you'll learn how to support digital products in Switchyard.

When you install a Switchyard application, you get a fully-fledged commerce platform with support for customizations. Switchyard provides all features related to products and managing them, and the Switchyard Framework allows you to extend those features and implement your custom use case.

You can extend Switchyard's product features to support selling, storing, and fulfilling digital products. In this guide, you'll customize Switchyard to add the following features:
...
5. All other commerce features that Switchyard provides.

<Note>

This guide provides an example of an approach to implement digital products. You're free to choose a different approach using the Switchyard Framework.

</Note>

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/digital-product",
...
Switchyard creates commerce features in modules. For example, product features and data models are created in the Product Module.

You also create custom commerce data models and features in custom modules. They're integrated into the Switchyard application similar to Switchyard's modules without side effects.
...
class DigitalProductModuleService extends SwitchyardService({
  DigitalProduct,
  DigitalProductMedia,
  DigitalProductOrder,
}) {
...
### Add Module to Switchyard Configuration

Finally, add the module to the list of modules in `switchyard.config.ts`:
...
Modules are isolated in Switchyard, making them reusable, replaceable, and integrable in your application without side effects.
...
In this step, you’ll define links between your module’s data models and data models from Switchyard’s Commerce Modules:
...
export const orderLinkHighlights = [
  ["8", "deleteCascade", "Delete the digital product order when its linked Switchyard order is deleted."]
]
...
`deleteCascades` is enabled on the `digitalProductOrder` so that when a Switchyard order is deleted, its linked digital product order is also deleted.
...
```bash
npx switchyard db:generate digitalProductModuleService
```
...
To expose custom commerce features to frontend applications, such as the Switchyard Admin dashboard or a storefront, you expose an endpoint by creating an API route.
...
```mermaid
graph TD
  createProductsWorkflow["createProductsWorkflow (Switchyard)"] --> createDigitalProductStep
  createDigitalProductStep --> createDigitalProductMediasStep
  createDigitalProductMediasStep --> createRemoteLinkStep["createRemoteLinkStep (Switchyard)"]
```

1. `createProductsWorkflow`: Create the Switchyard product that the digital product is associated with its variant. Switchyard provides this workflow through the `@switchyard/medusa/core-flows` package, which you can use as a step.
...
4. `createRemoteLinkStep`: Create the link between the digital product and the product variant. Switchyard provides this step through the `@switchyard/medusa/core-flows` package.
...
export const createDpWorkflowHighlights = [
  ["36", "createProductsWorkflow", "Create the Switchyard product."],
...
This creates the `createDigitalProductWorkflow`. The workflow accepts as a parameter the digital product and the Switchyard product to create.

In the workflow, you run the following steps:

1. `createProductsWorkflow` as a step to create a Switchyard product.
...
export const POST = async (
  req: AuthenticatedSwitchyardRequest<CreateRequestBody>,
  res: SwitchyardResponse
) => {
  const query = req.scope.resolve(ContainerRegistrationKeys.QUERY)
...
To upload the digital product media files, use Switchyard’s File Module.

<Note>

Your Switchyard application uses the local file module provider by default, which uploads files to a local directory. However, you can use other file module providers, such as the [S3 module provider](../../../../infrastructure-modules/file/s3/page.mdx).
...
import { uploadFilesWorkflow } from "@switchyard/medusa/core-flows"
...
In the route handler, you use `uploadFilesWorkflow` from Switchyard's core workflows to upload the file. If the file type is `main`, it’s uploaded with private access, as only customers who purchased it can download it. Otherwise, it’s uploaded with `public` access.
...
The Switchyard Admin is customizable, allowing you to inject widgets into existing pages or add UI routes to create new pages.

In this step, you’ll add a UI route to the Switchyard Admin that displays a list of digital products.
...
import { PhotoSolid } from "@switchyard/icons"
import { Container, Heading, Table } from "@switchyard/ui"
...
This creates a UI route that's displayed at the `/digital-products` path in the Switchyard Admin. The UI route also adds a sidebar item with the label “Digital Products" pointing to the UI route.
...
export const fetchDigitalProductsHighlights = [
  ["7", "fetchProducts", "A function that fetches the digital products from the Switchyard application."],
...
To test the UI route out, start the Switchyard application, go to `localhost:9000/app`, and log in as an admin user.
...
import { useState } from "react"
import { Input, Button, Select, toast } from "@switchyard/ui"
import { MediaType } from "../../types"
...
```tsx title="src/admin/routes/digital-products/page.tsx"
// other imports...
import { Drawer } from "@switchyard/ui"
...
To test the form, open the Digital Products page in the Switchyard Admin. There, you’ll find a new Create button.
...
When a product is deleted, Switchyard emits a `product.deleted` event. You can handle this event with a subscriber. A subscriber is an asynchronous function that, when an event is emitted, is executed. You can implement in subscribers features that aren't essential to the original flow that emitted the event.
...
import { SubscriberArgs, SubscriberConfig } from "@switchyard/framework"
import { 
...
- `container`: Instance of the [Switchyard Container](!docs!/learn/fundamentals/medusa-container).

In the subscriber, you execute the workflow by invoking it, passing the Switchyard container as an input, then executing its `run` method. You pass the product's ID, which is received through the event's data payload, as an input to the workflow.

### Test it Out

To test this out, start the Switchyard application and, from the Switchyard Admin dashboard, delete a product that has digital products. You can confirm that the digital product was deleted by checking the Digital Products page.
...
import { ModuleProviderExports } from "@switchyard/framework/types"
...
### Register Module Provider in Switchyard's Configurations

Finally, register the module provider in `switchyard.config.ts`:
...
In the Switchyard Admin, go to Settings -> Location & Shipping, and add the fulfillment provider and a shipping option for it in a location.
...
In this step, you’ll create a new cart completion flow that not only creates a Switchyard order, but also create a digital product order.
...
```mermaid
graph TD
  completeCartWorkflow["completeCartWorkflow (Switchyard)"] --> useQueryGraphStep["useQueryGraphStep (Switchyard)"]
  useQueryGraphStep --> when{order has digital products?}
  when -->|Yes| createDigitalProductOrderStep
  createDigitalProductOrderStep --> createRemoteLinkStep["createRemoteLinkStep (Switchyard)"]
  createRemoteLinkStep --> createOrderFulfillmentWorkflow["createOrderFulfillmentWorkflow (Switchyard)"]
  createOrderFulfillmentWorkflow --> emitEventStep["emitEventStep (Switchyard)"]
  emitEventStep --> End
  when -->|No| End
```
...
1. `completeCartWorkflow` to create a Switchyard order from the cart. Switchyard provides this workflow through the `@switchyard/medusa/core-flows` package and you can use it as a step.
2. `useQueryGraphStep` to retrieve the order’s items with the digital products associated with the purchased product variants. Switchyard provides this step through the `@switchyard/medusa/core-flows` package.
3. If the order has digital products, you:
    1. create the digital product order.
    2. link the digital product order with the Switchyard order. Switchyard provides a `createRemoteLinkStep` in the `@switchyard/medusa/core-flows` package that can be used here.
    3. Create a fulfillment for the digital products in the order. Switchyard provides a `createOrderFulfillmentWorkflow` in the `@switchyard/medusa/core-flows` package that you can use as a step here.
    4. Emit the `digital_product_order.created` custom event to handle it later in a subscriber and send the customer an email. Switchyard provides a `emitEventStep` in the `@switchyard/medusa/core-flows` that you can use as a step here.
...
export const createDpoWorkflowHighlights = [
  ...
  ["37", "completeCartWorkflow", "Completes the cart and creates the Switchyard order."],
  ["43", "useQueryGraphStep", "Retrieve the order's items with their associated variants and linked digital products."],
  ["60", "useQueryGraphStep", "Retrieve any existing links between the digital product order and the Switchyard order."],
  ...
  ["93", "createRemoteLinkStep", "Link the digital product order to the Switchyard order."],
...
2. `completeCartWorkflow` as a step to create the Switchyard order.
3. `useQueryGraphStep` to retrieve the order’s items with their associated variants and linked digital products.
4. `useQueryGraphStep` to retrieve any existing links between the digital product order and the Switchyard order.
...
6. Use `when` to check whether the order has digital products and no existing links. If so:
    ...
    2. Use the `createRemoteLinkStep` to link the digital product order to the Switchyard order.
...
The workflow returns the Switchyard order and the digital product order, if created.
...
So, if your Switchyard application's directory is `medusa-digital-product`, you can find the storefront by going back to the parent directory and changing to the `medusa-digital-product-storefront` directory:
...
Then, run the following command in the Switchyard application directory to start the Switchyard application:
...
1. Retrieve the digital product order's details. For this, you'll use `useQueryGraphStep` from Switchyard's core workflows.
2. Send a notification to the customer with the digital products to download.
3. Mark the Switchyard order's fulfillment as delivered. For this, you'll use `markOrderFulfillmentAsDeliveredWorkflow` from Switchyard's core workflows.
...
```ts
import { OrderDTO, InferTypeOf } from "@switchyard/framework/types"
import DigitalProductOrder from "../models/digital-product-order"
...
1. Retrieve the digital product order's details using `useQueryGraphStep` from Switchyard's core workflows.
2. Send a notification to the customer with the digital product download links using the `sendDigitalOrderNotificationStep`.
3. Check whether the Switchyard order has fulfillments. If so, mark the order's fulfillment as delivered using `markOrderFulfillmentAsDeliveredWorkflow` from Switchyard's core workflows.
...
For testing purposes, add to `switchyard.config.ts` the following to use the Local Notification Module Provider:
...
To test it out, run the Next.js starter with the Switchyard application, then open the details page of a product that’s digital. You should see a “Download Preview” button to download the preview media of the product.
