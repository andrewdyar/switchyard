...
import { Github, PlaySolid, ArrowUpRightOnBox } from "@switchyard/icons"
import { Prerequisites, WorkflowDiagram, CardList, InlineIcon } from "docs-ui"

export const metadata = {
  title: `Implement Personalized Products in Switchyard`,
}

# {metadata.title}

In this tutorial, you will learn how to implement personalized products in Switchyard.

When you install a Switchyard application, you get a fully-fledged commerce platform with a Framework for customization. You can benefit from existing features and extend them to implement your business requirements.
...
- Extend the Switchyard Admin dashboard to show personalized product data in an order.

You can follow this tutorial whether you're new to Switchyard or an advanced Switchyard developer.

![Diagram showcasing requests sent from the storefront to the server to get custom price, add product to cart, and place order. Then the admin user views the personalized items in an order.](https://res.cloudinary.com/dza7lstvk/image/upload/v1753167036/Switchyard%20Resources/personalized-products_fgxy6s.jpg)

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/personalized-products",
    title: "Full Code",
...
So, to create a personalized product, you can just create a [regular product using the Switchyard Admin](!user-guide!/products/create).
...
To do that from the Switchyard Admin dashboard:
...
![Screenshot of Switchyard Admin metadata editor showing a form field with 'is_personalized' as the key and 'true' as the value](https://res.cloudinary.com/dza7lstvk/image/upload/v1753108787/Switchyard%20Resources/CleanShot_2025-07-21_at_14.55.50_2x_m7ynd7.png)
...
In Switchyard, you implement your custom commerce flows in workflows. Then, you execute those workflows from other customizations, such as API routes.
...
Switchyard provides the `useQueryGraphStep`, so you only need to implement the `getCustomPriceStep` step.
...
export async function POST(
  req: SwitchyardRequest<PostCustomPriceSchemaType>,
...
In the route handler, you execute the `getCustomPriceWorkflow` workflow by invoking it, passing the Switchyard container (stored in `req.scope`), then executing its `run` method.
...
You apply Switchyard's `validateAndTransformBody` middleware to `POST` requests sent to the `/store/variants/:id/price` route.
...
So, if your Switchyard application's directory is `medusa-personalized`, you can find the storefront by going back to the parent directory and changing to the `medusa-personalized-storefront` directory:
...
![Screenshot of a product page in the Next.js storefront showing a fabric product with two input fields labeled 'Width (cm)' and 'Height (cm)' below the product options, and a calculated price displayed underneath the dimension inputs](https://res.cloudinary.com/dza7lstvk/image/upload/v1753111095/Switchyard%20Resources/CleanShot_2025-07-21_at_18.17.55_2x_hnzfiz.png)
...
So, you'll create a workflow that wraps around Switchyard's existing add-to-cart logic to add the personalized product to the cart with the custom price. Then, you'll create an API route that executes this workflow.

<Note title="Tip">

If you're not using custom pricing, you can skip this step and keep on using Switchyard's existing [Add-to-Cart API route](!api!/store#carts_postcartsidlineitems).
...
export async function POST(
  req: SwitchyardRequest<PostAddCustomLineItemSchemaType>,
  res: SwitchyardResponse
) {
...
Switchyard's `addToCartWorkflow` workflow supports performing custom validation on the items being added to the cart using [workflow hooks](!docs!/learn/fundamentals/workflows/workflow-hooks). A workflow hook is a point in a workflow where you can inject custom functionality as a step function.
...
export const validatePersonalizedProductHighlights = [
  ...
  ["6", "query", "Resolve Query from the Switchyard container."],
...
1. Resolve [Query](!docs!/learn/fundamentals/module-links/query) from the Switchyard container to retrieve data across modules.
...
To test out the custom add-to-cart logic, ensure that both the Switchyard application and the Next.js Starter Storefront are running.
...
![Screenshot of the shopping cart page showing a personalized fabric product with 'Width: 100cm' and 'Height: 80cm' displayed below the product variant information, demonstrating how custom dimensions are preserved in the cart](https://res.cloudinary.com/dza7lstvk/image/upload/v1753112339/Switchyard%20Resources/CleanShot_2025-07-21_at_18.38.30_2x_ersc2m.png)
...
## Step 8: Show an Order's Personalized Items in Switchyard Admin

In this step, you'll customize the Switchyard Admin to show the personalized item's height and width values in an order's details page.

The Switchyard Admin dashboard is extensible, allowing you to either inject custom components into existing pages, or create new pages.
...
- A `config` object created with the `defineWidgetConfig` function. It accepts an object with the `zone` property that indicates where the widget will be rendered in the Switchyard Admin dashboard.
...
To test out the personalized order items widget, start the Switchyard application and open the Switchyard Admin dashboard in your browser at `http://localhost:9000/app`.
...
![Screenshot of Switchyard Admin order details page showing a custom widget titled 'Personalized Order Items' with a fabric product entry displaying the product image, title, and dimensions 'Width (cm): 100' and 'Height (cm): 80' in a clean list format](https://res.cloudinary.com/dza7lstvk/image/upload/v1753113169/Switchyard%20Resources/CleanShot_2025-07-21_at_18.52.21_2x_i7shff.png)

---

## Next Steps

You've now implemented personalized products in Switchyard, allowing customers to customize product dimensions and see the calculated price in the storefront. You can expand on this feature to:
