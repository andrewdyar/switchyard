...
This recipe provides the general steps to build a Point of Sale (POS) system with Switchyard.
...
Switchyard's modular architecture solves these challenges. Any frontend can utilize Switchyard's commerce features, such as sales channels or multi-warehouse features, through its REST APIs.

<Note title="Related use-case">

[How Tekla built a POS system with Switchyard](https://switchyard.com/blog/tekla-agilo-pos-case/).
...
Switchyard's modular architecture removes any restrictions you may have while making this choice. Any client or frontend can connect to the Switchyard application using its headless REST APIs.

![POS Tech Stack](https://res.cloudinary.com/dza7lstvk/image/upload/v1709034046/Switchyard%20Book/pos-tech-stack_fy8uiu.jpg)

<Card
  href="!api!/admin"
  title="Admin REST APIs"
  text="Check out available Admin REST APIs in Switchyard."
...
![Example flow of integrating a barcode scanner](https://res.cloudinary.com/dza7lstvk/image/upload/v1709034282/Switchyard%20Book/pos-scan-barcode_a8j8ew.jpg)
...
Switchyard's [Inventory](../../commerce-modules/inventory/page.mdx), [Stock Location](../../commerce-modules/stock-location/page.mdx), and [Sales Channel](../../commerce-modules/sales-channel/page.mdx) modules allow merchants to manage the inventory items and their availability across locations and sales channels.

![Using Multi-warehouse features with POS](https://res.cloudinary.com/dza7lstvk/image/upload/v1709034731/Switchyard%20Book/pos-multiwarehouse_r1z48x.jpg)
...
Using Switchyard's [Customer Module](../../commerce-modules/customer/page.mdx), you can retrieve a customer's details from the Switchyard application and place an order on the POS under their account. The customer can then view their order details on their online profile as if they had placed the order online.

In addition, using Switchyard's [Promotion Module](../../commerce-modules/promotion/page.mdx), store operators can create promotions on the fly for customers using the POS system and apply them to their orders.
...
Switchyard's architecture allows you to integrate any third-party payment provider for your POS and online storefront. For example, you can integrate [Stripe Terminal](https://stripe.com/terminal) to accept in-store payments.
...
Then, merchants can view all orders coming from different sales channels using the Switchyard Admin. This keeps logistics and order handling consistent between online and in-store customers.
