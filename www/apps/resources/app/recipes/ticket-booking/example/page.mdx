...
export const metadata = {
  title: `Implement a Ticket Booking System with Switchyard`,
}

# {metadata.title}

In this tutorial, you'll learn how to implement a ticket booking system using Switchyard.
...
Switchyard's [Framework](!docs!/learn/fundamentals/framework) facilitates customizing Switchyard's core features for your specific use case, such as ticket booking.

<Note>

This tutorial provides an approach to implement a ticket booking system using Switchyard. Depending on your specific requirements, you might need to adjust the implementation details or explore a different approach.
...
- Create data models for venues and tickets, and link them to Switchyard's data models.
- Customize the Switchyard Admin to manage venues and shows or events.
...
![Diagram showing the architecture of the ticket booking system with Switchyard](https://res.cloudinary.com/dza7lstvk/image/upload/v1757512301/Switchyard%20Resources/ticket-booking-diagram_egbpye.jpg)

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/ticket-booking",
    title: "Full Code",
...
You'll define data models to represent venues, tickets, and purchases. Later, you'll link these data models to Switchyard's data models, such as products and orders.
...
The `TicketProduct` data model represents a product purchased as a ticket, such as a show or event. It will be linked to Switchyard's `Product` data model.
...
- `id`: The primary key of the table.
- `product_id`: The ID of the linked product in Switchyard's `Product` data model.
...
The `TicketProductVariant` data model represents a variant of a ticket product, such as a specific row type. It will be linked to Switchyard's `ProductVariant` data model.
...
- `id`: The primary key of the table.
- `product_variant_id`: The ID of the linked product variant in Switchyard's `ProductVariant` data model.
...
The `TicketPurchase` data model represents the purchase of a seat for a specific `TicketProduct`. It will be linked to Switchyard's `Order` data model.
...
- `id`: The primary key of the table.
- `order_id`: The ID of the linked order in Switchyard's `Order` data model.
...
export class TicketBookingModuleService extends SwitchyardService({
  Venue,
  VenueRow,
  TicketProduct,
...
The `TicketBookingModuleService` extends `SwitchyardService`, which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.

The `TicketBookingModuleService` class now has methods like `createTicketProducts` and `retrieveVenue`.

<Note>

Find all methods generated by the `SwitchyardService` in [the Service Factory reference](../../../service-factory-reference/page.mdx).

</Note>

### d. Export Module Definition

The final piece of a module is its definition, which you export in an `index.ts` file at its root directory. This definition tells Switchyard the name of the module and its service.
...
Switchyard's CLI tool can generate the migrations for you. To generate a migration for the Ticket Booking Module, run the following command in your Switchyard application's directory:

```bash
npx switchyard db:generate ticketBooking
```

The `db:generate` command of the Switchyard CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/ticket-booking` that holds the generated migration.
...
## Step 3: Link Ticket Booking to Switchyard Data Models
...
In this step, you'll define a link between the data models in the Ticket Booking Module and Switchyard's Commerce Modules.

### a. TicketProduct \<\> Product Link

To define a link between the `TicketProduct` data model and Switchyard's `Product` data model, create the file `src/links/ticket-product.ts` with the following content:
...
In later steps, you'll learn how this link allows you to retrieve and manage ticket products and their related Switchyard products.
...
To define a link between the `TicketProductVariant` data model and Switchyard's `ProductVariant` data model, create the file `src/links/ticket-product-variant.ts` with the following content:
...
Finally, to define a link between the `TicketPurchase` data model and Switchyard's `Order` data model, create the file `src/links/ticket-purchase-order.ts` with the following content:
...
This command will create the necessary tables to manage the links between the Ticket Booking Module's data models and Switchyard's data models.
...
When you build commerce features in Switchyard that can be consumed by client applications, such as the Switchyard Admin dashboard or storefront, you need to implement:
...
The `useQueryGraphStep` is available through Switchyard's `@switchyard/medusa/core-flows` package. You'll implement other steps in the workflow.
...
In the step function, you resolve the Ticket Booking Module's service from the Switchyard container using its `resolve` method, passing it the module's name as a parameter.
...
export async function POST(
  req: SwitchyardRequest<CreateVenueSchema>,
...
You'll test this API route later when you customize the Switchyard Admin.
...
You apply Switchyard's `validateAndTransformBody` middleware to `POST` requests sent to the `/admin/venues` route.
...
In this step, you'll add an API route to retrieve a list of venues. You'll use this API route later to display a list of venues in the Switchyard Admin.
...
In the route handler, you resolve Query from the Switchyard container and use it to retrieve a list of venues.
...
You'll test out this API route later when you customize the Switchyard Admin.
...
1. A Zod schema to validate the query parameters. You use Switchyard's `createFindParams` utility function to create a schema that validates common query parameters like `limit`, `offset`, `fields`, and `order`.
...
## Step 6: Manage Venues in Switchyard Admin

In this step, you'll customize the Switchyard Admin to allow admin users to manage venues.
...
In this step, you'll create a new page or UI route in the Switchyard Admin to view a list of venues and create new venues.
...
Then, you use the `useDataTable` hook from Switchyard UI to create a data table instance with the fetched venues and the columns you defined earlier.
...
You use Switchyard UI's `FocusModal` component to render the modal. Inside the modal, you render a form with input fields for the venue's name and address, and fields to add rows.
...
You can now test the venues page in the Switchyard Admin.

Run the following command to start the Switchyard server:

```bash npm2yarn
npm run dev
```

Then, open `http://localhost:9000/app` in your browser to access the Switchyard Admin. Log in with the admin user you created earlier.
...
![Venues Page in the Switchyard Admin with an empty table](https://res.cloudinary.com/dza7lstvk/image/upload/v1757430925/Switchyard%20Resources/CleanShot_2025-09-09_at_18.15.02_2x_o0pf16.png)
...
![Create Venue Modal in the Switchyard Admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1757431196/Switchyard%20Resources/CleanShot_2025-09-09_at_18.19.45_2x_qyov67.png)
...
The workflow that creates a ticket product will also create the Switchyard product and its variants. It will create a variant for each show date and row type. For example, if a show has two dates and three row types, the workflow will create six variants for the Switchyard product.
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        type: "step",
        name: "useQueryGraphStep",
        description: "Retrieve default store configuration that are useful for creating the Switchyard product.",
        depth: 2,
        link: "/references/helper-steps/useQueryGraphStep",
      },
      {
        ...
        description: "Creates inventory items for the Switchyard product.",
        depth: 3,
      ...
      },
      {
        ...
        description: "Creates the Switchyard product and its variants.",
        depth: 4,
      ...
      },
      {
        ...
        description: "Create links between the ticket product and variants and the Switchyard product and variants.",
        depth: 7
      },
      {
        type: "step",
...
You only need to implement the `validateVenueAvailabilityStep`, `createTicketProductsStep`, and `createTicketProductVariantsStep` steps. The other steps are provided by Switchyard.
...
export const createTicketProductVariantsStep = createStep(
  ...
  async (input: CreateTicketProductVariantsStepInput, { container }) => {
    ...
    // Create ticket product variants for each Switchyard variant
...
import { useQueryGraphStep, createProductsWorkflow, createRemoteLinkStep, createInventoryItemsWorkflow } from "@switchyard/medusa/core-flows"
import { CreateProductWorkflowInputDTO, CreateMoneyAmountDTO } from "@switchyard/framework/types"
...
Then, you retrieve the default store configuration using the `useQueryGraphStep`. These configurations are useful when creating the Switchyard inventory items and product.

<Note>

Make sure you have [default location and sales channel set up in your Switchyard store](!user-guide!/settings/store).

</Note>

Next, you'll create inventory items for each variant before creating the Switchyard product. Replace the `TODO` with the following:
...
// TODO create the Switchyard product
```
...
Next, you'll create the Switchyard product and variants. Replace the `TODO` with the following:
...
You prepare the Switchyard product data using the `transform` function. You create options for "Date" and "Row Type" and create a variant for each combination of show date and row type.
...
Then, you create the Switchyard product using the `createProductsWorkflow`.
...
Finally, you'll create links between the ticket product and variants and the Switchyard product and variants, and retrieve the created ticket product with its relations. Replace the `TODO` with the following:
...
const linksData = transform({
  ...
  // Create links between ticket product and Switchyard product
  const productLinks = [{
  ...
  // Create links between ticket variants and Switchyard variants
...
You create links between the ticket product and the Switchyard product, and between the ticket product variants and the Switchyard product variants using the `createRemoteLinkStep`.
...
export async function POST(
  req: SwitchyardRequest<CreateTicketProductSchema>,
  res: SwitchyardResponse
) {
...
You'll test the API route when you customize the Switchyard Admin later.
...
In this step, you'll add an API route to list ticket products. This will be useful when you customize the Switchyard Admin to display ticket products.
...
## Step 9: Manage Ticket Products in Switchyard Admin

In this step, you'll customize the Switchyard Admin to add a new page that shows a list of ticket products and allows creating new ticket products.

### a. Define Ticket Product Type

Before you customize the Switchyard Admin, you'll define a type for the ticket product to use in your customizations.
...
import { defineRouteConfig } from "@switchyard/admin-sdk"
import { ReceiptPercent } from "@switchyard/icons"
import {
  createDataTableColumnHelper,
  Container,
  DataTable,
...
First, you define the columns for the data table that will display ticket products. You create columns for the ticket product's name, venue, dates, and a link to view the associated Switchyard product.
...
Then, you use Switchyard UI's `DataTable` component to render the table of ticket products.
...
In Switchyard, you can set the price of a product variant in multiple currencies and regions. This allows you to sell products in different markets with localized pricing. Learn more in the [Pricing documentation](../../../commerce-modules/pricing/concepts/page.mdx).
...
Next, you'll fetch the list of venues, regions, and store currencies from the Switchyard backend. Replace the `TODO` with the following:
...
You can now test the ticket products page in the Switchyard Admin.

Start the Switchyard server if it isn't already running. If you open the Switchyard Admin and log in, you should see a new "Shows" item in the sidebar. If you click on it, you can see a table of ticket products with a button to create a new show.

![Shows page with the table of ticket products and a button to create a new show](https://res.cloudinary.com/dza7lstvk/image/upload/v1757490157/Switchyard%20Resources/CleanShot_2025-09-10_at_10.42.00_2x_jsaq7v.png)
...
![Create Show modal with the Product Details step](https://res.cloudinary.com/dza7lstvk/image/upload/v1757490395/Switchyard%20Resources/CleanShot_2025-09-10_at_10.46.23_2x_v2pbuo.png)
...
![Create Show modal with the Pricing step](https://res.cloudinary.com/dza7lstvk/image/upload/v1757490725/Switchyard%20Resources/CleanShot_2025-09-10_at_10.51.55_2x_bijkab.png)

This will create a new ticket product with a Switchyard product. You can view the ticket product in the table, and you can click the link to view the associated Switchyard product.

You can edit the associated Switchyard product to add images, descriptions, and other details for the show.
...
Switchyard implements cart operations in workflows. Specifically, you'll focus on the `addToCartWorkflow`. Switchyard allows you to inject custom logic into workflows using [hooks](!docs!/learn/fundamentals/workflows/workflow-hooks).
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        ...
        description: "Complete the cart using Switchyard's default completeCartWorkflow",
        depth: 2,
...
You only need to implement the `createTicketPurchasesStep` and `validateTicketOrderStep` steps, as the other steps and workflows are provided by Switchyard.
...
2. Complete the cart using Switchyard's `completeCartWorkflow`.
...
export const GET = async (req: SwitchyardRequest, res: SwitchyardResponse) => {
  ...
  if (!ticketProduct) {
    throw new SwitchyardError(SwitchyardError.Types.NOT_FOUND, "Ticket product not found")
  }
...
1. Retrieve the ticket product by its associated Switchyard product ID, including its dates, venue, variants, and purchases.
...
1. Retrieve the ticket product by its associated Switchyard product ID, including its venue, rows, variants, and purchases.
...
To send an email in Switchyard when an order is placed, you'll need:
...
First, set up a Notification Module Provider in your Switchyard application.

For example, to set up the SendGrid Notification Module Provider, add it to `switchyard.config.ts`:
  ...
  modules: [
    // ...
    {
      ...
      options: {
        providers: [
          {
            resolve: "@switchyard/medusa/notification-sendgrid",
            id: "sendgrid",
            options: {
              channels: ["email"],
    ...
    <tr>
      <td align="center">
        <table role="presentation" class="container" cellpadding="0" cellspacing="0" width="100%">
          ...
          <tr>
            <td class="header">
              <div class="brand">Switchyard</div>
            </td>
          </tr>
...
Start by installing the `qrcode` package in your Switchyard application with the following command:
...
import { SubscriberArgs, type SubscriberConfig } from "@switchyard/medusa"
import { TICKET_BOOKING_MODULE } from "../modules/ticket-booking"
...
To test the order confirmation email, [customize the storefront](./storefront/page.mdx) first. Then, start the Switchyard application and the Next.js Starter Storefront.

Place an order with at least one ticket product. After placing the order, you'll see the following message in your Switchyard application's terminal:
...
export async function POST(req: SwitchyardRequest, res: SwitchyardResponse) {
  const { id } = req.params
...
To test the ticket verification functionality, start the Switchyard application.
