...
import { AcademicCapSolid, PuzzleSolid } from "@switchyard/icons";
...
This recipe provides the general steps to implement a ticket booking system in your Switchyard application.

<Note>

Follow the step-by-step [Ticket Booking System Example](./example/page.mdx) to learn how to implement a ticket booking system in your Switchyard application.
...
Switchyard's [Framework](!docs!/learn/fundamentals/framework) facilitates building a ticket booking system on top of the existing commerce functionalities, such as products, pricing, inventory, carts, and orders. All of these are provided by Switchyard's [Commerce Modules](../../commerce-modules/page.mdx).

To support ticket-booking features, you can customize the Switchyard application by creating a Ticket Booking Module, linking its data models to Switchyard's existing models, and building flows around the module.

---

## Create Ticket Booking Module

Your custom features and functionalities are implemented inside modules. The module is integrated into the Switchyard application without any implications on existing functionalities.
...
Your module’s main service holds data-management and other related features. Then, in other resources, such as an API route, you can resolve the service from the Switchyard container and use its functionalities.

Switchyard facilitates implementing data-management features using the service factory. Your module's main service can extend this service factory, and it generates data-management methods for your data models.
...
You can then utilize the API routes that execute these workflows in your client applications, such as your Switchyard Admin customizations or your storefront.
...
By linking the `TicketProductVariant` model to the Product Module's `ProductVariant` model, you can leverage Switchyard's inventory management features, implemented in the Inventory Module, to manage ticket product variants' inventory.
...
By default, Switchyard product variants require shipping, which prompts the customer to provide a shipping address and choose a shipping method during checkout.
...
You can extend the Switchyard Admin to provide merchants with an interface to manage ticket-booking-related data, such as shows and venues. You can inject widgets into existing pages or create new pages.
...
<CardList items={[
  {
    ...
    text: "Learn how to create a widget in the Switchyard Admin.",
    icon: AcademicCapSolid,
  },
  {
    href: "!docs!/learn/fundamentals/admin/ui-routes",
    title: "Create UI Route",
    text: "Learn how to create a UI route in the Switchyard Admin.",
    icon: AcademicCapSolid,
  },
]} />
...
Switchyard's workflows provide hooks that allow you to inject custom logic at specific points in the workflow execution.
...
When an order is placed, Switchyard emits an `order.placed` event. You can listen to this event in a subscriber, which is an asynchronous function executed in the background when its associated event is emitted.
...
Switchyard provides a Next.js Starter Storefront with standard commerce features including listing products, placing orders, and managing accounts. You can customize the storefront and tailor its functionalities to support ticket booking features.
...
<CardList items={[
  ...
  },
  {
    ...
    text: "Learn how to build a storefront for your Switchyard application.",
    icon: AcademicCapSolid,
  },
]} />