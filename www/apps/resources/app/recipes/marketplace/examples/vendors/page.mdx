...
In this guide, you'll learn how to build a marketplace with Switchyard.

When you install a Switchyard application, you get a fully-fledged commerce platform with support for customizations. While Switchyard doesn't provide marketplace functionalities natively, it provides features that you can extend and a Framework to support all your customization needs to build a marketplace.

## Summary

In this guide, you'll customize Switchyard to build a marketplace with the following features:
...
This guide provides an example of an approach to implement marketplaces. You're free to choose a different approach using the Switchyard Framework.

</Note>

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/marketplace",
...
A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/marketplace`.
...
class MarketplaceModuleService extends SwitchyardService({
  Vendor,
  VendorAdmin,
}) { }
...
The `MarketplaceModuleService` extends `SwitchyardService` from the Modules SDK which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.
...
Switchyard's CLI tool generates the migrations for you. To generate a migration for the Marketplace Module, run the following command in your Switchyard application's directory:

```bash
npx switchyard db:generate marketplace
```

The `db:generate` command of the Switchyard CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/marketplace` that holds the generated migration.
...
Switchyard represents the links you define in link tables similar to pivot tables. So, to sync the defined links to the database, run the `db:migrate` command:
...
An [actor type](../../../../commerce-modules/auth/auth-identity-and-actor-types/page.mdx#actor-types) is a type of user that can send an authenticated requests. Switchyard has two default actor types: `customer` for customers, and `admin` for admin users.
...
1. Retrieve a registration JWT token. Switchyard has a `/auth/{actor_type}/emailpass/register` route to retrieve a registration JWT token for the specified actor type.
...
3. Retrieve an authenticated JWT token using Switchyard's `/auth/{actor_type}/emailpass` route, which retrieves the token for the specified actor type if the credentials in the request body match a user in the database.

In the next steps, you'll implement the logic to create a vendor and its admin around the above authentication flow. You can also refer to the following documentation pages to learn more about authentication in Switchyard:
...
Switchyard provides the last two steps through its `@switchyard/medusa/core-flows` package. So, you only need to implement the first two steps.
...
export const POST = async (
  req: AuthenticatedSwitchyardRequest<RequestBody>,
...
You define middlewares in Switchyard in the `src/api/middlewares.ts` special file. So, create the file `src/api/middlewares.ts` with the following content:
...
To test out the above API route, start the Switchyard application:
...
The workflow's steps are all provided by Switchyard's `@switchyard/medusa/core-flows` package. So, you can create the workflow right away.
...
export const POST = async (
  req: AuthenticatedSwitchyardRequest<HttpTypes.AdminCreateProduct>,
  res: SwitchyardResponse
) => {
...
```ts title="src/api/middlewares.ts"
// other imports...
import { AdminCreateProduct } from "@switchyard/medusa/api/admin/products/validators"
...
Similar to before, you apply the `validateAndTransformBody` middleware on the `POST /vendors/products` API route. You pass to the middleware the `AdminCreateProduct` schema that Switchyard uses to validate the request body of the [Create Product Admin API Route](!api!/admin#products_postproducts).
...
To test out the new API routes, start the Switchyard application:
...
You only need to implement the `groupVendorItemsStep` and `createVendorOrdersStep` steps, as Switchyard provides the rest of the steps in its `@switchyard/medusa/core-flows` package.
...
import { CartLineItemDTO } from "@switchyard/framework/types"
import { ContainerRegistrationKeys, promiseAll } from "@switchyard/framework/utils"
...
If an error occurs, the created orders in the `createdOrders` array are canceled using Switchyard's `cancelOrderWorkflow` from the `@switchyard/medusa/core-flows` package.
...
The compensation function cancels all child orders received from the step. It uses the `cancelOrderWorkflow` that Switchyard provides in the `@switchyard/medusa/core-flows` package.
...
Then, you need to customize the storefront to use your complete cart API route rather than Switchyard's. In `src/lib/data/cart.ts`, find the following lines in the `src/lib/data/cart.ts`:
...
Now, the checkout flow uses your custom API route to place the order instead of Switchyard's.
...
export const getOrderHighlights = [
  ...
  ["19", "getOrdersListWorkflow", "Use Switchyard's workflow to retrieve the list of orders."],
]
...
import { getOrdersListWorkflow } from "@switchyard/medusa/core-flows"
...
You add a `GET` API route at `/vendors/orders`. In the route handler, you first use Query to retrieve the orders of the authenticated admin's vendor. Then, you use Switchyard's `getOrdersListWorkflow` to retrieve the list of orders with the specified fields.
...
If you want vendors to perform actions that are available for admin users through Switchyard's [Admin API routes](!api!/admin), such as managing their orders, you need to recreate them similar to the create product API route you created earlier.
...
Switchyard provides a Next.js Starter storefront, which you can customize to fit your specific use case.
...
The Switchyard Admin is extendable, allowing you to add custom widgets to existing pages or create entirely new pages. For example, you can add a new page showing the list of vendors. Learn more about it in [this documentation](!docs!/learn/fundamentals/admin).

Only super admins can access the Switchyard Admin, not vendor admins. So, if you need a dashboard specific to each vendor admin, you will need to build a custom dashboard with the necessary features.
