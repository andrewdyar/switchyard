...
In this guide, you'll learn how to build a restaurant-delivery marketplace platform, similar to Uber Eats, with Switchyard.

When you install a Switchyard application, you get a fully-fledged commerce platform with support for customizations. While Switchyard doesn't provide marketeplace functionalities natively, it provides features that you can extend and a Framework to support all your customization needs to build a marketplace.

In this guide, you'll customize Switchyard to build a restaurant-delivery platform with the following features:
...
<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/restaurant-marketplace",
...
This recipe is adapted from [Switchyard Eats](https://github.com/switchyard/medusa-eats), which offers more implementation details including a custom storefront to place and track orders.
...
class RestaurantModuleService extends SwitchyardService({
  Restaurant,
  RestaurantAdmin,
}) {}
...
### Add Restaurant Module to Switchyard Configuration
...
class DeliveryModuleService extends SwitchyardService({
  Delivery,
  Driver,
}) {}
...
### Add Delivery Module to Switchyard Configuration
...
```bash
npx switchyard db:generate restaurantModuleService
npx switchyard db:generate deliveryModuleService
```
...
export async function POST(req: SwitchyardRequest, res: SwitchyardResponse) {
  ...
  if (!validatedBody) {
    return SwitchyardError.Types.INVALID_DATA
  }
...
To test the API route out, start the Switchyard application:
...
export async function GET(req: SwitchyardRequest, res: SwitchyardResponse) {
...
Switchyard provides an authentication flow that allows you to authenticate custom user types:
...
2. Set the actor type of the user’s authentication identity (created by the `/auth/{actor_type}/{provider}/register` API route). For this step, you’ll use `setAuthAppMetadataStep` from Switchyard's core workflows.
...
import { setAuthAppMetadataStep } from "@switchyard/medusa/core-flows"
import {
  createWorkflow,
  transform,
  when,
...
4. Use `setAuthAppMetadataStep` from Switchyard's core workflows to update the authentication identity and associate it with the new user.
...
This applies the `authenticate` middleware from the Switchyard Framework on the `POST /users` API routes.
...
3. Unset the association between the auth identity and the restaurant admin using `setAuthAppMetadataStep` from Switchyard's core workflows.
...
This allows only restaurant admins and Switchyard Admin users to access routes under the `/restaurants/[id]` prefix if the request method is `POST` or `DELETE`.
...
1. Create the product using Switchyard’s `createProductsWorkflow` as a step. This workflow is available through Switchyard's core workflows.
2. Create a link between the restaurant and the products using `createRemoateLinkStep` from Switchyard's core workflows.
...
3. `createRemoteLinkStep` that creates links between the different data model records. This step is from Switchyard's core workflows.
...
import { Modules } from "@switchyard/framework/utils"
import { createRemoteLinkStep } from "@switchyard/medusa/core-flows"
import { DELIVERY_MODULE } from "../../../modules/delivery"
import { RESTAURANT_MODULE } from "../../../modules/restaurant"
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        ...
        description: "Creates the links returned by the previous step between the order and delivery. This is from Switchyard's core workflows.",
...
import { CreateOrderShippingMethodDTO } from "@switchyard/framework/types"
import {
  Modules,
  ContainerRegistrationKeys,
  SwitchyardError,
...
export async function POST(req: SwitchyardRequest, res: SwitchyardResponse) {
  ...
  const updatedDeliveryResult = await updateDeliveryWorkflow(req.scope)
    ...
    .catch((error) => {
      console.log(error)
      return SwitchyardError.Types.UNEXPECTED_STATE
    })

  if (typeof updatedDeliveryResult === "string") {
    throw new SwitchyardError(updatedDeliveryResult, "An error occurred")
  }
...
This request returns the updated delivery. If you also check the Switchyard application’s logs, you’ll find the following message:
...
export async function POST(req: AuthenticatedSwitchyardRequest, res: SwitchyardResponse) {
...
The request returns the updated delivery. If you check the Switchyard application’s logs, you’ll find the following message:
...
You can use a custom Next.js project, or use Switchyard's Next.js Starter storefront. If you haven't installed the Next.js Starter storefront in the first step, refer to [this documentation](../../../../nextjs-starter/page.mdx#approach-2-install-separately) to learn how to install it.
...
To retrieve the delivery from the Switchyard application, replace the first `TODO` with the following:
...
The Switchyard Admin is extendable, allowing you to add widgets to existing pages or create new pages. Learn more about it in [this documentation](!docs!/learn/fundamentals/admin).

### Storefront Development

Switchyard provides a Next.js Starter storefront that you can customize to your use case.
