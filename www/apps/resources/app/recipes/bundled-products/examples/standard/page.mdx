...
export const metadata = {
  title: `Implement Bundled Products in Switchyard`,
}

# {metadata.title}

In this tutorial, you'll learn how to implement bundled products in Switchyard.
...
Switchyard natively supports [inventory kits](../../../../commerce-modules/inventory/inventory-kit/page.mdx), which can be used to create bundled products. However, inventory kits don't support all features of bundled products, such as fulfilling the products in the bundle separately.

In this tutorial, you'll use Switchyard's customizable Framework to implement bundled products. By building the bundled products feature, you can expand on it based on what's necessary for your use case.
...
- Install and set up a Switchyard application.
- Define models for bundled products.
- Link bundled products to Switchyard's existing product model, allowing you to benefit from existing product features.
...
![Bundled products system architecture diagram showing the relationship between bundled products and individual products and variants](https://res.cloudinary.com/dza7lstvk/image/upload/v1745855513/Switchyard%20Resources/bundled-products-overview_r5zejm.jpg)

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/bundled-products",
...
Modules are created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/bundled-product`.
...
export default class BundledProductModuleService extends SwitchyardService({
  Bundle,
  BundleItem,
}) {
}
...
The `BundledProductModuleService` extends `SwitchyardService` from the Modules SDK which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.
...
Switchyard's CLI tool can generate the migrations for you. To generate a migration for the Bundled Product Module, run the following command in your Switchyard application's directory:

```bash
npx switchyard db:generate bundledProduct
```

The `db:generate` command of the Switchyard CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/bundled-product` that holds the generated migration.
...
## Step 3: Link Bundles to Switchyard Products
...
Switchyard creates a table in the database for each link you define. So, you must run the migrations again to create the necessary tables:
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        ...
        type: "workflow",
        description: "Create the Switchyard product associated with the bundle",
        depth: 1,
      ...
      },
      {
        ...
        type: "step",
        description: "Create the link between the bundle and the Switchyard product",
        depth: 1,
      ...
      },
      {
        name: "createRemoteLinkStep",
        type: "step",
        description: "Create the link between the bundle items and the Switchyard products",
        depth: 1,
        link: "/references/helper-steps/createRemoteLinkStep"
      },
      {
...
You only need to implement the first two steps, as Switchyard provides the rest in its `@switchyard/medusa/core-flows` package.
...
In the step function, you resolve the Bundled Product Module's service from the Switchyard container using its `resolve` method, passing it the module's name as a parameter.

Then, you create the bundle using the `createBundles` method. As you remember, the Bundled Product Module's service extends the `SwitchyardService` which generates data-management methods for you.
...
In the compensation function, you resolve the Bundled Product Module's service from the Switchyard container and call the `deleteBundles` method to delete the bundle created in the step.
...
export const createBundledProductWorkflowHighlights = [
  ...
  ["32", "createProductsWorkflow", "Create the Switchyard product associated with the bundle."],
  ["38", "createRemoteLinkStep", "Create a link between the bundle and the Switchyard product."],
  ["47", "transform", "Prepare the data to create links between bundle items and Switchyard products."],
  ["61", "createRemoteLinkStep", "Create the links between the bundle items and the Switchyard products."],
...
import { CreateProductWorkflowInputDTO } from "@switchyard/framework/types"
...
import { createProductsWorkflow, createRemoteLinkStep, useQueryGraphStep } from "@switchyard/medusa/core-flows"
import { BUNDLED_PRODUCT_MODULE } from "../modules/bundled-product"
...
3. Create the Switchyard product associated with the bundle using the `createProductsWorkflow`.
4. Create a link between the bundle and the Switchyard product using the `createRemoteLinkStep`.
...
5. Use `transform` to prepare the data to link bundle items to products.
    - You must use the `transform` function whenever you want to manipulate data in a workflow, as Switchyard creates an internal representation of the workflow when the application starts, not when the workflow is executed. Learn more in the [Transform Data documentation](!docs!/learn/fundamentals/workflows/variable-manipulation).
6. Create a link between the bundle items and the Switchyard products using the `createRemoteLinkStep`.
...
You'll test out this API route in a later step when you customize the Switchyard Admin dashboard.

---

## Step 5: Create Bundled Product API Route

Now that you have the logic to create a bundled product, you need to expose it so that frontend clients, such as the Switchyard Admin, can use it. You do this by creating an [API route](!docs!/learn/fundamentals/api-routes).
...
import { 
  AdminCreateProduct,
} from "@switchyard/medusa/api/admin/products/validators"
import { 
...
export async function POST(
  req: AuthenticatedSwitchyardRequest<PostBundledProductsSchema>,
...
In the route handler function, you execute the `createBundledProductWorkflow` by invoking it, passing it the Switchyard container (which is available on the `scope` property of the request object), then calling its `run` method.
...
The create bundled product route is now ready for use. You'll use it in an upcoming step when you customize the Switchyard Admin dashboard.

---

## Step 6: Retrieve Bundles API Route

Before you start customizing the Switchyard Admin, you need an API route that retrieves all bundles. You'll use this API route to show the bundles in a table on the Switchyard Admin dashboard.
...
export const getBundledProductsRouteHighlights = [
  ["5", "query", "Resolve Query from the Switchyard container."],
...
In the route handler, you resolve [Query](!docs!/learn/fundamentals/module-links/query) from the Switchyard container. Then, you call its `graph` method to retrieve the bundles.
...
1. A Zod schema to validate query parameters. You use Switchyard's `createFindParams` function, which creates a Zod schema containing the following query parameters:
...
Your API route is now ready for use. You'll test it out in the next step as you customize the Switchyard Admin dashboard.

---

## Step 7: Add Bundles Page to Switchyard Admin

Now that you have the necessary routes for admin users to manage and view bundled products, you'll customize the Switchyard Admin to allow admin users to use these features.

You can add a new page to the Switchyard Admin dashboard using a [UI route](!docs!/learn/fundamentals/admin/ui-routes). A UI route is a React component that specifies the content to be shown in a new page in the Switchyard Admin dashboard.

You'll create a UI route to display the list of bundled products in the Switchyard Admin. Later, you'll add a form to create a bundled product.
...
UI routes are created under the `src/admin/routes` directory in a `page.tsx` file. The file's path, relative to `src/admin/routes`, is used as the page's path in the Switchyard Admin dashboard.
...
import { CubeSolid } from "@switchyard/icons"
...
Next, you'll use the [DataTable](!ui!/components/data-table) component from Switchyard UI to show the list of bundled products in a table.
...
You define the table's columns using `createDataTableColumnHelper` from Switchyard UI. The table has the following columns:
...
- `Product`: A link to the Switchyard product associated with the bundle.
...
In the query function of `useQuery`, you use the JS SDK to send a `GET` request to `/admin/bundled-products` of the Switchyard server. You pass the `limit` and `offset` query parameters to support paginating the bundles.

Next, you initialize a table instance using the `useDataTable` hook from Switchyard UI. Finally, you render the table in the page.

### Test it Out

To test out the UI route, start the Switchyard application by running the following command:

```bash npm2yarn
npm run dev
```

Then, open the Switchyard Admin dashboard in your browser at `http://localhost:9000/app` and log in.
...
![Bundled Product page with empty table](https://res.cloudinary.com/dza7lstvk/image/upload/v1745919655/Switchyard%20Resources/Screenshot_2025-04-29_at_12.30.30_PM_nvsezf.png)
...
Next, you need to retrieve the list of products in Switchyard to show them in a selector input. Replace the `TODO` in the `CreateBundledProduct` with the following:
...
Then, you use the `useQuery` hook from Tanstack Query to retrieve the products from the Switchyard server. You call the `sdk.admin.product.list` method to retrieve the products, passing it the `limit` and `offset` query parameters.
...
Next, you'll define the logic to create the bundled product in the Switchyard server once the user submits the form.
...
- Close the modal and show a success message using the `toast` component from Switchyard UI.
...
export const createBundledProductComponentHighlights4 = [
  ...
  ["11", "products", "The products retrieved from the Switchyard server."],
...
- `products`: The list of products retrieved from the Switchyard server.
...
In the component, you render the selector field using the [Select](!ui!/components/select) component from Switchyard UI. You show the products as options in the select, and update the product ID in the `items` state variable whenever the user selects a product.
...
You use the [FocusModal](!ui!/components/focus-modal) component from Switchyard UI to show the form in a modal. The modal is opened when the "Create" button is clicked.
...
To test out the form, start the Switchyard application by running the following command:

```bash npm2yarn
npm run dev
```

Then, open the Switchyard Admin dashboard in your browser at `http://localhost:9000/app`, log in, and open the Bundled Products page.
...
![Create button shown at the top right of the Bundled Products page](https://res.cloudinary.com/dza7lstvk/image/upload/v1745922803/Switchyard%20Resources/Screenshot_2025-04-29_at_1.32.36_PM_yoo23i.png)
...
![Create bundled product form](https://res.cloudinary.com/dza7lstvk/image/upload/v1745923393/Switchyard%20Resources/Screenshot_2025-04-29_at_1.42.12_PM_mdyzsi.png)
...
![Associated product page](https://res.cloudinary.com/dza7lstvk/image/upload/v1745923661/Switchyard%20Resources/Screenshot_2025-04-29_at_1.46.52_PM_iuplxc.png)
...
You only need to implement the second step, as the other steps are provided by Switchyard's `@switchyard/medusa/core-flows` package.
...
import { InferTypeOf, ProductDTO } from "@switchyard/framework/types"
...
export async function POST(
  req: SwitchyardRequest<PostCartsBundledLineItemsSchema>,
  res: SwitchyardResponse
) {
...
In the route handler, you resolve [Query](!docs!/learn/fundamentals/module-links/query) from the Switchyard container.
...
In this step, you'll customize the [Next.js Starter Storefront](../../../../nextjs-starter/page.mdx) you installed with the Switchyard application to show a bundled product's items.
...
So, if your Switchyard application's directory is `medusa-bundled-products`, you can find the storefront by going back to the parent directory and changing to the `medusa-bundled-products-storefront` directory:
...
Since a bundle is linked to a Switchyard product, you can modify the request that retrieves the Switchyard product to retrieve its associated bundle, if there's any.

By retrieving the bundle's details, you can check which Switchyard product is a bundled product, then retrieve its full bundle details.
...
To test it out, start the Switchyard application by running the following command in the Switchyard application's directory:
...
![Product catalogue page with the bundled product showing](https://res.cloudinary.com/dza7lstvk/image/upload/v1745926664/Switchyard%20Resources/Screenshot_2025-04-29_at_2.37.22_PM_ktk4e5.png)

If you click on the bundled product, you can see in its details page the items in the bundle.

![Bundled products detail page showing the items](https://res.cloudinary.com/dza7lstvk/image/upload/v1745926778/Switchyard%20Resources/Screenshot_2025-04-29_at_2.39.16_PM_mskh01.png)
...
![Cart with the bundled items in it](https://res.cloudinary.com/dza7lstvk/image/upload/v1745929244/Switchyard%20Resources/Screenshot_2025-04-29_at_3.20.27_PM_qnbdds.png)

You can then place an order with the bundled items. Then, on the Switchyard Admin dashboard, you can fulfill and process the items separately.

![Example of fulfilling one item in the bundle](https://res.cloudinary.com/dza7lstvk/image/upload/v1745929701/Switchyard%20Resources/Screenshot_2025-04-29_at_3.22.02_PM_nvrvvz.png)
...
Switchyard provides all these steps and workflows in its `@switchyard/medusa/core-flows` package. So, you can create the workflow right away.
...
To test it out, start the Switchyard application and the Next.js Starter Storefront as you did in the previous step.
...
![Cart with the Remove bundle button showing for bundle items](https://res.cloudinary.com/dza7lstvk/image/upload/v1746011200/Switchyard%20Resources/Screenshot_2025-04-30_at_2.06.02_PM_cgtg45.png)
...
- Add more CRUD management features to the Bundled Products page in the Switchyard Admin.
