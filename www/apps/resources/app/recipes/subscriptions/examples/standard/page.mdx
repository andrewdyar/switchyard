...
In this guide, you'll learn how to support subscription purchases with Switchyard.

When you install a Switchyard application, you get a fully-fledged commerce platform with support for customizations. While Switchyard doesn't provide subscription-based purchases natively, it provides the Framework to support you in implementing this feature.

In this guide, you'll customize Switchyard to implement subscription-based purchases with the following features:
...
This guide provides an example of an approach to implement subscriptions. You're free to choose a different approach using the Switchyard Framework.

</Note>

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/subscription",
...
Once the installation finishes successfully, the Switchyard Admin dashboard will open with a form to create a new user. Enter the user's credentials and submit the form.
...
As mentioned in the introduction, you'll use Stripe as the payment provider for the subscription payments. In this step, you'll configure the [Stripe Module Provider](../../../../commerce-modules/payment/payment-provider/stripe/page.mdx) in your Switchyard application.
...
To add the Stripe Module Provider to the Switchyard configurations, add the following to the `switchyard.config.ts` file:
  ...
  modules: [
    {
      ...
      options: {
        providers: [
          {
            resolve: "@switchyard/medusa/payment-stripe",
            id: "stripe",
            options: {
...
The Switchyard configurations accept a `modules` property to add modules to your application. You'll learn more about modules in the next section.
...
To allow customers to use Stripe during checkout, you must enable it in at least one region. Customers can only choose from payment providers available in their region. You can enable the payment provider in the Switchyard Admin dashboard.

To do that, start the Switchyard application:
...
![Choose "Stripe (STRIPE)" in the payment provider dropdown](https://res.cloudinary.com/dza7lstvk/image/upload/v1740654408/Switchyard%20Resources/Screenshot_2025-02-27_at_1.06.12_PM_esql1c.png)
...
class SubscriptionModuleService extends SwitchyardService({
  Subscription,
}) {
}
...
### Register Module in Switchyard’s Configuration

Finally, add the module into `switchyard.config.ts`:
...
In this step, you’ll define links between the Subscription Module’s `Subscription` data model and the data models of Switchyard’s Commerce Modules:
...
```bash
npx switchyard db:generate subscriptionModuleService
```
...
Switchyard provides the first and last steps in the `@switchyard/medusa/core-flows` package, so you only need to implement the second step.
...
import { LinkDefinition } from "@switchyard/framework/types"
...
![Diagram showcasing payment flow overview](https://res.cloudinary.com/dza7lstvk/image/upload/v1740657136/Switchyard%20Resources/subscriptions-stripe_ycw4ig.jpg)
...
When you create the payment session, Switchyard creates an account holder for the customer. An account holder represents a customer's saved payment information, including saved methods, in a third-party provider and may hold data from that provider. Learn more in the [Account Holder](../../../../commerce-modules/payment/account-holder/page.mdx) documentation.
...
import { Button, clx, Heading, Text } from "@switchyard/ui"
...
This adds a component that displays a form to choose the subscription's interval and period during checkout. When the customer submits the form, you use the `updateSubscriptionData` function that sends a request to the Switchyard application to update the cart with the subscription details.
...
1. In the Switchyard application's directory, run the following command to start the application:
...
- A validation schema indicating which query parameters are accepted. You create the schema with [Zod](https://zod.dev/). Switchyard has a createFindParams utility that generates a Zod schema accepting four query parameters:
...
In the next section, you’ll extend the Switchyard Admin and use these API routes to show the subscriptions.
...
Switchyard provides a [JS SDK](../../../../js-sdk/page.mdx) that facilitates sending requests to the server. You can use this SDK in any JavaScript client-side application, including your admin customizations.
...
- `baseUrl`: The URL of the Switchyard server. You use the Vite environment variable `VITE_BACKEND_URL`.
...
You'll now create the subscriptions list UI route. Since you'll show the subscriptions in a table, you'll use the [DataTable component](!ui!/components/data-table) from Switchyard UI. It facilitates displaying data in a tabular format with sorting, filtering, and pagination.
...
import { ClockSolid } from "@switchyard/icons"
import { Container, Heading, Badge, createDataTableColumnHelper, useDataTable, DataTablePaginationState, DataTable } from "@switchyard/ui"
...
To test the UI routes, run the Switchyard application and go to `http://localhost:9000/app`.
...
```mermaid
graph TD
  useQueryGraphStep["Retrieve Cart (useQueryGraphStep by Switchyard)"] --> createPaymentCollectionStep["createPaymentCollectionStep (Switchyard)"]
  createPaymentCollectionStep["createPaymentCollectionStep (Switchyard)"] --> getPaymentMethodStep
  getPaymentMethodStep --> createPaymentSessionsWorkflow["createPaymentSessionsWorkflow (Switchyard)"]
  createPaymentSessionsWorkflow["createPaymentSessionsWorkflow (Switchyard)"] --> authorizePaymentSessionStep["authorizePaymentSessionStep (Switchyard)"]
  authorizePaymentSessionStep["authorizePaymentSessionStep (Switchyard)"] --> createSubscriptionOrderStep
  createSubscriptionOrderStep --> createRemoteLinkStep["Create Links (createRemoteLinkStep by Switchyard)"]
  createRemoteLinkStep["Create Links (createRemoteLinkStep by Switchyard)"] --> capturePaymentStep["capturePaymentStep (Switchyard)"]
  capturePaymentStep["capturePaymentStep (Switchyard)"] --> updateSubscriptionStep
```

1. Retrieve the subscription’s linked cart. Switchyard provides a `useQueryGraphStep` in the `@switchyard/medusa/core-flows` package that can be used as a step.
2. Create a payment collection for the new order. Switchyard provides a `createPaymentCollectionsStep` in the `@switchyard/medusa/core-flows` package that you can use.
3. Get the customer's saved payment method. This payment method will be used to charge the customer.
4. Create payment sessions in the payment collection. Switchyard provides a `createPaymentSessionsWorkflow` in the `@switchyard/medusa/core-flows` package that can be used as a step.
5. Authorize the payment session. Switchyard also provides the `authorizePaymentSessionStep` in the `@switchyard/medusa/core-flows` package, which can be used.
6. Create the subscription’s new order.
7. Create links between the subscription and the order using the `createRemoteLinkStep` provided in the `@switchyard/medusa/core-flows` package.
8. Capture the order’s payment using the `capturePaymentStep` provided by Switchyard in the `@switchyard/medusa/core-flows` package.
...
import { AccountHolderDTO, CustomerDTO, PaymentMethodDTO } from "@switchyard/framework/types"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
...
export const createSubscriptionOrderStep1Highlights = [
  ...
  ["33", "createOrderWorkflow", "Use Switchyard's workflow to create the order."],
...
This creates a `createSubscriptionOrderStep` that uses the `createOrdersWorkflow`, which Switchyard provides in the `@switchyard/medusa/core-flows` package. The step returns the created order and an array of links to be created.
...
To manage the orders created for a subscription, or other functionalities, use Switchyard’s existing [Admin API routes](!api!/admin).
...
Switchyard provides a Next.js Starter storefront that you can customize to your use case. You can also create a custom storefront. To learn how visit the [Storefront Development](../../../../storefront-development/page.mdx) section.
