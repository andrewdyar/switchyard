...
export const metadata = {
  title: `Integrate Odoo with Switchyard`,
}

# {metadata.title}

In this guide, you will learn how to implement the integration layer between Odoo and Switchyard.

When you install a Switchyard application, you get a fully-fledged commerce platform that supports customizations. However, your business might already be using other systems such as an ERP to centralize data and processes. The Switchyard Framework facilitates integrating the ERP system and using its data to enrich your commerce platform.
...
Afterward, the installation process will start, installing the Switchyard application in a directory with your project's name. If you chose to install the Next.js starter, it'll be installed in a separate directory with the `{project-name}-storefront` name.
...
Run the following command in the Switchyard application to install the `json-rpc-2.0` package:
...
To integrate third-party systems into Switchyard, you create a custom [module](!docs!/learn/fundamentals/modules). A module is a re-usable package with functionalities related to a single feature or domain. Switchyard integrates the module into your application without implications or side effects on your setup.
...
A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/odoo`.

![Diagram showcasing directory structure after creating the module directory](https://res.cloudinary.com/dza7lstvk/image/upload/v1740474975/Switchyard%20Resources/odoo-1_en3bso.jpg)
...
![Diagram showcasing directory structure after creating the service file](https://res.cloudinary.com/dza7lstvk/image/upload/v1740474976/Switchyard%20Resources/odoo-2_y76dfs.jpg)
...
You can fetch many data from Odoo based on your business requirements, or create data in Odoo. For this guide, you'll only learn how to fetch products. You will use this method later to sync products from Odoo to Switchyard.
...
You will use the `listProducts` method to sync products from Odoo to Switchyard in the next steps.
...
![Diagram showcasing directory structure after creating the module definition file](https://res.cloudinary.com/dza7lstvk/image/upload/v1740475883/Switchyard%20Resources/odoo-3_q5y803.jpg)
...
![My profile dropdown in Odoo](https://res.cloudinary.com/dza7lstvk/image/upload/v1740476295/Switchyard%20Resources/Screenshot_2025-02-25_at_11.36.23_AM_h9iind.png)

    - On your profile's page, click the "Account Security" tab, then the "New API Key" button.

![Profile page in Odoo](https://res.cloudinary.com/dza7lstvk/image/upload/v1740476296/Switchyard%20Resources/Screenshot_2025-02-25_at_11.36.55_AM_gl4wfu.png)
...
![Generate key pop-up](https://res.cloudinary.com/dza7lstvk/image/upload/v1740476443/Switchyard%20Resources/Screenshot_2025-02-25_at_11.40.19_AM_ntbx1c.png)
...
## Step 4: Sync Products from Odoo to Switchyard

There are different use cases you can implement when integrating an ERP like Odoo. One of them is syncing products from the ERP to Switchyard. This way, you can manage products in Odoo and have them reflected in your commerce platform.
...
In this section, you'll create a workflow that syncs products from Odoo to Switchyard. Then, you'll execute that workflow once a day using a [scheduled job](!docs!/learn/fundamentals/scheduled-jobs). The workflow has the following steps:
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        ...
        description: "Get Switchyard store configurations to use when creating the products.",
        depth: 1,
      ...
      },
      {
        type: "step",
        name: "useQueryGraphStep",
        description: "Get Switchyard sales channels to use when creating the products.",
        depth: 1,
        link: "/references/helper-steps/useQueryGraphStep",
      },
      {
        type: "step",
        name: "useQueryGraphStep",
        description: "Get Switchyard shipping profiles to use when creating the products.",
        depth: 1,
        link: "/references/helper-steps/useQueryGraphStep",
      },
      {
        ...
        description: "Create new products in Switchyard.",
        depth: 1,
      ...
      },
      {
        ...
        description: "Update existing products in Switchyard.",
        depth: 1,
...
The only step you'll need to implement is the `getProductsFromErp` step. The other steps are available through Switchyard's `@switchyard/medusa/core-flows` package.
...
![Diagram showcasing directory structure after creating the workflow file](https://res.cloudinary.com/dza7lstvk/image/upload/v1740479240/Switchyard%20Resources/odoo-4_jrh5mx.jpg)
...
You can now create the workflow that syncs the products from Odoo to Switchyard.
...
- Use the [useQueryGraphStep](/references/helper-steps/useQueryGraphStep) to fetch the Switchyard store configurations, sales channels, and shipping profiles. You'll use this data when creating the products in a later step.
...
In Switchyard, you can run a task at a specified interval using a [scheduled job](!docs!/learn/fundamentals/scheduled-jobs). A scheduled job is an asynchronous function that runs at a regular interval during the Switchyard application's runtime to perform tasks such as syncing products from Odoo to Switchyard.

To create a scheduled job, create the file `src/jobs/sync-products-from-erp.ts` with the following content:

![Diagram showcasing directory structure after creating the scheduled job file](https://res.cloudinary.com/dza7lstvk/image/upload/v1740480934/Switchyard%20Resources/odoo-5_xf0xug.jpg)
...
The scheduled job function accepts the Switchyard container as a parameter. In the function, you define the pagination options for the products to fetch from Odoo. You then run the `syncFromErpWorkflow` workflow with the pagination options. You increment the offset by the limit each time you run the workflow until you fetch all the products.

---

## Test it Out

To test out syncing the products from Odoo to Switchyard, first, change the schedule of the job in `src/jobs/sync-products-from-erp.ts` to run every minute:
...
You can also confirm that the products were synced by checking the products in the Switchyard Admin dashboard.
