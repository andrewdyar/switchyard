...
export const ogImage = "https://res.cloudinary.com/dza7lstvk/image/upload/v1740556002/Switchyard%20Resources/erp-guide_sucmxz.jpg"

export const metadata = {
  title: `Integrate ERP with Switchyard`,
  openGraph: {
    images: [
      {
        url: ogImage,
...
In this recipe, you'll learn about the general approach to integrating an ERP system with Switchyard.
...
The Switchyard Framework solves these challenges by giving you a durable execution engine to orchestrate operations through custom flows, and the flexibility to customize the platform's existing flows. You can wrap existing flows with custom logic, inject custom features into existing flows, and create new flows that interact with the ERP system and sync data between the two systems.

![ERP Integration Illustration](https://res.cloudinary.com/dza7lstvk/image/upload/v1740470820/Switchyard%20Resources/erp-medusa-integration_pxjpcx.jpg)

In this recipe, you'll learn how to implement some common use cases when integrating an ERP system with Switchyard. This includes how to purchase products with custom pricing, restrict products from purchase under conditions in the ERP, sync orders to the ERP, and more.

You can use the code snippets in the recipe as a starting point for your ERP integration, making changes as necessary for your use case. You can also implement other use cases using the same Switchyard concepts.

---

## Prerequisite: Install Switchyard

If you don't have a Switchyard application yet, you can install it with the following command:
...
Before you start integrating the ERP system into existing or new flows in Switchyard, you must build the integration layer that allows you to communicate with the ERP in your customizations.

In Switchyard, you implement integrations or features around a single commerce domain in a [module](!docs!/learn/fundamentals/modules). A module is a reusable package that can interact with the database or external APIs. The module integrates into your Switchyard application without side effects to the existing setup.
...
Refer to [this documentation](!docs!/learn/fundamentals/modules) on how to create a module. You can also refer to the [Odoo Integration guide](./odoo/page.mdx) as an example of how to build a module that integrates an ERP into Switchyard.
...
If you store products in the ERP system, you want to sync them into Switchyard to allow customers to purchase them. You may sync them once or periodically to keep the products in Switchyard up-to-date with the ERP.

Syncing data between systems is a big challenge and it's often the pitfall of most ecommerce platforms, as you need to ensure data consistency and handle errors gracefully. Switchyard solves this challenge by providing a durable execution engine to complete tasks that span multiple systems, allowing you to orchestrate your operations across systems in Switchyard instead of managing it yourself.

Switchyard's workflows are a series of queries and actions, called steps, that complete a task. You construct a workflow similar to how you create a JavaScript function, but with additional features like defining rollback logic for each step, performing long actions asynchronously, and tracking the progress of the steps. You can then use these workflows in other customizations, such as:
...
So, to sync products from the ERP system to Switchyard, you can create a custom workflow that fetches the products from the ERP system and adds them to Switchyard. Then, you can create a scheduled job that syncs the products once a day, for example.

### 1. Create Workflow to Sync Products

The workflow that syncs products from the ERP system to Switchyard will have a step that fetches the product from the ERP, and another step that adds the product to Switchyard.
...
import { createProductsWorkflow } from "@switchyard/medusa/core-flows"
...
export const syncFromErpWorkflow = createWorkflow(
  ...
  () => {
    ...
    const productsToCreate = transform({
      ...
      // TODO prepare ERP products to be created in Switchyard
...
In the above file, you first create a `getProductsFromErpStep` that resolves the ERP Module's service from the [Switchyard container](!docs!/learn/fundamentals/medusa-container), which is a registry of Framework and commerce tools, including your modules, that you can access in your customizations. You can then call the `getProducts` method in the ERP Module's service to fetch the products from the ERP and return them.

Then, you create a `syncFromErpWorkflow` that executes the `getProductsFromErpStep` to get the products from the ERP, then prepare the products to be created in Switchyard. For example, you can set the product's title, and specify its ID in the ERP using the `external_id` field. Also, assuming the ERP products have variants, you can map the variants to Switchyard's format, setting the variant's title and its ERP ID in the metadata.

Finally, you pass the products to be created to the `createProductsWorkflow`, which is a built-in Switchyard workflow that creates products.
...
After creating a workflow, you can create a scheduled job that runs the workflow periodically to sync the products from the ERP to Switchyard.
...
You create a scheduled job that runs once a day, executing the `syncFromErpWorkflow` to sync the products from the ERP to Switchyard.
...
The function then resolves the ERP Module's service from the [Switchyard container](!docs!/learn/fundamentals/medusa-container) and, assuming you have a `getErpPrice` method in your ERP Module's service, you call it to retrieve that price, and return it multiplied by the quantity.
...
import { addToCartWorkflow, useQueryGraphStep } from "@switchyard/medusa/core-flows"
...
In this workflow, you first fetch the details of the cart and the variant from the database using the [useQueryGraphStep](/references/helper-steps/useQueryGraphStep) that Switchyard defines.
...
Since Switchyard implements the add-to-cart functionality within a workflow, it allows you to inject custom logic into the workflow using [workflow hooks](!docs!/learn/fundamentals/workflows/workflow-hooks). A workflow hook is a point in a workflow where you can inject a step and perform a custom functionality.
...
In the step, you resolve the ERP Module's service, the Product Module's service, and the Customer Module's service from the [Switchyard container](!docs!/learn/fundamentals/medusa-container). You then retrieve the cart's customer and the product variants to be added to the cart.

Then, for each product variant, you use a `canCompanyPurchaseProduct` method in the ERP Module's service that checks if the customer's company is allowed to purchase the product. If not, you throw a `SwitchyardError` with a message that the company is not allowed to purchase the product.
...
After a customer places an order in Switchyard, you may want to sync the order to the ERP system where you handle its fulfillment and processing. However, you may also want to sync the order back to Switchyard, where you handle customer-service related operations, such as returns and refunds.
...
1. A workflow that syncs the order from Switchyard to the ERP system. You can execute this workflow in a [subscriber](!docs!/learn/fundamentals/events-and-subscribers) that is triggered when an order is created in Switchyard.
2. A workflow that syncs the order from the ERP system back to Switchyard. Then, you can create a webhook listener in Switchyard that executes the workflow, and use the webhook in the ERP system to send order updates to Switchyard.

### 1. Sync Order to ERP

To sync the order from Switchyard to the ERP system, you can create a custom workflow that sends the order's details from Switchyard to the ERP system. For example:

```ts title="src/workflows/sync-order-to-erp.ts"
import { createStep, createWorkflow, StepResponse, WorkflowResponse } from "@switchyard/framework/workflows-sdk"
import { updateOrderWorkflow, useQueryGraphStep } from "@switchyard/medusa/core-flows"
...
You first create a `syncOrderToErpStep` that receives an order's details, resolves the ERP Module's service from the Switchyard container, and calls a `createOrder` method in the ERP Module's service that creates the order in the ERP.
...
Then, you create a `syncOrderToErpWorkflow` that retrieves the order's details from the database using the `useQueryGraphStep`, then executes the `syncOrderToErpStep` to sync the order to the ERP system. Finally, you update the order in Switchyard to set the ERP order's ID in the `metadata` field.
...
### 2. Sync Order from ERP to Switchyard

To sync the order from the ERP system back to Switchyard, create first the workflow that receives the updated order data and reflects them in Switchyard:
...
Then, you call the `syncOrderFromErpWorkflow` to sync the order from the ERP system back to Switchyard.
...
You can now receive webhook requests from your ERP system and sync the order data back to Switchyard.
...
An ERP system often manages the inventory of products, with the ability to track the stock levels and availability of products. When a customer is purchasing a product through Switchyard, you want to ensure that the product is available in the ERP system before allowing the purchase.
...
The use cases covered in this guide are some common ERP integration scenarios that you can implement with Switchyard. However, you can implement more use cases based on your ERP system's capabilities and your business requirements.

Refer to the [main documentation](!docs!/learn) to learn more about Switchyard's concepts and how to implement customizations. You can also use the feedback form at the end of this guide to suggest more use cases you'd like to see implemented.
