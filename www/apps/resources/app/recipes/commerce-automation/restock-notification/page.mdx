...
export const ogImage = "https://res.cloudinary.com/dza7lstvk/image/upload/v1733326703/Switchyard%20Resources/restock-notification_sw5k7d.jpg"

export const metadata = {
  title: `Implement Restock Notifications in Switchyard`,
  openGraph: {
    images: [
      {
        url: ogImage,
...
In this guide, you'll learn how to notify customers when a variant is restocked in Switchyard.

When you install a Switchyard application, you get a fully-fledged commerce platform with a Framework for customization. The Switchyard application's commerce features are built around [Commerce Modules](../../../commerce-modules/page.mdx) which are available out-of-the-box. These features include managing the inventory of product variants in different stock locations and sales channels.

Customers browsing your store may be interested in a product that is currently out of stock. To keep the customer interested in your store and encourage them to purchase the product in the future, you can build customizations around Switchyard's commerce features to subscribe customers to receive a notification when the product is restocked.
...
<Card
  ...
  href="https://github.com/switchyard/examples/tree/main/restock-notification"
  icon={Github}
/>
...
Once the installation finishes successfully, the Switchyard Admin dashboard will open with a form to create a new user. Enter the user's credential and submit the form. Afterwards, you can login with the new user and explore the dashboard.
...
To add custom tables to the database, which are called data models, you create a module. A module is a re-usable package with functionalities related to a single feature or domain. Switchyard integrates the module into your application without implications or side effects on your setup.
...
A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/restock`.

![Diagram showcasing the module directory to create](https://res.cloudinary.com/dza7lstvk/image/upload/v1733222736/Switchyard%20Resources/restock-dir-overview-1_hiz58j.jpg)
...
In Switchyard, you have sales channels that indicate the channels you sell your products through, such as online storefront or offline store. A product's variants have different inventory quantities across stock locations, which are associated with sales channels.

![A diagram showcasing how a variant's inventory is stored across modules](https://res.cloudinary.com/dza7lstvk/image/upload/v1733224214/Switchyard%20Resources/inventory-details-example_nvx4cj.jpg)
...
![The directory structure of the Restock Module after adding this model.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733224503/Switchyard%20Resources/restock-dir-overview-2_chap79.jpg)
...
5. `customer_id`: The customer's ID in Switchyard. This is nullable in case the customer is a guest.
...
![The directory structure of the Restock Module after adding this service.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733224957/Switchyard%20Resources/restock-dir-overview-4_pkypup.jpg)
...
class RestockModuleService extends SwitchyardService({
  RestockSubscription,
}) { }
...
The `RestockModuleService` extends `SwitchyardService` from the Modules SDK which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.
...
![The directory structure of the Restock Module after adding the definition file.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733225036/Switchyard%20Resources/restock-dir-overview-5_dcam6u.jpg)
...
Switchyard's CLI tool generates the migrations for you. To generate a migration for the Restock Module, run the following command in your Switchyard application's directory:

```bash
npx switchyard db:generate restock
```

The `db:generate` command of the Switchyard CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/restock` that holds the generated migration.

![The directory structure of the Restock Module after generating the migration](https://res.cloudinary.com/dza7lstvk/image/upload/v1733225157/Switchyard%20Resources/restock-dir-overview-6_c2z6oi.jpg)
...
![The directory structure of the Switchyard Application after adding this link.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733225402/Switchyard%20Resources/restock-dir-overview-7_dln3fw.jpg)
...
3. An object of configurations for the module link. By default, Switchyard creates a table in the database to represent the link you define. However, in this guide, you only want this link to retrieve the variants associated with a subscription. So, you enable `readOnly` telling Switchyard not to create a table for this link.
...
![The directory structure of the Switchyard application after adding the step.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733227547/Switchyard%20Resources/restock-dir-overview-10_g3dbi3.jpg)

```ts title="src/workflows/create-restock-subscription/steps/validate-variant-out-of-stock.ts"
import { getVariantAvailability, SwitchyardError } from "@switchyard/framework/utils"
...
This step accepts the ID of the variant and the ID of the customer's sales channel. In the step, you use the `getVariantAvailability` from the Switchyard Framework to get the variant's quantity in the specified sales channels. If the variant's quantity is greater than `0`, you throw an error, stopping the workflow's execution.
...
![The directory structure of the Switchyard application after adding the step.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733227679/Switchyard%20Resources/restock-dir-overview-11_dyrpao.jpg)

export const createRSHighlights = [
  ["15", "restockModuleService", "Resolve the Restock Module's service from the Switchyard container."],
...
In the step, you resolve the Restock Module's service from the Switchyard container. Switchyard registers the service of custom and core modules in the container under the module's name.

Then, you use the service's `createRestockSubscriptions` method, which was generated by `SwitchyardService`, to create the restock subscription.
...
2. An object similar to the second parameter of a step function. It has a `container` property to resolve resources from the Switchyard container.
...
![The directory structure of the Switchyard application after adding the step.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733227952/Switchyard%20Resources/restock-dir-overview-12_vubtkp.jpg)
...
![The directory structure of the Switchyard application after adding the workflow.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733229373/Switchyard%20Resources/restock-dir-overview-13_zdwawe.jpg)
...
![The directory structure of the Switchyard application after adding the route file.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733230210/Switchyard%20Resources/restock-dir-overview-16_sv7yk2.jpg)
...
export async function POST(
  req: AuthenticatedSwitchyardRequest<PostStoreCreateRestockSubscription>,
...
Then, you execute the `createRestockSubscriptionWorkflow` by invoking it, passing it the Switchyard container which is stored in the `scope` property of the request object, and invoking its `run` method.
...
In Switchyard, you create validation schemas using [Zod](https://zod.dev/) in a TypeScript file under the `src/api` directory. So, create the file `src/api/store/restock-subscriptions/validators.ts` with the following content:

![The directory structure of the Switchyard application after adding the validator file.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733229734/Switchyard%20Resources/restock-dir-overview-14_au0h15.jpg)
...
![The directory structure of the Switchyard application after adding the middlewares file.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733229866/Switchyard%20Resources/restock-dir-overview-15_xvwkc0.jpg)
...
To test out this API route, start the Switchyard application by running the following command in the root directory of the Switchyard application:
...
![The variants table shows a product's variants. Click on a variant to open its details page.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733230534/Switchyard%20Resources/Screenshot_2024-12-03_at_2.55.11_PM_c1ml9l.png)

2. Under Inventory Items, click on an inventory item.

![The inventory items table shows the variant's items. Click on an item to open its details page.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733230649/Switchyard%20Resources/Screenshot_2024-12-03_at_2.57.01_PM_ccc9of.png)

3. Under Locations, click on the third-dots icon at the right of a location, then choose Edit from the dropdown.

![The locations are shown in a table. Click on the three-dots at a location's right side, then choose Edit from the dropdown.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733230730/Switchyard%20Resources/Screenshot_2024-12-03_at_2.58.18_PM_waeepw.png)

4. In the drawer form, enter `0` for the item's in-stock quantity.
5. Click the Save button.

![In the drawer form, enter `0` in the In stock field, then click the Save button at the bottom.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733230822/Switchyard%20Resources/Screenshot_2024-12-03_at_2.59.48_PM_vqaige.png)

6. Go back to the variant's page and click on the icon at the right of the JSON section.

![Click on the icon at the right of the JSON section.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733230912/Switchyard%20Resources/Screenshot_2024-12-03_at_3.01.28_PM_bcau0e.png)

7. In the JSON object, hover over the `id` field and click the copy icon.

![Click on the copy icon next to the ID field.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733230981/Switchyard%20Resources/Screenshot_2024-12-03_at_3.02.34_PM_ujyv5w.png)
...
The `useQueryGraphStep` is from Switchyard's workflows. So, you'll only implement the other steps.

### Optional Prerequisite: Notification Module Provider

Within this workflow, you'll use Switchyard's [Notification Module](../../../infrastructure-modules/notification/page.mdx) to send an email to the customer.
...
Alternatively, for development and debugging purposes, you can use the default Notification Module Provider that only logs a message in the terminal instead of sending an email. To do that, add the following to the `modules` array in `switchyard.config.ts`:
...
```ts title="src/modules/restock/service.ts"
// other imports...
import { InjectManager, SwitchyardContext } from "@switchyard/framework/utils"
...
![Directory structure of the Switchyard application after adding the step.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733399774/Switchyard%20Resources/restock-dir-overview-22_kzchmm.jpg)
...
![The directory structure of the Switchyard application after adding the step.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733234506/Switchyard%20Resources/restock-dir-overview-17_pdtees.jpg)

```ts title="src/workflows/send-restock-notifications/steps/get-restocked.ts"
import { getVariantAvailability, promiseAll } from "@switchyard/framework/utils"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
...
In this step, you loop over the restock subscriptions and use `getVariantAvailability` from the Switchyard Framework to retrieve a variant's quantity in the sales channel.
...
![The directory structure of the Switchyard application after adding the step.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733234507/Switchyard%20Resources/restock-dir-overview-18_uvlu0l.jpg)
...
This step resolves the Notification Module's service from the Switchyard container and, for each subscription, sends a notification to its subscribers.
...
![The directory structure of the Switchyard application after adding the step.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733234506/Switchyard%20Resources/restock-dir-overview-19_qfospx.jpg)
...
![The directory structure of the Switchyard application after adding the workflow.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733234507/Switchyard%20Resources/restock-dir-overview-20_mcqkkx.jpg)
...
A scheduled job is an asynchronous function that the Switchyard application runs at the schedule you specify during the Switchyard application's runtime. Scheduled jobs are useful for automating tasks at a fixed schedule.
...
![The directory structure of the Switchyard application after adding the scheduled job.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733234524/Switchyard%20Resources/restock-dir-overview-21_reaqp3.jpg)
...
export default async function myCustomJob(container: SwitchyardContainer) {
...
The scheduled job function accepts the Switchyard container as a parameter. In the function, you execute the `sendRestockNotificationsWorkflow` by invoking it, passing it the container, then executing its `run` method.

### Test Scheduled Job

To test out the scheduled job, start the Switchyard application:

```bash npm2yarn
npm run dev
```

Then, open the Switchyard Admin again at `http://localhost:9000/app` and log in. After that:
...
![In the drawer form, enter value greater than `0` in the In stock field, then click the Save button at the bottom.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733234710/Switchyard%20Resources/Screenshot_2024-12-03_at_3.53.55_PM_uwt06f.png)
...
You've now implemented restock notifications in Switchyard. You can also customize the [storefront](../../../nextjs-starter/page.mdx) to allow customers to subscribe to the restock notification using the new API route you added.
