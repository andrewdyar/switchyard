...
import { AcademicCapSolid, BoltSolid } from "@switchyard/icons"
...
This recipe provides an overview of Switchyard's features and how to use it as an Order Management System (OMS).
...
Switchyard's Commerce Modules and [Framework](!docs!/learn/fundamentals/framework) for customizations allows you to integrate it within a larger ecosystem. The Commerce Modules provide features to allow businesses to accept orders from any sales channel, benefit from multi-warehouse inventory features, and integrate third-party services for fulfillment, payment, and more.

<Note title="Related use-case">

[How Siam Makro used Switchyard an OMS](https://switchyard.com/blog/makro-pro/).

</Note>

---

## Source Orders into Switchyard

Sales channels in your commerce ecosystem must route their orders into the OMS.

![Routing orders into Switchyard OMS](https://res.cloudinary.com/dza7lstvk/image/upload/v1709032160/Switchyard%20Book/oms-orders_zf5ta9.jpg)

Switchyard's [Store REST APIs](!api!/store) let you integrate a checkout experience in any storefront. Alternatively, you can use Switchyard's [Draft Order APIs](!api!/admin#draft-orders) to place an order without direct involvement from the customer, such as when placing an order through a POS.

In addition, you can customize the Switchyard application to accept orders through a third-party checkout system. This gives you more flexibility over adding orders to Switchyard.

For example, you can support importing orders into Switchyard through a custom API Route that allows batch-inserting orders. Another example is creating a scheduled job that runs at a specified interval and imports orders from a third-party service.
...
To integrate third-party fulfillment providers with the Switchyard application, you can create a fulfillment module provider.

Switchyard uses the Fulfillment Module whenever a fulfillment action is performed, such as when a fulfillment is created for items in an order. The methods of the module's main service use the associated fulfillment module provider to handle the desired fulfillment actions.

![Fulfilling orders with Switchyard OMS](https://res.cloudinary.com/dza7lstvk/image/upload/v1709032184/Switchyard%20Book/oms-fulfillment_qfrpdd.jpg)
...
<CardList itemsPerRow={2} items={[
  {
    ...
    text: "Learn how to create a fulfillment provider in Switchyard.",
    icon: AcademicCapSolid,
  },
  {
...
To integrate third-party payment providers with the Switchyard application, you can create a payment module provider. Customers can pay for their orders using this providers, and admins can process order payments using it.
...
Switchyard's Inventory, Stock Location, and Sales Channel modules allow merchants to track inventory levels tied to sales channels across stock locations.
...
Switchyard also emits events related to these actions, such as `order.return_requested`. So, you can build a workflow that performs actions with the third-party fulfillment and payment providers, then execute it in a subscriber that's triggered whenever the event is emitted.
