...
The Local File Module Provider stores files uploaded to your Switchyard application in the `/uploads` directory.
...
- The Local File Module Provider will only read files uploaded through Switchyard. It will not read files uploaded manually to the `static` (or other configured) directory.
...
<Table>
  ...
  <Table.Body>
    <Table.Row>
      ...
      The directory to upload files to. Switchyard exposes the content of the `static` directory publicly. If you change the directory, it must be served and publicly accessible.
