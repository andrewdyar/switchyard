...
The File Module exposes the functionalities to upload assets, such as product images, to the Switchyard application. Switchyard uses the File Module in its core commerce features for all file operations, and you can use it in your custom features as well.
...
In the example above, you create a workflow that has a step. In the step, you resolve the service of the File Module from the [Switchyard container](!docs!/learn/fundamentals/medusa-container).
...
By default, Switchyard uses the [Local File Module](./local/page.mdx). This module uploads files to the `uploads` directory of your Switchyard application.
