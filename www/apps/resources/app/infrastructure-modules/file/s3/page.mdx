...
The S3 File Module Provider integrates Amazon S3 and services following a compatible API (such as MinIO or DigitalOcean Spaces) to store files uploaded to your Switchyard application.

<Note title="Looking for managed storage?">

Cloud offers a managed file storage solution with AWS S3 for your Switchyard application. Refer to the [S3](!cloud!/s3) Cloud documentation for more details.
