...
In your Switchyard application, you can use the Analytics Module to send data to third-party analytics services like PostHog or Segment, enabling you to gain insights into user behavior and system performance.

![Analytics Module workflow diagram showing the event tracking flow: when a customer places an order in the storefront, the Switchyard application uses the Analytics Module to capture the event data and forwards it to configured third-party analytics services for business intelligence and user behavior analysis](https://res.cloudinary.com/dza7lstvk/image/upload/v1747832107/Switchyard%20Resources/analytics-module-overview_egz7xg.jpg)
...
Switchyard provides two Analytics Module Providers: Local and PostHog module providers.
...
In the step, you resolve the service of the Analytics Module from the [Switchyard container](!docs!/learn/fundamentals/medusa-container) and use its `track` method to track the event. This method will use the underlying provider configured in `switchyard.config.ts` to track the event.
...
You'll now track the order placement event whenever an order is placed in your Switchyard application. You can test this out by placing an order and checking the provider you integrated with (for example, PostHog) for the tracked event.
