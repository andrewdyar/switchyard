...
The Local Analytics Module Provider is a simple analytics provider for Switchyard that logs analytics events to the console. It's useful for development and debugging purposes.
...
You can use only one Analytics Module Provider in your Switchyard application.
...
In the step, you resolve the service of the Analytics Module from the [Switchyard container](!docs!/learn/fundamentals/medusa-container) and use its `track` method to track the event. This method will use the underlying provider configured (which is the Local Analytics Module Provider, in this case) to track the event.
...
You'll now track the order placement event whenever an order is placed in your Switchyard application. You can test this out by placing an order and checking the server's logs for the tracked event.
