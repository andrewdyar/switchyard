...
The PostHog Analytics Module Provider allows you to integrate [PostHog](https://posthog.com/) with Switchyard.
...
By integrating PostHog with Switchyard, you can track events such as cart additions, order completions, and user sign-ups, enabling you to gain insights into user behavior and optimize your application accordingly.

<Note>

The Analytics Module and its providers are available starting [Switchyard v2.8.3](https://github.com/switchyard/medusa/releases/tag/v2.8.3).
...
You can use only one provider in your Switchyard application.
  ...
  modules: [
    {
      ...
      options: {
        providers: [
          {
            resolve: "@switchyard/analytics-posthog",
            id: "posthog",
            options: {
...
In the step, you resolve the service of the Analytics Module from the [Switchyard container](!docs!/learn/fundamentals/medusa-container) and use its `track` method to track the event. This method will use the underlying provider configured (which is the PostHog Analytics Module Provider, in this case) to track the event.
...
You'll now track the order placement event whenever an order is placed in your Switchyard application. You can test this out by placing an order and checking your PostHog dashboard for the tracked event.
