...
The Local Notification Module Provider can also be used to send notifications to the [Switchyard Admin's notification panel](!user-guide!#check-notifications). 
...
- Resolve the Notification Module's main service from the [Switchyard container](!docs!/learn/fundamentals/medusa-container).
...
Then, place an order. The subscriber will run, sending a notification to the Switchyard Admin's notification panel.
