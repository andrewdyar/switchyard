...
The Caching Module is available starting [Switchyard v2.11.0](https://github.com/switchyard/medusa/releases/tag/v2.11.0).

</Note>

<Note title="This guide is for developers who have">

1. Set up a Cache Module in `switchyard.config.ts`.
...
![Diagram illustrating the change in architecture from Cache Module to Caching Module](https://res.cloudinary.com/dza7lstvk/image/upload/v1759845565/Switchyard%20Resources/cache-to-caching_yxaped.jpg)
...
<Prerequisites
  items={[
    {
      text: "Updated your Switchyard application to v2.11.0 or later",
      link: "https://github.com/switchyard/medusa/releases/tag/v2.11.0"
    }
  ]}
/>
...
The first step is to remove the Cache Module from your `switchyard.config.ts` file.

For example, if you set up the Redis Cache Module, remove the following code from `switchyard.config.ts`:

```ts title="switchyard.config.ts" highlights={[["5"], ["6"], ["7"], ["8"], ["9"], ["10"]]}
