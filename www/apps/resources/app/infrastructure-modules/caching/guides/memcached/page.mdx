...
In this tutorial, you'll learn how to create a Memcached [Caching Module Provider](../../providers/page.mdx) for your Switchyard application.
...
By the end of this tutorial, you'll be able to cache data in your Switchyard application using Memcached.

<Note>

Refer to the [Caching Module](../../page.mdx) documentation to learn more about caching in Switchyard.

</Note>

![Diagram illustrating the Memcached caching module provider in a Switchyard application](https://res.cloudinary.com/dza7lstvk/image/upload/v1759925175/Switchyard%20Resources/memcached-summary_ztlanl.jpg)

<Card
  title="Full Code"
  href="https://github.com/switchyard/examples/tree/main/memcached-caching"
...
To interact with Memcached, you'll need to install the `memjs` client. You can install it in your Switchyard project by running the following command:
...
A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/memcached` in your Switchyard project.
...
You first define module options that are passed to the Memcached Module Provider. You'll set those up later when you [register the module in your Switchyard application](#register-memcached-module-provider). The module accepts the following options:
...
- `options`: The options passed to the module when it's registered in the Switchyard application.
...
2. The options passed to the module when it's registered in the Switchyard application.
...
The final piece of a module provider is its definition, which you export in an `index.ts` file at its root directory. This definition tells Switchyard which module this provider belongs to, its loaders, and its service.
...
The last step is to register the Memcached Module Provider in your Switchyard application.
...
Then, in `switchyard.config.ts`, add a new entry in the `modules` array to register the Memcached Module Provider:
...
You register the `@switchyard/medusa/caching` module and add the Memcached Module Provider to its `providers` array.
...
To test that the Memcached Caching Provider is working, start the Switchyard application with the following command:
...
If you set the `is_default` option to `true` in the provider registration, the Memcached Caching Provider will be used for all caching operations in the Switchyard application.
