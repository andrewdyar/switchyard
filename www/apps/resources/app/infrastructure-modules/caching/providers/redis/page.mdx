...
To use the Redis Caching Module Provider, you need to register it in the `providers` array of the Caching Module in your `switchyard.config.ts`.
...
`caching-redis` is the ID you set in the `switchyard.config.ts` file when registering the Redis Caching Module Provider.
...
Finally, start your Switchyard server with the following command:
