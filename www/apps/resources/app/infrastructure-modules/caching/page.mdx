...
Refer to the [Switchyard Cache Cloud](!cloud!/cache) guide for setting up the Caching Module in Cloud.

</Note>

<Note>

The Caching Module is available starting [Switchyard v2.11.0](https://github.com/switchyard/medusa/releases/tag/v2.11.0). It replaces the deprecated [Cache Module](../cache/page.mdx).

</Note>

## What is the Caching Module?

The Caching Module provides functionality to cache data in your Switchyard application, improving performance and reducing latency for frequently accessed data.

For example, Switchyard uses the Caching Module to cache product information, and you can cache custom data such as brand information.

The Caching Module stores and retrieves cached data using the caching service you integrate, such as [Redis](./providers/redis/page.mdx) or [Memcached](./guides/memcached/page.mdx). This provides flexibility in customizing your Switchyard application's infrastructure to meet your performance and scalability requirements.

![Diagram illustrating the Caching Module architecture](https://res.cloudinary.com/dza7lstvk/image/upload/v1759846791/Switchyard%20Resources/caching-overview_tz91tw.jpg)

### Caching Module vs Cache Module

Before Switchyard v2.11.0, you used the [Cache Module](../cache/page.mdx) to cache data. The Cache Module is now deprecated and has been replaced by the Caching Module.
...
The Caching Module is installed by default in your application. To use it, enable the caching feature flag and register the module in your `switchyard.config.ts` file.
...
This enables you to use the Caching Module and activates caching features in Switchyard's core.

### 2. Register the Caching Module

Next, add the Caching Module to the `modules` property of the exported object in `switchyard.config.ts`:
...
Refer to the [Caching Module Providers guide](./providers/page.mdx) to learn more about Caching Module Providers in Switchyard, and how to configure the default provider.
...
After you enable the Caching Module, Switchyard automatically caches data for several business-critical APIs to boost performance and throughput. This includes all cart-related operations, where the following data is cached:
...
You can also use the Caching Module's service directly to cache custom data in your Switchyard application.
...
In the example above, you create a step that resolves the Caching Module's service from the [Switchyard container](!docs!/learn/fundamentals/medusa-container).
...
You can pass the following options to the Caching Module when registering it in your `switchyard.config.ts` file:
