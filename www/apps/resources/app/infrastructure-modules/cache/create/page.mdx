...
The Cache Module is deprecated starting from [Switchyard v2.11.0](https://github.com/switchyard/medusa/releases/tag/v2.11.0). [Create a Caching Module Provider](#) instead.
...
The Cache Module's main service must implement the `ICacheService` interface imported from `@switchyard/framework/types`:
...
To use your Cache Module, add it to the `modules` object exported as part of the configurations in `switchyard.config.ts`. A Cache Module is added under the `cacheService` key.
