...
The PostgreSQL Locking Module Provider uses PostgreSQL's advisory locks to control and manage locks across multiple instances of Switchyard. Advisory locks are lightweight locks that do not interfere with other database transactions. By using PostgreSQL's advisory locks, Switchyard can create distributed locks directly through the database.
...
To register the PostgreSQL Locking Module Provider, add it to the list of providers of the Locking Module in `switchyard.config.ts`:
  ...
  modules: [
    {
      ...
      options: {
        providers: [
          {
            resolve: "@switchyard/medusa/locking-postgres",
            id: "locking-postgres",
...
```ts title="switchyard.config.ts" highlights={defaultHighlights}
module.exports = defineConfig({
  // ...
  modules: [
    {
