...
Switchyard's architectural functionalities, such as emitting and subscribing to events or caching data, are all implemented in Infrastructure Modules. An Infrastructure Module is a package that can be installed and used in any Switchyard application. These modules allow you to choose and integrate custom services for architectural purposes.
...
This section of the documentation showcases Switchyard's Infrastructure Modules, how they work, and how to use them in your Switchyard application.
...
{/* The Analytics Module has module providers that implement the underlying logic of integrating third-party services for tracking analytics. The following Analytics Module Providers are provided by Switchyard. You can also create a custom provider as explained in the [Create Analytics Module Provider guide](/references/analytics/provider). */}
...
The following Caching modules are provided by Switchyard. You can also create a custom Caching Module Provider as explained in the [Create Caching Module Provider guide](#).
...
The following Event modules are provided by Switchyard. You can also create your own event module as explained in [this guide](./event/create/page.mdx).
...
The Locking Module uses module providers that implement the underlying logic of the locking mechanism. The following Locking Module Providers are provided by Switchyard. You can also create a custom provider as explained in the [Create Locking Module Provider guide](/references/locking-module-provider).
...
The Notification Module has module providers that implement the underlying logic of sending notifications, typically through integrating a third-party service. The following modules are provided by Switchyard. You can also create a custom provider as explained in the [Create Notification Module Provider guide](/references/notification-provider-module).
...
The following Workflow Engine modules are provided by Switchyard.
