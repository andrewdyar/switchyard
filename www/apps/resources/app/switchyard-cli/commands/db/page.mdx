...
export const metadata = {
  title: `db Commands - Switchyard CLI Reference`,
}

# {metadata.title}

Commands in the Switchyard CLI starting with `db:` perform actions on the database.

## db\:setup

Creates a database for the Switchyard application with the specified name, if it doesn't exist. Then, it runs migrations and syncs links.

It also updates your `.env` file with the database name.

```bash
npx switchyard db:setup --db <name>
```

<Note title="Tip">

Use this command if you're setting up a Switchyard project or database manually.
...
Creates a database for the Switchyard application with the specified name, if it doesn't exist.

It also updates your `.env` file with the database name.

```bash
npx switchyard db:create --db <name>
```
...
```bash
npx switchyard db:generate <module_names...>
```
...
Use this command if you've updated the Switchyard packages, or you've created customizations and want to reflect them in the database.
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      [Switchyard v2.3.0](https://github.com/switchyard/medusa/releases/tag/v2.3.0).
...
```bash
npx switchyard db:rollback <module_names...>
```
...
Sync the database with the link definitions in your application, including the definitions in Switchyard's modules.
