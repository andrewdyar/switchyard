...
- Create a callback page that the third-party service redirects to after authentication. This page receives query parameters from the third-party service and uses them to validate the authentication in Switchyard.
...
<Prerequisites
  items={[
    ...
    {
      text: "Install the Google Auth Module Provider in your Switchyard application, or the provider you're using.",
...
You'll use it to decode the token that Switchyard returns after validating the authentication callback.
...
Finally, you'll add to the page a function that validates the authentication callback in Switchyard and creates or retrieves the customer account. It will use the functions added earlier.
...
export const validateReactHighlights = [
  ["2", "sendCallback", "Validate the callback in Switchyard and retrieve the authentication token"],
...
2. Decode the token to check if it has an `actor_id` property.
  ...
  - If the property doesn't exist, this is the first time the customer is authenticating with the third-party service. So:
      1. Create a customer. The `user_metadata` in the decoded token will hold the customer's information in the third-party provider, such as their `email`. You can use this value as the customer's email in Switchyard.
...
If you already set up the [Auth Module Provider](../../../commerce-modules/auth/auth-providers/page.mdx) in your Switchyard application, you can log in a customer with a third-party service, such as Google or GitHub, using the following flow:

![Diagram illustrating the authentication flow between the storefront, Switchyard application, and the third-party service.](https://res.cloudinary.com/dza7lstvk/image/upload/v1725531068/Switchyard%20Resources/Social_Media_Graphics_third-party-auth-customer_kfn3k3.jpg)
...
3. In the storefront's callback page, send a request to the [Validate Authentication Callback API route](!api!/store#auth_postactor_typeauth_providercallback). Pass the query parameters (`code`, `state`, etc.) received from the third-party service.
    - Switchyard validates the authentication with the third-party service.
