...
In this tutorial, you'll learn how to create a mobile app that connects to your Switchyard backend with React Native and Expo. You can then publish your app to the Apple App Store and Google Play Store.

When you install a Switchyard application, you get a fully-fledged commerce server and an admin dashboard to manage the commerce store's data. Switchyard's [architecture](!docs!/learn/introduction/architecture) is flexible and customizable, allowing you to build your own custom storefronts using any technology you prefer.
...
1. Set up a Switchyard application.
2. Create an app with React Native and Expo.
3. Connect the Expo app to the Switchyard backend.
4. Implement essential ecommerce features such as product listing, cart management, and checkout.

You can follow this guide whether you're new to Switchyard or an advanced Switchyard developer. However, this tutorial assumes you have a basic understanding of React Native and JavaScript.

![Screenshot of home screen of the Expo app](https://res.cloudinary.com/dza7lstvk/image/upload/v1762333811/Switchyard%20Resources/CleanShot_2025-11-05_at_11.09.27_lxl2tw.png)

<Card
  title="Full Code"
  text="Find the full code for this tutorial in this repository."
  href="https://github.com/switchyard/examples/tree/main/react-native-expo"
  icon={Github}
/>
...
Afterwards, the installation process will start, which will install the Switchyard application in a directory with your project's name. Once the installation finishes successfully, the Switchyard Admin dashboard will open in your browser at `http://localhost:9000/app` with a form to create a new user. Enter the user's credentials and submit the form.
...
In a directory separate from your Switchyard application, run the following command to create a new Expo project:
...
```bash npm2yarn
npm install @switchyard/js-sdk @switchyard/types @react-native-async-storage/async-storage @react-native-picker/picker @react-navigation/drawer
```

You install the following packages:

- `@switchyard/js-sdk`: Switchyard's [JS SDK](../../../js-sdk/page.mdx) to interact with the Switchyard backend.
- `@switchyard/types`: TypeScript types for Switchyard, which are useful when working with Switchyard's JS SDK.
...
## Step 5: Initialize Switchyard JS SDK

In this step, you'll set up Switchyard's [JS SDK](../../../js-sdk/page.mdx) in your Expo app. You'll use the SDK to interact with the Switchyard backend.
...
- Switchyard backend URL, which is read from the environment variable `EXPO_PUBLIC_MEDUSA_URL`. If the variable is not set, it defaults to `http://localhost:9000`.
...
Next, you'll set the environment variables in your Expo project. Before you do that, you need to retrieve the Switchyard backend URL and the Publishable API key.

The Switchyard backend URL is the URL where your Switchyard server is running. If you're running the Switchyard server locally, it should be the IP address of your machine at the port `9000`. For example, `http://192.168.1.100:9000`.
...
Next, to get the Publishable API key, start the Switchyard application by running the following command in its directory:
...
1. Open the Switchyard Admin dashboard at `http://localhost:9000/app` and log in.
...
Once you have the Switchyard backend URL and the Publishable API key, create the file `.env` in the root directory of your Expo project with the following content:
...
Make sure to replace the values with your actual Switchyard backend URL and Publishable API key.

### Optional: Update CORS in Switchyard

If you plan to test the Expo app in a web browser, you'll need to update the CORS settings in the Switchyard backend to allow requests from the Expo development server.

In your Switchyard application's directory, add the Expo development server URL to the `STORE_CORS` and `AUTH_CORS` environment variables in the `.env` file:
...
Make sure to restart the Switchyard server after making changes to the `.env` file.
...
export const regionContextHighlights = [
  ["7", "regions", "Available regions from the Switchyard backend."],
...
1. `regions`: An array of available regions fetched from the Switchyard backend.
...
Next, you'll implement the logic to load the regions from the Switchyard backend, and manage the selected region state.

Replace the `// TODO load and select regions` comment with the following:

export const regionLoadingHighlights = [
  ["1", "loadRegions", "Load regions from the Switchyard backend."],
...
You define the `loadRegions` function that fetches the regions from the Switchyard backend using the JS SDK. It also loads any previously selected region and country from `AsyncStorage`, or defaults to the first region and its first country.
...
Next, you'll implement the logic to load the cart from the Switchyard backend. Replace the `// TODO load cart` comment with the following:
...
The `loadCart` function creates or retrieves the saved cart from the Switchyard backend using the JS SDK. This function runs when the component mounts using the `useEffect` hook.
...
In this step, you'll create the home screen of the app. It will display a hero image with a list of products retrieved from the Switchyard backend.
 ...
 * Note: Switchyard stores prices in major units (e.g., dollars, euros)
...
export const homeScreenHighlights = [
  ["22", "fetchProducts", "Fetch products from the Switchyard backend based on the selected region."],
...
The home screen component fetches products from the Switchyard backend based on the selected region. It manages loading, refreshing, and error states.
...
Before you run your app, start the Switchyard backend by running the following command in the Switchyard project directory:
...
When the app opens, you'll see a home screen displaying a hero image and a list of products fetched from the Switchyard application.
...
![Home screen showing the hero image and list of products](https://res.cloudinary.com/dza7lstvk/image/upload/v1762344360/Switchyard%20Resources/CleanShot_2025-11-05_at_13.54.13_2x_efopgu.png)
...
![Drawer navigator showing the region selector](https://res.cloudinary.com/dza7lstvk/image/upload/v1762344361/Switchyard%20Resources/CleanShot_2025-11-05_at_13.54.38_2x_r0tb6i.png)
...
The `fetchProduct` function retrieves product details from the Switchyard backend using the product ID. The region ID is passed as a query parameter to get prices specific to the selected region.
...
To test out the product detail screen, run your Switchyard application and Expo app.
...
![Product Detail Screen](https://res.cloudinary.com/dza7lstvk/image/upload/v1762417976/Switchyard%20Resources/CleanShot_2025-11-06_at_10.29.04_2x_girdxx.png)
...
To test out the cart screen, run your Switchyard application and Expo app.
...
![Cart screen with items in it](https://res.cloudinary.com/dza7lstvk/image/upload/v1762358342/Switchyard%20Resources/CleanShot_2025-11-05_at_17.58.18_2x_w5gmly.png)
...
The function only handles Switchyard's built-in manual payment provider `pp_system_default`. You can expand it later to include more providers that you integrate.
...
You add a `fetchShippingOptions` function that retrieves shipping options, and a `fetchPaymentProviders` function that retrieves payment providers from the Switchyard backend.
...
To test the checkout flow, run your Switchyard application and Expo app.
...
![Delivery step with address filled](https://res.cloudinary.com/dza7lstvk/image/upload/v1762360359/Switchyard%20Resources/CleanShot_2025-11-05_at_18.31.49_2x_jl3q4g.png)

Then, in the Shipping step, select a shipping method and click "Continue" to proceed to the Payment step.

![Shipping step with shipping method selected](https://res.cloudinary.com/dza7lstvk/image/upload/v1762360359/Switchyard%20Resources/CleanShot_2025-11-05_at_18.31.49_2x_jl3q4g.png)
...
![Payment step with payment provider selected](https://res.cloudinary.com/dza7lstvk/image/upload/v1762360464/Switchyard%20Resources/CleanShot_2025-11-05_at_18.33.59_2x_eled5o.png)
...
The `fetchOrder` function retrieves order details from the Switchyard backend using the order ID from the URL parameters.
...
To test the order confirmation screen, run your Switchyard application and Expo app.
...
![Order confirmation screen](https://res.cloudinary.com/dza7lstvk/image/upload/v1762360994/Switchyard%20Resources/CleanShot_2025-11-05_at_18.42.42_2x_prvkba.png)

---

## Next Steps

You now have an ecommerce app built with React Native and Switchyard, complete with product listing, cart management, checkout flow, and order confirmation.
