...
export const ogImage = "https://res.cloudinary.com/dza7lstvk/image/upload/v1739360151/Switchyard%20Resources/Express_Checkout_Guide_2025_yizn34.jpg"
...
![Express checkout storefront interface showing a single-page checkout flow with product selection](https://res.cloudinary.com/dza7lstvk/image/upload/v1739344861/Switchyard%20Resources/Screenshot_2025-02-12_at_9.20.09_AM_ufqdi3.png)
...
<Card
  ...
  href="https://github.com/switchyard/examples/tree/main/express-checkout-storefront"
  icon={Github}
/>
...
Afterwards, the installation process will start, which will install the Switchyard application in a directory with your project's name. Once the installation finishes successfully, the Switchyard Admin dashboard will open in your browser at `http://localhost:9000/app` with a form to create a new user. Enter the user's credential and submit the form.
...
The Switchyard application enforces Cross Origin Resource Sharing (CORS) security, which means you can only access the Switchyard API from configured origins.

The Next.js development server will run on port `3000` by default, which isn't a configured origin in the Switchyard application. To add it to configured origins, change the value of `STORE_CORS` and `AUTH_CORS` in the Switchyard application's `.env` file to the following:
...
Learn more about CORS configurations in [this guide](!docs!/learn/configurations/switchyard.config#httpauthcors).

---

## Step 2: Install Next.js Project

You'll create the express checkout storefront using Next.js. So, to create a Next.js project, run the following command in a directory separate from the Switchyard application:
...
## Step 3: Install Switchyard Dependencies
...
### Install Switchyard UI

Switchyard provides a [UI Library](!ui!) that provides the primitives to build applications based on Switchyard's design system. To install the UI library, run the following command in your Next.js project:

```bash npm2yarn
npm install @switchyard/ui@latest @switchyard/ui-preset@latest @switchyard/icons@latest
```

You install three packages:

- `@switchyard/ui`: The main [UI library](!ui!).
- `@switchyard/ui-preset`: The design and style presets for the UI library.
- `@switchyard/icons`: A package that has [Switchyard icons](!ui!/icons/overview) you can use in your application.

Next, to use Switchyard's design system in your storefront, change the content of `tailwind.config.ts` to the following:
...
This adds Switchyard's UI presets to Tailwind CSS's presets and includes the Switchyard UI components in the Tailwind CSS content, ensuring that the styles of the UI components aren't removed by tree-shaking.

### Install Switchyard JS SDK

To interact with the [Switchyard API](!api!/store) in your storefront, you'll use the [Switchyard JS SDK](../../../js-sdk/page.mdx). Install the SDK by running the following command in your Next.js project:

```bash npm2yarn
npm install @switchyard/js-sdk@latest @switchyard/types@latest
```

Aside from the SDK, you also install the `@switchyard/types` package, which provides TypeScript types useful for your development with the SDK.
...
![Project directory structure showing the newly created 'lib' folder containing the 'sdk.ts' file, which exports a configured Switchyard SDK instance for connecting to the backend API](https://res.cloudinary.com/dza7lstvk/image/upload/v1735815487/Switchyard%20Resources/express-checkout-1_vrudhk.jpg)
...
This file exports a configured instance of the Switchyard SDK that you can use to send requests to the Switchyard application. To configure the SDK, you use the following environment variables:

- `NEXT_PUBLIC_MEDUSA_BACKEND_URL`: The URL of the Switchyard application server. By default, it's set to `http://localhost:9000`.
- `NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY`: The publishable API key of the Switchyard application. You can find this key in the Switchyard Admin dashboard under Settings -> Publishable API Keys.

![Navigation path in Switchyard Admin dashboard: Settings showing "Publishable API Key" option, with the publishable API key displayed and a copy button to retrieve the token for storefront configuration](https://res.cloudinary.com/dza7lstvk/image/upload/v1735810830/Switchyard%20Resources/Screenshot_2025-01-02_at_11.36.20_AM_gw7i7h.png)
...
- The selected region. In Switchyard, a region represents one or more countries you serve customers in. Each region can have different settings, such as currency codes, prices, payment providers, and more. You create regions in the Switchyard Admin, and you can allow the customer to choose their region in the storefront.
...
- `regions`: A list of regions available in the Switchyard application.
...
export const regionProviderHighlights2 = [
  ["16", "regions", "A state variable that will hold the list of regions available in the Switchyard application."],
...
- `regions`: A state variable that will hold the list of regions available in the Switchyard application.
...
export const regionProviderHighlights3 = [
  ["6", "list", "Retrieve the list of regions from the Switchyard application."]
]
...
When the component mounts, you fetch the regions from the Switchyard application using the JS SDK. You then set the regions in the `regions` state variable.
...
export const regionProviderHighlights4 = [
  ...
  ["16", "retrieve", "Retrieve the details of the selected region from the Switchyard application."],
]
...
If the region isn't set in the state variable, you try to retrieve the previously selected value from the local storage. If it's not found, you set the first region in the `regions` list as the selected region. Otherwise, you retrieve the previously selected region from the Switchyard application using the JS SDK.
...
![Directory structure after creating cart provider file](https://res.cloudinary.com/dza7lstvk/image/upload/v1735816212/Switchyard%20Resources/express-checkout-3_ajve6d.jpg)
...
export const cartProviderHighlights3 = [
  ...
  ["16", "retrieve", "Retrieve the cart from the Switchyard application if the cart ID is found in the local storage."],
]
...
export const cartProviderHighlights4 = [
  ["6", "update", "Update the cart's region in the Switchyard application if it's changed."]
]
...
export const cartProviderHighlights5 = [
  ["6", "create", "Create the cart in the Switchyard application."],
...
export const cartProviderHighlights6 = [
  ...
  ["7", "createLineItem", "Add the product variant to the cart in the Switchyard application."],
...
export const cartProviderHighlights7 = [
  ["13", "update", "Update the cart's general details in the Switchyard application."],
  ["17", "addShippingMethod", "Set the cart's shipping method in the Switchyard application."],
...
export const cartProviderHighlights8 = [
  ["2", "updateLineItem", "Update the quantity of an item in the cart in the Switchyard application."],
...
![Directory structure after creating second column file](https://res.cloudinary.com/dza7lstvk/image/upload/v1735904919/Switchyard%20Resources/express-checkout-4_ocan8h.jpg)

export const secondColHighlights = [
  ["7", "region", "The selected region"],
  ["7", "regions", "The list of regions in the Switchyard application"],
  ...
  return (
    <div className={clx(
      ...
      <div className="flex flex-col justify-center gap-1">
        ...
        <img 
          src="https://res.cloudinary.com/dza7lstvk/image/upload/v1735642745/Switchyard%20Resources/medusa-express-logo_gqu5qy.png" 
          alt="Switchyard" 
          width={67}
          height={16}
        />
...
You create a `SecondCol` component that shows the Switchyard logo and a dropdown to change the selected region. You use the `useRegion` hook to access the region context and manage region data.
...
Each product in Switchyard has a `handle` property, which is a slug-like value that can be used on client applications to create friendly URLs.
...
![Directory structure after creating Router component file](https://res.cloudinary.com/dza7lstvk/image/upload/v1735819072/Switchyard%20Resources/express-checkout-5_bhpx0w.jpg)
...
![Directory structure after creating express checkout page file](https://res.cloudinary.com/dza7lstvk/image/upload/v1735820075/Switchyard%20Resources/express-checkout-6_w6qf7d.jpg)
...
![Directory structure after creating Card component file](https://res.cloudinary.com/dza7lstvk/image/upload/v1735820328/Switchyard%20Resources/express-checkout-7_a8ys4g.jpg)
...
import { CheckCircle } from "@switchyard/icons"
import { clx, Heading } from "@switchyard/ui"
...
![Directory structure after creating Product component file](https://res.cloudinary.com/dza7lstvk/image/upload/v1735821135/Switchyard%20Resources/express-checkout-8_vloodv.jpg)

export const productHighlights1 = [
  ["17", "loading", "Whether an operation is loading."],
  ["18", "product", "The product's details retrieved from the Switchyard application."],
...
- `product`: A state variable that holds the product's details, which you'll retrieve from the Switchyard application.
...
Next, you'll retrieve the product's details from the Switchyard application. Start by adding the following imports to the top of the file:
...
export const productHighlights2 = [
  ["6", "list", "Retrieve the product from the Switchyard application by filtering products with the unique handle."],
...
Most of Switchyard's API routes accept the `fields` query parameter, allowing you to specify the fields you need to retrieve. Learn more about its usage in [this documentation](!api!/store#select-fields-and-relations)
...
Next, you'll set the price to show to the customer. The Switchyard application returns the price as a number. To display it with currency, you'll create a utility that you'll re-use whenever you show a price.

Create the file `lib/price.ts` with the following content:

![Directory structure after creating price utility file](https://res.cloudinary.com/dza7lstvk/image/upload/v1735825687/Switchyard%20Resources/express-checkout-9_nvhjqo.jpg)
...
In Switchyard, each product variant has different inventory quantity. So, after the customer selects a variant, you'll check whether it's in stock before allowing them to add it to the cart.
...
- The variant's `manage_inventory` property is set to `false`, meaning Switchyard doesn't manage the variant's inventory quantity, so, it's always considered in stock;
...
import { Button, Input, Select } from "@switchyard/ui"
```
...
- If `loading` is enabled, you show a spinner icon imported from `@switchyard/icons`.
...
You also loop over the product's options and show a select input, imported from `@switchyard/ui`, to allow the customer to select the value of each option.
...
To test out what you've implemented so far, first, start the Switchyard application by running the following command in the Switchyard project's directory:

```bash npm2yarn
npm run dev
```

This will run the Switchyard application at `http://localhost:9000`.

Then, while the Switchyard application is running, run the following command in the Next.js project to start the development server:
...
When you first created the Switchyard application, four products, including the `sweatpants` product, were created by default. You can view and add products to your application by going to the Switchyard Admin dashboard at `http://localhost:9000/app`.

</Note>

![Product step showing the product's details and add to cart button](https://res.cloudinary.com/dza7lstvk/image/upload/v1735828022/Switchyard%20Resources/Screenshot_2025-01-02_at_4.26.40_PM_crvwsg.png)
...
![Directory structure after creating Address component file](https://res.cloudinary.com/dza7lstvk/image/upload/v1735828307/Switchyard%20Resources/express-checkout-10_xlfasn.jpg)
...
export const addressHighlights2 = [
  ["8", "updateCart", "Update the cart's shipping address, billing address, and email in the Switchyard application."],
...
While both the Switchyard application and the Next.js Starter Storefront are running, if you refresh the page you had opened or go to `http://localhost:3000/sweatpants?step=address`, you should see the address form where you can input your address details.

![Address step showing the address form and a continue button](https://res.cloudinary.com/dza7lstvk/image/upload/v1735831238/Switchyard%20Resources/Screenshot_2025-01-02_at_5.20.22_PM_leqt17.png)
...
![Directory structure after creating Shipping component file](https://res.cloudinary.com/dza7lstvk/image/upload/v1735831410/Switchyard%20Resources/express-checkout-11_myblmq.jpg)
...
export const shippingHighlights2 = [
  ["6", "listCartOptions", "Retrieve the shipping options for the cart from the Switchyard application."]
]
...
So, to retrieve the prices of calculated shipping options, you use Switchyard's [Calculate Shipping Option Price](!api!/store#shipping-options_postshippingoptionsidcalculate) API route.
...
export const shippingHighlights3 = [
  ...
  ["9", "fetch", "Retrieve the calculated price from the Switchyard application."],
...
In the `useEffect` hook, you filter the `shippingOptions` to get only the calculated shipping options. Then, you loop over these options to retrieve their calculated price from the Switchyard application. You pass to the Calculate Shipping Option API route the cart's ID and any custom data that the fulfillment provider might need to calculate the price.
...
export const shippingHighlights4 = [
  ...
  ["12", "updateCart", "Update the cart with the selected shipping method in the Switchyard application."],
...
```tsx title="components/Shipping/index.tsx"
import { Card } from "../Card"
import { Button, RadioGroup } from "@switchyard/ui"
```
...
While both the Switchyard application and the Next.js Starter Storefront are running, if you refresh the page you had opened or go to `http://localhost:3000/sweatpants?step=shipping`, you should see the Shipping step where you can choose a shipping method.

![Shipping step showing the shipping options and a go to payment button](https://res.cloudinary.com/dza7lstvk/image/upload/v1735832462/Switchyard%20Resources/Screenshot_2025-01-02_at_5.40.45_PM_jplcn3.png)
...
![Directory structure after creating Payment component file](https://res.cloudinary.com/dza7lstvk/image/upload/v1735832586/Switchyard%20Resources/express-checkout-12_j2xldp.jpg)
...
export const paymentHighlights1 = [
  ...
  ["20", "loading", "Whether an operation is loading."],
  ["21", "paymentProviders", "The available payment providers retrieved from the Switchyard application."],
...
- `loading`: A boolean state variable indicating whether an operation is loading.
- `paymentProviders`: A state variable that holds the available payment providers in the Switchyard application.
- `selectedPaymentProvider`: A state variable that holds the ID of the selected payment provider.
- `router`: The router instance to navigate between steps.

### Retrieve Payment Providers

In Switchyard, each region has a set of enabled payment providers. So, during checkout, you retrieve the payment providers specific to a cart's region and allow the customer to choose from them.
...
export const paymentHighlights2 = [
  ["6", "listPaymentProviders", "Retrieve the payment providers of the cart's region from the Switchyard application."]
]
...
So, you'll implement the logic to first save the payment provider selection in the Switchyard application, then optionally show additional UI based on the chosen provider.
...
export const paymentHighlights3 = [
  ["1", "handleSelectProvider", "A function to save the selected payment provider in the Switchyard application."],
  ["8", "initiatePaymentSession", "Set the chosen payment method in the Switchyard application."],
...
export const paymentHighlights4 = [
  ["7", "complete", "Complete the cart and place an order in the Switchyard application."],
...
```tsx title="components/Payment/index.tsx"
import { Card } from "../Card"
import { Button, Input, RadioGroup } from "@switchyard/ui"
```
...
While both the Switchyard application and the Next.js Starter Storefront are running, if you refresh the page you had opened or go to `http://localhost:3000/sweatpants?step=payment`, you should see the Payment step where you can choose a payment method.

![Payment step showing the payment providers and a pay button](https://res.cloudinary.com/dza7lstvk/image/upload/v1735833736/Switchyard%20Resources/Screenshot_2025-01-02_at_6.01.58_PM_usjlli.png)
...
The confirmation page will receive the order's ID as a path parameter. Then, it'll retrieve the order's details from the Switchyard application and shows those details to the customer.

To create the confirmation page, create the file `app/confirmation/[id]/page.tsx` with the following content:

![Directory structure after creating confirmation page file](https://res.cloudinary.com/dza7lstvk/image/upload/v1735890379/Switchyard%20Resources/express-checkout-13_shwtvh.jpg)

export const confirmationHighlights = [
  ["11", "orderId", "Retrieve the order's ID from the path parameter."],
  ["13", "order", "Retrieve the order's details from the Switchyard application."],
]
...
The `ConfirmationPage` component receives the order ID as a path parameter. In the component, you retrieve the order's details from the Switchyard application using the JS SDK.

Then, you show the order's details, including the order number, order date, and a thank you message.

### Test it Out

While both the Switchyard application and the Next.js Starter Storefront are running, either refresh the page you had open from the previous step, or restart the express checkout flow to place an order. You'll see the confirmation page showing the order's details.

![Confirmation page showing the order's ID and date](https://res.cloudinary.com/dza7lstvk/image/upload/v1735834096/Switchyard%20Resources/Screenshot_2025-01-02_at_6.07.38_PM_um4p0a.png)

---

## Next Steps

By following this guide, you now have an express checkout storefront built with Next.js that connects to a Switchyard commerce application.
