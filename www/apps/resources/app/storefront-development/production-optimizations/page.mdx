...
There are other important optimizations like lazy-loading images, code-splitting, and using CDNs. These optimizations depend on the frontend framework you use and your setup. This guide focuses on Switchyard-specific optimizations in your storefront.
...
When fetching data from your Switchyard backend, consider using a fetching library optimized for performance and caching. There are many options available, but one of the most popular is [TanStack Query](https://tanstack.com/query/latest).
...
When fetching data from your Switchyard backend, optimize your queries to fetch only the necessary data in the context of a component or page. This reduces the response size and time, improving your storefront's performance.

Switchyard's API routes accept a `fields` parameter that allows you to specify which fields and relations to include in the response.
...
- You defined separate cart data functions that use the [JS SDK](../../js-sdk/page.mdx) to call the Switchyard backend.
