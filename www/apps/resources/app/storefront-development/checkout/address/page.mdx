...
A customer's address and a cart's address are represented by different data models in the Switchyard application, as they're managed by the [Customer Module](../../../commerce-modules/customer/page.mdx) and the [Cart Module](../../../commerce-modules/cart/page.mdx), respectively. So, addresses that the customer used previously during checkout aren't automatically saved to their account. You need to save the customer's address using the [Create Customer Address API route](!api!/store#customers_postcustomersmeaddresses).
...
export const fetch2Highlights = [
  ...
  ["3", "retrieveCustomerAddresses", "Retrieve the customer's addresses from Switchyard."],
