...
![Diagram showing the different steps of the shipping flow in storefront checkout](https://res.cloudinary.com/dza7lstvk/image/upload/v1743085465/Switchyard%20Resources/shipping-checkout-flow_mfzdsh.jpg)
...
2. For shipping options whose `price_type=calculated`, you retrieve their calculated price using the [Calculate Shipping Option Price API Route](!api!/store#shipping-options_postshippingoptionsidcalculate).
    - The Switchyard application calculates the price using the associated fulfillment provider's logic, which may require sending a request to a third-party service.
...
- For each shipping option, you retrieve its calculated price from the Switchyard application using the [Calculate Shipping Option Price API Route](!api!/store#shipping-options_postshippingoptionsidcalculate).
