...
In this guide, you'll learn how to create and use the publishable API key in your storefront to interact with the Switchyard application's Store API routes.
...
Publishable API keys are associated with sales channels. Then, when the publishable API key is passed in the header of a request, the Switchyard application automatically infers what sales channel is being used.

---

## How to Create a Publishable API Key?

You create a publishable API key either using the [Switchyard Admin](!user-guide!/settings/developer/publishable-api-keys) or the [Admin API routes](!api!/admin#api-keys_postapikeys).
...
To add sales channels to the publishable API key's scope, you can either use the [Switchyard Admin](!user-guide!/settings/developer/publishable-api-keys#manage-publishable-api-keys-sales-channels), or send a request to the [Manage Sales Channels API route](!api!/admin#api-keys_postapikeysidsaleschannels):
...
In your storefront, pass the `x-publishable-api-key` in the header of all your requests to the Switchyard application.
...
The JS SDK will now take care of passing the publishable API key in the header of all requests to the Switchyard application.