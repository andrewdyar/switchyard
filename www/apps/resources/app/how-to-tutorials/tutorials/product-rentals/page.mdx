...
import { Github, PlaySolid, EllipsisHorizontal } from "@switchyard/icons"
import { Prerequisites, WorkflowDiagram, CardList, InlineIcon } from "docs-ui"

export const metadata = {
  title: `Implement Product Rentals in Switchyard`,
}

# {metadata.title}

In this tutorial, you'll learn how to implement product rentals in Switchyard.

When you install a Switchyard application, you get a fully-fledged commerce platform with a Framework for customization. The Switchyard application's commerce features are built around [Commerce Modules](../../../commerce-modules/page.mdx) that are available out-of-the-box.
...
- Install and set up Switchyard with the Next.js Starter Storefront.
...
![Diagram of the Rentals Module and its connection with Switchyard's Product, Order, and Customer Modules](https://res.cloudinary.com/dza7lstvk/image/upload/v1761722702/Switchyard%20Resources/product-rentals-summary_rhjwjn.jpg)

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/product-rentals",
    title: "Full Code",
...
Afterward, the installation process will start, which will install the Switchyard application in a directory with your project's name, and the Next.js Starter Storefront in a separate directory with the `{project-name}-storefront` name.
...
Once the installation finishes successfully, the Switchyard Admin dashboard will open with a form to create a new user. Enter the user's credentials and submit the form. Afterward, you can log in with the new user and explore the dashboard.
...
- `product_id`: The ID of the Switchyard product associated with the rental configuration.
...
Notice that you'll handle pricing and inventory through Switchyard's existing [Product](../../../commerce-modules/product/page.mdx) and [Inventory](../../../commerce-modules/inventory/page.mdx) modules.
...
- `id`: The primary key of the table.
- `variant_id`: The ID of the Switchyard product variant being rented.
- `customer_id`: The ID of the customer renting the product.
- `order_id`: The ID of the Switchyard order associated with the rental.
- `line_item_id`: The ID of the Switchyard line item associated with the rental.
...
class RentalModuleService extends SwitchyardService({
  Rental,
  RentalConfiguration,
}) {
...
The `RentalModuleService` extends `SwitchyardService`, which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.

So, the `RentalModuleService` class now has methods like `createRentals` and `retrieveRentalConfiguration`.

<Note>

Find all methods generated by the `SwitchyardService` in [the Service Factory](../../../service-factory-reference/page.mdx) reference.
...
In `switchyard.config.ts`, add a `modules` property and pass it an array with your custom module:
...
Switchyard's CLI tool can generate the migrations for you. To generate a migration for the Rental Module, run the following command in your Switchyard application's directory:

```bash
npx switchyard db:generate rental
```

The `db:generate` command of the Switchyard CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/rental` that holds the generated migration.
...
Since Switchyard isolates modules to integrate them into your application without side effects, you can't directly create relationships between data models of different modules.

Instead, Switchyard provides a mechanism to define links between data models and retrieve and manage linked records while maintaining module isolation.
...
In this step, you'll define links between the data models in the Rental Module and the data models in Switchyard's modules:
...
To sync the links to the database, run the migrations command again in the Switchyard application's directory:
...
In this step, you'll implement the logic to create or update a rental configuration for a product. Later, you'll execute this logic from an API route, and allow admin users to manage rental configurations from the Switchyard Admin dashboard.
...
Switchyard provides the `useQueryGraphStep` and `createRemoteLinkStep` out-of-the-box. So, you only need to implement the other steps.
...
In the step function, you resolve the Rental Module's service from the Switchyard container, and use it to create a rental configuration.
...
In this step, you'll create API routes that allow you to retrieve and manage rental configurations. Later, you'll use these API routes in the Switchyard Admin dashboard to allow admin users to manage rental configurations.
...
export const POST = async (
  req: SwitchyardRequest<z.infer<typeof PostRentalConfigBodySchema>>,
...
In the API route handler, you execute the `upsertRentalConfigWorkflow` by invoking it, passing it the Switchyard container from the request's scope. Then, you call its `run` method, passing the workflow's input from the request's parameters and validated body.
...
You expose a `GET` API route at `/admin/products/:id/rental-config`. In the route handler, you resolve [Query](!docs!/learn/fundamentals/module-links/query) from the Switchyard container and use it to query the rental configuration for the product.

Then, you return the rental configuration in the response.

In the next step, you'll consume these API routes in the Switchyard Admin dashboard.

---

## Step 6: Manage Rental Configurations in Switchyard Admin

In this step, you'll customize the Switchyard Admin dashboard to allow admin users to manage rental configurations for products.

The Switchyard Admin dashboard is customizable, allowing you to insert widgets into existing pages, or create new pages.
...
To send requests to the Switchyard server, you'll use the [JS SDK](../../../js-sdk/page.mdx). It's already installed in your Switchyard project, but you need to initialize it before using it in your customizations.

Create the file `src/admin/lib/sdk.ts` with the following content:

export const sdkHighlights = [
  ["3", "new Switchyard", "Initialize the Switchyard JS SDK."],
...
- A `config` object created with `defineWidgetConfig` from the Admin SDK. It accepts an object with the `zone` property that indicates where the widget will be rendered in the Switchyard Admin dashboard.
...
2. `confirm` to show confirmation prompts using [Switchyard's UI package](!ui!/components/prompt).
...
You can now test the rental configuration widget in the Switchyard Admin dashboard.

Run the following command in your Switchyard application's directory to start the Switchyard server:

```bash npm2yarn
npm run dev
```

Then, open the Switchyard Admin dashboard in your browser at `http://localhost:9000/app` and login with the user you created in the first step.
...
![Rental Configuration Widget](https://res.cloudinary.com/dza7lstvk/image/upload/v1761650341/Switchyard%20Resources/CleanShot_2025-10-28_at_13.17.54_2x_ip1stl.png)
...
![Rental configuration form with minimum and maximum rental days fields](https://res.cloudinary.com/dza7lstvk/image/upload/v1761650449/Switchyard%20Resources/CleanShot_2025-10-28_at_13.20.02_2x_zvixfc.png)
...
![Rental configuration details in the widget](https://res.cloudinary.com/dza7lstvk/image/upload/v1761650526/Switchyard%20Resources/CleanShot_2025-10-28_at_13.21.43_2x_t7ktgq.png)
...
If any validation fails, you throw a `SwitchyardError` with the `INVALID_DATA` type.
...
import { SwitchyardError, QueryContext } from "@switchyard/framework/utils"
import { z } from "zod"
import { RENTAL_MODULE } from "../../../../../modules/rental"
import RentalModuleService from "../../../../../modules/rental/service"
...
export const GET = async (
  req: SwitchyardRequest<{}, z.infer<typeof GetRentalAvailabilitySchema>>, 
  res: SwitchyardResponse
) => {
...
So, if your Switchyard application's directory is `medusa-product-rental`, you can find the storefront by going back to the parent directory and changing to the `medusa-product-rental-storefront` directory:
...
First, run the following command in the Switchyard application's directory to start the Switchyard server:
...
![Rental options on product details page in the storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1761653559/Switchyard%20Resources/CleanShot_2025-10-28_at_14.12.25_2x_g09h85.png)

---

## Step 9: Add Rental Products to Cart

In this step, you'll implement the logic to add rentable products to the cart in the Switchyard application. You'll wrap Switchyard's existing add-to-cart logic to include rental-specific data and validation.
...
Switchyard provides all the steps and workflows out-of-the-box, except for the `validateRentalCartItemStep` step, which you'll implement.
...
```ts title="src/utils/has-cart-overlap.ts" badgeLabel="Switchyard Application" badgeColor="green"
...
```ts title="src/workflows/steps/validate-rental-cart-item.ts" badgeLabel="Switchyard Application" badgeColor="green" collapsibleLines="1-8" expandButtonLabel="Show Imports"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
import { SwitchyardError } from "@switchyard/framework/utils"
import { RENTAL_MODULE } from "../../modules/rental"
import RentalModuleService from "../../modules/rental/service"
import { InferTypeOf, ProductVariantDTO } from "@switchyard/framework/types"
...
```ts title="src/workflows/steps/validate-rental-cart-item.ts" badgeLabel="Switchyard Application" badgeColor="green"
// Validate metadata
...
```ts title="src/workflows/add-to-cart-with-rental.ts" badgeLabel="Switchyard Application" badgeColor="green" collapsibleLines="1-18" expandButtonLabel="Show Imports"
import { 
  createWorkflow, 
  WorkflowResponse, 
  transform,
...
export const addToCartWithRentalWorkflow = createWorkflow(
  ...
  (input: AddToCartWorkflowInput) => {
    ...
    const itemToAdd = transform({
      ...
      // For non-rental products, don't specify unit_price (let Switchyard calculate it)
      return [baseItem]
    })
...
5. Prepare the item to add to the cart.
    ...
    - For non-rentable products, you don't specify the `unit_price`; Switchyard will use the variant's price.
...
```ts title="src/api/store/carts/[id]/line-items/rentals/route.ts" badgeLabel="Switchyard Application" badgeColor="green" collapsibleLines="1-9" expandButtonLabel="Show Imports"
import type { 
  SwitchyardRequest, 
  SwitchyardResponse,
} from "@switchyard/framework/http"
...
export const POST = async (
  req: SwitchyardRequest<z.infer<typeof PostCartItemsRentalsBody>>,
  res: SwitchyardResponse
) => {
...
First, run both the Switchyard server and the Next.js Starter Storefront.
...
![Rental product added to cart in the storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1761660048/Switchyard%20Resources/CleanShot_2025-10-28_at_15.38.24_2x_yfo50v.png)

---

## Step 11: Create Rental Orders

In this step, you'll implement the logic to create rental orders in the Switchyard application. You'll wrap Switchyard's existing order creation logic to handle rental-specific data and validation.
...
You'll implement the `validateRentalStep` and `createRentalsStep` steps used in the workflow. The rest are provided by Switchyard out-of-the-box.
...
```ts title="src/workflows/steps/validate-rental.ts" badgeLabel="Switchyard Application" badgeColor="green"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
import { SwitchyardError } from "@switchyard/framework/utils"
import { RENTAL_MODULE } from "../../modules/rental"
import RentalModuleService from "../../modules/rental/service"
...
```ts title="src/workflows/steps/create-rentals-for-order.ts" badgeLabel="Switchyard Application" badgeColor="green"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
import { RENTAL_MODULE } from "../../modules/rental"
import RentalModuleService from "../../modules/rental/service"
...
```ts title="src/workflows/create-rentals.ts" badgeLabel="Switchyard Application" badgeColor="green" collapsibleLines="1-21" expandButtonLabel="Show Imports"
import { 
  createWorkflow, 
  WorkflowResponse, 
  transform,
...
```ts title="src/api/store/rentals/[cart_id]/route.ts" badgeLabel="Switchyard Application" badgeColor="green"
import type { SwitchyardRequest, SwitchyardResponse } from "@switchyard/framework/http"
...
![Rental order confirmation in the storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1761661870/Switchyard%20Resources/CleanShot_2025-10-28_at_16.30.14_2x_v8wbb7.png)

---

## Step 13: Manage Rentals in Admin

In this step, you'll allow admin users to manage rentals in the Switchyard Admin Dashboard. You will:
...
```ts title="src/api/admin/orders/[id]/rentals/route.ts" badgeLabel="Switchyard Application" badgeColor="green"
import type { SwitchyardRequest, SwitchyardResponse } from "@switchyard/framework/http"
...
```ts title="src/workflows/steps/update-rental.ts" badgeLabel="Switchyard Application" badgeColor="green"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
import { RENTAL_MODULE } from "../../modules/rental"
import RentalModuleService from "../../modules/rental/service"
import { SwitchyardError } from "@switchyard/framework/utils"
...
```ts title="src/workflows/update-rental.ts" badgeLabel="Switchyard Application" badgeColor="green"
...
```ts title="src/api/admin/rentals/[id]/route.ts" badgeLabel="Switchyard Application" badgeColor="green"
import type { SwitchyardRequest, SwitchyardResponse } from "@switchyard/framework/http"
...
export const POST = async (
  req: SwitchyardRequest<z.infer<typeof PostRentalStatusBodySchema>>,
  res: SwitchyardResponse
) => {
  const { id } = req.params
...
```tsx title="src/admin/widgets/order-rental-items.tsx" badgeLabel="Switchyard Application" badgeColor="green" collapsibleLines="1-18" expandButtonLabel="Show Imports"
import { defineWidgetConfig } from "@switchyard/admin-sdk"
import {
  Container,
  Heading,
...
import { useQuery, useMutation } from "@tanstack/react-query"
import { sdk } from "../lib/sdk"
import { DetailWidgetProps, AdminOrder } from "@switchyard/framework/types"
import { useEffect, useState } from "react"
...
```tsx title="src/admin/widgets/order-rental-items.tsx" badgeLabel="Switchyard Application" badgeColor="green"
...
You can now test managing rentals in the Switchyard Admin Dashboard.

First, start the Switchyard application and log in.
...
![Rental items in admin order details page](https://res.cloudinary.com/dza7lstvk/image/upload/v1761662993/Switchyard%20Resources/CleanShot_2025-10-28_at_16.49.29_2x_usufnn.png)

You can click the "Update Status" button for a rental item to open the drawer and edit its status.

![Update rental status drawer in admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1761663069/Switchyard%20Resources/CleanShot_2025-10-28_at_16.50.35_2x_bzmaoq.png)
...
Switchyard Admin users can cancel orders. So, in this step, you'll customize the order cancellation flow to validate that rental items in the order can be cancelled based on their rental status. You'll also update the rental statuses when an order is cancelled.
...
```ts title="src/workflows/hooks/validate-order-cancel.ts" badgeLabel="Switchyard Application" badgeColor="green"
import { cancelOrderWorkflow } from "@switchyard/medusa/core-flows"
import { SwitchyardError, ContainerRegistrationKeys } from "@switchyard/framework/utils"
...
To test the order cancellation validation, try to cancel an order from the Switchyard Admin that has rental items with `returned` status. The order cancellation should fail.

### b. Update Rental Statuses on Order Cancellation

Next, you'll update the rental statuses when an order is cancelled.

When an order is cancelled, Switchyard emits an `order.canceled` event. You can handle this event in a [subscriber](!docs!/learn/fundamentals/events-and-subscribers).
...
```ts title="src/subscribers/order-canceled.ts" badgeLabel="Switchyard Application" badgeColor="green"
...
To test the rental status update on order cancellation, cancel an order from the Switchyard Admin that has rental items with `pending` or `active` status.

Then, refresh the page. You'll see that the rental items' statuses have been updated to `cancelled`.

![Cancelled rentals in admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1761665415/Switchyard%20Resources/CleanShot_2025-10-28_at_17.29.55_2x_apkc5n.png)
...
A scheduled job is an asynchronous function that runs tasks at specific intervals while the Switchyard application is running. You can use scheduled jobs to change a rental's status based on the rental period.
...
```ts title="src/jobs/activate-rentals.ts" badgeLabel="Switchyard Application" badgeColor="green"
...
export default async function activateRentalsJob(container: SwitchyardContainer) {
  const logger = container.resolve("logger")
  const query = container.resolve(ContainerRegistrationKeys.QUERY)
...
Then, start the Switchyard application and wait for a minute. You should see log messages indicating that the job is running and has activated any pending rentals whose start date is today.

### b. Event-Driven Rental Status Updates

You can also update rental statuses based on specific [events](/references/events) in Switchyard.
...
```ts title="src/subscribers/shipment-created.ts" badgeLabel="Switchyard Application" badgeColor="green"
import { SubscriberArgs, type SubscriberConfig } from "@switchyard/framework"
import { ContainerRegistrationKeys } from "@switchyard/framework/utils"
import { updateRentalWorkflow } from "../workflows/update-rental"
...
To test the event-driven rental status update, create a fulfillment for an order that has rental items with `pending` status from the Switchyard Admin. Then, mark the fulfillment as shipped.

If you refresh the order details page, you should see that the rental items' statuses have been updated to `active`.

![Activated rentals after shipment creation in admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1761665929/Switchyard%20Resources/CleanShot_2025-10-28_at_17.38.04_2x_xgwwan.png)
...
1. Removing the associated shipping profile of the product. You can do this from the Switchyard Admin.
...
Switchyard provides optional [inventory management for product variants](../../../commerce-modules/product/variant-inventory/page.mdx). If enabled, Switchyard will increment and decrement inventory levels for product variants when orders are placed, fulfilled, or returned.
...
Switchyard creates a reservation for the inventory of a product variant when an order is placed, and decrements the inventory when the order is fulfilled. So, you don't need to manually adjust inventory when a rental is created or activated.
...
import { updateRentalStep } from "./steps/update-rental"
import { adjustInventoryLevelsStep, useQueryGraphStep } from "@switchyard/medusa/core-flows"
...
You have now implemented product rentals in your Switchyard application. You can expand on this foundation by adding more features, such as allowing customers to view and manage their rentals from the storefront.
