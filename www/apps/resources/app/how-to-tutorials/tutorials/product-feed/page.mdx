...
In this tutorial, you'll learn how to create a product feed in Switchyard that can be used for Meta and Google.
...
Businesses that are selling on social media platforms like Meta (Instagram and Facebook) and Google need to upload their product catalog to those platforms and keep them in sync with their Switchyard store. Creating a product feed allows you to automate this process.
...
![Products in the Meta Catalogue](https://res.cloudinary.com/dza7lstvk/image/upload/v1756732400/Switchyard%20Resources/CleanShot_2025-09-01_at_16.12.33_2x_oszhkt.png)

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/product-feed",
    title: "Full Code",
    ...
    steps: [
      {
        ...
        description: "Retrieve Switchyard products as items for the feed.",
        depth: 1,
      },
      {
        type: "step",
...
The `getProductFeedItemsStep` will retrieve the Switchyard products with pagination, and format their product variants as items to be added to the feed.
...
import { CalculatedPriceSet } from "@switchyard/framework/types"
...
Then, you resolve the following resources from the Switchyard container:

- [Query](!docs!/learn/fundamentals/module-links/query) that allows you to retrieve data across modules.
- [Switchyard Configurations](!docs!/learn/configurations/switchyard.config) that are defined in `switchyard.config.ts`.

You use the Switchyard configurations to retrieve the storefront URL, which you'll use to build the product links in the feed. If the storefront URL is not set in the configurations, you fall back to the `STOREFRONT_URL` environment variable.
...
Then, you execute the `generateProductFeedWorkflow` by invoking it, passing it the Switchyard container (which is `req.scope`), and calling its `run` method. You pass the workflow's input to the `run` method.
...
import { defineMiddlewares, validateAndTransformQuery } from "@switchyard/framework/http"
import { z } from "zod"
...
To test out the product feed API route, start the Switchyard server with the following command:
...
![Meta Data Sources Setup Form](https://res.cloudinary.com/dza7lstvk/image/upload/v1756731936/Switchyard%20Resources/CleanShot_2025-09-01_at_16.04.51_2x_kht1lx.png)
...
![Meta Catalogue Products](https://res.cloudinary.com/dza7lstvk/image/upload/v1756732208/Switchyard%20Resources/CleanShot_2025-09-01_at_16.09.52_2x_tbs52f.png)

---

## Next Steps

You've now set up the product feed API route for your Switchyard application. Meta and Google will pull products from this feed periodically, ensuring your product listings are always up to date.
