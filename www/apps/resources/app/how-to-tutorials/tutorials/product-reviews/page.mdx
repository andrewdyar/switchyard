...
export const ogImage = "https://res.cloudinary.com/dza7lstvk/image/upload/v1742389227/Switchyard%20Resources/product-reviews_zia79g.jpg"

export const metadata = {
  title: `Implement Product Reviews in Switchyard`,
  openGraph: {
    images: [
      {
        url: ogImage,
...
In this tutorial, you'll learn how to implement product reviews in Switchyard.
...
Switchyard doesn't provide product reviews out-of-the-box, but the Switchyard Framework facilitates implementing customizations like product reviews. In this tutorial, you'll learn how to customize the Switchyard server, Admin dashboard, and Next.js Starter Storefront to implement product reviews.
...
- Define product reviews models and implement their management features in the Switchyard server.
- Customize the Switchyard Admin to allow merchants to view and manage product reviews.
- Customize the Next.js Starter Storefront to display product reviews and allow customers to submit reviews.

![Diagram showcasing the product review features in the storefront and admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1741941058/Switchyard%20Resources/reviews-overview_nufybf.jpg)

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/product-reviews",
...
Afterwards, the installation process will start, which will install the Switchyard application in a directory with your project's name, and the Next.js Starter Storefront in a separate directory with the `{project-name}-storefront` name.
...
A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/product-review`.
...
class ProductReviewModuleService extends SwitchyardService({
  Review,
}) {
}
...
The `ProductReviewModuleService` extends `SwitchyardService` from the Modules SDK which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.
...
Switchyard's CLI tool can generate the migrations for you. To generate a migration for the Review Module, run the following command in your Switchyard application's directory:

```bash
npx switchyard db:generate productReview
```

The `db:generate` command of the Switchyard CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/product-review` that holds the generated migration.
...
The `useQueryGraphStep` step is provided by Switchyard in its `@switchyard/medusa/core-flows` package. So, you only need to implement the `createReviewStep` step.
...
If you get a type error on resolving the Product Review Module, run the Switchyard application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](!docs!/learn/fundamentals/generated-types).
...
In the step function, you resolve the Review Module's service from the Switchyard container using its `resolve` method, passing it the module's name as a parameter.

Then, you create the review using the `createReviews` method. As you remember, the Review Module's service extends the `SwitchyardService` which generates data-management methods for you.
...
In the compensation function, you resolve the Review Module's service from the Switchyard container and call the `deleteReviews` method to delete the review created in the step.

### Add createReviewWorkflow

You can now create the workflow using the step provided by Switchyard and your custom step.
...
export const POST = async (
  req: AuthenticatedSwitchyardRequest<PostStoreReviewReq>,
...
In the route handler, you execute the `createReviewWorkflow` workflow by invoking it, passing it the Switchyard container (which is stored in the `scope` property of a request object). Then, you call its `run` method, passing to the workflow the request body as input.
...
Before creating a review, you need the ID of a product. You can either copy one from the Switchyard Admin, or send the following request:
...
Make sure to replace `{your_publishable_api_key}` with the publishable API key you retrieved from the Switchyard Admin.
...
In this step, you'll create an API route that lists the reviews of a product. You'll use this route in the Switchyard Admin customizations to allow admin users to view and manage product reviews.
...
Then, you export a `GET` function, which exposes a `GET` API Route at the path `/admin/reviews`. In the route handler you resolve [Query](!docs!/learn/fundamentals/module-links/query) from the Switchyard container, which allows you to retrieve data across modules.
...
You'll test the API route as you customize the Switchyard Admin in the next step.
...
Now that you have an API route that retrieves reviews, you'll customize the Switchyard Admin to add a new "Reviews" page by creating a [UI Route](!docs!/learn/fundamentals/admin/ui-routes).

A UI route is a React component that specifies the content to be shown in a new page in the Switchyard Admin dashboard. You'll create a UI route to display the list of reviews in the Switchyard Admin.
...
To display the table, you'll use the [DataTable](!ui!/components/data-table) component from Switchyard UI. To define the columns of the table, you use the `createDataTableColumnHelper` function from Switchyard UI, which returns a `columnHelper` object. You then use the `columnHelper` object to define the table's columns.
...
- Use the `useDataTable` hook from Switchyard UI to create a DataTable instance. You pass the columns, data, and pagination configurations to the hook.
...
The file also exports a configuration object created with `defineRouteConfig`. You export this object to tell Switchyard that you want to add the new route to the Switchyard Admin's sidebar. You specify the sidebar's item and title.

### Test the UI Route

To test out the UI route, start the Switchyard application:
...
![Reviews page showing list of reviews](https://res.cloudinary.com/dza7lstvk/image/upload/v1741935325/Switchyard%20Resources/Screenshot_2025-03-14_at_8.54.14_AM_tfhnyu.png)
...
export async function POST(
  req: SwitchyardRequest<z.infer<typeof PostAdminUpdateReviewsStatusSchema>>, 
  res: SwitchyardResponse
) {
...
The `DataTable` component from Switchyard UI supports a command bar that is triggered by a select (or checkbox) column in the table.
...
You first initialize the command helper using the `createDataTableCommandHelper` function from Switchyard UI. Then, you create a custom hook `useCommands` that returns an array of commands created with the command helper.
...
Then, open the Switchyard Admin dashboard and go to the Reviews page. You'll see a new column with checkboxes that allow you to select multiple reviews.
...
![Checkboxes are now shown next to the items in the table, and when you click on them the command bar shows at the bottom of the page with Approve and Reject commands](https://res.cloudinary.com/dza7lstvk/image/upload/v1741937101/Switchyard%20Resources/Screenshot_2025-03-14_at_9.24.29_AM_y9vhac.png)
...
```ts title="src/modules/product-review/service.ts"
import { InjectManager, SwitchyardService, SwitchyardContext } from "@switchyard/framework/utils"
import Review from "./models/review"
...
- Pass as the last parameter a context parameter that has the `SwitchyardContext` decorator.

By doing the above, Switchyard injects the method with a context parameter that has a `manger` property whose value is a [forked entity manager](https://mikro-orm.io/docs/identity-map#forking-entity-manager).
...
Then, you export a `GET` function, and that exposes a `GET` API Route at the path `/store/products/[id]/reviews`. In the route handler you resolve [Query](!docs!/learn/fundamentals/module-links/query) from the Switchyard container, which allows you to retrieve data across modules.
...
Next, you'll add two functions that fetch and submit reviews using the API routes you created earlier. To send requests to the API routes, you can use Switchyard's [JS SDK](../../../js-sdk/page.mdx).
...
import { Button, Input, Label, Textarea, toast, Toaster } from "@switchyard/ui"
...
To test out both the server and storefront customizations, first, start the Switchyard application by running the following command in its directory:
...
![Product page showing the average rating and reviews of the product](https://res.cloudinary.com/dza7lstvk/image/upload/v1741937534/Switchyard%20Resources/Screenshot_2025-03-14_at_9.31.58_AM_tw9vui.png)
...
![Product page showing the Add a review button](https://res.cloudinary.com/dza7lstvk/image/upload/v1741937731/Switchyard%20Resources/Screenshot_2025-03-14_at_9.35.11_AM_w1wzdp.png)

If you click on the button, a form will appear where you can fill in the review's details and submit it.

![Product page showing the Add a review form](https://res.cloudinary.com/dza7lstvk/image/upload/v1741938961/Switchyard%20Resources/Screenshot_2025-03-14_at_9.55.37_AM_epnfz0.png)

After submitting the review, you can approve or reject it from the Switchyard Admin dashboard.

---

## Next Steps

You've now implemented product-review features in Switchyard. There's still more that you can implement to enhance these features:
