...
In this tutorial, you will learn how to implement phone number authentication in your Switchyard application.
...
![Diagram showcasing the phone authentication flow](https://res.cloudinary.com/dza7lstvk/image/upload/v1747744941/Switchyard%20Resources/phone-auth-overview_bt5yrp.jpg)
...
<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/phone-auth",
...
In Switchyard, you integrate custom authentication providers by creating an [Authentication Module Provider](../../../commerce-modules/auth/auth-providers/page.mdx). Then, you can use that provider to authenticate users using custom logic.
...
To install the package, run the following command in the Switchyard application directory:
...
- `options`: Options that are passed to the module provider when it's registered in Switchyard's configurations. You define the following option:
    - `jwtSecret`: A secret used to sign and verify the OTPs.

<Note>

You'll learn how to set this option when you [add the module provider to Switchyard's configurations](#h-add-module-provider-to-medusas-configurations).
...
Switchyard uses the `register` method to create an auth identity that will be associated with the customer when they register. You can learn more in the [Authentication Flows](../../../commerce-modules/auth/auth-flows/page.mdx) documentation.

![Diagram showcasing the relation between a customer and auth identity](https://res.cloudinary.com/dza7lstvk/image/upload/v1747747179/Switchyard%20Resources/customer-auth-identity_je1bvh.jpg)
...
![Diagram showcasing authentication with callback verification](https://res.cloudinary.com/dza7lstvk/image/upload/v1747749683/Switchyard%20Resources/authentication-flow_ey59hw.jpg)
...
When an authentication provider requires a callback to verify the user, the Switchyard application calls the `validateCallback` method.

You can use this method to verify the OTP that the user entered. If valid, you return the logged in user, and the Switchyard application will return a JWT token that the user can use to authenticate in the application.

![Diagram showcasing how callback verification fits in the authentication flow](https://res.cloudinary.com/dza7lstvk/image/upload/v1747749734/Switchyard%20Resources/callback-flow_yney5a.jpg)
...
2. An object with a required property `services` indicating the module provider's services. Each of these services will be registered as authentication providers in Switchyard.

### h. Add Module Provider to Switchyard's Configurations

Once you finish building the module, add it to Switchyard's configurations to start using it.

In `switchyard.config.ts`, add a `modules` property:
...
To pass an Auth Module Provider to the Auth Module, you add the `modules` property to the Switchyard configuration and pass the Auth Module in its value.
...
In `switchyard.config.ts`, add to `projectConfig.http` a new `authMethodsPerActor` property:

export const authMethodsPerActorHighlights = [
  ["8", "customer", "Enable the emailpass and phone-auth providers for customers."],
]

```ts title="switchyard.config.ts" highlights={authMethodsPerActorHighlights}
module.exports = defineConfig({
  projectConfig: {
    // ...
    http: {
...
In this section, you'll test out the Phone Authentication Module Provider using Switchyard's API routes. You can, instead, test it out later using the [Next.js Starter Storefront](#step-4-use-phone-authentication-in-the-nextjs-starter-storefront).
...
1. Open the Switchyard Admin dashboard at `http://localhost:9000/admin` and log in.
...
- `{publishable_api_key}` with the publishable API key you retrieved from the Switchyard Admin dashboard.
...
If you check the logs of the Switchyard application, you'll see that the OTP was generated and logged:
...
Run the following command in the Switchyard application directory:
...
A Notification Module Provider must have an `identifier` static property, which is a unique identifier for the module. This identifier is used to register the module in the Switchyard application.
...
You'll learn how to set these options when you [add the module provider to Switchyard's configurations](#g-add-module-provider-to-medusas-configurations).
...
The only required method for a Notification Module Provider is the `send` method. When the Switchyard application needs to send a notification using the provider's channel (such as SMS), it calls this method of the registered provider.
...
2. An object with a required property `services` indicating the module provider's services. Each of these services will be registered as notification providers in Switchyard.

### g. Add Module Provider to Switchyard's Configurations

You'll now add the Twilio Notification Module Provider to Switchyard's configurations to start using it.

In `switchyard.config.ts`, add the following to the `modules` property:
...
![Twilio console homepage showing the account SID, phone number, and auth token](https://res.cloudinary.com/dza7lstvk/image/upload/v1747736641/Switchyard%20Resources/CleanShot_2025-05-20_at_13.22.55_2x_sztmfo.png)

### i. Handle OTP Generated Event

Now that you have integrated Twilio into Switchyard, you can use it to send the OTP to the customer. To do that, you need to handle the `phone-auth.otp.generated` event that you emitted in the `authenticate` method of the Phone Authentication Module Provider.
...
- `container`: The [Switchyard container](!docs!/learn/fundamentals/medusa-container), which you can use to resolve Framework and commerce resources.

In the subscriber function, you resolve the Notification Module's service from the Switchyard container. Then, you use its `createNotifications` method to send the OTP to the user.
...
So, if your Switchyard application's directory is `medusa-phone-auth`, you can find the storefront by going back to the parent directory and changing to the `medusa-phone-auth-storefront` directory:
...
Before adding the forms, you'll add the functions that send requests to the Switchyard API to authenticate the customer.
...
return (
  ...
  >
    <h1 className="text-large-semi uppercase mb-6">
      Become a Switchyard Store Member
    </h1>
    <p className="text-center text-base-regular text-ui-fg-base mb-4">
      Create your Switchyard Store Member profile, and get access to an enhanced
      shopping experience.
    </p>
    <form className="w-full flex flex-col" onSubmit={handleSubmit}>
      ...
      <span className="text-center text-ui-fg-base text-small-regular mt-6">
        By creating an account, you agree to Switchyard Store&apos;s{" "}
        <LocalizedClientLink
...
First, start the Switchyard application by running the following command in the Switchyard project's directory:
...
![Login form with phone number input](https://res.cloudinary.com/dza7lstvk/image/upload/v1747739945/Switchyard%20Resources/CleanShot_2025-05-20_at_14.18.36_2x_ubuika.png)

You can also switch to the registration form by clicking on the "Join" link below the login form.

![Registration form with phone number input](https://res.cloudinary.com/dza7lstvk/image/upload/v1747739988/Switchyard%20Resources/CleanShot_2025-05-20_at_14.19.35_2x_btlc2s.png)
...
![OTP form](https://res.cloudinary.com/dza7lstvk/image/upload/v1747740102/Switchyard%20Resources/CleanShot_2025-05-20_at_14.21.18_2x_yrkuyg.png)

After you enter the six digits, you'll be logged in and you'll see your profile page.

![Profile page](https://res.cloudinary.com/dza7lstvk/image/upload/v1747740166/Switchyard%20Resources/CleanShot_2025-05-20_at_14.22.30_2x_kshb83.png)
...
![Phone number showing in profile page](https://res.cloudinary.com/dza7lstvk/image/upload/v1747740378/Switchyard%20Resources/CleanShot_2025-05-20_at_14.25.59_2x_x0pxvt.png)
...
![Profile page without email and phone fields](https://res.cloudinary.com/dza7lstvk/image/upload/v1747740515/Switchyard%20Resources/CleanShot_2025-05-20_at_14.28.09_2x_ugab7d.png)

### c. Disable Phone Updates in Switchyard

While removing the email and phone fields from the profile page prevents customers using the storefront from updating their phone number, it doesn't prevent them from updating it using Switchyard's API.
...
To add a middleware in your Switchyard application, create the file `src/api/middlewares.ts` with the following content:
...
You've now implemented phone authentication in Switchyard with Twilio integration. You can further customize the phone authentication feature based on your business use case.
...
To do that, first, enable the `phone-auth` authentication strategy in `switchyard.config.ts` for the actor types. For example:
...
The login form of Switchyard Admin can't be customized, so you'll have to build a custom admin dashboard to support phone authentication for admin users.
