...
export const metadata = {
  title: `Implement Customer Tiers in Switchyard`,
}

# {metadata.title}

In this tutorial, you'll learn how to implement a customer tiers system in Switchyard.
...
- Customize the Switchyard Admin to manage tiers.
...
![Diagram illustrating how the customer tiers system works, starting from the customer adding a product to the cart, Switchyard applying the tier promotion automatically, customer placing the order, and Switchyard updating the customer's tier.](https://res.cloudinary.com/dza7lstvk/image/upload/v1764079847/Switchyard%20Resources/customer-tiers_hx504i.jpg)

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/customer-tiers",
...
In Switchyard, you can build custom features in a [module](!docs!/learn/fundamentals/modules). A module is a reusable package with functionalities related to a single feature or domain. Switchyard integrates the module into your application without affecting your setup.
...
Modules are created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/tier`.

### Create Data Models

A data model represents a table in the database. You create data models using Switchyard's Data Model Language (DML), which simplifies defining a table's columns, relations, and indexes with straightforward methods and configurations.
...
class TierModuleService extends SwitchyardService({
  Tier,
  TierRule,
}) {
}
...
The `TierModuleService` extends `SwitchyardService` from the Modules SDK, which generates a class with data-management methods for your module's data models. This saves you time implementing Create, Read, Update, and Delete (CRUD) methods.
...
Switchyard's CLI tool can generate the migrations for you. To generate a migration for the Tier Module, run the following command in your Switchyard application's directory:

```bash
npx switchyard db:generate tier
```

The `db:generate` command of the Switchyard CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/tier` that holds the generated migration.
...
Switchyard integrates modules into your application without side effects by isolating them from one another. This means you can't directly create relationships between data models in your module and data models in other modules.

Instead, Switchyard provides a mechanism to define links between data models and to retrieve and manage linked records while maintaining module isolation. Links are useful for defining associations between data models in different modules or for extending a model in another module to associate custom properties with it.
...
import { defineLink } from "@switchyard/framework/utils"
import TierModule from "../modules/tier"
import PromotionModule from "@switchyard/medusa/promotion"
...
Later, you'll customize the Switchyard Admin to allow creating tiers from the dashboard.

### a. Create Tier Workflow

To build custom commerce features in Switchyard, you create a [workflow](!docs!/learn/fundamentals/workflows). A workflow is a series of queries and actions, called steps, that complete a task. You can track the workflow's execution progress, define rollback logic, and configure other advanced features.
...
Switchyard provides the last step out of the box. You'll create the other steps before creating the workflow.
...
In the step function, you resolve the Tier Module's service from the Switchyard container and create the tier using the `createTiers` method.
...
export async function POST(
  req: SwitchyardRequest<CreateTierInput>,
...
In the route handler function, you execute the `createTierWorkflow` by invoking it, passing it the Switchyard container, then executing its `run` method.

You return the created tier in the response.

You'll test out this API route later when you customize the Switchyard Admin dashboard.
...
In this step, you'll add an API route that retrieves tiers. You'll use this API route later when you customize the Switchyard Admin to display tiers in the Switchyard Admin.
...
In the route handler, you resolve Query from the Switchyard container and use it to retrieve a list of tiers.
...
## Step 6: Manage Customer Tiers in Switchyard Admin

In this step, you'll customize the Switchyard Admin to display and create tiers.
...
Next, you'll create a UI route that displays the list of tiers in the Switchyard Admin.

A UI route is a React component that specifies the content to be shown in a new page in the Switchyard Admin dashboard.
...
import { UserGroup } from "@switchyard/icons"
...
- Create a DataTable instance from [Switchyard UI](!ui!). You pass the columns, data, and pagination configurations to the hook.
...
import { FocusModal, Heading, Label, Input, Button, Select, IconButton, toast } from "@switchyard/ui"
...
You display a `FocusModal` from Switchyard UI. In the modal, you render a form with the following fields:
...
### Test Customer Tiers in Switchyard Admin

To test out the customer tiers in the Switchyard Admin, start the Switchyard application:

```bash npm2yarn
npm run dev
```

Then, open the Switchyard Admin dashboard at `http://localhost:9000/app` and log in using the credentials you set up earlier.

You'll find a new sidebar item labeled "Customer Tiers." Click on it to view the list of tiers.

![Customer Tiers page showing list of tiers](https://res.cloudinary.com/dza7lstvk/image/upload/v1764065196/Switchyard%20Resources/CleanShot_2025-11-25_at_12.05.51_2x_u2khaj.png)
...
![Create tier modal showing form to create a tier](https://res.cloudinary.com/dza7lstvk/image/upload/v1764065361/Switchyard%20Resources/CleanShot_2025-11-25_at_12.08.18_2x_nid87p.png)
...
In the route handler, you resolve Query from the Switchyard container and use it to retrieve the tier with the given ID.
...
Switchyard provides the `useQueryGraphStep` out-of-the-box, and you've implemented the `createTierRulesStep`. You'll create the other steps before creating the workflow.
...
export async function POST(
  req: SwitchyardRequest<UpdateTierInput>,
  res: SwitchyardResponse
): Promise<void> {
  const { id } = req.params
  const { name, promo_id, tier_rules } = req.validatedBody
...
You'll test out the API route later when you customize the Switchyard Admin dashboard.
...
In this step, you'll add an API route that retrieves customers in a tier. This will be useful to show the customers in the tier's page on the Switchyard Admin.
...
In the route handler, you resolve Query from the Switchyard container and use it to retrieve customers linked to the tier with the given ID.
...
To install the Index Module, run the following command in your Switchyard application's directory:

```bash npm2yarn
npm install @switchyard/index
```

Then, add the following to your Switchyard application's configuration file:
...
Lastly, start the Switchyard application to ingest the data into the Index Module:

```bash npm2yarn
npm run dev
```

You can now use the Index Module to filter customers by their tier. You'll test out the API route when you customize the Switchyard Admin dashboard in the next step.
...
import { Drawer, Heading, Label, Input, Button, Select, IconButton, toast } from "@switchyard/ui"
import { useForm, Controller, FormProvider } from "react-hook-form"
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query"
import { sdk } from "../lib/sdk"
...
You display a `Drawer` from Switchyard UI. In the drawer, you render a form with the following fields:
...
import { Code, Container, Heading, Text } from "@switchyard/ui"
...
import { Heading, DataTable, createDataTableColumnHelper, useDataTable, Container } from "@switchyard/ui"
import { Tier } from "../routes/tiers/page"
...
import { Heading, DataTable, createDataTableColumnHelper, useDataTable, Container, DataTablePaginationState } from "@switchyard/ui"
import { sdk } from "../lib/sdk"
import { useQuery } from "@tanstack/react-query"
...
![Tier details page showing tier details, rules, and customers](https://res.cloudinary.com/dza7lstvk/image/upload/v1764067752/Switchyard%20Resources/CleanShot_2025-11-25_at_12.48.45_2x_ahjflr.png)
...
![Edit tier drawer showing form to edit tier details](https://res.cloudinary.com/dza7lstvk/image/upload/v1764068307/Switchyard%20Resources/CleanShot_2025-11-25_at_12.58.17_2x_me8gbo.png)
...
You only need to create the `validateCustomerStep` and `determineTierStep` steps. Switchyard provides the other steps out of the box.
...
In the step, you resolve the Tier Module's service from the Switchyard container and call the `calculateQualifyingTier` method to determine the qualifying tier.
...
- `container`: The Switchyard container. Use it to resolve modules' main services and other registered resources.

In the subscriber function, you resolve the logger from the Switchyard container and execute the workflow. If an error occurs, you log it.
...
So, if your Switchyard application's directory is `medusa-customer-tiers`, you can find the storefront by going back to the parent directory and changing to the `medusa-customer-tiers-storefront` directory:
...
First, start the Switchyard application by running the following command in the Switchyard application's directory:
...
4. After you place the order, open the Switchyard Admin dashboard at `http://localhost:9000/app` and log in.
...
![Customer in tier's customers list section](https://res.cloudinary.com/dza7lstvk/image/upload/v1764070515/Switchyard%20Resources/CleanShot_2025-11-25_at_13.34.32_2x_wro2yk.png)
...
You only need to create the `validateTierPromotionStep`. Switchyard provides the other steps and workflows out-of-the-box.
...
To test the automated tier promotion, make sure both the Switchyard application and the Next.js Starter Storefront are running.
...
![Cart showing the promotion of the tier applied](https://res.cloudinary.com/dza7lstvk/image/upload/v1764072761/Switchyard%20Resources/CleanShot_2025-11-25_at_13.47.49_2x_xmewsm.png)
...
import { updateCartPromotionsWorkflow } from "@switchyard/medusa/core-flows"
import { SwitchyardError } from "@switchyard/framework/utils"
import { PromotionActions } from "@switchyard/framework/utils"
...
To test the promotion addition validation, make sure both the Switchyard application and the Next.js Starter Storefront are running.

Then, in the Next.js Starter Storefront, log in as a customer in a tier, and add a product to the cart.

Try to add a promotion that belongs to a different tier. You should see an error message.

![Error message when trying to add a promotion of a different tier to the cart](https://res.cloudinary.com/dza7lstvk/image/upload/v1764074555/Switchyard%20Resources/CleanShot_2025-11-25_at_14.42.18_2x_uxkfgk.png)
...
import { completeCartWorkflow } from "@switchyard/medusa/core-flows"
import { SwitchyardError } from "@switchyard/framework/utils"
...
export async function GET(
  req: AuthenticatedSwitchyardRequest<{}, NextTierInput>,
  res: SwitchyardResponse
): Promise<void> {
...
- Resolve Query and the Tier Module service from the Switchyard container.
...
First, you'll define types for the tier information received from the Switchyard server.
...
The component receives the tier data retrieved from the Switchyard server as a prop. It then calculates and displays a progress bar indicating how much the customer needs to spend to unlock the next tier.
...
To test the customer tier component on the account page, make sure both the Switchyard server and the Next.js Starter Storefront are running.
...
![Customer tier component on account page](https://res.cloudinary.com/dza7lstvk/image/upload/v1764078181/Switchyard%20Resources/CleanShot_2025-11-25_at_15.42.44_2x_hhguzm.png)
...
To test the customer tier component on the order confirmation page, make sure both the Switchyard server and the Next.js Starter Storefront are running.
...
![Customer tier component on order confirmation page](https://res.cloudinary.com/dza7lstvk/image/upload/v1764078663/Switchyard%20Resources/CleanShot_2025-11-25_at_15.50.50_2x_liqi8r.png)

---

## Next Steps

You've now implemented customer tiers in Switchyard. You can expand on this feature to add more features like:

- Automated emails to customers when they reach a new tier. Cloud users can benefit from zero-config email setup with [Switchyard Emails](!cloud!/emails).
...
If you're new to Switchyard, check out the [main documentation](!docs!/learn) for a more in-depth understanding of the concepts you've used in this guide and more.

To learn more about the commerce features Switchyard provides, check out [Commerce Modules](../../../commerce-modules/page.mdx).
