...
export const metadata = {
  title: `Implement First-Purchase Discount in Switchyard`,
}

# {metadata.title}

In this tutorial, you'll learn how to implement first-purchase discounts in Switchyard.
...
The first-purchase discount feature encourages customers to sign up and make their first purchase by offering them a discount. In this tutorial, you'll learn how to implement this feature in Switchyard.
...
![Diagram showcasing the flow of first-time purchase discounts](https://res.cloudinary.com/dza7lstvk/image/upload/v1750846212/Switchyard%20Resources/first-purchase-promo-overview_jbiwa9.jpg)

<Card
  title="View on Github"
  icon={Github}
  href="https://github.com/switchyard/examples/tree/main/first-purchase-discount"
...
Afterward, the installation process will start, which will install the Switchyard application in a directory with your project's name and the Next.js Starter Storefront in a separate directory named `{project-name}-storefront`.
...
Start your Switchyard application with the following command:

```bash npm2yarn
npm run dev
```

Then, open the Switchyard Admin dashboard at `http://localhost:9000/app` and log in with the user you created in the previous step.
...
Make sure to set the promotion's code to `FIRST_PURCHASE`, as you'll be using this code in your Switchyard customization. If you want to use a different code, make sure to update the code in the next steps accordingly.

<Note title="Need help?">

Refer to the [Create Promotions User Guide](!user-guide!/promotions/create) to learn how to create promotions in Switchyard.

</Note>

Once you create and publish the promotion, you can proceed to the next steps.

![First-purchase promotion in Switchyard Admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1750846696/Switchyard%20Resources/CleanShot_2025-06-25_at_13.18.00_2x_j46emw.png)

---

## Step 3: Apply the First-Purchase Discount to Cart

In this step, you'll customize the Switchyard application to automatically apply the first-purchase promotion to a cart.
...
Since you'll refer to the first-purchase promotion code in multiple places, it's a good idea to store it as a constant in your Switchyard application.
...
Switchyard provides all these steps in its `@switchyard/medusa/core-flows` package, so you can implement the workflow right away.
...
import { updateCartPromotionsStep, useQueryGraphStep } from "@switchyard/medusa/core-flows"
...
The subscriber function receives an object as a parameter that has a `container` property, which is the [Switchyard container](!docs!/learn/fundamentals/medusa-container). The Switchyard container holds Framework and commerce tools that you can resolve and use in your customizations.

In the subscriber function, you execute the `applyFirstPurchasePromoWorkflow` by invoking it, passing it the Switchyard container, then calling its `run` method. You pass the `cart_id` from the event payload as an input to the workflow.
...
So, if your Switchyard application's directory is `medusa-first-promo`, you can find the storefront by going back to the parent directory and changing to the `medusa-first-promo-storefront` directory:
...
![Cart page with first-purchase promotion applied](https://res.cloudinary.com/dza7lstvk/image/upload/v1750842319/Switchyard%20Resources/CleanShot_2025-06-25_at_12.02.17_2x_bbu8vt.png)
...
In this step, you'll customize Switchyard's existing workflows to validate the first-purchase promotion usage. You can do that by consuming the [workflows' hooks](!docs!/learn/fundamentals/workflows/workflow-hooks). A workflow hook is a point in a workflow where you can inject custom functionality as a step function.
...
- The hook or step context object. Most notably, it has a `container` property, which is the Switchyard container.
...
If any of these validations fail, you throw a `SwitchyardError` with the appropriate error message. This will prevent the promotion from being applied to the cart.

<Note title="Tip">

To retrieve the customer's details, you use [Query](!docs!/learn/fundamentals/module-links/query). Query allows you to retrieve data across modules in your Switchyard application.
...
If any of these validations fail, you throw a `SwitchyardError` with the appropriate error message. This will prevent the order from being placed if the first-purchase promotion is used by a customer who is not a first-time customer.

### Test it Out

To test the custom validation, start the Switchyard application and the Next.js Starter Storefront as you did in the previous steps.
...
import { Button, Heading, Text } from "@switchyard/ui"
...
To test the pop-up, start the Switchyard application and the Next.js Starter Storefront as you did in the previous steps.
...
![Discount pop-up in the Next.js Starter Storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1750844087/Switchyard%20Resources/CleanShot_2025-06-25_at_12.34.35_2x_f1f5jh.png)

---

## Next Steps

You've now implemented the first-purchase discount feature in Switchyard. You can add more features to build customer loyalty, such as a [loyalty points system](../loyalty-points/page.mdx) or [product reviews](../product-reviews/page.mdx).
