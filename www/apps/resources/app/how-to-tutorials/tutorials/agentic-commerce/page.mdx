...
In this tutorial, you'll learn how to implement [Agentic Commerce](https://developers.openai.com/commerce) specifications in Switchyard that allow you to sell through ChatGPT.
...
The [Agentic Commerce Protocol](https://developers.openai.com/commerce) supports instant checkout experiences within AI agents. By implementing Agentic Commerce specifications in your Switchyard application, customers can purchase products through ChatGPT and other AI agents.
...
![Diagram showing the Agentic Commerce integration between user, ChatGPT, and Switchyard application](https://res.cloudinary.com/dza7lstvk/image/upload/v1759333742/Switchyard%20Resources/agentic-commerce_jqr5wn.jpg)

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/agentic-commerce",
    title: "Full Code",
...
Begin by installing the Switchyard application on your machine with the following command:
...
After that, the installation process will begin. This will install the Switchyard application in a directory with your project's name and the Next.js Starter Storefront in a separate directory named `{project-name}-storefront`.
...
Once the installation finishes successfully, the Switchyard Admin dashboard will open with a form to create a new user. Enter the user's credentials and submit the form. Then, you can log in with the new user and explore the dashboard.
...
A module is created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/agentic-commerce`.
...
- The module's options that you'll later pass when registering the module in the Switchyard application. You can add more options based on your integration.
...
The final piece of a module is its definition, which you export in an `index.ts` file at the root directory. This definition tells Switchyard the module name and its service.
...
### d. Add Module to Switchyard's Configurations
...
import { getVariantAvailability, QueryContext } from "@switchyard/framework/utils"
import { CalculatedPriceSet, ShippingOptionDTO } from "@switchyard/framework/types"
    ...
    for (const product of products) {
      ...
      for (const variant of product.variants) {
        ...
        feedItems.push({
          ...
          size: size || undefined,
          seller_name: "Switchyard", // TODO add seller name if supported
    ...
    const itemsXml = input.items.map((item) => {
      return (
        `<item>` +
          ...
          `<brand>${escape(item.brand || "Switchyard")}</brand>` +
...
If you get a type error on resolving the Agentic Commerce Module, run the Switchyard application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](!docs!/learn/fundamentals/generated-types).
...
In the step, you resolve the Agentic Commerce Module's service from the Switchyard container and call its `sendProductFeed` method to send the product feed to the AI agent.
...
Next, you'll create a [scheduled job](!docs!/learn/fundamentals/scheduled-jobs) that executes the `sendProductFeedWorkflow` every fifteen minutes. A scheduled job is an asynchronous function that the Switchyard application runs at the interval you specify during the Switchyard application's runtime.
...
export default async function syncProductFeed(container: SwitchyardContainer) {
...
1. An asynchronous function that holds the job's logic. The function receives the [Switchyard container](!docs!/learn/fundamentals/medusa-container) as a parameter.
...
In the scheduled job function, you use Query to retrieve regions in your Switchyard application, including their countries and currency codes.
...
To use the scheduled job, start the Switchyard application with the following command:
...
You'll implement the `POST /checkout_sessions` API route for creating checkout sessions. AI agents call this endpoint when customers want to purchase products. This is equivalent to creating a new cart in Switchyard.
...
These steps and workflows are available in Switchyard out-of-the-box. So, you can implement the workflow without creating custom steps.
...
- `cart_id`: Cart ID in Switchyard, which is also the checkout session ID.
...
export const POST = async (
  ...
  try {
    ...
    const medusaError = error as SwitchyardError
...
import { SwitchyardNextFunction, SwitchyardRequest, SwitchyardResponse } from "@switchyard/framework"
import { AGENTIC_COMMERCE_MODULE } from "../../modules/agentic-commerce"
...
The headers are expected based on [Agentic Commerce specifications](https://developers.openai.com/commerce/specs/checkout#rest-endpoints). You can create an API key that AI agents can use in the [Secret API Key Settings](!user-guide!/settings/developer/secret-api-keys) of the Switchyard Admin dashboard.
...
- Create an API key in the [Secret API Key Settings](!user-guide!/settings/developer/secret-api-keys) of the Switchyard Admin dashboard.
...
Then, start the Switchyard application with the following command:
...
Make sure you [have a region with the US added to its countries](!user-guide!/settings/regions#create-region) in your Switchyard store.
...
- `{your_api_key}` with the API key you created in the Switchyard Admin dashboard.
...
import { refreshPaymentCollectionForCartWorkflow } from "@switchyard/medusa/core-flows"
      ...
      .run({
        input: {
          ...
          messages: [
            {
              type: "error",
              code: medusaError.type === SwitchyardError.Types.PAYMENT_AUTHORIZATION_ERROR ? 
...
- `{api_key}` with the API key you created in the Switchyard Admin dashboard.
...
export const GET = async (
  ...
  try {
    ...
    const medusaError = error as SwitchyardError
    const statusCode = medusaError.type === SwitchyardError.Types.NOT_FOUND ? 404 : 500
...
You use `transform` to prepare the input for the `prepareCheckoutSessionDataWorkflow` workflow and to map the payment provider from the input to the payment provider ID used in your Switchyard store.
...
To support payments with Stripe, you'll need to set up the [Stripe Payment Module Provider](../../../commerce-modules/payment/payment-provider/stripe/page.mdx) in your Switchyard store.

In `switchyard.config.ts`, add a new entry to the `modules` array:
...
This will add Stripe as a payment provider in your Switchyard store.
...
You only need to implement the `validateCartCancelationStep` and `cancelPaymentSessionsStep` steps. The other steps and workflows are available in Switchyard out-of-the-box.
...
import { CartDTO, OrderDTO, PaymentCollectionDTO } from "@switchyard/framework/types"
import { SwitchyardError } from "@switchyard/framework/utils"
...
import { acquireLockStep, releaseLockStep, updateCartWorkflow, useQueryGraphStep } from "@switchyard/medusa/core-flows"
...
The subscriber function receives an object as a parameter that has a `container` property, which is the [Switchyard container](!docs!/learn/fundamentals/medusa-container).
...
4. Check the logs of your Switchyard server to see that `order.placed` events were emitted and webhook events were sent to AI agents.

---

## Next Steps

You've now built Agentic Commerce integration in your Switchyard store. You can use it once you apply to ChatGPT's [Instant Checkout](https://chatgpt.com/merchants) and set up the integration in Instant Checkout settings.

If you're new to Switchyard, check out the [main documentation](!docs!/learn), where you'll get more in-depth understanding of all concepts used in this guide and more.

To learn more about commerce features that Switchyard provides, check out Switchyard's [Commerce Modules](../../../commerce-modules/page.mdx).
