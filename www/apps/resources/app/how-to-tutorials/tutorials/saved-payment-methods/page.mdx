...
Switchyard's architecture facilitates integrating third-party services, such as payment providers. These payment providers can process payments and securely store customers' payment methods for future use.

In this tutorial, you'll expand on Switchyard's [Stripe Module Provider](../../../commerce-modules/payment/payment-provider/stripe/page.mdx) to allow customers to re-use their saved payment methods during checkout.
...
- Install and set up Switchyard and the Next.js Starter Storefront.
- Set up the Stripe Module Provider in Switchyard.
...
![Diagram illustrating the features of this guide](https://res.cloudinary.com/dza7lstvk/image/upload/v1745309355/Switchyard%20Resources/saved-payment-methods_orjnix.jpg)

<Card
  href="https://github.com/switchyard/examples/tree/main/stripe-saved-payment"
...
Switchyard's [Payment Module](../../../commerce-modules/payment/page.mdx) provides payment-related models and the interface to manage and process payments. However, it delegates the actual payment processing to module providers that integrate third-party payment services.

The [Stripe Module Provider](../../../commerce-modules/payment/payment-provider/stripe/page.mdx) is a Payment Module Provider that integrates Stripe into your Switchyard application to process payments. It can also save payment methods securely.

In this section, you'll set up the Stripe Module Provider in your Switchyard application.
...
To register the Stripe Module Provider in your Switchyard application, add it to the array of providers passed to the Payment Module in `switchyard.config.ts`:
...
The Switchyard configuration accepts a `modules` array, which contains the modules to be loaded. While the Payment Module is loaded by default, you need to add it again when registering a new provider.
...
- `id`: The ID of the provider. This is used to identify the provider in the Switchyard application.
...
![Secret API Key in the Stripe dashboard](https://res.cloudinary.com/dza7lstvk/image/upload/v1745313823/Switchyard%20Resources/Screenshot_2025-04-22_at_12.20.08_PM_w6rxxo.png)

### Enable Stripe in a Region

In Switchyard, each [region](../../../commerce-modules/region/page.mdx) (which is a geographical area where your store operates) can have different payment methods enabled. So, after registering the Stripe Module Provider, you need to enable it in a region.

To enable it in a region, start the Switchyard application with the following command:
...
![Stripe in the dropdown](https://res.cloudinary.com/dza7lstvk/image/upload/v1745245433/Switchyard%20Resources/Screenshot_2025-04-21_at_5.23.07_PM_yd7xji.png)
...
So, if your Switchyard application's directory is `medusa-payment`, you can find the storefront by going back to the parent directory and changing to the `medusa-payment-storefront` directory:
...
The Payment Module uses [account holders](../../../commerce-modules/payment/account-holder/page.mdx) to represent a customer's details that are stored in a third-party payment provider. Switchyard creates an account holder for each customer, allowing you later to retrieve the customer's saved payment methods in the third-party provider.

![Diagram illustrating the relation between customers and account holders in Switchyard, and customers in Stripe](https://res.cloudinary.com/dza7lstvk/image/upload/v1745314344/Switchyard%20Resources/customer-account-stripe_in5ei6.jpg)
...
export const apiRouteHighlights = [
  ["9", "query", "Resolve Query from the Switchyard container"],
  ["10", "paymentModuleService", "Resolve the Payment Module's service from the Switchyard container."],
...
The request object has a `scope` property, which is an instance of the [Switchyard container](!docs!/learn/fundamentals/medusa-container). The Switchyard container is a registry of Framework and commerce tools that you can access in the API route.

You use the Switchyard container to resolve:

- [Query](!docs!/learn/fundamentals/module-links/query), which is a tool that retrieves data across modules in the Switchyard application.
...
import { authenticate, defineMiddlewares } from "@switchyard/framework/http"
...
![Saved payment method on the Stripe dashboard](https://res.cloudinary.com/dza7lstvk/image/upload/v1745249064/Switchyard%20Resources/Screenshot_2025-04-21_at_6.24.00_PM_peotrd.png)
...
export const paymentHighlights = [
  ...
  ["3", "provider_id", "The ID of the provider in the Switchyard application,\nsuch as Stripe's ID."],
...
- `provider_id`: The ID of the provider in the Switchyard application, such as Stripe's ID.
...
![Saved payment method in checkout](https://res.cloudinary.com/dza7lstvk/image/upload/v1745308047/Switchyard%20Resources/Screenshot_2025-04-21_at_4.07.34_PM_krudjq.png)

Once the order is placed successfully, you can check it in the Switchyard Admin dashboard. You can view the order and capture the payment.

![Order in the Switchyard Admin dashboard](https://res.cloudinary.com/dza7lstvk/image/upload/v1745318042/Switchyard%20Resources/Screenshot_2025-04-22_at_1.33.39_PM_uynlfp.png)

---

## Next Steps

You've added support for saved payment methods in your Switchyard application and Next.js Starter Storefront, allowing customers to save their payment methods during checkout and use them in future orders.

You can add more features to the saved payment methods, such as allowing customers to delete saved payment methods. You can use [Stripe's APIs](https://docs.stripe.com/api/payment_methods/detach) in the storefront or add an [API route](!docs!/learn/fundamentals/api-routes) in Switchyard to delete the saved payment method.
