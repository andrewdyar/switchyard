...
export const metadata = {
  title: `Implement Loyalty Points System in Switchyard`,
}

# {metadata.title}

In this tutorial, you'll learn how to implement a loyalty points system in Switchyard.
...
A loyalty point system allows customers to earn points for purchases, which can be redeemed for discounts or rewards. In this tutorial, you'll learn how to customize the Switchyard application to implement a loyalty points system.
...
![Diagram illustrating redeem loyalty points flow](https://res.cloudinary.com/dza7lstvk/image/upload/v1744126213/Switchyard%20Resources/redeem-points-flow_kzgkux.jpg)

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/loyalty-points",
...
Modules are created under the `src/modules` directory of your Switchyard application. So, create the directory `src/modules/loyalty`.
...
import { SwitchyardError, SwitchyardService } from "@switchyard/framework/utils"
...
class LoyaltyModuleService extends SwitchyardService({
  LoyaltyPoint,
}) {
  // TODO add methods
}
...
The `LoyaltyModuleService` extends `SwitchyardService` from the Modules SDK which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.
...
In the method, you retrieve the customer's existing loyalty points using the `listLoyaltyPoints` method, which is automatically generated by the `SwitchyardService`. If the customer has existing points, you update them with the new points using the `updateLoyaltyPoints` method.
...
Otherwise, you update the customer's loyalty points with the new value using the `updateLoyaltyPoints` method, which is automatically generated by `SwitchyardService`.
...
Switchyard's CLI tool can generate the migrations for you. To generate a migration for the Loyalty Module, run the following command in your Switchyard application's directory:

```bash
npx switchyard db:generate loyalty
```

The `db:generate` command of the Switchyard CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/loyalty` that holds the generated migration.
...
Switchyard provides the `useQueryGraphStep` and `updatePromotionsStep` in its `@switchyard/medusa/core-flows` package. So, you'll only implement the other steps.
...
import { CustomerDTO } from "@switchyard/framework/types"
...
import { PromotionDTO, CustomerDTO, CartDTO } from "@switchyard/framework/types"
...
If you get a type error on resolving the Loyalty Module, run the Switchyard application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](!docs!/learn/fundamentals/generated-types).
...
In the step, you resolve the Loyalty Module's service from the Switchyard container. Then, you use the `calculatePointsFromAmount` method to calculate the points to deduct from the promotion's amount.
...
In the compensation function, you resolve the Loyalty Module's service from the Switchyard container. Then, you call the `addPoints` method to restore the points deducted from the customer's loyalty points if an error occurs.
...
In the step, you resolve the Loyalty Module's service from the Switchyard container. Then, you use the `calculatePointsFromAmount` method to calculate the points to add from the order's amount.
...
import { updatePromotionsStep, useQueryGraphStep } from "@switchyard/medusa/core-flows"
...
Switchyard has an event system that allows you to listen to events emitted by the Switchyard server using a [subscriber](!docs!//learn/fundamentals/events-and-subscribers). A subscriber is an asynchronous function that's executed when its associated event is emitted. In a subscriber, you can execute a workflow that performs actions in result of the event.
...
- `container`: The Switchyard container, which you can use to resolve services and tools.

In the subscriber function, you execute the `handleOrderPointsWorkflow` by invoking it, passing it the Switchyard container, then using its `run` method, passing it the workflow's input.
...
To test out the loyalty points flow, you'll use the [Next.js Starter Storefront](../../../nextjs-starter/page.mdx) that you installed in the first step. As mentioned in that step, the storefront will be installed in a separate directory from the Switchyard application, and its name is `{project-name}-storefront`, where `{project-name}` is the name of your Switchyard application's directory.

So, run the following command in the Switchyard application's directory to start the Switchyard server:
...
The Next.js Starter Storefront will be running on `http://localhost:8000`, and the Switchyard server will be running on `http://localhost:9000`.
...
After you place the order, you'll see the following message in your Switchyard application's terminal:
...
In the route handler, you resolve the Loyalty Module's service from the Switchyard container (which is available at `req.scope`).
...
To test out the customizations to the checkout flow, make sure both the Switchyard application and Next.js Starter Storefront are running.
...
![Loyalty Points Section at the end of the summary section at the right](https://res.cloudinary.com/dza7lstvk/image/upload/v1744195223/Switchyard%20Resources/Screenshot_2025-04-09_at_1.39.34_PM_l5oltc.png)
...
Most of the workflow's steps are either provided by Switchyard in the `@switchyard/medusa/core-flows` package or steps you've already implemented. You only need to implement the `getCartLoyaltyPromoAmountStep` step.
...
import { PromotionDTO, CustomerDTO } from "@switchyard/framework/types"
import { SwitchyardError } from "@switchyard/framework/utils"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
import LoyaltyModuleService from "../../modules/loyalty/service"
import { LOYALTY_MODULE } from "../../modules/loyalty"
...
In the step, you resolve the Loyalty Module's service from the Switchyard container. Then, you call the `getPoints` method to retrieve the customer's loyalty points. If the customer has no loyalty points, you throw an error.
...
To test out applying the loyalty points on the cart, start the Switchyard application and Next.js Starter Storefront.
...
![Discounted amount is shown as part of the summary and the promotion is shown as part of the applied promotions](https://res.cloudinary.com/dza7lstvk/image/upload/v1744200895/Switchyard%20Resources/Screenshot_2025-04-09_at_3.14.19_PM_abmtjh.png)
...
To test out removing the loyalty points from the cart, start the Switchyard application and Next.js Starter Storefront.
...
![The "Remove Loyalty Points" button is shown in the "Loyalty Points" section](https://res.cloudinary.com/dza7lstvk/image/upload/v1744204436/Switchyard%20Resources/Screenshot_2025-04-09_at_4.13.24_PM_xt5trh.png)
...
So, in this step, you'll hook into Switchyard's cart completion flow to perform the validation.

Since Switchyard uses workflows in its API routes, it allows you to hook into them and perform custom functionalities using [Workflow Hooks](!docs!/learn/fundamentals/workflows/workflow-hooks). A workflow hook is a point in a workflow where you can inject custom functionality as a step function, called a hook handler.

Switchyard uses the [completeCartWorkflow](/references/medusa-workflows/completeCartWorkflow) hook to complete the cart and place an order. This workflow has a `validate` hook that allows you to perform custom validation before the cart is completed.
...
export const completeCartWorkflowHookHighlights = [
  ...
  ["49", "SwitchyardError", "Throw an error if the customer doesn't have enough loyalty points."],
]
...
- The step context object, which contains the Switchyard container. You can use it to resolve services and perform actions.
...
You've now implement a loyalty points system in Switchyard. There's still more that you can implement based on your use case:
...
- Customize the Switchyard Admin to show a new page or [UI Route](!docs!/learn/fundamentals/admin/ui-routes) for loyalty points information and analytics.
