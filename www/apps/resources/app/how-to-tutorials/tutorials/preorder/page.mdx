...
export const metadata = {
  title: `Implement Pre-Orders in Switchyard`,
}

# {metadata.title}

In this tutorial, you'll learn how to implement pre-orders in Switchyard.
...
This tutorial will help you implement pre-orders in your Switchyard application.
...
- Customize the Switchyard Admin dashboard to allow merchants to:
...
![Diagram showcasing the pre-order features from this tutorial](https://res.cloudinary.com/dza7lstvk/image/upload/v1752756906/Switchyard%20Resources/pre-order-summary_epvssb.jpg)

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/preorders",
...
- `variant_id`: The ID of the product variant that this pre-order variant configurations applies to.
    - Later, you'll learn how to link this data model to Switchyard's `ProductVariant` data model.
...
- `id`: The primary key of the table.
- `order_id`: The ID of the Switchyard order that this pre-order belongs to.
    - Later, you'll learn how to link this data model to Switchyard's `Order` data model.
...
export default class PreorderModuleService extends SwitchyardService({
  PreorderVariant,
  Preorder,
}) {}
```

The `PreorderModuleService` extends `SwitchyardService`, which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.
...
Switchyard's CLI tool can generate the migrations for you. To generate a migration for the Preorder Module, run the following command in your Switchyard application's directory:

```bash
npx switchyard db:generate preorder
```

The `db:generate` command of the Switchyard CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/preorder` that holds the generated migration.
...
## Step 3: Link Preorder and Switchyard Data Models
...
In this step, you'll define a link between the data models in the Preorder Module and the data models in Switchyard's Commerce Modules.
...
Next, you'll define a read-only link between the `Preorder` data model and Switchyard's `Order` data model. Read-only links allow you to retrieve the order that a pre-order belongs to without the need to manage the link in the database.
...
When you build commerce features in Switchyard that can be consumed by client applications, such as the Switchyard Admin dashboard or a storefront, you need to implement:
...
The `useQueryGraphStep` and `createRemoteLinkStep` are available through Switchyard's `@switchyard/medusa/core-flows` package. You'll implement other steps in the workflow.
...
If you get a type error on resolving the Preorder Module, run the Switchyard application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](!docs!/learn/fundamentals/generated-types).

</Note>

export const updatePreorderVariantStepHighlights = [
  ["16", "preorderModuleService", "Resolve the Preorder Module's service from the Switchyard container."],
...
In the step function, you resolve the Preorder Module's service from the Switchyard container using its `resolve` method, passing it the module's name as a parameter.
...
import { createRemoteLinkStep, useQueryGraphStep } from "@switchyard/medusa/core-flows"
...
export const POST = async (
  req: AuthenticatedSwitchyardRequest<UpsertPreorderVariantSchema>,
...
You apply Switchyard's `validateAndTransformBody` middleware to `POST` requests sent to the `/admin/variants/:id/preorders` API route.
...
You'll test out this functionality when you customize the Switchyard Admin in the next step.

---

## Step 5: Add Widget in Product Variant Admin Page

In this step, you'll customize the Switchyard Admin to allow admin users to manage pre-order configurations of product variants.
...
To send requests to the Switchyard server with support for caching and refetching capabilities, you'll wrap JS SDK methods with [Tanstack (React) Query](https://tanstack.com/query/latest).
...
- `preorderVariant`: The pre-order variant. It's retrieved from Switchyard's [Retrieve Variant](!api!/admin#products_getproductsidvariantsvariant_id) API route, which allows expanding linked records using the `fields` query parameter.
...
import { Calendar, EllipsisHorizontal, Pencil, Plus, XCircle } from "@switchyard/icons"
...
To show the pre-order configuration form, you use the [Drawer](!ui!/components/drawer) component from Switchyard UI.

### Test the Customizations

You can now test the customizations you made in the Switchyard server and admin dashboard.

Start the Switchyard application with the following command:

```bash npm2yarn
npm run dev
```

Then, open the Switchyard Admin dashboard at `http://localhost:9000/app` and log in.

Go to any product, then click on one of its variants. You'll find a new "Pre-order" section in the side column.

![Pre-order variant widget in the Switchyard Admin dashboard](https://res.cloudinary.com/dza7lstvk/image/upload/v1752735553/Switchyard%20Resources/CleanShot_2025-07-17_at_09.52.30_2x_ftojti.png)
...
![Pre-order variant widget showing the pre-order configuration](https://res.cloudinary.com/dza7lstvk/image/upload/v1752735733/Switchyard%20Resources/CleanShot_2025-07-17_at_10.01.03_2x_bdvzo9.png)
...
In this step, you'll wrap custom logic around Switchyard's cart completion logic in a workflow, then execute that workflow in a custom API route.
...
import { CartLineItemDTO, ProductVariantDTO } from "@switchyard/framework/types"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
...
The step function receives the ID of the Switchyard order and the IDs of the pre-order variants in the cart.

In the step function, you create a `Preorder` record for each pre-order variant. You set the `order_id` of the `Preorder` record to the ID of the Switchyard order.
...
export const completeCartPreorderWorkflowHighlights = [
  ...
  ["94", "order", "Return the created Switchyard order."],
]
...
2. Complete the cart using the [completeCartWorkflow](/references/medusa-workflows/completeCartWorkflow) as a step. This is Switchyard's cart completion logic.
...
9. Return the created Switchyard order.
...
In those cases, you can perform custom logic within Switchyard's add-to-cart logic to validate an item before it's added to the cart.
...
export const validateCartHookHighlights = [
  ...
  ["55", "throw SwitchyardError", "Throw an error if the cart has mixed pre-order and available items."],
]
...
So, if your Switchyard application's directory is `medusa-preorder`, you can find the storefront by going back to the parent directory and changing to the `medusa-preorder-storefront` directory:
...
import { StorePrice } from "@switchyard/types"
```
...
First, you'll retrieve the pre-order variant information along with the product variant from the Switchyard server.
...
To test out the changes to the product page, start the Switchyard application with the following command:
...
![Pre-order product variant in the Next.js Starter Storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1752739520/Switchyard%20Resources/CleanShot_2025-07-17_at_11.04.56_2x_fefjo6.png)
...
To test the changes to the cart page, ensure that both the Switchyard and Next.js applications are running.
...
![Pre-order item in the cart page of the Next.js Starter Storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1752740244/Switchyard%20Resources/CleanShot_2025-07-17_at_11.17.04_2x_u4opzf.png)
...
To test the cart completion with pre-order items, ensure that both the Switchyard and Next.js applications are running.
...
To test the changes to the order confirmation page, ensure that both the Switchyard and Next.js applications are running.
...
![Pre-order item in the order confirmation page of the Next.js Starter Storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1752741220/Switchyard%20Resources/CleanShot_2025-07-17_at_11.33.17_2x_anitlq.png)
...
In this step, you'll customize the Switchyard Admin to show pre-order information in the order details page.
...
import { defineWidgetConfig } from "@switchyard/admin-sdk"
import { DetailWidgetProps, HttpTypes } from "@switchyard/framework/types"
...
The `PreordersWidget` will be injected into the side column of the order details page in the Switchyard Admin dashboard.
...
To test the pre-order widget, ensure that the Switchyard application is running.

Then, in the Switchyard Admin dashboard, go to any order that has pre-order items. You should see the "Pre-orders" section in the side column with the pre-order information.

![Pre-orders Section in the Switchyard Admin dashboard](https://res.cloudinary.com/dza7lstvk/image/upload/v1752741736/Switchyard%20Resources/CleanShot_2025-07-17_at_11.41.55_2x_w8aiw2.png)
    ...
    steps: [
      {
        type: "step",
        name: "useQueryGraphStep",
        description: "Retrieve the Switchyard order details.",
        link: "/references/helper-steps/useQueryGraphStep",
        depth: 1
      },
      {
      ...
      },
      {
        ...
        description: "Create a fulfillment for the Switchyard order.",
...
export const retrieveItemsToFulfillHighlights = [
  ...
  ["7", "line_items", "The line items in the Switchyard order."],
...
import { InferTypeOf, OrderLineItemDTO } from "@switchyard/framework/types"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
import { PreorderVariant } from "../../modules/preorder/models/preorder-variant"
...
The step receives the pre-order variant to be fulfilled as an input, and the line items in the Switchyard order.

In the step, you find the items in the Switchyard order that are associated with the pre-order variant and return them.
...
export const fulfillPreorderWorkflowHighlights = [
  ...
  ["12", "order_id", "The ID of the Switchyard order containing the pre-ordered variant."],
  ["18", "useQueryGraphStep", "Retrieve the Switchyard order."],
  ["29", "retrieveItemsToFulfillStep", "Retrieve the items to fulfill in the order."],
  ["34", "createOrderFulfillmentWorkflow.runAsStep", "Create a fulfillment for the Switchyard order."],
...
import { PreorderVariant } from "../modules/preorder/models/preorder-variant"
import { createWorkflow, WorkflowResponse } from "@switchyard/framework/workflows-sdk"
import { createOrderFulfillmentWorkflow, emitEventStep, useQueryGraphStep } from "@switchyard/medusa/core-flows"
...
- `order_id`: The ID of the Switchyard order containing the pre-ordered variant.

In the workflow, you:

- Retrieve the Switchyard order using the [useQueryGraphStep](/references/helper-steps/useQueryGraphStep).
- Retrieve the items to fulfill in the order using the `retrieveItemsToFulfillStep`.
- Create a fulfillment for the Switchyard order using the [createOrderFulfillmentWorkflow](/references/medusa-workflows/createOrderFulfillmentWorkflow) as a step.
...
By default, Switchyard authorizes an order's payment when it's placed. The admin user can capture the payment later manually.
...
To do that, you can use Switchyard's [capturePaymentWorkflow](/references/medusa-workflows/capturePaymentWorkflow) to capture the payment for the order in the workflow.
...
import { PreorderVariant } from "../modules/preorder/models/preorder-variant"
import { createWorkflow, transform, when, WorkflowResponse } from "@switchyard/framework/workflows-sdk"
import { capturePaymentWorkflow, createOrderFulfillmentWorkflow, emitEventStep, useQueryGraphStep } from "@switchyard/medusa/core-flows"
import { retrieveItemsToFulfillStep, RetrieveItemsToFulfillStepInput } from "./steps/retrieve-items-to-fulfill"
import { updatePreordersStep } from "./steps/update-preorders"
import { PreorderStatus } from "../modules/preorder/models/preorder"
...
export default async function fulfillPreordersJob(container: SwitchyardContainer) {
  const query = container.resolve("query")
...
So far, you only resolve Query and the Logger utility from the Switchyard container passed as input to the scheduled job.
...
Next, start the Switchyard application:
...
The subscriber receives among its parameters the data payload of the emitted event, which includes the IDs of the Switchyard order and the pre-order variant.
...
import { emitEventStep } from "@switchyard/medusa/core-flows"
import { updatePreordersStep } from "./steps/update-preorders"
...
To test order cancelation, start the Switchyard application.
...
![Pre-order item's status is canceled in a canceled order](https://res.cloudinary.com/dza7lstvk/image/upload/v1752752393/Switchyard%20Resources/CleanShot_2025-07-17_at_14.38.50_2x_cxqscy.png)
...
import { InferTypeOf, OrderDTO, OrderLineItemDTO, ProductVariantDTO } from "@switchyard/framework/types"
import { PreorderVariant } from "../../modules/preorder/models/preorder-variant"
import { Preorder, PreorderStatus } from "../../modules/preorder/models/preorder"
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
...
import { createWorkflow, transform, WorkflowResponse } from "@switchyard/framework/workflows-sdk"
import { emitEventStep, useQueryGraphStep } from "@switchyard/medusa/core-flows"
...
import { SubscriberArgs, type SubscriberConfig } from "@switchyard/framework"
import { OrderChangeActionDTO } from "@switchyard/framework/types"
...
To test out the order edit handler, start the Switchyard application.

Then, open an order in the Switchyard Admin. It can be an order you want to remove a pre-order item from, or an order you want to add a new pre-order item to.
...
![Pre-order items updated in an order edit](https://res.cloudinary.com/dza7lstvk/image/upload/v1752754220/Switchyard%20Resources/CleanShot_2025-07-17_at_15.09.55_2x_xkkfef.png)

---

## Next Steps

You've now implemented the pre-order feature in Switchyard. You can expand on this feature based on your use case. You can add features like:
