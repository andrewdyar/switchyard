...
In this tutorial, you'll learn how to add images to product categories in Switchyard.

When you install a Switchyard application, you get a fully-fledged commerce platform with the Framework for customization. The Switchyard application's commerce features are built around [Commerce Modules](../../../commerce-modules/page.mdx), which are available out of the box.

Switchyard doesn't natively support adding images to product categories. However, it provides the customization capabilities you need to implement this feature.
...
- Customize the Switchyard Admin dashboard to manage category images.
...
![Diagram showing the relation between product categories and their images](https://res.cloudinary.com/dza7lstvk/image/upload/v1760522310/Switchyard%20Resources/category-images-summary_l1duwj.jpg)

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/category-images",
    title: "Full Code",
...
class ProductMediaModuleService extends SwitchyardService({
...
The `ProductMediaModuleService` extends `SwitchyardService`, which generates a class with data-management methods for your module's data models. This saves you time implementing Create, Read, Update, and Delete (CRUD) methods.
...
The final piece of a module is its definition, which you export in an `index.ts` file at the module's root directory. This definition tells Switchyard the module's name and its service.
...
After building the module, add it to Switchyard's configurations to start using it.

In `switchyard.config.ts`, add a `modules` property with an array containing your custom module:
...
Switchyard's CLI tool can generate migrations for you. To generate a migration for the Product Media Module, run the following command in your Switchyard application's directory:

```bash
npx switchyard db:generate productMedia
```

The `db:generate` command of the Switchyard CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/product-media` that holds the generated migration.
...
When building commerce features in Switchyard that client applications consume, such as the Switchyard Admin dashboard or a storefront, you need to implement:
...
You won't implement the functionality to upload the image, as Switchyard already exposes an API route to upload files. You'll use that route in the Switchyard Admin dashboard to upload images and get their URLs and file IDs.
...
2. An async function that receives two parameters:
    ...
    - An object with properties including the [Switchyard container](!docs!/learn/fundamentals/medusa-container), which is a registry of Framework and commerce tools accessible in the step.
...
1. Resolve the `ProductMediaModuleService` from the Switchyard container to manage product category images.
...
export const createCategoryImagesStepHighlights = [
  ...
  ["36", "SwitchyardError", "Throw an error if more than one thumbnail is being added for a category."],
...
export async function POST(
  req: SwitchyardRequest<CreateCategoryImagesInput>,
...
You apply Switchyard's `validateAndTransformBody` middleware to `POST` requests sent to the `/admin/categories/:category_id/images` route. The middleware function accepts a Zod schema that you created in the API route's file.
...
You'll test this API route next when you customize the Switchyard Admin.

---

## Step 5: Create Product Category Images in Switchyard Admin

In this step, you'll customize the Switchyard Admin dashboard to manage a product category's images.
...
import { DetailWidgetProps, AdminProductCategory } from "@switchyard/framework/types"
...
1. `uploadFilesMutation`: A mutation that uploads files using Switchyard's existing API route for uploads. This will upload the images to the [configured File Module Provider](../../../infrastructure-modules/file/page.mdx#what-is-a-file-module-provider).
...
import { FocusModal, Button, Heading, toast } from "@switchyard/ui"
...
You render a modal using the `FocusModal` component from [Switchyard UI](!ui!). The modal displays the `CategoryImageGallery` component on the left and the `CategoryImageUpload` component on the right.
...
You can now test the media widget in the Switchyard Admin dashboard.

Run the following command in the Switchyard project directory to start the Switchyard server:
...
Images are uploaded to the [configured File Module Provider](../../../infrastructure-modules/file/page.mdx#what-is-a-file-module-provider). If you haven't configured one, images will be uploaded to the `static` folder in your Switchyard project.

</Note>

![Media widget showing images with upload form](https://res.cloudinary.com/dza7lstvk/image/upload/v1760455846/Switchyard%20Resources/CleanShot_2025-10-14_at_18.29.49_2x_vhbn6z.png)
...
![Media widget showing images after upload](https://res.cloudinary.com/dza7lstvk/image/upload/v1760456027/Switchyard%20Resources/CleanShot_2025-10-14_at_18.33.27_2x_au7nxg.png)
...
- Updating the Switchyard Admin modal to allow updating image types.
...
Switchyard provides the `useQueryGraphStep`, and you've already created the `convertCategoryThumbnailsStep` in [step 3](#convertcategorythumbnailsstep). You only need to create the `updateCategoryImagesStep`.
...
export async function POST(
  req: SwitchyardRequest<UpdateCategoryImagesInput>,
  res: SwitchyardResponse
): Promise<void> {
...
```tsx title="src/admin/components/category-media/category-image-item.tsx"
import { Checkbox, clx } from "@switchyard/ui"
```
...
```tsx title="src/admin/components/category-media/category-media-modal.tsx"
import { CommandBar } from "@switchyard/ui"
```

You'll use the `CommandBar` component from Switchyard UI to show actions like "Set as Thumbnail" and "Delete".
...
You can now test the update functionality in the Switchyard Admin dashboard.

Start the Switchyard server if it's not already running, and go to a category's details page:
...
![Media widget showing command bar with set as thumbnail action](https://res.cloudinary.com/dza7lstvk/image/upload/v1760514879/Switchyard%20Resources/CleanShot_2025-10-15_at_10.54.18_2x_qfrwgy.png)

You'll now see the thumbnail icon on the image in the Media section of the category details page.

![Media widget showing updated thumbnail](https://res.cloudinary.com/dza7lstvk/image/upload/v1760515015/Switchyard%20Resources/CleanShot_2025-10-15_at_10.56.24_2x_suv0qr.png)
...
3. Updating the Switchyard Admin modal to allow deleting images.
...
The first two steps are available out-of-the-box in Switchyard. You only need to create the last step.
...
import { deleteFilesWorkflow, useQueryGraphStep } from "@switchyard/medusa/core-flows"
...
export async function DELETE(
  req: SwitchyardRequest<DeleteCategoryImagesInput>,
  res: SwitchyardResponse
): Promise<void> {
...
You can now test the delete functionality in the Switchyard Admin dashboard.
...
![Media widget showing command bar with delete action](https://res.cloudinary.com/dza7lstvk/image/upload/v1760516694/Switchyard%20Resources/CleanShot_2025-10-15_at_11.23.42_2x_k1folo.png)

You'll see the selected images are removed from the Media section of the category details page.

![Media widget showing updated images after deletion](https://res.cloudinary.com/dza7lstvk/image/upload/v1760516693/Switchyard%20Resources/CleanShot_2025-10-15_at_11.24.30_2x_rmgtqn.png)
...
So, if your Switchyard application's directory is `medusa-category-images`, you can find the storefront by going back to the parent directory and changing to the `medusa-category-images-storefront` directory:
...
Before customizing the storefront, you need a way to retrieve a category's images from the Switchyard backend.
...
```ts title="src/links/product-category-image.ts" badgeLabel="Switchyard Application" badgeColor="green"
...
To test out the megamenu, start the Switchyard application with the following command:
...
![Storefront showing megamenu with categories and their thumbnails](https://res.cloudinary.com/dza7lstvk/image/upload/v1760518332/Switchyard%20Resources/CleanShot_2025-10-15_at_11.51.56_2x_mifvqx.png)
...
To test out the category banner, ensure both the Switchyard application and the Next.js Starter Storefront are running.
...
You'll see the banner image at the top of the category page. If you don't see a banner image, ensure that the category has an image of type "image" (not "thumbnail") in the Switchyard Admin dashboard.

![Storefront showing category page with banner image](https://res.cloudinary.com/dza7lstvk/image/upload/v1760518495/Switchyard%20Resources/CleanShot_2025-10-15_at_11.54.44_2x_muadjw.png)

---

## Next Steps

You've now added support for category images in Switchyard. You can expand on this by:
