...
export const metadata = {
  title: `Add Gift Message to Line Items in Switchyard`,
}

# {metadata.title}

In this tutorial, you will learn how to add a gift message to items in carts and orders in Switchyard.
...
You can customize the Switchyard application and storefront to add a gift message to items in the cart. This feature allows customers to add a personalized message to their gifts, enhancing the shopping experience.
...
- Customize the Switchyard Admin to show gift items with messages in an order.
...
<Card
  ...
  icon={Github}
  href="https://github.com/switchyard/examples/tree/main/order-gift-message"
...
The Next.js Starter Storefront has an `updateLineItem` function that sends a request to the Switchyard server to update the cart item. However, it doesn't support updating the `metadata` property.
...
You pass the `metadata` property to the Switchyard server, which will update the cart item with the new metadata.
...
import { Checkbox, Textarea, Button, Label } from "@switchyard/ui"
```

You import components from the [Switchyard UI library](!ui!) that will be useful for the gift inputs.
...
First, start the Switchyard application by running the following command in the Switchyard application directory:
...
![Gift checkbox on checkout page](https://res.cloudinary.com/dza7lstvk/image/upload/v1750923117/Switchyard%20Resources/CleanShot_2025-06-26_at_10.31.25_2x_wcqiff.png)
...
![Gift item with message added](https://res.cloudinary.com/dza7lstvk/image/upload/v1750924150/Switchyard%20Resources/CleanShot_2025-06-26_at_10.32.52_2x_qir2t5.png)
...
![Gift message on order confirmation page](https://res.cloudinary.com/dza7lstvk/image/upload/v1750924429/Switchyard%20Resources/CleanShot_2025-06-26_at_10.53.32_2x_htzvtw.png)

---

## Step 4: Show Gift Options in Admin Dashboard

In this step, you'll customize the Switchyard Admin dashboard to show the gift items with their messages in an order.

### What is a Widget?

The Switchyard Admin dashboard's pages are customizable to insert [widgets](!docs!/learn/fundamentals/admin/widgets) of custom content in predefined injection zones. You create these widgets as React components that allow admin users to perform custom actions.
...
You create a widget in a `.tsx` file under the `src/admin/widgets` directory. So, in the Switchyard application directory, create the file `src/admin/widgets/order-gift-items-widget.tsx` with the following content:

```tsx title="src/admin/widgets/order-gift-items-widget.tsx" badgeLabel="Switchyard Application" badgeColor="green"
import { defineWidgetConfig } from "@switchyard/admin-sdk"
import { Container, Text } from "@switchyard/ui"
...
### Test Switchyard Admin Changes

To test out the Switchyard Admin widget, start the Switchyard application by running the following command in the Switchyard application directory:

```bash npm2yarn badgeLabel="Switchyard Application" badgeColor="green"
npm run dev
```

Then, open the Switchyard Admin at `http://localhost:9000/app` and log in with the user you created earlier.
...
![Order details page in the Switchyard Admin with the "Gift Items & Messages" section highlighted](https://res.cloudinary.com/dza7lstvk/image/upload/v1750925501/Switchyard%20Resources/CleanShot_2025-06-26_at_11.11.18_2x_jrrc2s.png)
...
```ts title="Fulfillment Module Provider's Service" badgeLabel="Switchyard Application" badgeColor="green" highlights={createFulfillmentHighlights}
...
You've now added gift messages to items in carts and orders in Switchyard.
