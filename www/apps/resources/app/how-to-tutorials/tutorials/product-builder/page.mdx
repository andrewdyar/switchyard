...
export const metadata = {
  title: `Implement Product Builder in Switchyard`,
}

# {metadata.title}

In this tutorial, you'll learn how to implement a product builder in Switchyard.
...
- Allow admin users to manage the builder configurations of a product from Switchyard Admin.
...
- Customize the order details page on the Switchyard Admin to reflect the selected builder configurations of items.

You can follow this tutorial whether you're new to Switchyard or an advanced Switchyard developer.

![Screenshot of how the product builder will look like in the storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1755101580/Switchyard%20Resources/CleanShot_2025-08-13_at_19.12.48_2x_tf4goa.png)

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/product-builder",
    title: "Full Code",
...
- `product_id`: The ID of the product that this builder configuration applies to.
    - Later, you'll learn how to link this data model to Switchyard's `Product` data model.
...
- `product_id`: The ID of the complementary product to suggest.
    - Later, you'll learn how to link this to Switchyard's `Product` data model.
- `product_builder`: A relation back to the parent `ProductBuilder` configuration.
...
class ProductBuilderModuleService extends SwitchyardService({
  ProductBuilder,
...
The `ProductBuilderModuleService` extends `SwitchyardService`, which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.
...
Switchyard's CLI tool can generate the migrations for you. To generate a migration for the Product Builder Module, run the following command in your Switchyard application's directory:

```bash
npx switchyard db:generate productBuilder
```

The `db:generate` command of the Switchyard CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/product-builder` that holds the generated migration.
...
In this step, you'll define links between the data models in the Product Builder Module and the data models in Switchyard's Product Module:

- `ProductBuilder` ↔ `Product`: A product builder record represents the builder configurations of a product.
- `ProductBuilderComplementary` ↔ `Product`: A complementary product record suggests a Switchyard product related to the main product.
- `ProductBuilderAddon` ↔ `Product`: An add-on product record suggests a Switchyard product that can be added to the main product in the cart.
...
export const productBuilderLinkHighlights = [
  ...
  ["10", "ProductModule.linkable.product", "Link to Switchyard's Product data model."]
]
...
This command will create the necessary tables to store the links between your Product Builder Module and Switchyard's Product Module.
        ...
        steps: [
          {
            type: "step",
            name: "createRemoteLinkStep",
            description: "Link new complementary products to their Switchyard product.",
            link: "/references/helper-steps/createRemoteLinkStep",
            depth: 1
          }
        ],
        ...
        steps: [
          {
            ...
            description: "Dismiss links between deleted complementary products and their Switchyard products.",
        ...
        steps: [
          {
            type: "step",
            name: "createRemoteLinkStep",
            description: "Link new addon products to their Switchyard product.",
            link: "/references/helper-steps/createRemoteLinkStep",
            depth: 1
          }
        ],
        ...
        steps: [
          {
            type: "step",
            name: "dismissRemoteLinkStep",
            description: "Dismiss links between deleted addon products and their Switchyard products.",
            link: "/references/helper-steps/dismissRemoteLinkStep",
            depth: 1
          }
        ],
...
The `useQueryGraphStep`, `createRemoteLinkStep`, and `dismissRemoteLinkStep` are available through Switchyard's `@switchyard/medusa/core-flows` package. You'll implement other steps in the workflow.
...
If you get a type error on resolving the Product Builder Module, run the Switchyard application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](!docs!/learn/fundamentals/generated-types).
...
In the step function, you resolve the Product Builder Module's service from the Switchyard container and create a product builder record.
...
import { createWorkflow, parallelize, transform, when, WorkflowResponse } from "@switchyard/framework/workflows-sdk"
import { createRemoteLinkStep, dismissRemoteLinkStep } from "@switchyard/medusa/core-flows"
...
In a workflow, you can't manipulate data or check conditions because Switchyard stores an internal representation of the workflow on application startup. Learn more in the [Data Manipulation](!docs!/learn/fundamentals/workflows/variable-manipulation) and [Conditions](!docs!/learn/fundamentals/workflows/conditions) documentation.
...
export const upsertProductBuilderWorkflowHighlights3 = [
  ...
  ["24", "complementaryProductLinks", "Links to create between complementary Switchyard products."],
  ["25", "deletedComplementaryProductLinks", "Links to dismiss between complementary Switchyard products."],
  ["54", "createRemoteLinkStep", "Create links between complementary and Switchyard products."],
  ["63", "dismissRemoteLinkStep", "Dismiss links between complementary and Switchyard products."]
]
...
- Prepare the links to be created or deleted between the complementary products and the Switchyard products.
...
export const upsertProductBuilderWorkflowHighlights4 = [
  ...
  ["21", "addonProductLinks", "Links to create between addon Switchyard products."],
  ["22", "deletedAddonProductLinks", "Links to dismiss between addon Switchyard products."],
  ["51", "createRemoteLinkStep", "Create links between addon and Switchyard products."],
  ["60", "dismissRemoteLinkStep", "Dismiss links between addon and Switchyard products."]
]
...
This part of the workflow is similar to the complementary products management, but it handles addon products instead. You create and delete addon products, then create and dismiss links between them and Switchyard products.
...
export const POST = async (
  req: AuthenticatedSwitchyardRequest<typeof UpsertProductBuilderSchema>,
...
In the route handler, you execute the `upsertProductBuilderWorkflow` passing it the Switchyard container, which is available in the `req.scope` property, and executing its `run` method.

You return the product builder in the response.

You'll test this API route later when you customize the Switchyard Admin dashboard.
...
export const GET = async (
  req: AuthenticatedSwitchyardRequest<{ id: string }>,
  res: SwitchyardResponse
) => {
...
In this step, you'll customize the Switchyard Admin to allow admin users to manage a product's builder configurations.
...
![Screenshot of how the custom fields tab will look like](https://res.cloudinary.com/dza7lstvk/image/upload/v1755089825/Switchyard%20Resources/CleanShot_2025-08-13_at_15.56.15_2x_uwej4x.png)
...
![Screenshot of how the complementary products tab will look like](https://res.cloudinary.com/dza7lstvk/image/upload/v1755089830/Switchyard%20Resources/CleanShot_2025-08-13_at_15.56.20_2x_jxix2n.png)
...
import { AdminProduct } from "@switchyard/framework/types"
import { 
  Heading, 
  Checkbox, 
...
![Screenshot of how the addons tab will look like](https://res.cloudinary.com/dza7lstvk/image/upload/v1755089831/Switchyard%20Resources/CleanShot_2025-08-13_at_15.56.24_2x_zmryk1.png)
...
import { Container, Heading, Text, Button } from "@switchyard/ui"
import { useQuery } from "@tanstack/react-query"
import { sdk } from "../lib/sdk"
import { 
  DetailWidgetProps, 
...
1. Start the Switchyard application with the following command:

```bash npm2yarn
npm run dev
```

2. Open the Switchyard Admin dashboard at `localhost:9000/app` and login.
...
![Builder configuration widget in the product details page](https://res.cloudinary.com/dza7lstvk/image/upload/v1755096182/Switchyard%20Resources/CleanShot_2025-08-13_at_17.30.06_2x_hnfsdl.png)
...
![Updated builder configuration data showing in the widget](https://res.cloudinary.com/dza7lstvk/image/upload/v1755096296/Switchyard%20Resources/CleanShot_2025-08-13_at_17.44.45_2x_zqonoy.png)
...
Alongside the variant options like color and size, which are already available in Switchyard, you'll show:
...
So, if your Switchyard application's directory is `medusa-product-builder`, you can find the storefront by going back to the parent directory and changing to the `medusa-product-builder-storefront` directory:
...
- `ProductWithBuilder`: A Switchyard product with an associated product builder configuration.
...
![Screenshot of how the product builder configuration component will look like](https://res.cloudinary.com/dza7lstvk/image/upload/v1755098570/Switchyard%20Resources/CleanShot_2025-08-13_at_18.22.29_2x_zdtjlk.png)
...
![Product details page with builder configurations](https://res.cloudinary.com/dza7lstvk/image/upload/v1755101580/Switchyard%20Resources/CleanShot_2025-08-13_at_19.12.48_2x_tf4goa.png)
...
You only need to implement the `validateProductBuilderConfigurationStep`, as Switchyard provides the rest.
...
To test out the add-to-cart functionality with builder configurations, make sure that both the Switchyard application and the Next.js Starter Storefront are running.
...
![Cart dropdown showing the product with its complementary product](https://res.cloudinary.com/dza7lstvk/image/upload/v1755160166/Switchyard%20Resources/CleanShot_2025-08-14_at_11.28.53_2x_bcskgr.png)
...
![Screenshot showcasing which areas of the cart page will be updated and how they'll look like](https://res.cloudinary.com/dza7lstvk/image/upload/v1755160329/Switchyard%20Resources/CleanShot_2025-08-14_at_11.31.50_2x_tpxlht.png)

#### Styling Changes

You'll first update the style of the quantity changer component for a better design.

![Screenshot showcasing the updated quantity changer component](https://res.cloudinary.com/dza7lstvk/image/upload/v1755160526/Switchyard%20Resources/CleanShot_2025-08-14_at_11.35.07_2x_vtmnuz.png)
...
![Screenshot showcasing the updated delete button](https://res.cloudinary.com/dza7lstvk/image/upload/v1755160603/Switchyard%20Resources/CleanShot_2025-08-14_at_11.36.29_2x_myhwyf.png)
...
![Screenshot showcasing the updated item price components](https://res.cloudinary.com/dza7lstvk/image/upload/v1755160898/Switchyard%20Resources/CleanShot_2025-08-14_at_11.41.23_2x_eitu29.png)
...
To test out the design changes to the cart page, make sure both the Switchyard application and the Next.js Starter Storefront are running.
...
![Screenshot showcasing the updated cart page with custom fields and addon products](https://res.cloudinary.com/dza7lstvk/image/upload/v1755161965/Switchyard%20Resources/CleanShot_2025-08-14_at_11.59.16_2x_pfo20r.png)
...
To test out the cart dropdown changes, make sure both the Switchyard application and the Next.js Starter Storefront are running.
...
![Updated cart dropdown with updated total count and filtered items](https://res.cloudinary.com/dza7lstvk/image/upload/v1755160166/Switchyard%20Resources/CleanShot_2025-08-14_at_11.28.53_2x_bcskgr.png)
...
Switchyard provides all of these steps, so you can create the workflow without needing to implement any custom steps.
...
To test out the changes, make sure both the Switchyard application and the Next.js Starter Storefront are running.
...
To test out the changes in the order confirmation page, make sure both the Switchyard application and the Next.js Starter Storefront are running.
...
![Updated order confirmation page](https://res.cloudinary.com/dza7lstvk/image/upload/v1755168706/Switchyard%20Resources/CleanShot_2025-08-14_at_13.51.33_2x_bdadlh.png)
...
1. Make sure the Switchyard Application is running.
2. Open the Switchyard Admin dashboard and log in.
3. Go to Orders.
...
![Widget on the order details page showing the builder configurations for each item](https://res.cloudinary.com/dza7lstvk/image/upload/v1755169611/Switchyard%20Resources/CleanShot_2025-08-14_at_14.06.40_2x_om7f6e.png)

---

## Next Steps

You've now implemented the product builder feature in Switchyard. You can expand on this feature based on your use case. You can:
