...
export const metadata = {
  title: `Generate Invoices for Orders in Switchyard`,
}

# {metadata.title}

In this tutorial, you will learn how to generate invoices for orders in your Switchyard application.
...
You can extend the Switchyard application to automatically generate invoices (or sales orders), manage invoice configurations through the admin dashboard, and provide customers with easy access to their invoices through the storefront.
...
- Store default invoice configurations and manage them from the Switchyard Admin dashboard.
...
![Diagram illustrating the flow from the customer placing the order, them receiving the invoice, and the admin downloading the invoice](https://res.cloudinary.com/dza7lstvk/image/upload/v1753793501/Switchyard%20Resources/invoices-summary_ieja9e.jpg)

<CardList items={[
  {
    href: "https://github.com/switchyard/examples/tree/main/invoice-generator",
    title: "Full Code",
...
class InvoiceGeneratorService extends SwitchyardService({
  InvoiceConfig,
  Invoice,
}) { }
...
The `InvoiceGeneratorService` extends `SwitchyardService`, which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.
...
Switchyard's CLI tool can generate the migrations for you. To generate a migration for the Invoice Generator Module, run the following command in your Switchyard application's directory:

```bash
npx switchyard db:generate invoice-generator
```

The `db:generate` command of the Switchyard CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/invoice-generator` that holds the generated migration.
...
In this step, you'll create default invoice configurations that admins can later manage through the Switchyard Admin dashboard.

Since you need to create the default configurations when the Switchyard application starts, you'll use a [loader](!docs!/learn/fundamentals/modules/loaders). A loader is a script in your module that Switchyard runs on application startup.
...
export default async function createDefaultConfigLoader({
  ...
  const service: ISwitchyardInternalService<
...
In the loader, you resolve a service that Switchyard generates for the `InvoiceConfig` data model. You use that service to create default configurations if none exist.
...
To run the loader, start the Switchyard application:

```bash npm2yarn
npm run dev
```

The Switchyard application will run your loader to create the default invoice configurations in the database. You'll verify that it worked in the next step after adding the admin settings page.

---

## Step 4: Allow Admins to Manage Invoice Configurations

In this step, you'll customize the Switchyard application to allow admin users to manage the default invoice configurations through the Switchyard Admin dashboard.
...
3. A [settings page](!docs!/learn/fundamentals/admin/ui-routes#create-settings-page) in the Switchyard Admin dashboard that allows admin users to manage the invoice configurations.
...
If you get a type error on resolving the Invoice Generator Module, run the Switchyard application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](!docs!/learn/fundamentals/generated-types).
...
In the step function, you resolve the Invoice Generator Module's service from the Switchyard container. Then, you either retrieve the invoice configuration by its ID, or list all invoice configurations and use the first one.
...
export const retrieveInvoiceConfigHighlights = [
  ...
  ["7", "query", "Resolve Query from the Switchyard container."],
...
In the route handler, you resolve [Query](!docs!/learn/fundamentals/module-links/query) from the Switchyard container. It allows you to retrieve data across modules.
...
export async function POST(
  req: SwitchyardRequest<PostInvoiceConfig>,
  res: SwitchyardResponse
) {
...
In the route handler, you execute the `updateInvoiceConfigWorkflow` by invoking it, passing it the Switchyard container, then executing its `run` method.
...
import { defineMiddlewares, validateAndTransformBody } from "@switchyard/framework/http"
...
You apply Switchyard's `validateAndTransformBody` middleware to `POST` requests sent to the `/admin/invoice-config` API route.
...
Next, you'll create a settings page in the Switchyard Admin dashboard that allows admin users to manage the invoice configurations.
...
Settings pages are [UI routes](!docs!/learn/fundamentals/admin/ui-routes#create-settings-page) created under the `/src/admin/routes/settings` directory. Switchyard will then add the settings page under the Settings section of the Switchyard Admin dashboard.
...
import { Container, Heading, Button, Input, Label, Textarea, toast } from "@switchyard/ui"
...
export const InvoiceConfigPageHighlights = [
  ...
  ["37", "uploadLogo", "Upload a logo using Switchyard's Upload API route."],
...
6. Define an `uploadLogo` function that uploads a logo using Switchyard's [Upload API route](!api!/admin#uploads_postuploads).
...
Refer to the [Admin Components](../../../admin-components/components/forms/page.mdx) documentation to learn more about creating forms in the Switchyard Admin dashboard.
...
First, start the Switchyard application with the following command:
...
1. Open the Switchyard Admin dashboard in your browser at `http://localhost:9000/admin` and log in.
...
![Invoice Configurations Settings Page](https://res.cloudinary.com/dza7lstvk/image/upload/v1753788482/Switchyard%20Resources/CleanShot_2025-07-29_at_13.54.11_2x_zs2h9w.png)
...
`axios` is available in your Switchyard project by default, so you don't need to install it.
...
You only need to implement the third and fourth steps, as the other two steps are helper steps that Switchyard provides.
...
import { createStep, StepResponse } from "@switchyard/framework/workflows-sdk"
import { INVOICE_MODULE } from "../../modules/invoice-generator"
import { OrderDTO, OrderLineItemDTO } from "@switchyard/framework/types"
...
export const orderInvoiceWidgetHighlights1 = [
  ...
  ["12", "zone", "Where the widget should be injected in the Switchyard Admin."]
]
...
import { AdminOrder, DetailWidgetProps } from "@switchyard/framework/types"
import { sdk } from "../lib/sdk"
...
2. A widget configuration object that defines where the widget should be injected in the Switchyard Admin.
...
To test the admin widget, start the Switchyard Admin application again.
...
So, if your Switchyard application's directory is `medusa-invoice`, you can find the storefront by going back to the parent directory and changing to the `medusa-invoice-storefront` directory:
...
In the Switchyard Admin, open the order's detail page. You'll see an "Invoice" section at the top right of the page. Click on the "Download Invoice" button to generate and download the invoice PDF.

![Download Invoice Button in Admin Widget](https://res.cloudinary.com/dza7lstvk/image/upload/v1753788505/Switchyard%20Resources/CleanShot_2025-07-29_at_14.27.08_2x_rnn1er.png)
...
To test the storefront changes, ensure both the Switchyard application and the Next.js Starter Storefront are running.
...
![Download Invoice Button in Storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1753789117/Switchyard%20Resources/CleanShot_2025-07-29_at_14.38.21_2x_oyydri.png)
...
You can listen to events that occur in your Switchyard application, such as when an order is placed, using [subscribers](!docs!/learn/fundamentals/events-and-subscribers). A subscriber is an asynchronous function that is executed whenever its associated event is emitted.
...
To test the order confirmation email, make sure that the Switchyard application and the Next.js Starter Storefront are running.

Then, place an order in the storefront. You'll see the following message in the Switchyard application's console:
...
Refer to the [Events Reference](/references/events) for a complete list of events that Switchyard emits.

</Note>

### Test Order Updates

To test the order updates subscriber, start the Switchyard application and go to an order's page.
...
You've successfully implemented the invoice generator feature in Switchyard. You can expand on this feature to:
...
3. Trigger sending invoices to customers from the Switchyard Admin dashboard.
