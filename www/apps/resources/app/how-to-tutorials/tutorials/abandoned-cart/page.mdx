...
export const ogImage = "https://res.cloudinary.com/dza7lstvk/image/upload/v1743097105/Switchyard%20Resources/abandoned-cart_eze9ng.jpg"

export const metadata = {
  title: `Send Abandoned Cart Notifications in Switchyard`,
  openGraph: {
    images: [
      {
        url: ogImage,
...
Switchyard's [Notification Module](../../../infrastructure-modules/notification/page.mdx) allows you to send notifications to users or customers, such as password reset emails, order confirmation SMS, or other types of notifications.
...
![Diagram illustrating the flow of the abandoned-cart functionalities](https://res.cloudinary.com/dza7lstvk/image/upload/v1742460588/Switchyard%20Resources/abandoned-cart-summary_fcf2tn.jpg)

<Card
  title="View on Github"
  icon={Github}
  href="https://github.com/switchyard/examples/tree/main/abandoned-cart"
...
Switchyard's Notification Module provides the general functionality to send notifications, but the sending logic is implemented in a module provider. This allows you to integrate the email provider of your choice.

To send the cart-abandonment emails, you will use SendGrid. Switchyard provides a [SendGrid Notification Module Provider](../../../infrastructure-modules/notification/sendgrid/page.mdx) that you can use to send emails.
...
To set up SendGrid, add the SendGrid Notification Module Provider to `switchyard.config.ts`:
...
You can now use SendGrid to send emails in Switchyard.
...
Switchyard provides the second step in its `@switchyard/medusa/core-flows` package. So, you only need to implement the first one.
...
import { CartDTO, CustomerDTO } from "@switchyard/framework/types"
...
import { updateCartsStep } from "@switchyard/medusa/core-flows"
import { CartDTO } from "@switchyard/framework/types"
...
- Use the `updateCartsStep` from Switchyard's core flows to update the carts' metadata with the last notification date.

Notice that you use the `transform` function to prepare the `updateCartsStep`'s input. Switchyard does not support direct data manipulation in a workflow's constructor function. You can learn more about it in the [Data Manipulation in Workflows documentation](!docs!/learn/fundamentals/workflows/variable-manipulation).
...
![Button is at the top right of the page](https://res.cloudinary.com/dza7lstvk/image/upload/v1742457153/Switchyard%20Resources/Screenshot_2025-03-20_at_9.51.38_AM_g5nk80.png)
...
![The template is a collapsible in the middle of the page,with the "Add Version" button shown in the middle](https://res.cloudinary.com/dza7lstvk/image/upload/v1742458096/Switchyard%20Resources/Screenshot_2025-03-20_at_10.07.54_AM_y2dys7.png)
...
Once you are done, copy the template ID from SendGrid and set it as an environment variable in your Switchyard project:
...
To run a task at a scheduled interval, you can use a [scheduled job](!docs!/learn/fundamentals/scheduled-jobs). A scheduled job is an asynchronous function that the Switchyard application runs at the interval you specify during the Switchyard application's runtime.
...
Finally, start the Switchyard application with the following command:
...
- Try to retrieve the cart from the Switchyard application. The `retrieveCart` function is already available in the Next.js Starter Storefront. If the cart is not found, you return a 404 response.
...
![Cart page in the storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1742459552/Switchyard%20Resources/Screenshot_2025-03-20_at_10.32.17_AM_frmbup.png)

---

## Next Steps

You have now implemented the logic to send abandoned cart notifications in Switchyard. You can implement other customizations with Switchyard, such as:
...
If you are new to Switchyard, check out the [main documentation](!docs!/learn), where you will get a more in-depth learning of all the concepts you have used in this guide and more.
