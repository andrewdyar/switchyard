...
export const metadata = {
  title: `How to Add Custom Authentication in Switchyard Admin`,
}

# {metadata.title}

In this guide, you'll learn how to add custom authentication provider to the Switchyard Admin.

<Note>

Authenticating into the Switchyard Admin with third-party providers is supported from [Switchyard v2.12.0](https://github.com/switchyard/medusa/releases/tag/v2.12.0).

</Note>

## Overview

By default, the Switchyard Admin allows users to authenticate using their email and password. Switchyard uses the [Emailpass Authentication Provider](../../../../commerce-modules/auth/auth-providers/emailpass/page.mdx) to handle this authentication method.

You can also integrate a custom or third-party authentication provider to allow users registered with that provider to log in to the Switchyard Admin. This delegates the authentication process to the third-party provider, which is useful if your organization uses a centralized authentication system.

<Note type="warning">

Ensure the third-party authentication provider allows **only** your organization's users to log in to the Switchyard Admin. For example, integrating a social login provider like Google or Facebook without restrictions will allow any user with an account on those platforms to access your Switchyard Admin.

</Note>

### Summary of Custom Switchyard Admin Authentication
...
2. Change the authentication type in the Switchyard Admin dashboard to use JWT authentication.
...
![Overview of the custom authentication process in Switchyard Admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1764153489/Switchyard%20Resources/admin-third-party-auth_phknrs.jpg)
...
It's recommended to restrict the custom authentication provider to admin users only. To do this, set the `http.authMethodsPerActor` configuration in `switchyard.config.ts` to enable the custom authentication provider only for the `user` actor type:
...
## Step 2: Change Authentication Type in Switchyard Admin

Next, you need to change the authentication type in the Switchyard Admin dashboard to use JWT authentication.

To do this, set the `ADMIN_AUTH_TYPE` environment variable to `jwt` in your Switchyard application's environment variables:

```bash
ADMIN_AUTH_TYPE=jwt
```

Make sure to also create a JS SDK instance in your Switchyard Admin customizations with the `auth.type` option set to `jwt`. For example, create the file `src/lib/sdk.ts` with the following content:
...
Refer to the [JS SDK Authentication guide](../../../../js-sdk/auth/overview/page.mdx) to learn more about configuring authentication in the Switchyard JS SDK.
...
import { createUsersWorkflow, setAuthAppMetadataStep } from "@switchyard/medusa/core-flows"
...
export const POST = async (req: AuthenticatedSwitchyardRequest<CreateUserBody>, res: SwitchyardResponse) => {
...
Finally, you need to add a [widget](!docs!/learn/fundamentals/admin/widgets) to the Switchyard Admin login page that provides the option to log in through the third-party provider.

The widget should display a button that authenticates or redirects the user to the third-party provider. The widget should also handle the redirection back to the Switchyard Admin after successful authentication.
...
You can't remove the default email and password login form from the Switchyard Admin. You can only add additional login options through widgets.
...
3. Upon redirection back, you:
    ...
    - If the user doesn't exist in Switchyard, call the [custom API route you created](#step-3-create-user-api-route) to create the user, then refresh the authentication token.
    - Finally, if authentication is successful, navigate to the Orders page of the admin dashboard.

---

## Test Custom Authentication in Switchyard Admin

To test the custom authentication in the Switchyard Admin, run the following command to start your Switchyard application:

```bash npm2yarn
npm run dev
```

Then, open the Switchyard Admin at `http://localhost:9000/app`, which will redirect you to the login page.
...
If authentication is successful, you'll have access to the Switchyard Admin dashboard. Otherwise, you can troubleshoot the issue by checking your Switchyard application's logs or the browser's console for errors.
