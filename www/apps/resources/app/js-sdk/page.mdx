...
export const metadata = {
  title: `Switchyard JS SDK`,
}

# {metadata.title}

In this documentation, you'll learn how to install and use Switchyard's JS SDK.

## What is Switchyard JS SDK?

Switchyard's JS SDK is a library to easily send requests to your Switchyard application. You can use it in your admin customizations or custom storefronts.

---

## How to Install Switchyard JS SDK?

The Switchyard JS SDK is available in your Switchyard application by default. So, you don't need to install it before using it in your admin customizations.

To install the Switchyard JS SDK in other projects, such as a custom storefront, run the following command:
...
- `@switchyard/js-sdk`: the Switchyard JS SDK.
- `@switchyard/types`: Switchyard's types library, which is useful if you're using TypeScript in your development.
...
<CodeTabs group="sdk-project">
  <CodeTab label="Admin (Switchyard project)" value="admin-medusa-project">
...
```
  ...
  <CodeTab label="Admin (Switchyard Plugin)" value="admin-medusa-plugin">
...
In Switchyard Admin customizations that are created in a Switchyard project, you use `import.meta.env` to access environment variables, whereas in customizations built in a Switchyard plugin, you use the global variable `__BACKEND_URL__` to access the backend URL. You can learn more in the [Admin Environment Variables](!docs!/learn/fundamentals/admin/environment-variables) chapter.

</Note>

### JS SDK Configurations

The `Switchyard` initializer accepts as a parameter an object with the following properties:
...
<Table>
  ...
  <Table.Body>
    <Table.Row>
      ...
      A required string indicating the URL to the Switchyard backend.
    ...
    <Table.Row>
      ...
      A string indicating the publishable API key to use in the storefront. You can retrieve it from the Switchyard Admin.
    ...
    <Table.Row>
      ...
      - `custom` to define custom storage using the `auth.storage` configuration. If `auth.storage` isn't defined, the SDK throws an error. This option is only available after Switchyard v2.5.1.
    ...
    <Table.Row>
      ...
      This option is only available after [Switchyard v2.1.1](https://github.com/switchyard/medusa/releases/tag/v2.1.1).
...
The sidebar shows the different methods that you can use to send requests to Switchyard's API routes.
...
Tanstack Query is installed by default in your Switchyard application.
...
import { Button, Container } from "@switchyard/ui"
