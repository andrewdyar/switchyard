...
This component is available after [Switchyard v2.4.0+](https://github.com/switchyard/medusa/releases/tag/v2.4.0).

</Note>

The [DataTable component in Switchyard UI](!ui!/components/data-table) allows you to display data in a table with sorting, filtering, and pagination. It's used across the Switchyard Admin dashboard to showcase a list of items, such as a list of products.

![Example of a table in the product listing page](https://res.cloudinary.com/dza7lstvk/image/upload/v1728295658/Switchyard%20Resources/list_ddt9zc.png)

You can use this component in your Admin Extensions to display data in a table format, especially if you're retrieving them from API routes of the Switchyard application.

This guide focuses on how to use the `DataTable` component while fetching data from the backend. Refer to the [Switchyard UI documentation](!ui!/components/data-table) for detailed information about the DataTable component and its different usages.
...
Start by initializing the columns in the data table. To do that, use the `createDataTableColumnHelper` from Switchyard UI:
...
Next, you'll fetch the products from the Switchyard application. Assuming you have the JS SDK configured as explained in [this guide](../../../js-sdk/page.mdx), add the following imports at the top of the file:
...
You use the `useQuery` hook to fetch the products from the Switchyard application. In the `queryFn`, you call the `sdk.admin.product.list` method to fetch the products. You pass the following query parameters to the method:
...
Next, you'll configure the data table. Switchyard UI provides a `useDataTable` hook that helps you configure the data table. Add the following imports at the top of the file:
...
<TypeList
  types={[
    ...
    { name: "data", type: "`array`", description: "The products fetched from the Switchyard application." },
...
If you start your Switchyard application and go to `localhost:9000/app/custom`, you'll see the data table showing the list of products with pagination, filtering, searching, and sorting functionalities.
...
import { useMemo, useState } from "react"
import { Container } from "../../components/container"
import { HttpTypes, ProductStatus } from "@switchyard/framework/types"
