...
export const ogImage = "https://res.cloudinary.com/dza7lstvk/image/upload/v1734007558/Switchyard%20Resources/integrations-stripe_qfnwtf.jpg"
...
- Set up a Switchyard application with the Stripe Module Provider.
- Customize the Next.js Starter Storefront to use Stripe's Payment Element.

---

## Step 1: Set Up Switchyard Project

In this step, you'll set up a Switchyard application and configure the Stripe Module Provider. You can skip this step if you already have a Switchyard application running with the Stripe Module Provider configured.

### a. Install Switchyard Application
...
To install the Switchyard application, run the following command:
...
Next, you'll configure the [Stripe Module Provider](../../../commerce-modules/payment/payment-provider/stripe/page.mdx) in your Switchyard application. The Stripe Module Provider allows you to accept payments through Stripe in your Switchyard application.
...
The Stripe Module Provider is installed by default in your application. To use it, add it to the array of providers passed to the Payment Module in `switchyard.config.ts`:
...
For more details about other available options and the webhook URLs that Switchyard provides, refer to the [Stripe Module Provider](../../../commerce-modules/payment/payment-provider/stripe/page.mdx) documentation.

### c. Set Environment Variables

Next, make sure to add the necessary environment variables for the above options in `.env` in your Switchyard application:
...
Finally, you need to add Stripe as a payment provider to one or more regions in your Switchyard Admin. This will allow customers to use Stripe as a payment method during checkout.

To do that:

1. Log in to your Switchyard Admin dashboard at `http://localhost:9000/app`.
...
const initStripe = async () => {
  try {
    await initiatePaymentSession(cart, {
      // TODO: change the provider ID if using a different ID in switchyard.config.ts
...
You add an `initStripe` function that initiates a payment session in the Switchyard server. Notice that you set the provider ID to `pp_stripe_stripe`, which is the ID of the Stripe payment provider in your Switchyard application if the ID in `switchyard.config.ts` is `stripe`.
...
You replace the radio group with the payment methods available in Switchyard with the `PaymentElement` component from the Stripe SDK. You pass it the following props:
...
import { CheckCircleSolid, CreditCard } from "@switchyard/icons"
import { Button, Container, Heading, Text, clx } from "@switchyard/ui"
...
- `icon`: A JSX element representing the icon for the payment method. You can use icons from [Switchyard UI](!ui!/icons/overview) or custom icons.
...
At this point, you can proceed to the Review step, but you can't confirm the payment with Stripe and place an order in Switchyard. You'll customize the payment button in the next step to handle the payment confirmation with Stripe.

![The Stripe Payment Element allows you to pay with different payment methods like PayPal](https://res.cloudinary.com/dza7lstvk/image/upload/v1734005395/Switchyard%20Resources/Screenshot_2024-12-10_at_18.04.52_wemqwg.jpg)

---

## Step 3: Update the Payment Button

Next, you'll update the payment button in the Review step of the checkout process. This button is used to confirm the payment with Stripe, then place the order in Switchyard.
...
const paymentSession = cart.payment_collection?.payment_sessions?.find(
  // TODO change the provider_id if using a different ID in switchyard.config.ts
...
- `paymentSession`: The Switchyard payment session for the Stripe payment provider. This is used to get the `clientSecret` needed to confirm the payment.
    - Notice that the provider ID is set to `pp_stripe_stripe`, which is the ID of the Stripe payment provider in your Switchyard application if the ID in `switchyard.config.ts` is `stripe`. If you used a different ID, change the `stripe` in the middle accordingly.
...
Start the Switchyard application and the Next.js Starter Storefront as you did in the previous steps, and proceed to checkout.
...
![Example of handling payment through PayPal](https://res.cloudinary.com/dza7lstvk/image/upload/v1734006541/Switchyard%20Resources/Screenshot_2024-12-10_at_19.27.13_q0w6u1.jpg)
...
![Stripe 3D Secure pop-up](https://res.cloudinary.com/dza7lstvk/image/upload/v1752577384/Switchyard%20Resources/CleanShot_2025-07-15_at_11.12.44_2x_ibjxvx.png)

### Server Webhook Verification

Webhook verification is useful to ensure that payment events are handled despite connection issues. The Stripe Module Provider in Switchyard provides webhook verification out of the box, so you don't need to implement it yourself.
