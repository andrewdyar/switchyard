...
In this guide, you'll learn about the general approach to revalidating cache in the Next.js Starter Storefront when data is updated in the Switchyard application.

## Approach Overview

By default, the data that the Next.js Starter Storefront retrieves from the Switchyard application is cached in the browser. This cache is used to improve the performance and speed of the storefront.

In some cases, you may need to revalidate the cache in the storefront when data is updated in the Switchyard application. For example, when a product variant's price is updated in the Switchyard application, you may want to revalidate the cache in the storefront to reflect the updated price.

You're free to choose the approach that works for your use case, custom requirements, and tech stack. The approach that Switchyard recommends is:

1. Create a [subscriber](!docs!/learn/fundamentals/events-and-subscribers) in the Switchyard application that listens for the event that triggers the data update. For example, you can listen to the `product.updated` event.
...
Refer to the [Events Reference](/references/events) for a full list of events that the Switchyard application emits.
...
Start by creating the following subscriber in the Switchyard application:
...
Make sure to set the `STOREFRONT_URL` environment variable in the Switchyard application to the URL of the Next.js Starter Storefront.
...
To test out this mechanism, run the Switchyard application and the Next.js Starter Storefront.

Then, update a product in the Switchyard application. You can see in the Next.js Starter Storefront's terminal that a request was sent to the `/api/revalidate` endpoint, meaning that the cache was revalidated successfully.