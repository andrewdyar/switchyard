...
import { Github, EllipsisHorizontal } from "@switchyard/icons"
...
Switchyard supports automated Return Merchandise Authorization (RMA) flows for orders. Customers can create return requests for their orders, and merchants can manage these requests through the Switchyard Admin dashboard. Switchyard provides the necessary API routes and workflows to handle returns efficiently.
...
![Request return page in storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1758009041/Switchyard%20Resources/CleanShot_2025-09-16_at_10.50.27_2x_g4rwjr.png)

<Card
  title="Example Repository"
  text="Find the full code of the guide in this repository."
  href="https://github.com/switchyard/examples/tree/main/returns-storefront"
  icon={Github}
/>
...
In this step, you'll add server functions to the Next.js Starter Storefront that let you send requests to the Switchyard application for order returns. You'll use these functions later in the storefront pages.

<Note title="Reminder" forceMultiline>

If you installed the Next.js Starter Storefront with the Switchyard backend, the storefront was installed in a separate directory. The directory's name is `{your-project}-storefront`.

So, if your Switchyard application's directory is `medusa-returns`, you can find the storefront by going back to the parent directory and changing to the `medusa-returns-storefront` directory:
...
![Preview of the return item selector on the request return page](https://res.cloudinary.com/dza7lstvk/image/upload/v1758008962/Switchyard%20Resources/CleanShot_2025-09-16_at_10.48.41_2x_myun8q.png)
...
import { HttpTypes } from "@switchyard/types"
import { Badge, IconButton, Select, Textarea } from "@switchyard/ui"
...
![Preview of the return shipping selector on the request return page](https://res.cloudinary.com/dza7lstvk/image/upload/v1758010574/Switchyard%20Resources/CleanShot_2025-09-16_at_11.13.19_2x_tvxnrq.png)

The component will display the available shipping options for returns, along with their name and price. For shipping options whose price type is calculated by the associated fulfillment provider, the component will retrieve its calculated price from the Switchyard application.
...
import { Loader } from "@switchyard/icons"
...
If a shipping option's price type is `calculated`, you display the calculated price retrieved from the Switchyard application.
...
First, run the following command from the Switchyard application's directory to start the development server:
...
Before you can test the return request page, you need to create return shipping options in the Switchyard Admin. To do that:

1. Open the Switchyard Admin in your browser at `localhost:9000/app` and log in.
...
![Form to create a return shipping option in the Switchyard Admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1758012545/Switchyard%20Resources/CleanShot_2025-09-16_at_11.47.47_2x_fx6tan.png)

#### Create Return Reasons

Next, you need to create return reasons in the Switchyard Admin. To do that:

1. In the Switchyard Admin, go to Settings -> Return Reasons.
...
![Form to create a return reason in the Switchyard Admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1758012657/Switchyard%20Resources/CleanShot_2025-09-16_at_11.50.47_2x_qcrqj6.png)
...
![Form to create a customer account in the Next.js Starter Storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1758012811/Switchyard%20Resources/CleanShot_2025-09-16_at_11.52.58_2x_msuuqx.png)
...
![Order confirmation page in the Next.js Starter Storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1758012923/Switchyard%20Resources/CleanShot_2025-09-16_at_11.55.12_2x_zyat0i.png)

#### Deliver Items in the Order

Finally, you need to deliver the items in the order from the Switchyard Admin to make them returnable. To do that:

1. In the Switchyard Admin, go to Orders.
...
![Order details page in the Switchyard Admin showing the fulfillment status as "Delivered"](https://res.cloudinary.com/dza7lstvk/image/upload/v1758013188/Switchyard%20Resources/CleanShot_2025-09-16_at_11.59.29_2x_tlqaoo.png)
...
![Order details page in the Next.js Starter Storefront showing the "Request Return" button](https://res.cloudinary.com/dza7lstvk/image/upload/v1758013344/Switchyard%20Resources/CleanShot_2025-09-16_at_12.02.03_2x_ehirsj.png)
...
![Request Return page in the Next.js Starter Storefront showing the item and shipping selectors](https://res.cloudinary.com/dza7lstvk/image/upload/v1758013438/Switchyard%20Resources/CleanShot_2025-09-16_at_12.03.41_2x_ksmlpt.png)

You should see a success message after submitting the return request.

If you open the order's details page in the Switchyard Admin, you'll see the requested return in the Activity section. You'll also see a note below the order's items that were requested for return.

![Order details page in the Switchyard Admin showing the requested return in the Activity section](https://res.cloudinary.com/dza7lstvk/image/upload/v1758013557/Switchyard%20Resources/CleanShot_2025-09-16_at_12.05.31_2x_wv9z8o.png)

---

## Next Steps

### Handle Return Requests in the Switchyard Admin

As a merchant, you can now handle return requests from the Switchyard Admin. You can:
...
![Form to mark return items as received in the Switchyard Admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1758013778/Switchyard%20Resources/CleanShot_2025-09-16_at_12.09.23_2x_r07tcb.png)
...
![Button to refund the outstanding amount in the Switchyard Admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1758014212/Switchyard%20Resources/CleanShot_2025-09-16_at_12.16.35_2x_ibmkdq.png)
...
![Button to cancel a return request in the Switchyard Admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1758013651/Switchyard%20Resources/CleanShot_2025-09-16_at_12.07.09_2x_husdak.png)
