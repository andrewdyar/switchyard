...
export const metadata = {
  title: `Switchyard and Module Container Resources`,
}

# {metadata.title}

This reference provides the list of resources registered in the Switchyard container and module containers.
...
Refer to the [Switchyard Container](!docs!/learn/fundamentals/medusa-container) documentation for more information on how to use these resources.

</Note>

<Note title="Tip">

You can use the `ContainerRegistrationKeys` enum imported from `@switchyard/framework/utils` where specified.
...
<Table>
  ...
  <Table.Body>
    <Table.Row>
      ...
      [Switchyard Configurations](!docs!/learn/configurations/switchyard.config)

      </Table.Cell>
      <Table.Cell>

      The configurations defined in `switchyard.config.ts`.
    ...
    <Table.Row>
      ...
      The main service of every module added in `switchyard.config.ts`.

      </Table.Cell>
      <Table.Cell>

      - For custom modules, the registration name is the first parameter of `Module` exported in the `index.ts` file of the module.
      - For Switchyard's Commerce and Infrastructural Modules, use the `Modules` enum imported from `@switchyard/framework/utils`.
...
Use the `ContainerRegistrationKeys` and `Modules` enums imported from `@switchyard/framework/utils` where specified.
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      The service of every data model passed to the `SwitchyardService` function that your module's service extends. The service has the methods described in the [service factory reference](../service-factory-reference/page.mdx).
