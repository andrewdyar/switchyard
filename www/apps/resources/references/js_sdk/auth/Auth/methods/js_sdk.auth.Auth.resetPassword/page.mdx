...
This documentation provides a reference to the `sdk.auth.resetPassword` method used to send requests to Switchyard's Authentication API routes. It can be used for admin users, customers, or custom actor types.

This method requests a reset password token for an admin user, customer, or custom actor type.
It sends a request to the [Generate Reset Password Token API route](https://docs.switchyard.com/api/admin#auth\_postactor\_typeauth\_providerresetpassword).
