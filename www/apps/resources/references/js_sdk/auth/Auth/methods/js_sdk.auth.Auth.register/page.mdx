...
This documentation provides a reference to the `sdk.auth.register` method used to send requests to Switchyard's Authentication API routes. It can be used for admin users, customers, or custom actor types.
...
[Retrieve Registration Token API route](https://docs.switchyard.com/api/store#auth\_postactor\_typeauth\_provider\_register).

Then, it stores the returned token and passes it in the header of subsequent requests. So, you can call the
[store.customer.create](https://docs.switchyard.com/resources/references/js-sdk/store/customer#create) method,
