...
This documentation provides a reference to the `sdk.auth.login` method used to send requests to Switchyard's Authentication API routes. It can be used for admin users, customers, or custom actor types.

This method retrieves the JWT authenticated token for an admin user, customer, or custom
actor type. It sends a request to the [Authenticate API Route](https://docs.switchyard.com/api/admin#auth\_postactor\_typeauth\_provider).
...
[Third-Party Login in Storefront](https://docs.switchyard.com/resources/storefront-development/customers/third-party-login) guide.
...
[Set Authentication Session API route](https://docs.switchyard.com/api/admin#auth\_postsession).
