---
slug: /references/medusa-workflows/markPaymentCollectionAsPaid
sidebar_label: markPaymentCollectionAsPaid
sidebar_description: Mark a payment collection for an order as paid.
tags:
  - logger
  - payment
  - workflow
  - link
  - remote query
  - order
  - event bus
---

import { TypeList, WorkflowDiagram } from "docs-ui"

# markPaymentCollectionAsPaid - Switchyard Core Workflows Reference

This documentation provides a reference to the `markPaymentCollectionAsPaid`. It belongs to the `@switchyard/medusa/core-flows` package.

This workflow marks a payment collection for an order as paid. It's used by the
[Mark Payment Collection as Paid Admin API Route](https://docs.switchyard.com/api/admin#payment-collections\_postpaymentcollectionsidmarkaspaid).

You can use this workflow within your customizations or your own custom workflows, allowing you to wrap custom logic around
marking a payment collection for an order as paid.

<SourceCodeLink link="https://github.com/switchyard/medusa/blob/da5d0e3a1430979ab9cda7f239ea7066ded1f7ab/packages/core/core-flows/src/order/workflows/mark-payment-collection-as-paid.ts#L97" />

## Examples

<CodeTabs group="workflow-exection">
    <CodeTab label="API Route" value="api-route">
    
```ts title="src/api/workflow/route.ts"
import type {
  SwitchyardRequest,
  SwitchyardResponse,
} from "@switchyard/framework/http"
import { markPaymentCollectionAsPaid } from "@switchyard/medusa/core-flows"

export async function POST(
  req: SwitchyardRequest,
  res: SwitchyardResponse
) {
  const { result } = await markPaymentCollectionAsPaid(req.scope)
    .run({
      input: {
        order_id: "order_123",
        payment_collection_id: "paycol_123",
      }
    })

  res.send(result)
}
```

    </CodeTab>
    <CodeTab label="Subscriber" value="subscriber">
    
```ts title="src/subscribers/order-placed.ts"
import {
  type SubscriberConfig,
  type SubscriberArgs,
} from "@switchyard/framework"
import { markPaymentCollectionAsPaid } from "@switchyard/medusa/core-flows"

export default async function handleOrderPlaced({
  event: { data },
  container,
}: SubscriberArgs < { id: string } > ) {
  const { result } = await markPaymentCollectionAsPaid(container)
    .run({
      input: {
        order_id: "order_123",
        payment_collection_id: "paycol_123",
      }
    })

  console.log(result)
}

export const config: SubscriberConfig = {
  event: "order.placed",
}
```

    </CodeTab>
    <CodeTab label="Scheduled Job" value="scheduled-job">
    
```ts title="src/jobs/message-daily.ts"
import { SwitchyardContainer } from "@switchyard/framework/types"
import { markPaymentCollectionAsPaid } from "@switchyard/medusa/core-flows"

export default async function myCustomJob(
  container: SwitchyardContainer
) {
  const { result } = await markPaymentCollectionAsPaid(container)
    .run({
      input: {
        order_id: "order_123",
        payment_collection_id: "paycol_123",
      }
    })

  console.log(result)
}

export const config = {
  name: "run-once-a-day",
  schedule: "0 0 * * *",
}
```

    </CodeTab>
    <CodeTab label="Another Workflow" value="another-workflow">
    
```ts title="src/workflows/my-workflow.ts"
import { createWorkflow } from "@switchyard/framework/workflows-sdk"
import { markPaymentCollectionAsPaid } from "@switchyard/medusa/core-flows"

const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const result = markPaymentCollectionAsPaid
      .runAsStep({
        input: {
          order_id: "order_123",
          payment_collection_id: "paycol_123",
        }
      })
  }
)
```

    </CodeTab>
  </CodeTabs>

## Steps

<WorkflowDiagram workflow={{"name":"markPaymentCollectionAsPaid","steps":[{"type":"step","name":"throwUnlessPaymentCollectionNotPaid","description":"This step validates that the payment collection is not paid. If not valid,\nthe step will throw an error.\n\n:::note\n\nYou can retrieve a payment collection's details using [Query](https://docs.switchyard.com/learn/fundamentals/module-links/query),\nor [useQueryGraphStep](https://docs.switchyard.com/resources/references/medusa-workflows/steps/useQueryGraphStep).\n\n:::","link":"../core_flows.Order.Workflows_Order.throwUnlessPaymentCollectionNotPaid/page.mdx","depth":1},{"type":"workflow","name":"createPaymentSessionsWorkflow","description":"Create payment sessions.","link":"../../../../Payment_Collection/Workflows_Payment_Collection/functions/core_flows.Payment_Collection.Workflows_Payment_Collection.createPaymentSessionsWorkflow/page.mdx","depth":2},{"type":"step","name":"authorizePaymentSessionStep","description":"This step authorizes a payment session.","link":"../../../../Payment/Steps_Payment/functions/core_flows.Payment.Steps_Payment.authorizePaymentSessionStep/page.mdx","depth":3},{"type":"workflow","name":"capturePaymentWorkflow","description":"Capture a payment.","link":"../../../../Payment/Workflows_Payment/functions/core_flows.Payment.Workflows_Payment.capturePaymentWorkflow/page.mdx","depth":4}]}} />

## Input

<TypeList types={[{"name":"MarkPaymentCollectionAsPaidInput","type":"[MarkPaymentCollectionAsPaidInput](../../../../types/core_flows.MarkPaymentCollectionAsPaidInput/page.mdx)","optional":false,"defaultValue":"","description":"The data to mark a payment collection as paid.","expandable":false,"children":[{"name":"payment_collection_id","type":"`string`","description":"The ID of the payment collection to mark as paid.","optional":false,"defaultValue":"","expandable":false,"children":[]},{"name":"order_id","type":"`string`","description":"The ID of the order that the payment collection belongs to.","optional":false,"defaultValue":"","expandable":false,"children":[]},{"name":"captured_by","type":"`string`","description":"The ID of the user marking the payment collection as completed.","optional":true,"defaultValue":"","expandable":false,"children":[]}]}]} expandUrl="https://docs.switchyard.com/learn/fundamentals/data-models/manage-relationships#retrieve-records-of-relation" openedLevel={1} sectionTitle="markPaymentCollectionAsPaid"/>

## Output

<TypeList types={[{"name":"null \\| PaymentDTO","type":"`null` \\| [PaymentDTO](../../../../../payment/interfaces/payment.PaymentDTO/page.mdx)","optional":false,"defaultValue":"","description":"","expandable":false,"children":[{"name":"null \\| PaymentDTO","type":"`null` \\| [PaymentDTO](../../../../../payment/interfaces/payment.PaymentDTO/page.mdx)","optional":false,"defaultValue":"","description":"","expandable":false,"children":[]}]}]} expandUrl="https://docs.switchyard.com/learn/fundamentals/data-models/manage-relationships#retrieve-records-of-relation" openedLevel={1} sectionTitle="markPaymentCollectionAsPaid"/>

## Emitted Events

This section lists the events that are either triggered by the `emitEventStep` in the workflow, or by another workflow executed within this workflow.

:::note

The `emitEventStep` only emits the event after the workflow has finished successfully. So, even if it's executed in the middle of the workflow, it won't actually emit the event until the workflow has completed successfully. If the workflow fails, it won't emit the event at all.

:::

You can listen to these events in a subscriber, as explained in the [Subscribers](https://docs.switchyard.com/learn/fundamentals/events-and-subscribers) documentation.

<Table>
  <Table.Header>
    <Table.Row>
      <Table.HeaderCell>
Event
</Table.HeaderCell>
      <Table.HeaderCell>
Description
</Table.HeaderCell>
      <Table.HeaderCell>
Payload
</Table.HeaderCell>
      <Table.HeaderCell>
Action
</Table.HeaderCell>
    </Table.Row>
  </Table.Header>
  <Table.Body>
    <Table.Row>
      <Table.Cell>
`payment.captured`
</Table.Cell>
      <Table.Cell>
Emitted when a payment is captured.
</Table.Cell>
      <Table.Cell>
```ts blockStyle="inline"
{
  id, // the ID of the payment
}
```
</Table.Cell>
      <Table.Cell>
<CopyGeneratedSnippetButton tooltipText="Copy subscriber for event" type="subscriber" options={{
        event: "payment.captured",
        payload: `\`\`\`ts
{
  id, // the ID of the payment
}
\`\`\``
        }}/>
</Table.Cell>
    </Table.Row>
  </Table.Body>
</Table>
