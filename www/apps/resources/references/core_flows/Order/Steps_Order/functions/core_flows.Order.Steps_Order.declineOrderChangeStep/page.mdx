...
# declineOrderChangeStep - Switchyard Core Workflows Reference

This documentation provides a reference to the `declineOrderChangeStep`. It belongs to the `@switchyard/medusa/core-flows` package.

This step declines an order change.

```ts title="src/workflows/my-workflow.ts"
import { createWorkflow } from "@switchyard/framework/workflows-sdk"
import { declineOrderChangeStep } from "@switchyard/medusa/core-flows"

const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const data = declineOrderChangeStep({
      "id": "id_IAUWMu2DMcBUfZaM5I"
    })
  }
)
```
