...
# updateReturnsStep - Switchyard Core Workflows Reference

This documentation provides a reference to the `updateReturnsStep`. It belongs to the `@switchyard/medusa/core-flows` package.

This step updates one or more returns.

```ts title="src/workflows/my-workflow.ts"
import { createWorkflow } from "@switchyard/framework/workflows-sdk"
import { updateReturnsStep } from "@switchyard/medusa/core-flows"

const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const data = updateReturnsStep({
      "id": "id_10AZadukWwqLezVV"
    })
  }
)
```
