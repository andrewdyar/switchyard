...
# updateCartPromotionsStep - Switchyard Core Workflows Reference

This documentation provides a reference to the `updateCartPromotionsStep`. It belongs to the `@switchyard/medusa/core-flows` package.

This step updates the promotions applied on a cart.

```ts title="src/workflows/my-workflow.ts"
import { createWorkflow } from "@switchyard/framework/workflows-sdk"
import { updateCartPromotionsStep } from "@switchyard/medusa/core-flows"

const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const data = updateCartPromotionsStep({
      "id": "id_3Th2INdBccudYvZ"
    })
  }
)
```
