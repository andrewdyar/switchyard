---
slug: /references/medusa-workflows/steps/removeLineItemAdjustmentsStep
sidebar_label: removeLineItemAdjustmentsStep
tags:
  - cart
  - step
---

import { TypeList, WorkflowDiagram } from "docs-ui"

# removeLineItemAdjustmentsStep - Switchyard Core Workflows Reference

This documentation provides a reference to the `removeLineItemAdjustmentsStep`. It belongs to the `@switchyard/medusa/core-flows` package.

This step removes line item adjustments from a cart.

```ts title="src/workflows/my-workflow.ts"
import { createWorkflow } from "@switchyard/framework/workflows-sdk"
import { removeLineItemAdjustmentsStep } from "@switchyard/medusa/core-flows"

const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const data = removeLineItemAdjustmentsStep({
      "lineItemAdjustmentIdsToRemove": [
        "{value}"
      ]
    })
  }
)
```
  

## Input

<TypeList types={[{"name":"RemoveLineItemAdjustmentsStepInput","type":"[RemoveLineItemAdjustmentsStepInput](../../../../interfaces/core_flows.RemoveLineItemAdjustmentsStepInput/page.mdx)","optional":false,"defaultValue":"","description":"The details of the line item adjustments to remove.","expandable":false,"children":[{"name":"lineItemAdjustmentIdsToRemove","type":"`string`[]","description":"The IDs of the line item adjustments to remove.","optional":false,"defaultValue":"","expandable":false,"children":[]}]}]} expandUrl="https://docs.switchyard.com/learn/fundamentals/data-models/manage-relationships#retrieve-records-of-relation" openedLevel={1} sectionTitle="removeLineItemAdjustmentsStep"/>
