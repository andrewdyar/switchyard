...
# createShippingOptionsPriceSetsStep - Switchyard Core Workflows Reference

This documentation provides a reference to the `createShippingOptionsPriceSetsStep`. It belongs to the `@switchyard/medusa/core-flows` package.
...
import { createShippingOptionsPriceSetsStep } from "@switchyard/medusa/core-flows"

const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const data = createShippingOptionsPriceSetsStep({
      "id": "id_KsCIjDU4aJoDEmC",
      "prices": [{
        "currency_code": "ars",
        "amount": 37
      }]
