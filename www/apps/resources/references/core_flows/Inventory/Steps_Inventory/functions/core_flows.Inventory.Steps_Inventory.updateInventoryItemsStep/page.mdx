...
# updateInventoryItemsStep - Switchyard Core Workflows Reference

This documentation provides a reference to the `updateInventoryItemsStep`. It belongs to the `@switchyard/medusa/core-flows` package.

This step updates one or more inventory items.

```ts title="src/workflows/my-workflow.ts"
import { createWorkflow } from "@switchyard/framework/workflows-sdk"
import { updateInventoryItemsStep } from "@switchyard/medusa/core-flows"

const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const data = updateInventoryItemsStep({
      "id": "id_ckVyYPgE0KWM7T"
    })
  }
)
```
