...
# updateRefundReasonsStep - Switchyard Core Workflows Reference

This documentation provides a reference to the `updateRefundReasonsStep`. It belongs to the `@switchyard/medusa/core-flows` package.

This step updates one or more refund reasons.

```ts title="src/workflows/my-workflow.ts"
import { createWorkflow } from "@switchyard/framework/workflows-sdk"
import { updateRefundReasonsStep } from "@switchyard/medusa/core-flows"

const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const data = updateRefundReasonsStep({
      "id": "id_e0lV6HG6ghsLl3BuPtpJ"
    })
  }
)
```
