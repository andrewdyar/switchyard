---
slug: /references/medusa-workflows/deleteRefundReasonsWorkflow
sidebar_label: deleteRefundReasonsWorkflow
sidebar_description: Delete refund reasons.
tags:
  - payment
  - workflow
---

import { TypeList, WorkflowDiagram } from "docs-ui"

# deleteRefundReasonsWorkflow - Switchyard Core Workflows Reference

This documentation provides a reference to the `deleteRefundReasonsWorkflow`. It belongs to the `@switchyard/medusa/core-flows` package.

This workflow deletes one or more refund reasons. It's used by the
[Delete Refund Reason Admin API Route](https://docs.switchyard.com/api/admin#refund-reasons\_deleterefundreasonsid).

You can use this workflow within your own customizations or custom workflows, allowing you
to delete refund reasons in your custom flows.

<SourceCodeLink link="https://github.com/switchyard/medusa/blob/da5d0e3a1430979ab9cda7f239ea7066ded1f7ab/packages/core/core-flows/src/payment-collection/workflows/delete-refund-reasons.ts#L38" />

## Examples

<CodeTabs group="workflow-exection">
    <CodeTab label="API Route" value="api-route">
    
```ts title="src/api/workflow/route.ts"
import type {
  SwitchyardRequest,
  SwitchyardResponse,
} from "@switchyard/framework/http"
import { deleteRefundReasonsWorkflow } from "@switchyard/medusa/core-flows"

export async function POST(
  req: SwitchyardRequest,
  res: SwitchyardResponse
) {
  const { result } = await deleteRefundReasonsWorkflow(req.scope)
    .run({
      input: {
        ids: ["refres_123"]
      }
    })

  res.send(result)
}
```

    </CodeTab>
    <CodeTab label="Subscriber" value="subscriber">
    
```ts title="src/subscribers/order-placed.ts"
import {
  type SubscriberConfig,
  type SubscriberArgs,
} from "@switchyard/framework"
import { deleteRefundReasonsWorkflow } from "@switchyard/medusa/core-flows"

export default async function handleOrderPlaced({
  event: { data },
  container,
}: SubscriberArgs < { id: string } > ) {
  const { result } = await deleteRefundReasonsWorkflow(container)
    .run({
      input: {
        ids: ["refres_123"]
      }
    })

  console.log(result)
}

export const config: SubscriberConfig = {
  event: "order.placed",
}
```

    </CodeTab>
    <CodeTab label="Scheduled Job" value="scheduled-job">
    
```ts title="src/jobs/message-daily.ts"
import { SwitchyardContainer } from "@switchyard/framework/types"
import { deleteRefundReasonsWorkflow } from "@switchyard/medusa/core-flows"

export default async function myCustomJob(
  container: SwitchyardContainer
) {
  const { result } = await deleteRefundReasonsWorkflow(container)
    .run({
      input: {
        ids: ["refres_123"]
      }
    })

  console.log(result)
}

export const config = {
  name: "run-once-a-day",
  schedule: "0 0 * * *",
}
```

    </CodeTab>
    <CodeTab label="Another Workflow" value="another-workflow">
    
```ts title="src/workflows/my-workflow.ts"
import { createWorkflow } from "@switchyard/framework/workflows-sdk"
import { deleteRefundReasonsWorkflow } from "@switchyard/medusa/core-flows"

const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const result = deleteRefundReasonsWorkflow
      .runAsStep({
        input: {
          ids: ["refres_123"]
        }
      })
  }
)
```

    </CodeTab>
  </CodeTabs>

## Steps

<WorkflowDiagram workflow={{"name":"deleteRefundReasonsWorkflow","steps":[{"type":"step","name":"deleteRefundReasonsStep","description":"This step deletes one or more refund reasons.","link":"../../../Steps_Payment_Collection/functions/core_flows.Payment_Collection.Steps_Payment_Collection.deleteRefundReasonsStep/page.mdx","depth":1}]}} />

## Input

<TypeList types={[{"name":"DeleteRefundReasonsWorkflowInput","type":"[DeleteRefundReasonsWorkflowInput](../../../../types/core_flows.DeleteRefundReasonsWorkflowInput/page.mdx)","optional":false,"defaultValue":"","description":"The data to delete refund reasons.","expandable":false,"children":[{"name":"ids","type":"`string`[]","description":"The refund reasons to delete.","optional":false,"defaultValue":"","expandable":false,"children":[]}]}]} expandUrl="https://docs.switchyard.com/learn/fundamentals/data-models/manage-relationships#retrieve-records-of-relation" openedLevel={1} sectionTitle="deleteRefundReasonsWorkflow"/>
