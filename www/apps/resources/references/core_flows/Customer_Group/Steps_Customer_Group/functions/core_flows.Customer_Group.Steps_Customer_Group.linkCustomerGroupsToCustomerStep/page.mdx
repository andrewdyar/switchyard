---
slug: /references/medusa-workflows/steps/linkCustomerGroupsToCustomerStep
sidebar_label: linkCustomerGroupsToCustomerStep
tags:
  - customer
  - step
---

import { TypeList, WorkflowDiagram } from "docs-ui"

# linkCustomerGroupsToCustomerStep - Switchyard Core Workflows Reference

This documentation provides a reference to the `linkCustomerGroupsToCustomerStep`. It belongs to the `@switchyard/medusa/core-flows` package.

This step manages the customer groups of a customer.

## Example

```ts
const data = linkCustomerGroupsToCustomerStep({
  id: "cus_123",
  add: ["cusgrp_123"],
  remove: ["cusgrp_456"]
})
```

## Input

<TypeList types={[{"name":"LinkWorkflowInput","type":"[LinkWorkflowInput](../../../../../types/CommonTypes/types/types.CommonTypes.LinkWorkflowInput/page.mdx)","optional":false,"defaultValue":"","description":"","expandable":false,"children":[{"name":"id","type":"`string`","description":"The ID of the data model to create links to or remove links from.","optional":false,"defaultValue":"","expandable":false,"children":[]},{"name":"add","type":"`string`[]","description":"The IDs of the other data models to create links to the record specified in `id`.","optional":true,"defaultValue":"","expandable":false,"children":[]},{"name":"remove","type":"`string`[]","description":"The IDs of the other data models to remove links from the record specified in `id`.","optional":true,"defaultValue":"","expandable":false,"children":[]}]}]} expandUrl="https://docs.switchyard.com/learn/fundamentals/data-models/manage-relationships#retrieve-records-of-relation" openedLevel={1} sectionTitle="linkCustomerGroupsToCustomerStep"/>
