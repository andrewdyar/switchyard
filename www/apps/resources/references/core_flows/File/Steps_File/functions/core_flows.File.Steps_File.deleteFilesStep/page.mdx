---
slug: /references/medusa-workflows/steps/deleteFilesStep
sidebar_label: deleteFilesStep
tags:
  - file
  - step
---

import { TypeList, WorkflowDiagram } from "docs-ui"

# deleteFilesStep - Switchyard Core Workflows Reference

This documentation provides a reference to the `deleteFilesStep`. It belongs to the `@switchyard/medusa/core-flows` package.

This step deletes one or more files using the installed
[File Module Provider](https://docs.switchyard.com/resources/infrastructure-modules/file). The files
will be removed from the database and the storage.

## Example

```ts
const data = deleteFilesStep([
  "id_123"
])
```

## Input

<TypeList types={[{"name":"DeleteFilesStepInput","type":"[DeleteFilesStepInput](../../../../types/core_flows.DeleteFilesStepInput/page.mdx)","optional":false,"defaultValue":"","description":"The IDs of the files to delete.","expandable":false,"children":[{"name":"DeleteFilesStepInput","type":"`string`[]","description":"The IDs of the files to delete.","optional":false,"defaultValue":"","expandable":false,"children":[]}]}]} expandUrl="https://docs.switchyard.com/learn/fundamentals/data-models/manage-relationships#retrieve-records-of-relation" openedLevel={1} sectionTitle="deleteFilesStep"/>
