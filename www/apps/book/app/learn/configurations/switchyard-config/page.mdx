...
export const metadata = {
  title: `${pageNumber} Switchyard Application Configuration`,
}

# {metadata.title}

In this chapter, you'll learn available configurations in the Switchyard application. You can change the application's configurations to customize the behavior of the application, its integrated modules and plugins, and more.

## Configuration File

All configurations of the Switchyard application are stored in the `switchyard.config.ts` file. The file exports an object created using the `defineConfig` utility. For example:
...
- [projectConfig](#project-configurations-projectConfig): Essential configurations related to the Switchyard application, such as database and CORS configurations.
- [admin](#admin-configurations-admin): Configurations related to the Switchyard Admin.
...
- [featureFlags](#feature-flags-featureFlags): Configurations to manage enabled beta features in the Switchyard application.
...
The `projectConfig` object contains essential configurations related to the Switchyard application, such as database and CORS configurations.

### cookieOptions

<Note>

This option is available since Switchyard [v2.8.5](https://github.com/switchyard/medusa/releases/tag/v2.8.5).
...
After setting the database credentials, you can create and setup the database using the [db:setup](!resources!/switchyard-cli/commands/db#dbsetup) command of the Switchyard CLI.
...
After setting the database URL, you can create and setup the database using the [db:setup](!resources!/switchyard-cli/commands/db#dbsetup) command of the Switchyard CLI.
...
- Or a regular expression pattern that can match more than one origin. For example, `.example.com`. The regex pattern that Switchyard tests for is `^([\/~@;%#'])(.*?)\1([gimsuy]*)$`.
...
If you enable HTTP compression and you want to disable it for specific API Routes, you can pass in the request header `"x-no-compression": true`. Learn more in the [API Reference](https://docs.switchyard.com/api/store#http-compression).
...
The `projectConfig.redisOptions` configuration defines options to pass to `ioredis`, which creates the Redis connection used to store the Switchyard server session. Refer to [ioredis’s RedisOptions documentation](https://redis.github.io/ioredis/index.html#RedisOptions)
...
The `projectConfig.redisPrefix` configuration defines a prefix on all keys stored in Redis for the Switchyard server session. The default value is `sess:`.
...
The `projectConfig.redisUrl` configuration specifies the connection URL to Redis to store the Switchyard server session. When specified, the Switchyard server uses Redis to store the session data. Otherwise, the session data is stored in-memory.
...
The `projectConfig.sessionOptions` configuration defines additional options to pass to [express-session](https://www.npmjs.com/package/express-session), which is used to store the Switchyard server session.
...
The `projectConfig.workerMode` configuration specifies the worker mode of the Switchyard application. You can learn more about it in the [Worker Mode chapter](../../production/worker-mode/page.mdx).
...
The `admin` object contains configurations related to the Switchyard Admin.

### backendUrl

The `admin.backendUrl` configuration specifies the URL of the Switchyard application. Its default value is the browser origin. This is useful to set when running the admin on a separate domain.
...
The `admin.disable` configuration specifies whether to disable the Switchyard Admin. If disabled, the Switchyard Admin will not be compiled and you can't access it at `/app` path of your application. The default value is `false`.
...
The `admin.storefrontUrl` configuration specifies the URL of the Switchyard storefront application. This URL is used as a prefix to some links in the admin that require performing actions in the storefront.

For example, this URL is used as a prefix to shareable payment links for orders with outstanding amounts.

#### Example

```js title="switchyard.config.js"
module.exports = defineConfig({
  admin: {
...
The `admin.vite` configration specifies Vite configurations for the Switchyard Admin. Its value is a function that receives the default Vite configuration and returns the modified configuration. The default value is `undefined`.
...
The `modules` configuration allows you to register and configure the [modules](../../fundamentals/modules/page.mdx) registered in the Switchyard application. Switchyard's commerce and Infrastructure Modules are configured by default. So, you only need to pass your custom modules, or override the default configurations of the existing modules.
...
You can also override the default configurations of Switchyard's modules. For example, to add a Notification Module Provider to the Notification Module:
...
The `plugins` configuration allows you to register and configure the [plugins](../../fundamentals/plugins/page.mdx) registered in the Switchyard application. Plugins include re-usable Switchyard customizations, such as modules, workflows, API routes, and more.

Aside from installing the plugin with NPM, you must also register it in the `switchyard.config.ts` file.
...
When you register a plugin, its modules are automatically registered in the Switchyard application. You don't have to register them manually in the `modules` configuration.
...
The `featureFlags` configuration allows you to manage enabled beta features in the Switchyard application.
...
The `logger` configuration allows you to override the default [Logger](../../debugging-and-testing/logging/page.mdx) used in the Switchyard application with your custom implementation.
