...
In this chapter, you'll learn what feature flags are, what the available feature flags in Switchyard are, and how to toggle them.
...
Switchyard uses feature flags to ship new versions even when some features are not fully ready. This approach allows our team to continuously deliver updates and improvements while stabilizing new features.

---

## Available Feature Flags in Switchyard

For a list of features hidden behind a feature flag in Switchyard, check out the [feature-flags](https://github.com/switchyard/medusa/tree/develop/packages/medusa/src/feature-flags) directory in the `@switchyard/medusa` package.

---

## Toggle Feature Flags

There are multiple ways to enable or disable feature flags in Switchyard:

- In the `switchyard.config.ts` file;
- Or using environment variables.

### 1. Using the `switchyard.config.ts` file

The Switchyard configurations in `switchyard.config.ts` accept a `featureFlags` configuration to toggle feature flags. Its value is an object whose key is the feature flag key (defined in the feature flag's file), and the value is a boolean indicating whether the feature is enabled.

For example, to enable the [Index Module](../../fundamentals/module-links/index-module/page.mdx)'s feature flag in `switchyard.config.ts`:
