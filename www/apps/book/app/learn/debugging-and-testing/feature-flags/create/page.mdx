...
In this chapter, you'll learn how to create a custom feature flag in Switchyard.

## Why Create a Custom Feature Flag?

As explained in the [Feature Flags](../page.mdx) chapter, feature flags allow the Switchyard team to ship new features that are still under development and testing, but not ready for production or wide use yet.

You can also create custom feature flags for your Switchyard project or plugin to control the availability of experimental or in-development features. Feature flags allow you to test features in staging, and only enable them in production when they are ready.
...
To create a custom feature flag, you need to define it in a new file under the `src/feature-flags` directory of your Switchyard project or plugin. The file must export a configuration object.
...
To enable or disable your custom feature flag, you can either add it to your `switchyard.config.ts` file:
...
Then, the Switchyard application will load your customizations hidden behind the feature flag as part of the integration tests.