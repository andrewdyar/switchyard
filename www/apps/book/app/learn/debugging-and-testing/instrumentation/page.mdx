...
In this chapter, you'll learn about observability in Switchyard and how to configure instrumentation with OpenTelemetry.
...
Switchyard uses [OpenTelemetry](https://opentelemetry.io/) for instrumentation and reporting. When configured, it reports traces for:
...
## How to Configure Instrumentation in Switchyard?
...
As of [Switchyard v2.11.0](https://github.com/switchyard/medusa/releases/tag/v2.11.0), OpenTelemetry dependencies are installed by default in new Switchyard projects. If you're using an older version of Switchyard, you need to install the `@opentelemetry/sdk-node`, `@opentelemetry/resources`, `@opentelemetry/sdk-trace-node`, and `@opentelemetry/instrumentation-pg` dependencies.
...
In the `instrumentation.ts` file, you export a `register` function that uses Switchyard's `registerOtel` utility function. You also initialize an instance of the exporter, such as Zipkin, and pass it to the `registerOtel` function.
...
The `NodeSDKConfiguration` properties are accepted since Switchyard v2.5.1.
...
Then, start your Switchyard application:

```bash npm2yarn
npm run dev
```

Try to open the Switchyard Admin or send a request to an API route.
...
- [Integrate Sentry with Switchyard](!resources!/integrations/guides/sentry)