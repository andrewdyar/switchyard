...
In this chapter, you’ll learn how to use Switchyard’s logging utility.

## Logger Class

Switchyard provides a `Logger` class with advanced logging functionalities. This includes configuring logging levels or saving logs to a file.
...
The Switchyard application registers the `Logger` class in the Switchyard container and each module's container as `logger`. 

---

## How to Log a Message

Resolve the `logger` using the Switchyard container to log a message in your resource.
...
This creates a scheduled job that resolves the `logger` from the Switchyard container and uses it to log a message.

### Test the Scheduled Job

To test out the above scheduled job, start the Switchyard application:
...
Aside from showing the logs in the terminal, you can save the logs in a file by setting the `LOG_FILE` environment variable to the path of the file relative to the Switchyard server’s root directory.
...
Your logs are now saved in the `all.log` file at the root of your Switchyard application.
...
The `Logger` class has an `activity` method used to log a message of level `info`. If the Switchyard application is running in a development environment, a spinner starts to show the activity's progress.
