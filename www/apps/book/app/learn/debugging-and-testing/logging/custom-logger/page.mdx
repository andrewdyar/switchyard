...
In this guide, you'll learn how to override the default [Logger](../page.mdx) in Switchyard with your custom implementation.

## Why Override the Logger?

Switchyard's default `Logger` logs your application's events, errors, and general log messages into the console. However, you might want to customize this behavior for your specific use case.
...
After overriding the `Logger`, Switchyard will use your custom logger whenever it needs to log a message.
...
1. Create a class that extends the `Logger` interface from `@switchyard/framework/types`.
2. Pass the class in the Switchyard configurations.
...
Notice that you must export an instance of your custom logger class to use it in the Switchyard configurations.
...
### Step 2: Pass the Custom Logger in Switchyard Configurations

Next, to use the custom logger, set the `logger` configuration in your `switchyard.config.ts` file:
...
To test that your custom logger is working, start the Switchyard application:
