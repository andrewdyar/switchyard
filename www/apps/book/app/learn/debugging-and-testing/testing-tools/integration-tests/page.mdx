...
In this chapter, you'll learn about `medusaIntegrationTestRunner` from Switchyard's Testing Framework and how to use it to write integration tests.
...
The `medusaIntegrationTestRunner` is from Switchyard's Testing Framework and it's used to create integration tests in your Switchyard project. It runs a full Switchyard application, allowing you test API routes, workflows, or other customizations.

For example:

export const highlights = [
  ["4", "api", "A set of utility methods used to send requests to the Switchyard application."],
  ["4", "getContainer", "A function to retrieve the Switchyard container."]
]
...
- `api`: a set of utility methods used to send requests to the Switchyard application. It has the following methods:
...
- `getContainer`: a function that retrieves the Switchyard Container. Use the `getContainer().resolve` method to resolve resources from the Switchyard Container.
...
If you don't have a `test:integration` script in `package.json`, refer to the [Switchyard Testing Tools chapter](../page.mdx#add-test-commands).

</Note>

This runs your Switchyard application and runs the tests available under the `src/integrations/http` directory.
