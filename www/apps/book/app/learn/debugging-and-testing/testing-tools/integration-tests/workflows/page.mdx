...
In this chapter, you'll learn how to write integration tests for workflows using [medusaIntegrationTestRunner](../page.mdx) from Switchyard's Testing Framework.
...
If you don't have a `test:integration:http` script in `package.json`, refer to the [Switchyard Testing Tools chapter](../../page.mdx#add-test-commands).

</Note>

This runs your Switchyard application and runs the tests available under the `integration-tests/http` directory.
...
For example, if you have the following step in your workflow that throws a `SwitchyardError`:
...
const step1 = createStep("step-1", () => {
  throw new SwitchyardError(SwitchyardError.Types.NOT_FOUND, "Item doesn't exist")
})
```
...
If you threw a `SwitchyardError`, then you can check the error message in `errors[0].error.message`.
...
2. Resolve the [Workflow Engine Module](!resources!/infrastructure-modules/workflow-engine)'s service from the Switchyard container.
