...
In the previous chapters, you [defined a link](../define-link/page.mdx) between the [custom Brand Module](../../custom-features/module/page.mdx) and Switchyard's [Product Module](!resources!/commerce-modules/product), then [extended the create-product flow](../extend-create-product/page.mdx) to link a product to a brand.

In this chapter, you'll learn how to retrieve a product's brand (and vice-versa) in two ways: Using Switchyard's existing API route, or in customizations, such as a custom API route.
...
Switchyard's existing API routes accept a `fields` query parameter that allows you to specify the fields and relations of a model to retrieve. So, when you send a request to the [List Products](!api!/admin#products_getproducts), [Get Product](!api!/admin#products_getproductsid), or any product-related store or admin routes that accept a `fields` query parameter, you can specify in this parameter to return the product's brands.

<Note>

Learn more about using the `fields` query parameter to retrieve custom linked data models in the [Retrieve Custom Linked Data Models from Switchyard's API Routes](../../../fundamentals/api-routes/retrieve-custom-links/page.mdx) chapter.
...
You can also retrieve linked records using Query. Query allows you to retrieve data across modules with filters, pagination, and more. You can resolve Query from the Switchyard container and use it in your API route or workflow.
...
export const highlights = [
  ["11", "resolve", "Resolve Query from the Switchyard container."],
...
This adds a `GET` API route at `/admin/brands`. In the API route, you resolve Query from the Switchyard container. Query has a `graph` method that runs a query to retrieve data. It accepts an object having the following properties:
...
## Next Steps: Customize Switchyard Admin

Clients, such as the Switchyard Admin dashboard, can now use brand-related features, such as creating a brand or setting the brand of a product.

In the next chapters, you'll learn how to customize the Switchyard Admin to show a product's brand on its details page, and to show a new page with the list of brands in your store.
