...
After linking the [custom Brand data model](../../custom-features/module/page.mdx) and Switchyard's [Product Module](!resources!/commerce-modules/product) in the [previous chapter](../define-link/page.mdx), you'll extend the create product workflow and API route to allow associating a brand with a product.
...
![Directory structure after creating the hook's file.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733384338/Switchyard%20Book/brands-hook-dir-overview_ltwr5h.jpg)
...
2. An object of properties related to the step's context. It has a `container` property whose value is the [Switchyard container](../../../fundamentals/medusa-container/page.mdx) to resolve Framework and commerce tools.
...
![Diagram showcasing how the order of defining a link affects creating the link](https://res.cloudinary.com/dza7lstvk/image/upload/v1733386156/Switchyard%20Book/remote-link-brand-product-exp_fhjmg4.jpg)
...
![Directory structure after adding the middelwares file](https://res.cloudinary.com/dza7lstvk/image/upload/v1733386868/Switchyard%20Book/brands-middleware-dir-overview_uczos1.jpg)
...
Make sure to replace `{token}` with the token you received from the previous request, `shipping_profile_id` with the ID of a shipping profile in your application, and `{brand_id}` with the ID of a brand in your application. You can retrieve the ID of a shipping profile either from the Switchyard Admin, or the [List Shipping Profiles API route](!api!/admin#shipping-profiles_getshippingprofiles).

The request creates a product and returns it.

In the Switchyard application's logs, you'll find the message `Linked brand to products`, indicating that the workflow hook handler ran and linked the brand to the products.
