...
An API Route is an endpoint that acts as an entry point for other clients to interact with your Switchyard customizations, such as the admin dashboard, storefronts, or third-party systems.

The Switchyard core application provides a set of [admin](!api!/admin) and [store](!api!/store) API routes out-of-the-box. You can also create custom API routes to expose your custom functionalities.
...
![Directory structure of the Switchyard application after adding the route](https://res.cloudinary.com/dza7lstvk/image/upload/v1732869882/Switchyard%20Book/brand-route-dir-overview-2_hjqlnf.jpg)
...
export const POST = async (
  req: SwitchyardRequest<PostAdminCreateBrandType>,
...
The function receives two parameters: a `SwitchyardRequest` object to access request details, and `SwitchyardResponse` object to return or manipulate the response. The `SwitchyardRequest` object's `scope` property is the [Switchyard container](../../../fundamentals/medusa-container/page.mdx) that holds Framework tools and custom and core modules' services.

<Note title="Tip">

`SwitchyardRequest` accepts the request body's type as a type argument.

</Note>

In the API route's handler, you execute the `createBrandWorkflow` by invoking it and passing the Switchyard container `req.scope` as a parameter, then invoking its `run` method. You pass the workflow's input in the `input` property of the `run` method's parameter. You pass the request body's parameters using the `validatedBody` property of `SwitchyardRequest`.
...
Switchyard uses [Zod](https://zod.dev/) to create validation schemas. These schemas are then used to validate incoming request bodies or query parameters.
...
![Directory structure of Switchyard application after adding validators file](https://res.cloudinary.com/dza7lstvk/image/upload/v1732869806/Switchyard%20Book/brand-route-dir-overview-1_yfyjss.jpg)
...
Switchyard provides a `validateAndTransformBody` middleware that accepts a Zod validation schema and returns a response error if a request is sent with body parameters that don't satisfy the validation schema.
...
![Directory structure of the Switchyard application after adding the middleware](https://res.cloudinary.com/dza7lstvk/image/upload/v1732869977/Switchyard%20Book/brand-route-dir-overview-3_kcx511.jpg)
...
The Switchyard application will now validate the body parameters of `POST` requests sent to `/admin/brands` to ensure they match the Zod validation schema. If not, an error is returned in the response specifying the issues to fix in the request body.

---

## Test API Route

To test out the API route, start the Switchyard application with the following command:
...
Now that you have brands in your Switchyard application, you want to associate a brand with a product, which is defined in the [Product Module](!resources!/commerce-modules/product).
