...
![Directory structure in the Switchyard project after adding the file for createBrandStep](https://res.cloudinary.com/dza7lstvk/image/upload/v1732869184/Switchyard%20Book/brand-workflow-dir-overview-1_fjvf5j.jpg)
...
The step function receives two parameters: input passed to the step when it's invoked, and an object of general context and configurations. This object has a `container` property, which is the Switchyard container.

The [Switchyard container](../../../fundamentals/medusa-container/page.mdx) is a registry of Framework and commerce tools accessible in your customizations, such as a workflow's step. The Switchyard application registers the services of core and custom modules in the container, allowing you to resolve and use them.

So, In the step function, you use the Switchyard container to resolve the Brand Module's service and use its generated `createBrands` method, which accepts an object of brands to create.
...
In the compensation function, you resolve the Brand Module's service from the Switchyard container, then use its generated `deleteBrands` method to delete the brand created by the step. This method accepts the ID of the brand to delete.
