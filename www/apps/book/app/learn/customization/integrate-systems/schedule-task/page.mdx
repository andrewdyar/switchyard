...
In the previous chapters, you've [integrated a third-party CMS](../service/page.mdx) and implemented the logic to [sync created brands](../handle-event/page.mdx) from Switchyard to the CMS.

However, when you integrate a third-party system, you want the data to be in sync between the Switchyard application and the system. One way to do so is by automatically syncing the data once a day.

You can create an action to be automatically executed at a specified interval using scheduled jobs. A scheduled job is an asynchronous function with a specified schedule of when the Switchyard application should run it. Scheduled jobs are useful to automate repeated tasks.
...
In this chapter, you'll create a scheduled job that triggers syncing the brands from the third-party CMS to Switchyard once a day. You'll implement the syncing logic in a workflow, and execute that workflow in the scheduled job.
...
2. `createBrandsStep` to create the brands retrieved in the first step that don't exist in Switchyard.
3. `updateBrandsStep` to update the brands retrieved in the first step that exist in Switchyard.
...
![Directory structure of the Switchyard application after creating the file.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733494196/Switchyard%20Book/cms-dir-overview-6_z1omsi.jpg)
...
The brands retrieved in the first step may have brands that don't exist in Switchyard. So, you'll create a step that creates those brands. Add the step to the same `src/workflows/sync-brands-from-cms.ts` file:

export const createBrandsHighlights = [
  ["22", "createBrands", "Create the brands in Switchyard"],
  ["35", "deleteBrands", "Delete the brands from Switchyard"]
]
...
The brands retrieved in the first step may also have brands that exist in Switchyard. So, you'll create a step that updates their details to match that of the CMS. Add the step to the same `src/workflows/sync-brands-from-cms.ts` file:

export const updateBrandsHighlights = [
  ["19", "prevUpdatedBrands", "Retrieve the data of the brands before the update."],
  ["23", "updateBrands", "Update the brands in Switchyard."],
...
The `updateBrandsStep` receives the brands to update in Switchyard. In the step, you retrieve the brand's details in Switchyard before the update to pass them to the compensation function. You then update the brands using the Brand Module's `updateBrands` generated method.
...
Finally, you'll create the workflow that uses the above steps to sync the brands from the CMS to Switchyard. Add to the same `src/workflows/sync-brands-from-cms.ts` file the following:
...
In `transform`'s function, you loop over the brands array to check which should be created or updated. This logic assumes that a brand in the CMS has an `external_id` property whose value is the brand's ID in Switchyard.
...
You first run the `createBrandsStep` to create the brands that don't exist in Switchyard, then the `updateBrandsStep` to update the brands that exist in Switchyard. You pass the arrays returned by `transform` as the inputs for the steps.
...
You now have the workflow to sync the brands from the CMS to Switchyard. Next, you'll create a scheduled job that runs this workflow once a day to ensure the data between Switchyard and the CMS are always in sync.
...
![Directory structure of the Switchyard application after adding the scheduled job](https://res.cloudinary.com/dza7lstvk/image/upload/v1733494592/Switchyard%20Book/cms-dir-overview-7_dkjb9s.jpg)
...
export default async function (container: SwitchyardContainer) {
...
The scheduled job function accepts as a parameter the [Switchyard container](../../../fundamentals/medusa-container/page.mdx) used to resolve Framework and commerce tools. You then execute the `syncBrandsFromCmsWorkflow` and use its result to log how many brands were created or updated.

Based on the cron expression specified in `config.schedule`, Switchyard will run the scheduled job every day at midnight. You can also change it to `* * * * *` to run it every minute for easier debugging.
...
By following the previous chapters, you utilized the Switchyard Framework and orchestration tools to perform and automate tasks that span across systems.

With Switchyard, you can integrate any service from your commerce ecosystem with ease. You don't have to set up separate applications to manage your different customizations, or worry about data inconsistency across systems. Your efforts only go into implementing the business logic that ties your systems together.
