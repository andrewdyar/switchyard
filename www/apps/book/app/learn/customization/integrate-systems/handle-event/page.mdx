...
export const metadata = {
  title: `${pageNumber} Guide: Sync Brands from Switchyard to Third-Party`,
}
...
Switchyard has an event system that emits events when an operation is performed. It allows you to listen to those events and perform an asynchronous action in a function called a [subscriber](../../../fundamentals/events-and-subscribers/page.mdx). This is useful to perform actions that aren't integral to the original flow, such as syncing data to a third-party system.

<Note>

Learn more about Switchyard's event system and subscribers in [this chapter](../../../fundamentals/events-and-subscribers/page.mdx).
...
Switchyard provides an `emitEventStep` that allows you to emit an event in your workflows. So, in the `createBrandWorkflow` defined in `src/workflows/create-brand.ts`, use the `emitEventStep` helper step after the `createBrandStep`:
...
- `useQueryGraphStep`: a step that Switchyard provides to retrieve data using [Query](../../../fundamentals/module-links/query/page.mdx). You'll use this to retrieve the brand's details using its ID.
...
![Directory structure of the Switchyard application after adding the file](https://res.cloudinary.com/dza7lstvk/image/upload/v1733493547/Switchyard%20Book/cms-dir-overview-4_u5t0ug.jpg)
...
You create the `syncBrandToCmsStep` that accepts a brand as an input. In the step, you resolve the CMS Module's service from the [Switchyard container](../../../fundamentals/medusa-container/page.mdx) and use its `createBrand` method. This method will create the brand in the third-party CMS.
...
![Directory structure of the Switchyard application after adding the subscriber](https://res.cloudinary.com/dza7lstvk/image/upload/v1733493774/Switchyard%20Book/cms-dir-overview-5_iqqwvg.jpg)
...
export const subscriberHighlights = [
  ...
  ["9", "container", "The Switchyard container used to resolve resources."],
...
- `container`: The Switchyard container used to resolve Framework and commerce tools.

In the function, you execute the `syncBrandToCmsWorkflow`, passing it the data payload as an input. So, everytime a brand is created, Switchyard will execute this function, which in turn executes the workflow to sync the brand to the CMS.
...
## Next Chapter: Sync Brand from Third-Party CMS to Switchyard

You can also automate syncing data from a third-party system to Switchyard at a regular interval. In the next chapter, you'll learn how to sync brands from the third-party CMS to Switchyard once a day.
