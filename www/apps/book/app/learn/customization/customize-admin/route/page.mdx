...
Switchyard provides a `validateAndTransformQuery` middleware that validates the accepted query parameters for a request and sets the default Query configuration. So, in `src/api/middlewares.ts`, add a new middleware configuration object:
...
- A [Zod](https://zod.dev/) schema that a request's query parameters must satisfy. Switchyard provides `createFindParams` that generates a Zod schema with the following properties:
...
In your custom UI route, you'll retrieve the brands by sending a request to the Switchyard server. Switchyard has a [JS SDK](!resources!/js-sdk) that simplifies sending requests to the core API route.
...
![The directory structure of the Switchyard application after adding the file](https://res.cloudinary.com/dza7lstvk/image/upload/v1733414606/Switchyard%20Book/brands-admin-dir-overview-1_jleg0t.jpg)
...
- `baseUrl`: The URL to the Switchyard server.
- `debug`: Whether to enable logging debug messages. This should only be enabled in development.
- `auth.type`: The authentication method used in the client application, which is `session` in the Switchyard Admin dashboard.

Notice that you use `import.meta.env` to access environment variables in your customizations because the Switchyard Admin is built on top of Vite. Learn more in [this chapter](../../../fundamentals/admin/environment-variables/page.mdx).

You can now use the SDK to send requests to the Switchyard server.
...
![Directory structure of the Switchyard application after adding the UI route.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733472011/Switchyard%20Book/brands-admin-dir-overview-3_syytld.jpg)
...
import { TagSolid } from "@switchyard/icons"
import { 
  Container,
} from "@switchyard/ui"
...
So far, you only show a container. In admin customizations, use components from the [Switchyard UI package](!ui!) to maintain a consistent user interface and design in the dashboard.
...
To display the brands, you'll use Switchyard UI's [DataTable](!ui!/components/data-table) component. So, add the following imports in `src/admin/routes/brands/page.tsx`:
...
Then, you use `useQuery` from [Tanstack (React) Query](https://tanstack.com/query/latest) to query the Switchyard server. Tanstack Query provides features like asynchronous state management and optimized caching.
...
![A new sidebar item is added for the new brands UI route. The UI route shows the table of brands with pagination.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733421074/Switchyard%20Book/Screenshot_2024-12-05_at_7.46.52_PM_slcdqd.png)
...
- Created a UI route in the Switchyard Admin that shows the list of brands.
...
In the next chapters, you'll learn about the concepts that facilitate integrating third-party systems in your application. You'll integrate a dummy third-party system and sync the brands between it and the Switchyard application.
