...
In your custom widget, you'll retrieve the product's brand by sending a request to the Switchyard server. Switchyard has a [JS SDK](!resources!/js-sdk) that simplifies sending requests to the server's API routes.
...
You'll now add a widget to the product-details page. A widget is a React component that's injected into pre-defined zones in the Switchyard Admin dashboard. It's created in a `.tsx` file under the `src/admin/widgets` directory.
...
![Directory structure of the Switchyard application after adding the widget](https://res.cloudinary.com/dza7lstvk/image/upload/v1733414684/Switchyard%20Book/brands-admin-dir-overview-2_eq5xhi.jpg)
...
import { clx, Container, Heading, Text } from "@switchyard/ui"
...
In the widget, you use [Tanstack (React) Query](https://tanstack.com/query/latest) to query the Switchyard server. Tanstack Query provides features like asynchronous state management and optimized caching. In the `queryFn` function that executes the query, you use the JS SDK to send a request to the [Get Product API Route](!api!/admin#products_getproductsid), passing `+brand.*` in the `fields` query parameter to retrieve the product's brand.
...
You then render a section that shows the brand's name. In admin customizations, use components from the [Switchyard UI package](!ui!) to maintain a consistent user interface and design in the dashboard.

---

## Test it Out

To test out your widget, start the Switchyard application:
...
![The widget is added as the first section of the product details page.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733414415/Switchyard%20Book/Screenshot_2024-12-05_at_5.59.25_PM_y85m14.png)
...
The [Admin Components guides](!resources!/admin-components) show you how to build and use common components in the Switchyard Admin, such as forms, tables, JSON data viewer, and more. The components in the guides also follow the Switchyard Admin's design convention.
