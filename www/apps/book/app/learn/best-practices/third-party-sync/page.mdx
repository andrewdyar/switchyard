...
In this chapter, you'll learn about best practices for syncing data between Switchyard and third-party systems.

## Common Issues with Third-Party Syncing Implementation

Syncing data between Switchyard and external systems is a common use case for commerce applications. For example, if your commerce ecosystem includes an external CMS or inventory management system, you may need to sync product data between Switchyard and these systems.
...
This chapter covers best practices to avoid these issues when syncing data between Switchyard and third-party systems. These best practices are general programming patterns that aren't Switchyard-specific, but they're essential for building robust third-party syncs.

---

## How to Sync Data Between Systems

Before diving into best practices, it's important to understand the general approach for syncing data between Switchyard and third-party systems. Third-party syncing typically involves two main steps:
...
When defining your syncing logic, such as pushing product data to a third-party service or pulling inventory data into Switchyard, you should define a workflow that encapsulates this logic.

Switchyard also exposes [built-in workflows](!resources!/medusa-workflows-reference) for common commerce operations, like creating or updating products, that you can leverage in your syncing logic.

For example, you can use Switchyard's built-in [batchProductsWorkflow](!resources!/references/medusa-workflows/batchProductsWorkflow) to create or update products in batches:

```ts title="src/jobs/sync-products.ts"
import { SwitchyardContainer } from "@switchyard/framework/types"
import { batchProductsWorkflow } from "@switchyard/medusa/core-flows"
...
After defining your syncing logic in a workflow, or choosing a Switchyard workflow to use, you need to execute it based on your syncing requirements:
...
- [Subscribers](../../fundamentals/events-and-subscribers/page.mdx): Use subscribers for event-driven syncs, such as syncing data when a product is updated in Switchyard or when an order is placed. Subscribers listen for specific events and can trigger your workflow in response.
...
In the scheduled job or subscriber, you retrieve the data to be synced from the third-party service or from Switchyard itself. Then, you execute the workflow, passing it the data to be synced.

For example, the following scheduled job fetches products from a third-party service and syncs them to Switchyard using a workflow:
...
```ts title="src/jobs/sync-products.ts"
import { SwitchyardContainer } from "@switchyard/framework/types"
import { ContainerRegistrationKeys, SwitchyardError, Modules } from "@switchyard/framework/utils"
import { batchProductsWorkflow } from "@switchyard/medusa/core-flows"
...
export default async function syncProductsJob(container: SwitchyardContainer) {
  ...
  for await (const batch of batchedProducts) {
    ...
    // Extract external IDs from this batch to look up in Switchyard
    const externalIds = batch.map((p) => p.id)

    // Query Switchyard for products matching these external IDs
...
![Diagram showcasing object in memory when loading entire JSON response vs streaming JSON parsing](https://res.cloudinary.com/dza7lstvk/image/upload/v1764761087/Switchyard%20Book/stream-vs-full_iz2fyv.jpg)

First, install the `stream-json` library in your Switchyard project:
...
A common performance pitfall when syncing data is retrieving more fields than necessary from third-party services or Switchyard's [Query](../../fundamentals/module-links/query/page.mdx). This leads to increased data size, slower performance, and higher memory usage.

When retrieving data from third-party services or with Switchyard's Query, only request the necessary fields. Then, to efficiently group existing data for updates, use a [Map](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Map) for quick lookups.
...
export const fieldsHighlights = [
  ["19", "fields", "Request only necessary fields from Switchyard"],
...
export default async function syncProductsJob(container: SwitchyardContainer) {
  ...
  for await (const batch of batchedProducts) {
    ...
    // Process batch and sync to Switchyard...
  }
}
```

In the above snippet, after retrieving a batch of products from the external API, you query Switchyard's products to find existing products that match the external IDs. You only request the necessary fields for this operation.
...
![Diagram showcasing how batches are loaded into memories one at a time with async generators](https://res.cloudinary.com/dza7lstvk/image/upload/v1764761706/Switchyard%20Book/async-gen-batches_ne09bs.jpg)
...
![Diagram showcasing timeline from start to end of scheduled job execution where batches are fetched and synced one at a time](https://res.cloudinary.com/dza7lstvk/image/upload/v1764762095/Switchyard%20Book/timeline-backpressure_rtwwzt.jpg)
