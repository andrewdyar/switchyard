export const metadata = {
  title: `${pageNumber} Build Switchyard Application`,
}

# {metadata.title}

In this chapter, you'll learn how to create a production build of your Switchyard application for deployment to a hosting provider.

The next chapter explains how to deploy your Switchyard application.

<Note title="Deploy with Cloud" type="success">

The below instructions are useful for self-hosting your Switchyard application. You can also use [Cloud](!cloud!) to deploy and manage your Switchyard application without worrying about infrastructure. Switchyard will handle the configurations, deployment, and scaling for you.

</Note>

## build Command

The Switchyard CLI tool provides a [build](!resources!/switchyard-cli/commands/build) command that creates a standalone build of the Switchyard application that:

- Doesn't rely on the source TypeScript files.
- Can be copied to a production server reliably.

So, to create the production build, run the following command in the root of your Switchyard application:
...
## Start Built Switchyard Application

To start the Switchyard application after running the `build` command:
...
When `NODE_ENV=production`, the Switchyard application loads the environment variables from `.env.production`. Learn more in the [environment variables guide](../fundamentals/environment-variables/page.mdx).
...
4. Start the Switchyard application from `.medusa/server`:
...
When the Switchyard application is started in production (`NODE_ENV=production`) or staging mode, cookie settings are stricter. Cookie authentication only works if the client application is served from the same domain as the Switchyard server, and the domain is not `localhost` or part of the [public suffix list](https://publicsuffix.org/).

So if you try to access the Switchyard Admin locally in production mode, you won't be able to log in.

To access the Switchyard Admin locally in production mode, set the `projectConfig.cookieOptions` in your `switchyard.config.ts` file to be less strict. For example:
...
Then rebuild the Switchyard application and start it again as described in the [steps above](#start-built-medusa-application). You can now access the Switchyard Admin locally in production mode.
...
![.medusa/server directory structure](https://res.cloudinary.com/dza7lstvk/image/upload/v1760709518/Switchyard%20Book/output-dir_dlklk6.jpg)
...
- `switchyard.config.js`: The compiled Switchyard configuration file.
- `package.json` and a lock file: The dependencies required to run the Switchyard application in production.
...
If you have other assets, such as JSON files or fonts, that you need in your Switchyard backend at runtime, you need to copy them to the `.medusa/server` directory as part of your build process.
...
The admin dashboard is built with [Vite](!https://vitejs.dev/). If you need to customize the build process to include additional static assets, you can modify the Vite configuration with the [admin.vite](../configurations/switchyard.config/page.mdx) option in your `switchyard.config.ts` file.
...
```bash
npx switchyard build --admin-only
```
