...
Switchyard provides an `InjectManager` decorator from the Modules SDK that injects a service's method with a [forked entity manager](https://mikro-orm.io/docs/identity-map#forking-entity-manager).

So, to run database queries in a service:

1. Add the `InjectManager` decorator to the method.
2. Add as a last parameter an optional `sharedContext` parameter that has the `SwitchyardContext` decorator from the Modules SDK. This context holds database-related context, including the manager injected by `InjectManager`
...
You add two methods `getCount` and `getCountSql` that have the `InjectManager` decorator. Each of the methods also accept the `sharedContext` parameter which has the `SwitchyardContext` decorator.

The entity manager is injected to the `sharedContext.manager` property, which is an instance of [EntityManager from the `@switchyard/framework/mikro-orm/knex` package](https://mikro-orm.io/api/knex/class/EntityManager).
...
Both methods must accept as a last parameter an optional `sharedContext` parameter that has the `SwitchyardContext` decorator from the Modules SDK. It holds database-related contexts passed through the Switchyard application.
...
export const opHighlights = [
  ...
  ["17", "SwitchyardContext", "A decorator to use Switchyard's shared context."],
...
If your protected transactional method uses other methods that accept a Switchyard context, pass the shared context to those methods.
...
For every data model in your module, Switchyard generates a data model repository that has methods to perform database operations.
...
When the Switchyard application injects a data model repository into a module's container, it formats the registration name by:
...
import { SwitchyardService } from "@switchyard/framework/utils"
import Post from "./models/post"
import { DAL } from "@switchyard/framework/types"
...
import { Context } from "@switchyard/framework/types"
import { EntityManager } from "@switchyard/framework/mikro-orm/knex"
import { IsolationLevel } from "@switchyard/framework/mikro-orm/core"
