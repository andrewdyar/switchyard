...
A module has its own container, as explained in the [Module Container](../container/page.mdx) chapter. This container includes the module's resources, such as services and data models, and some Framework resources that the Switchyard application provides.
...
- Integrate your module into any Switchyard application without side-effects to your setup.
...
Some Framework APIs and tools are not registered in the module's container. For example, [Query](../../module-links/query/page.mdx) is only registered in the Switchyard container.
...
```ts title="Example Workflow"
import { createWorkflow, createStep } from "@switchyard/framework/workflows-sdk"
...
In those cases, you can inject the dependencies to your module's service in `switchyard.config.ts` using the `dependencies` property of the module's configuration.
