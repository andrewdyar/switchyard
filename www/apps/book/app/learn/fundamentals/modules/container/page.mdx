...
So, resources in the module, such as services or loaders, can only resolve other resources registered in the module's container, and some Framework tools that the Switchyard application registers in the module's container.
...
Consider a module that has a main service `BrandModuleService`, and an internal service `CmsService`. Switchyard will register both of these services in the module's container.
...
Instead, if your main service extends the `SwitchyardService` [service factory](../service-factory/page.mdx), you can resolve the internal services generated for each data model passed to the `SwitchyardService` function.
...
Instead, to build commerce features that span multiple modules, you can create [workflows](../../workflows/page.mdx). In those workflows, you can resolve services of all modules registered in the Switchyard application, including the services of the Product and Blog modules.
