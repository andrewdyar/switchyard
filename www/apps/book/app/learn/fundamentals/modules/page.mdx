...
Switchyard removes this overhead by allowing you to easily write custom modules that integrate into the Switchyard application without affecting the existing setup. You can also re-use your modules across Switchyard projects.

As you learn more about Switchyard, you will see that modules are central to customizations and integrations. With modules, your Switchyard application can turn into a middleware solution for your commerce ecosystem.
...
In a module, you define data models that represent new tables in the database, and you manage these models in a class called a service. Then, the Switchyard application registers the module's service in the [Switchyard container](../../fundamentals/medusa-container/page.mdx) so that you can build commerce flows and features around the functionalities provided by the module.
...
A data model represents a table in the database. You create data models using Switchyard's data modeling language (DML). It simplifies defining a table's columns, relations, and indexes with straightforward methods and configurations.
...
![Blog module directory structure showing the organized file hierarchy with the newly created 'models' folder containing the 'post.ts' data model file, which defines the data model for blog posts using Switchyard's data modeling language](https://res.cloudinary.com/dza7lstvk/image/upload/v1732806790/Switchyard%20Book/blog-dir-overview-1_jfvovj.jpg)
...
You perform database operations on your data models in a service, which is a class exported by the module and acts like an interface to its functionalities. Switchyard registers the service in its [container](../../fundamentals/medusa-container/page.mdx), allowing you to resolve and use it when building custom commerce flows.
...
![Updated directory overview after adding the service](https://res.cloudinary.com/dza7lstvk/image/upload/v1732807230/Switchyard%20Book/blog-dir-overview-2_avzb9l.jpg)

export const highlights = [
  ["4", "SwitchyardService", "The service factory function."],
...
Your module's service extends a class generated by `SwitchyardService` from the Modules SDK. This class comes with generated methods for data-management Create, Read, Update, and Delete (CRUD) operations on each of your modules, saving you time that can be spent on building custom business logic.

The `SwitchyardService` function accepts an object of data models to generate methods for. You can pass all data models in your module in this object.
...
Find all methods generated by the `SwitchyardService` in [this reference](!resources!/service-factory-reference)

</Note>

If a module doesn't have data models, such as when it's integrating a third-party service, it doesn't need to extend `SwitchyardService`.

### 3. Export Module Definition

The final piece to a module is its definition, which is exported in an `index.ts` file at its root directory. This definition tells Switchyard the name of the module and its main service. Switchyard will then register the main service in the container under the module's name.

So, to export the definition of the Blog Module, create the file `src/modules/blog/index.ts` with the following content:

![Updated directory overview after adding the module definition](https://res.cloudinary.com/dza7lstvk/image/upload/v1732808511/Switchyard%20Book/blog-dir-overview-3_dcgjaa.jpg)
...
### 4. Add Module to Switchyard's Configurations
...
Once you finish building the module, add it to Switchyard's configurations to start using it. Switchyard will then register the module's main service in the Switchyard container, allowing you to resolve and use it in other customizations.

In `switchyard.config.ts`, add a `modules` property and pass an array with your custom module:

```ts title="switchyard.config.ts" highlights={[["7"]]}
...
You don't have to write migrations yourself. Switchyard's CLI tool has a command that generates the migrations for you. You can also use this command again when you make changes to the module at a later point, and it will generate new migrations for that change.

To generate a migration for the Blog Module, run the following command in your Switchyard application's directory:

<Note>

If you're creating the module in a plugin, use the [plugin\:db\:generate command](!resources!/switchyard-cli/commands/plugin#plugindbgenerate) instead.

</Note>

```bash
npx switchyard db:generate blog
```

The `db:generate` command of the Switchyard CLI accepts one or more module names to generate the migration for. It will create a migration file for the Blog Module in the directory `src/modules/blog/migrations` similar to the following:

<Note>

As of [Switchyard v2.11.0](https://github.com/switchyard/medusa/releases/tag/v2.11.0), MikroORM dependencies are included in the `@switchyard/framework` package. If you're using an older version of Switchyard, change the import statement to `@mikro-orm/migrations`.
...
If you're creating the module in a plugin, run this command on the Switchyard application that the plugin is installed in.
...
Since the module's main service is registered in the Switchyard container, you can resolve it in other customizations to use its methods.
...
export const workflowHighlights = [
  ...
  ["17", "resolve", "Resolve the Blog Module's service from the Switchyard container."],
...
The workflow has a single step `createPostStep` that creates a post. In the step, you resolve the Blog Module's service from the Switchyard container, which the step receives as a parameter. Then, you create the post using the method `createPosts` of the service, which was generated by `SwitchyardService`.
...
This adds a `POST` API route at `/blog/posts`. In the API route, you execute the `createPostWorkflow` by invoking it, passing it the Switchyard container in `req.scope`, then invoking the `run` method. In the `run` method, you pass the workflow's input in the `input` property.

To test this out, start the Switchyard application:
