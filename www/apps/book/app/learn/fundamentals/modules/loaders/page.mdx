...
When building a commerce application, you'll often need to execute an action the first time the application starts. For example, if your application needs to connect to databases other than Switchyard's PostgreSQL database, you might need to establish a connection on application startup.

In Switchyard, you can execute an action when the application starts using a loader. A loader is a function exported by a [module](../page.mdx), which is a package of business logic for a single domain. When the Switchyard application starts, it executes all loaders exported by configured modules.

Loaders are useful to register custom resources, such as database connections, in the [module's container](../container/page.mdx), which is similar to the [Switchyard container](../../medusa-container/page.mdx) but includes only [resources available to the module](!resources!/medusa-container-resources#module-container-resources). Modules are isolated, so they can't access resources outside of them, such as a service in another module.

<Note title="Why are modules isolated?">

Switchyard isolates modules to ensure that they're re-usable across applications, aren't tightly coupled to other resources, and don't have implications when integrated into the Switchyard application. Learn more about why modules are isolated in [this chapter](../isolation/page.mdx), and check out [this reference for the list of resources in the module's container](!resources!/medusa-container-resources#module-container-resources).
...
![Example of loader file in the application's directory structure](https://res.cloudinary.com/dza7lstvk/image/upload/v1732865671/Switchyard%20Book/loader-dir-overview_eg6vtu.jpg)
...
After implementing the loader, you must export it in the module's definition in the `index.ts` file at the root of the module's directory. Otherwise, the Switchyard application will not run it.
...
The second parameter of the `Module` function accepts a `loaders` property whose value is an array of loader functions. The Switchyard application will execute these functions when it starts.

### Test the Loader

Assuming your module is [added to Switchyard's configuration](../page.mdx#4-add-module-to-medusas-configurations), you can test the loader by starting the Switchyard application:
...
```plain
info:   [HELLO MODULE] Just started the Switchyard application!
```
...
When you start the Switchyard application, it executes the loaders of all modules in their registration order.
...
Since loaders are executed when the Switchyard application starts, heavy operations will increase the startup time of the application.
...
<Prerequisites
  items={[
    ...
    {
      text: "Install the MongoDB SDK in your Switchyard application.",
...
As of [Switchyard v2.11.0](https://github.com/switchyard/medusa/releases/tag/v2.11.0), Awilix dependencies are included in the `@switchyard/framework` package. If you're using an older version of Switchyard, change the import statement to `awilix`.
...
The loader function accepts in its object parameter an `options` property, which is the options passed to the module in Switchyard's configurations. For example:

export const optionHighlights = [
  ["6", "options", "The options to pass to the module."]
]

```ts title="switchyard.config.ts" highlights={optionHighlights}
...
2. The resource to register in the container, which is the MongoDB client you created. However, you don't pass the resource as-is. Instead, you need to use an `asValue` function imported from the [`@switchyard/framework/awilix` package](https://github.com/jeffijoe/awilix), which is the package used to implement the container functionality in Switchyard.
...
You can test the connection out by starting the Switchyard application. If it's successful, you'll see the following message logged in the terminal:
