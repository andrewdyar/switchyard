...
Consider a scenario where you need to access related records from another module, but don't want the overhead of managing or storing the links between them. This can include cases where you're working with external data models not stored in your Switchyard database, such as third-party systems.
...
For example, Switchyard creates a read-only module link from the `Cart` data model of the [Cart Module](!resources!/commerce-modules/cart) to the `Customer` data model of the [Customer Module](!resources!/commerce-modules/customer). This link allows you to access the details of the cart's customer without managing the link. Instead, the customer's ID is stored in the `Cart` data model.

![Diagram illustrating the read-only module link from cart to customer](https://res.cloudinary.com/dza7lstvk/image/upload/v1742212508/Switchyard%20Book/cart-customer_w6vk59.jpg)
...
Unlike the stored module link, Switchyard will not create a table in the database for this link. Instead, Switchyard uses the ID stored in the specified field of the first data model to retrieve the linked record.
...
In the above example, you retrieve a post and its linked product. Switchyard will use the ID of the product in the post's `product_id` field to determine which product should be retrieved.
...
Read-only module links are most useful when working with data models that aren't stored in your Switchyard database. For example, data that is stored in a third-party system.

In those cases, you can define a read-only module link between a data model in Switchyard and the data model in the external system, facilitating the retrieval of the linked data.

To define the read-only module link to a virtual data model, you must:

1. Define the read-only module link from the Switchyard data model to the virtual data model.
2. Create a `list` method in the custom module's service. This method retrieves the linked records filtered by the ID(s) of the Switchyard data model.
    - You can also create a `listAndCount` method to retrieve the related records with pagination.
3. Use Query to retrieve the Switchyard data model and its linked records from the virtual data model.

For example, consider you have a CMS Module that integrates a third-party Content-Management System (CMS) with Switchyard, and you want to retrieve the posts in the CMS associated with a product in Switchyard. The next steps showcase how to implement this.

### a. Define Read-Only Module Link

Start by defining a read-only module link from the `Product` data model in Switchyard to the external `post` data model in the CMS:
...
1. An object with the linkable configuration of the data model in Switchyard, and the fields that will be passed as a filter to the CMS service.
...
2. An object with the linkable configuration of the virtual data model in the CMS. This object must have the following properties:
    - `serviceName`: The name of the service, which is the CMS Module's name. Switchyard uses this name to resolve the module's service from the [Switchyard container](../../medusa-container/page.mdx).
...
import { FindConfig } from "@switchyard/framework/types"
