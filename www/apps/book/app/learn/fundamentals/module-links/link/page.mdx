...
As of [Switchyard v2.2.0](https://github.com/switchyard/medusa/releases/tag/v2.2.0), Remote Link has been deprecated in favor of Link. They have the same usage, so you only need to change the key used to resolve the tool from the Switchyard container as explained below.

</Note>

## What is Link?

Link is a class with utility methods to manage links between data models. It’s registered in the Switchyard container under the `link` registration name.
...
Switchyard enforces integrity constraints on links based on the link's relation type. So, an error is thrown in the following scenarios:
