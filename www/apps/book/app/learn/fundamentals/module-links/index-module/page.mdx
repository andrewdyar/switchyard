...
![Diagram showcasing how data is retrieved from the Index Module's data store](https://res.cloudinary.com/dza7lstvk/image/upload/v1747988533/Switchyard%20Book/index-module_epurmt.jpg)

### Ingested Data Models

Data models are ingested if they're linked to other data models with the `filterable` property set, as you'll learn in the [Ingest Custom Data Models](#how-to-ingest-custom-data-models) section. Switchyard also ingests some core data models by default, such as `Product`, `ProductVariant`, `Price`, and `SalesChannel`.

<Note>

Prior to [Switchyard v2.10.2](https://github.com/switchyard/medusa/releases/tag/v2.10.2), only the `Product`, `ProductVariant`, `Price`, and `SalesChannel` data models were ingested. Make sure to update to the latest version to ingest all core data models.
...
To install the Index Module, run the following command in your Switchyard project to install its package:

```bash npm2yarn
npm install @switchyard/index
```

Then, add the Index Module to your Switchyard configuration in `switchyard.config.ts`:
...
The Index Module only ingests data when you start your Switchyard server. So, to [ingest data models](#ingested-data-models), start the Switchyard application:
...
But first, you must run the migrations to sync the link, then start the Switchyard application:
...
The Index API routes are available from [Switchyard v2.11.2](https://github.com/switchyard/medusa/releases/tag/v2.11.2).

</Note>

Switchyard provides API routes to view and trigger index reingestion or syncing. This is useful if you want to reingest data manually, for example, after a large data import.
