...
In the CLI script, use your custom workflows or Switchyard's existing workflows, which you can browse in [this reference](!resources!/medusa-workflows-reference), to seed data.
...
You create the generated products using the `createProductsWorkflow` imported previously from `@switchyard/medusa/core-flows`. It accepts the product data as input, and returns the created products.
...
Finally, you use the `createInventoryLevelsWorkflow` from Switchyard's core workflows to create the inventory levels.
...
```bash
npx switchyard exec ./src/scripts/demo-products.ts
```

This seeds the products to your database. If you run your Switchyard application and view the products in the dashboard, you'll find fifty new products.
