...
The Switchyard Admin dashboard is customizable, allowing you to add new pages, called UI routes. You create a UI route as a React component showing custom content that allows admin users to perform custom actions.

For example, you can add a new page to show and manage product reviews, which aren't available natively in Switchyard.

You can create a UI route directly in your Switchyard application, or in a [plugin](../../plugins/page.mdx) if you want to share the UI route across multiple Switchyard applications.
...
![Example of UI route file in the application's directory structure](https://res.cloudinary.com/dza7lstvk/image/upload/v1732867243/Switchyard%20Book/ui-route-dir-overview_tgju25.jpg)
...
In the route, you use [Switchyard UI](!ui!), a package that Switchyard maintains to allow you to customize the dashboard with the same components used to build it.
...
To test the UI route, start the Switchyard application:
...
The configuration object is created using `defineRouteConfig` from the Switchyard Framework. It accepts the following properties:
...
The above example adds a new sidebar item with the label `Custom Route` and an icon from the [Switchyard UI Icons package](!ui!/icons/overview).

### Specify UI Route Sidebar Rank

<Note>

UI route ranking is available starting [Switchyard v2.11.4](https://github.com/switchyard/medusa/releases/tag/v2.11.4).
...
import { defineRouteConfig } from "@switchyard/admin-sdk"
import { ChartBar } from "@switchyard/icons"
import { Container, Heading } from "@switchyard/ui"
...
Switchyard sorts custom UI routes based on their rank:

1. UI routes that have ranks are sorted in ascending order.
2. UI routes without a rank are added after the ranked UI routes.

Switchyard also applies the same sorting logic to UI routes at the nested level. Learn more in the [Nested UI Routes Ranking](#nested-ui-routes-ranking) section.
...
![Example of nested UI route file in the application's directory structure](https://res.cloudinary.com/dza7lstvk/image/upload/v1732867243/Switchyard%20Book/ui-route-dir-overview_tgju25.jpg)
...
![Example of settings UI route file in the application's directory structure](https://res.cloudinary.com/dza7lstvk/image/upload/v1732867435/Switchyard%20Book/setting-ui-route-dir-overview_kytbh8.jpg)
...
![Example of UI route file with path parameters in the application's directory structure](https://res.cloudinary.com/dza7lstvk/image/upload/v1732867748/Switchyard%20Book/path-param-ui-route-dir-overview_kcfbev.jpg)
...
If you run the Switchyard application and go to `http://localhost:9000/app/custom/123`, you'll see `123` printed in the page.

---

## Set UI Route Breadcrumbs

The Switchyard Admin dashboard shows breadcrumbs at the top of each page, if specified. This allows users to navigate through your custom UI routes.
...
If you set a breadcrumb for a nested UI route, and you open the route in the Switchyard Admin, you'll see the breadcrumbs starting from its parent route to the nested route.
...
To build admin customizations that match the Switchyard Admin's designs and layouts, refer to [this guide](!resources!/admin-components) to find common components.
