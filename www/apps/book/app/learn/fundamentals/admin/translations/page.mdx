...
In this chapter, you'll learn how to add translations to your Switchyard Admin widgets and UI routes.

<Note>

Translations for admin customizations are available from [Switchyard v2.11.1](https://github.com/switchyard/medusa/releases/tag/v2.11.1).

</Note>

## Translations in the Switchyard Admin

The Switchyard Admin dashboard supports [multiple languages](!user-guide!/tips/languages) for its interface. Switchyard uses [react-i18next](https://react.i18next.com/) to manage translations in the admin dashboard.

<Note>

Switchyard Admin translations apply to the interface only. Switchyard doesn't support translating store content, such as product names or descriptions. To implement localization for store content, [integrate a CMS](!resources!/integrations#cms).

</Note>

When you create [Widgets](../widgets/page.mdx) or [UI Routes](../ui-routes/page.mdx) to customize the Switchyard Admin, you can provide translations for the text content in your customizations, allowing users to view your customizations in their preferred language.

You can add translations for your admin customizations within your Switchyard project or as part of a plugin.
...
English is the default language for Switchyard Admin, so it's recommended to always include an English translation file.

</Note>

![Directory structure showing where to place translation files](https://res.cloudinary.com/dza7lstvk/image/upload/v1761555573/Switchyard%20Book/translations-json_m2pvet.jpg)
...
![Directory structure showing where to place translation files](https://res.cloudinary.com/dza7lstvk/image/upload/v1761555573/Switchyard%20Book/translations-json-es_s1etna.jpg)
...
![Directory structure showing i18n index file](https://res.cloudinary.com/dza7lstvk/image/upload/v1761555573/Switchyard%20Book/translations-index_cgrj0t.jpg)
...
The `react-i18next` package is already included in the Switchyard Admin's dependencies, so you don't need to install it separately. However, `pnpm` users may need to install it manually due to package resolution issues.
...
To test the translations, start the Switchyard application with the following command:

```bash npm2yarn
npm run dev
```

Then, go to a product details page in the Switchyard Admin dashboard. If your default language is set to English, you'll see the widget displaying text in English.
...
Translation auto-completion is available from [Switchyard v2.11.2](https://github.com/switchyard/medusa/releases/tag/v2.11.2).

</Note>

To enhance your development experience, you can set up auto-completion for translation keys. This will allow you to auto-complete translation keys from Switchyard's default translations as well as your custom translations.
...
// Import Switchyard keys
import type { Resources } from "@switchyard/dashboard"
...
When you load the translations with the `translation` key in `src/admin/i18n/index.ts`, your custom Switchyard Admin translations are merged with the default Switchyard Admin translations:

- Translation keys in your custom translations override the default Switchyard Admin translations.
- The default Switchyard Admin translations are used as a fallback when a key is not defined in your custom translations.
...
- `brands.description`: Not defined in your custom translation file or the default Switchyard Admin translations, so it outputs the key itself: `brands.description`.
- `actions.save`: Defined in your custom translation file, so it outputs `Custom Save`.
- `actions.delete`: Not defined in your custom translation file, so it falls back to the default Switchyard Admin translation and outputs `Delete`.

### Custom Translation Namespaces

To avoid potential key conflicts between your custom translations and the default Switchyard Admin translations, you can use custom namespaces. This is particularly useful when developing plugins that add admin customizations, as it prevents naming collisions with other plugins or the default Switchyard Admin translations.
...
Translations are now loaded only from the `brands` namespace without conflicting with other translation keys in the Switchyard Admin.
