...
The `storeCors` and `adminCors` properties of Switchyard's `http` configuration set the allowed origins for routes starting with `/store` and `/admin` respectively.
...
Learn more about the CORS configurations in [this resource guide](../../../configurations/switchyard.config/page.mdx#http).
...
export const highlights = [["25", "parseCorsOrigins", "A utility function that parses the CORS configurations in `switchyard.config.ts`"]]
...
This retrieves the configurations exported from `switchyard.config.ts` and applies the `storeCors` to routes starting with `/custom`.
