...
To send a JSON response, use the `json` method of the `SwitchyardResponse` object that is passed as the second parameter of your API route handler.
...
To change the status code, use the `status` method of the `SwitchyardResponse` object.
...
To return response data other than a JSON object, you can either use the `set`, `setHeader`, or `writeHead` methods of the `SwitchyardResponse` object. They allow you to set the response headers, including the content type.
...
The `SwitchyardResponse` type is based on [Express's Response](https://expressjs.com/en/api.html#res). Refer to their API reference for other uses of responses.