...
By default, the body parser limits the request body size to `100kb`. If a request body exceeds that size, the Switchyard application throws an error.
...
You can't override the request body size limit for existing routes, such as Switchyard's Store and Admin API routes, due to how [middlewares are applied](../middlewares/page.mdx#how-are-middlewares-ordered-and-applied).
...
The `multer` package is available through the `@switchyard/medusa` package, so you don't need to install it. However, for better TypeScript support, install the `@types/multer` package as a development dependency:
