export const metadata = {
  title: `${pageNumber} Retrieve Custom Links from Switchyard's API Route`,
}

# {metadata.title}

In this chapter, you'll learn how to retrieve custom data models linked to existing Switchyard data models from Switchyard's API routes.

## Why Retrieve Custom Linked Data Models?

Often, you'll link custom data models to existing Switchyard data models to implement custom features or expand on existing ones.
...
Most of Switchyard's API routes accept a `fields` query parameter that allows you to specify the fields and relations to retrieve in the resource, such as a product.
...
Some of Switchyard's API routes restrict the fields and relations you can retrieve, which means you can't pass your custom linked data models in the `fields` query parameter. Switchyard makes this restriction to ensure the API routes are performant and secure.
...
import { defineMiddlewares } from "@switchyard/medusa"
