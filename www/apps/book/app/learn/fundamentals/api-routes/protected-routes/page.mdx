...
You can protect API routes using the `authenticate` [middleware](../middlewares/page.mdx) from the Switchyard Framework. When applied to a route, the middleware checks that:
...
### Override Authentication for Switchyard's API Routes

In some cases, you may want to override the authentication requirement for Switchyard's API routes. For example, you may want to allow custom actor types to access existing protected API routes.
...
To access the authentication details in an API route, such as the logged-in user's ID, set the type of the first request parameter to `AuthenticatedSwitchyardRequest`. It extends `SwitchyardRequest`:

```ts highlights={[["7", "AuthenticatedSwitchyardRequest"]]}
import type {
  AuthenticatedSwitchyardRequest,
  SwitchyardResponse,
} from "@switchyard/framework/http"
...
The `auth_context.actor_id` property of `AuthenticatedSwitchyardRequest` holds the ID of the authenticated user or customer. If there isn't any authenticated user or customer, `auth_context` is `undefined`.
...
In this example, you retrieve the ID of the authenticated user, customer, or custom actor type from the `auth_context` property of the `AuthenticatedSwitchyardRequest` object.
...
In this example, you retrieve the customer's ID and resolve Query from the [Switchyard container](../../medusa-container/page.mdx).
...
You can access the logged-in admin user’s ID in all API routes starting with `/admin` using the `auth_context.actor_id` property of the `AuthenticatedSwitchyardRequest` object. You can then use [Query](../../module-links/query/page.mdx) to retrieve the user details, or pass the ID to a [workflow](../../workflows/page.mdx) that performs business logic.
...
In this example, you retrieve the admin user's ID and resolve Query from the [Switchyard container](../../medusa-container/page.mdx).
