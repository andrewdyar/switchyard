...
In this chapter, you'll learn how to create a Switchyard plugin and publish it.

A [plugin](../page.mdx) is a package of reusable Switchyard customizations that you can install in any Switchyard application. By creating and publishing a plugin, you can reuse your Switchyard customizations across multiple projects or share them with the community.

<Note>

Plugins are available starting from [Switchyard v2.3.0](https://github.com/switchyard/medusa/releases/tag/v2.3.0).

</Note>

## 1. Create a Plugin Project

Plugins are created in a separate Switchyard project. This makes the development and publishing of the plugin easier. Later, you'll install that plugin in your Switchyard application to test it out and use it.

Switchyard's `create-switchyard-app` CLI tool provides the option to create a plugin project. Run the following command to create a new plugin project:

```bash
npx create-switchyard-app my-plugin --plugin
```

This will create a new Switchyard plugin project in the `my-plugin` directory.
...
- `src/`: Contains the Switchyard customizations.
...
Before developing, testing, and publishing your plugin, make sure its name in `package.json` is correct. This is the name you'll use to install the plugin in your Switchyard application.
...
Switchyard scrapes NPM for a list of plugins that integrate third-party services, to later showcase them on the Switchyard website. If you want your plugin to appear in that listing, make sure to add the `medusa-v2` and `medusa-plugin-integration` keywords to the `keywords` field in `package.json`.

<Note title="Important">

Only plugins that integrate third-party services are listed in the Switchyard integrations page. If your plugin doesn't integrate a third-party service, you can skip this step.
...
In the `package.json` file you must have the Switchyard dependencies as `devDependencies` and `peerDependencies`. In addition, you must have `@swc/core` as a `devDependency`, as it's used by the plugin CLI tools.

For example, assuming `2.5.0` is the latest Switchyard version:

```json title="package.json"
{
  "devDependencies": {
    "@switchyard/admin-sdk": "2.5.0",
    "@switchyard/cli": "2.5.0",
    "@switchyard/framework": "2.5.0",
...
Your plugin project will already have the exports mentioned in this section. Unless you made changes to the exports or you created your plugin before [Switchyard v2.7.0](https://github.com/switchyard/medusa/releases/tag/v2.7.0), you can skip this section.
...
- `./package.json`: The `package.json` file. Switchyard needs to access the `package.json` when registering the plugin.
- `./workflows`: The workflows exported in `./src/workflows/index.ts`.
- `./.medusa/server/src/modules/*`: The definition file of modules. This is useful if you create links to the plugin's modules in the Switchyard application.
- `./providers/*`: The definition file of module providers. This is useful if your plugin includes a module provider, allowing you to register the plugin's providers in Switchyard applications. Learn more in the [Create Module Providers](#create-module-providers) section.
...
Switchyard's CLI tool provides commands to simplify developing and testing your plugin in a local Switchyard application. You start by publishing your plugin in the local package registry, then install it in your Switchyard application. You can then watch for changes in the plugin as you develop it.
...
<Prerequisites
  items={[
    {
      text: "Switchyard application installed.",
      link: "/learn/installation",
    }
  ]}
/>

The first time you create your plugin, you need to publish the package into a local package registry, then install it in your Switchyard application. This is a one-time only process.
...
Next, navigate to your Switchyard application:

```bash title="Switchyard application"
cd ~/path/to/medusa-app
```

Make sure to replace `~/path/to/medusa-app` with the path to your Switchyard application.

Then, if your project was created before v2.3.1 of Switchyard, make sure to install `yalc` as a development dependency:

```bash npm2yarn badgeLabel="Switchyard Application" badgeColor="green"
npm install --save-dev yalc
```

After that, run the following Switchyard CLI command to install the plugin:

```bash title="Switchyard application"
npx switchyard plugin:add @myorg/plugin-name
```

Make sure to replace `@myorg/plugin-name` with the name of your plugin as specified in `package.json`. Your plugin will be installed from the local package registry into your Switchyard application.

### Register Plugin in Switchyard Application

After installing the plugin, you need to register it in your Switchyard application in the configurations defined in `switchyard.config.ts`.

Add the plugin to the `plugins` array in the `switchyard.config.ts` file:

export const pluginHighlights = [
  ["5", `"@myorg/plugin-name"`, "Replace with your plugin name."],
]

```ts title="switchyard.config.ts" highlights={pluginHighlights}
...
```ts title="switchyard.config.ts" highlights={pluginOptionsHighlight}
module.exports = defineConfig({
  // ...
  plugins: [
    {
...
While developing your plugin, you can watch for changes in the plugin and automatically update the plugin in the Switchyard application using it. This is the only command you'll continuously need during your plugin development.
...
- Publish the plugin changes to the local package registry. This will automatically update the plugin in the Switchyard application using it. You can also benefit from real-time HMR updates of admin extensions.

### Start Switchyard Application

You can start your Switchyard application's development server to test out your plugin:

```bash npm2yarn badgeLabel="Switchyard Application" badgeColor="green"
npm run dev
```

While your Switchyard application is running and the plugin is being watched, you can test your plugin while developing it in the Switchyard application.
...
While building those customizations, you can test them in your Switchyard application by [watching the plugin changes](#watch-plugin-changes-during-development) and [starting the Switchyard application](#start-medusa-application).
...
Finally, run these migrations on the Switchyard application that the plugin is installed in using the `db:migrate` command:
...
These exports allow you to import your plugin resources in your Switchyard application, including workflows, links and modules.

For example, to import the plugin's workflow in your Switchyard application:
...
The [exported resources](#package-exports) also allow you to import module providers in your plugin and register them in the Switchyard application's configuration. A module provider is a module that provides the underlying logic or integration related to a commerce or Infrastructure Module.

For example, assuming your plugin has a [Notification Module Provider](!resources!/infrastructure-modules/notification) called `my-notification`, you can register it in your Switchyard application's configuration like this:
...
```ts highlights={[["9"]]} title="switchyard.config.ts"
module.exports = defineConfig({
  // ...
  modules: [
    {
...
Switchyard's CLI tool provides a command that bundles your plugin to be published to npm. Once you're ready to publish your plugin publicly, run the following command in your plugin project:
...
If you haven't logged in before with your NPM account, you'll be asked to log in first. Then, your package is published publicly to be used in any Switchyard application.

### Install Public Plugin in Switchyard Application
...
Then, register the plugin in your Switchyard application's configurations as explained in [this section](#register-plugin-in-medusa-application).
...
To update the Switchyard dependencies in a plugin, refer to [this documentation](../../../update/page.mdx#update-plugin-project).
