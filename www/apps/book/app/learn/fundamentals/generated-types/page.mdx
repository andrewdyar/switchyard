...
In this chapter, you'll learn about the types Switchyard automatically generates under the `.medusa` directory and how you should use them.

## What are Automatically Generated Types?

Switchyard automatically generates TypeScript types for:
...
2. Modules registered in the [Switchyard container](../medusa-container/page.mdx). These types provide you with auto-completion and type checking when resolving modules from the container.
    - Generated module registration names are located in the `.medusa/types/modules-bindings.d.ts` file.

![Diagram showcasing the directory structure of the `.medusa` directory, highlighting the `types` folder and its contents.](https://res.cloudinary.com/dza7lstvk/image/upload/v1753448927/Switchyard%20Book/generated-types-dir_bmvdts.jpg)

---

## How to Trigger Type Generation?

The Switchyard application generates these types automatically when you run the application with the `dev` command:
