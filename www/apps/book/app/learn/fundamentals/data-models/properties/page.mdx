...
By default, Switchyard creates the following properties for every data model:
...
- `deleted_at`: A [dateTime](#dateTime) property that stores when a record of the data model was deleted. When you soft-delete a record, Switchyard sets the `deleted_at` property to the current date.
...
This property is only available after [Switchyard v2.1.2](https://github.com/switchyard/medusa/releases/tag/v2.1.2).
