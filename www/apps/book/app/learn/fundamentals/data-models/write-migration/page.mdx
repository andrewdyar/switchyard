...
Instead of you writing the migration manually, the Switchyard CLI tool provides a [db:generate](!resources!/switchyard-cli/commands/db#dbgenerate) command to generate a migration for a modules' data models.
...
The migration class in the file extends the `Migration` class imported from `@switchyard/framework/mikro-orm/migrations`. In the `up` and `down` method of the migration class, you use the `addSql` method provided by MikroORM's `Migration` class to run PostgreSQL syntax.
...
```bash
npx switchyard db:rollback blog
```
...
For example, if you delete the migration of the Blog Module, then try to create a new one, Switchyard will create a brand new migration that re-creates the tables or indices. If those are still in the database, you might encounter errors.
...
To learn more about the Switchyard CLI's database commands, refer to [this CLI reference](!resources!/switchyard-cli/commands/db).
...
In those scenarios, you can instead create a data migration script. They are asynchronous function that the Switchyard application executes once when you run the `npx switchyard db:migrate command`.
...
export default async function migrateBlogData({ container }: ExecArgs) {
  ...
  if (!SwitchyardModule.isInstalled(BLOG_MODULE)) {
    return
  }
...
In the above example, you default export an asynchronous function that receives an object parameter with the [Switchyard Container](../../medusa-container/page.mdx) property.
...
Switchyard will run any pending migrations and migration scripts, including your script.

If the script runs successfully, Switchyard won't run the script again.

If there are errors in the script, you'll receive an error in the migration script logs. Switchyard will keep running the script every time you run the migration command until it runs successfully.
