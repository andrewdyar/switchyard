...
Locks are particularly useful in workflows where concurrent executions may lead to commerce risks. For example, Switchyard uses locks in its cart workflows to prevent issues like overselling products or double charging customers.
...
Switchyard provides two steps that you can use to create locks in your workflows:
...
For example, consider a custom workflow that executes Switchyard's [completeCartWorkflow](!resources!/references/medusa-workflows/completeCartWorkflow). The `completeCartWorkflow` acquires locks on the cart to prevent race conditions.
