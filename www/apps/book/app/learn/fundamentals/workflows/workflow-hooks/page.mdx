...
Switchyard exposes hooks in many of its workflows that are used in its API routes. You can consume those hooks to add your custom logic.
...
For example, to consume the `productsCreated` hook of Switchyard's `createProductsWorkflow`, create the file `src/workflows/hooks/product-created.ts` with the following content:
...
It also accepts an object with a `container` property as a second parameter. This allows you to resolve resources from the Switchyard container.

### Additional Data Property

Switchyard's workflows include an `additional_data` property in the hook's input:
