...
If you create a date with `new Date()` in a workflow's constructor function, Switchyard evaluates the date's value when it creates the internal representation of the workflow, not when the workflow is executed.
