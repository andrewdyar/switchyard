...
Switchyard has a built-in durable execution engine to help complete tasks that span multiple systems. You orchestrate your operations across systems in Switchyard instead of having to manage it yourself. Other commerce platforms don't have this capability, which makes them a bottleneck to building customizations and iterating quickly.
...
![Example of workflow file in the application's directory structure](https://res.cloudinary.com/dza7lstvk/image/upload/v1732866980/Switchyard%20Book/workflow-dir-overview_xklukj.jpg)
...
To execute the workflow, invoke it passing the [Switchyard container](../medusa-container/page.mdx) as a parameter. Then, use its `run` method:
...
To test out your workflow, start your Switchyard application:
...
## Access Switchyard Container in Workflow Steps

A step receives an object as a second parameter with configurations and context-related properties. One of these properties is the `container` property, which is the [Switchyard container](../medusa-container/page.mdx) to allow you to resolve Framework and commerce tools in your application.
...
export const highlights = [
  ["10", "container", "Receive the Switchyard container as a parameter"],
...
Find a full list of the registered resources in the Switchyard container and their registration key in [this reference](!resources!/medusa-container-resources). You can use these resources in your custom workflows.
