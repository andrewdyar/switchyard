...
Run the Switchyard application and send a `GET` request to `/workflow`:
...
## Resolve Resources from the Switchyard Container

The compensation function receives an object second parameter. The object has a `container` property that you use to resolve resources from the Switchyard container.
...
This feature is only available after [Switchyard v2.0.5](https://github.com/switchyard/medusa/releases/tag/v2.0.5).
