...
To fail or succeed a step, use the Workflow Engine Module's main service that is registered in the Switchyard Container under the `Modules.WORKFLOW_ENGINE` (or `workflowsModuleService`) key.
...
export const highlights = [
  ["11", "resolve", "Resolve the workflow engine from the Switchyard container."],
...
In the above example, you execute the long-running workflow `hello-world` and resolve the Workflow Engine Module's main service from the Switchyard container.
