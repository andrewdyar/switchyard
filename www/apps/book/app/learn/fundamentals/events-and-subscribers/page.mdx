...
In this chapter, you’ll learn about Switchyard's event system, and how to handle events with subscribers.

## Handle Core Commerce Flows with Events

When building commerce digital applications, you'll often need to perform an action after a commerce operation is performed. For example, sending an order confirmation email when the customer places an order, or syncing data that's updated in Switchyard to a third-party system.

Switchyard emits events when core commerce features are performed, and you can listen to and handle these events in asynchronous functions. You can think of Switchyard's events like you'd think about webhooks in other commerce platforms, but instead of having to setup separate applications to handle webhooks, your efforts only go into writing the logic right in your Switchyard codebase.
...
![A diagram showcasing an example of how an event is emitted when an order is placed.](https://res.cloudinary.com/dza7lstvk/image/upload/v1732277948/Switchyard%20Book/order-placed-event-example_e4e4kw.jpg)
...
![Example of subscriber file in the application's directory structure](https://res.cloudinary.com/dza7lstvk/image/upload/v1732866244/Switchyard%20Book/subscriber-dir-overview_pusyeu.jpg)
...
- `container`: The [Switchyard container](../medusa-container/page.mdx) that you can use to resolve registered resources.
...
To test the subscriber, start the Switchyard application:

```bash npm2yarn
npm run dev
```

Then, try placing an order either using Switchyard's API routes or the [Next.js Starter Storefront](!resources!/nextjs-starter). You'll see the following message in the terminal:
...
The subscription and emitting of events is handled by an Event Module, an Infrastructure Module that implements the pub/sub functionalities of Switchyard's event system.

Switchyard provides two Event Modules out of the box:
...
- [Redis Event Module](!resources!/infrastructure-modules/event/redis), which is useful in production. It uses [Redis](https://redis.io/) to implement Switchyard's pub/sub events system.

Switchyard's [architecture](../../introduction/architecture/page.mdx) also allows you to build a custom Event Module that uses a different service or logic to implement the pub/sub system. Learn how to build an Event Module in [this guide](!resources!/infrastructure-modules/event/create).
