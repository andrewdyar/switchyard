...
There are two types of events in Switchyard:

1. Workflow event: an event that's emitted in a workflow after a commerce feature is performed. For example, Switchyard emits the `order.placed` event after a cart is completed.
...
To emit a workflow event, use the `emitEventStep` helper step provided in the `@switchyard/medusa/core-flows` package. It allows you to emit an event from within a workflow, and it only emits the event after the workflow has finished successfully.
...
3. By default, the Event Module's service isn't injected into your module's container. To add it to the container, pass it in the module's registration object in `switchyard.config.ts` in the `dependencies` property:
...
```ts title="switchyard.config.ts" highlights={depsHighlight}
