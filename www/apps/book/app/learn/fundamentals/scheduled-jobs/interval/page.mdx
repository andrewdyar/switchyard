...
A scheduled job interval is a specific duration that must pass between the execution of scheduled jobs. The timer starts when the Switchyard application starts, and it resets every time a job is executed.
...
So, the above scheduled job will run every minute, starting one minute after the Switchyard application starts.
