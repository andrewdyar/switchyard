...
Switchyard removes this overhead by supporting this feature natively with scheduled jobs. A scheduled job is an asynchronous function that the Switchyard application runs at the interval you specify during the Switchyard application's runtime. Your efforts are only spent on implementing the functionality performed by the job, such as syncing products to an ERP.

<Note title="Don't use scheduled jobs if" type="error">

- You want the action to execute at a specified schedule while the Switchyard application **isn't** running. Instead, create a [custom CLI script](../custom-cli-scripts/page.mdx) and execute it using the operating system's equivalent of a cron job.
...
![Example of scheduled job file in the application's directory structure](https://res.cloudinary.com/dza7lstvk/image/upload/v1732866423/Switchyard%20Book/scheduled-job-dir-overview_ediqgm.jpg)

export const highlights = [
  ["3", "greetingJob", "The scheduled job function to execute at a specified interval."]
  ["3", "container", "Receive the Switchyard container as a parameter"],
...
You export an asynchronous function that receives the [Switchyard container](../medusa-container/page.mdx) as a parameter. In the function, you resolve the [Logger utility](../../debugging-and-testing/logging/page.mdx) from the Switchyard container and log a message.
...
To test out your scheduled job, start the Switchyard application:
...
The next time you start the Switchyard application, it will run this job every day at midnight.
