...
export const metadata = {
  title: `Manage Order Details in Switchyard Admin`,
}
...
If Switchyard manages the inventory of an ordered item, it reserves or allocates the ordered quantity from the inventory. This ensures that the item's available quantity is reduced by the ordered quantity.

Switchyard allocates the ordered quantity from the stock location of the chosen shipping option in the order. For example, if the customer chooses a shipping option that uses a stock location in the US, Switchyard allocates the ordered quantity from that stock location.
