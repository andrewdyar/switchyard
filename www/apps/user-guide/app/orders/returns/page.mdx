...
export const metadata = {
  title: `Manage Order Returns in Switchyard Admin`,
}
...
Switchyard allows you to automate the Return Merchandise Authorization (RMA) process by supporting two ways of creating returns:

1. The customer requests a return for an order through the storefront, which you can [view](#find-return-requests-of-an-order) in the Switchyard Admin dashboard;
...
You can create a return from the Switchyard Admin for [delivered items](../fulfillments/page.mdx#mark-fulfillment-as-delivered) in an order.
