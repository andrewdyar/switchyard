...
export const metadata = {
  title: `Manage Order Fulfillments in Switchyard Admin`,
}
...
5. In the form that opens:
    ...
    - For "Shipping method", choose the shipping method to fulfill the items with. By default, the shipping method is set to that chosen by the customer during checkout.
        ...
        - If you choose a shipping method with a different price, Switchyard doesn't change the shipping total of the order.
    - Under Items to fulfill, enter the quantity to fulfill for each item. This is also the quantity subtracted from the stock in the chosen location if Switchyard manages the variant's inventory.
