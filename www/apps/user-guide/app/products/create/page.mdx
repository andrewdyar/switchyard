...
import { DotsSix, EllipsisHorizontal, XMark, TaxExclusive, TaxInclusive } from "@switchyard/icons"

export const metadata = {
  title: `Create Product in Switchyard Admin`,
}

# {metadata.title}

In this guide, you’ll learn how to create a new product using the Switchyard Admin.

## Types of Products to Create

You can create different types of products in Switchyard:
...
3. In the column "Managed Inventory", you can enable inventory management for the variant. If disabled, the variant is always considered in stock. Otherwise, you can set the inventory quantity in different stock locations, and Switchyard will manage that inventory, such as decrementing it when the variant is purchased.
...
6. The next columns are price columns for all the currencies and regions you have set up in Switchyard. You can set the price for each variant in each currency and region. You can also add the prices after creating the product.

<Note title="Tips">

- Price columns that have the <InlineIcon Icon={TaxExclusive} alt="tax-exclusive" /> icon are tax-exclusive prices. Switchyard will calculate taxes during checkout and add them to this price.
- Price columns that have the <InlineIcon Icon={TaxInclusive} alt="tax-inclusive" /> icon are tax-inclusive prices. Switchyard will calculate taxes during checkout as part of this price.
