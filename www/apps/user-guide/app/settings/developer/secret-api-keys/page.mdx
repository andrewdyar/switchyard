...
export const metadata = {
  title: `Manage Secret API Keys in Switchyard Admin`,
}

# {metadata.title}

In this guide, you'll learn how to create and manage secret API keys in the Switchyard Admin.

<Note>

This guide is mainly useful for developers and technical teams building customizations for a Switchyard application.

</Note>

## What is a Secret API Key?

A secret API key is an authentication token that allows you to access Switchyard's Admin APIs. You can pass it in request headers to send requests as an authenticated admin user. So, it's important to not expose the secret API key in client-side code or public repositories.
...
To view the secret API keys of the currently logged-in user in the Switchyard Admin, go to Settings -> Secret API Keys.
...
Prior to [Switchyard v2.11.0], users could only have one active secret API key at a time. To create a new secret API key, you had to [revoke](#revoke-secret-api-key) the existing key first. If you can't create multiple secret API keys, request your technical team to [upgrade your Switchyard application](!docs!/learn/update).
