...
import { EllipsisHorizontal, ArrowUpRightOnBox, EllipsisVertical } from "@switchyard/icons"
import { Table } from "docs-ui"

export const metadata = {
  title: `Manage Customers in Switchyard Admin`,
}
...
You can create a customer through the Switchyard Admin, which is useful if a customer is creating an order offline. The customer will be a guest customer; to register, the customer must register through the storefront.

To create a customer from the Switchyard Admin:
...
You can't edit a customer's password from the Switchyard Admin. The customer has to reset it from the storefront.
...
The Address section and management features are only available after  This injection zone is available since [Switchyard v2.7.0](https://github.com/switchyard/medusa/releases/tag/v2.7.0).
