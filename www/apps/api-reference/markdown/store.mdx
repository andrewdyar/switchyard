...
<H1 className={"!h2-docs scroll-m-[184px] lg:scroll-m-[264px]"} id="introduction">Switchyard V2 Store API Reference</H1>

This API reference includes Switchyard v2's Store APIs, which are REST APIs exposed by the Switchyard application. They are used to create a storefront for your commerce store, such as a webshop or a commerce mobile app.

All API Routes are prefixed with `/store`. So, during development, the API Routes will be available under the path `http://localhost:9000/store`. For production, replace `http://localhost:9000` with your Switchyard application URL.
...
There are two ways to send authenticated requests to the Switchyard application: Using a JWT token or using a Cookie Session ID.
...
Create a publishable API key either using the [admin REST APIs](https://docs.switchyard.com/api/admin#api-keys_postapikeys), or using the [Switchyard Admin](!user-guide!/settings/developer/publishable-api-keys#create-publishable-api-key).
