...
<H1 className={"!h2-docs scroll-m-[184px] lg:scroll-m-[264px]"} id="introduction">Switchyard V2 Admin API Reference</H1>

This API reference includes Switchyard v2's Admin APIs, which are REST APIs exposed by the Switchyard application. They are used to perform admin functionalities or create an admin dashboard to access and manipulate your commerce store's data.

All API Routes are prefixed with `/admin`. So, during development, the API Routes will be available under the path `http://localhost:9000/admin`. For production, replace `http://localhost:9000` with your Switchyard application URL.
...
There are three ways to send authenticated requests to the Switchyard server: Using a JWT token in a bearer authorization header, using an admin user's API token, or using a cookie session ID.
...
Create the API key token either from the [Switchyard Admin](!user-guide!/settings/developer/secret-api-keys) or using the [Create API Key API Route](#api-keys_postapikeys).
...
Previous versions of Switchyard required passing the API key token as base64. This is still supported for backwards compatibility, but you can pass the API token without encoding it as well.
...
If you've enabled HTTP Compression in your Switchyard configurations, and you 
...
Many data models in Switchyard, such as products and carts, have a `metadata` field that allows you to store custom information in key-value pairs.

When setting or updating the `metadata` field using the relevant API routes, Switchyard will merge the new metadata with the existing metadata.
...
Switchyard doesn't merge nested objects, so if you pass a partial object, the existing properties in the nested object will be removed.
...
However, some API routes restrict the fields and relations you can retrieve. To learn about those API routes and how to override the allowed fields and relations, refer to the [Retrieve Custom Linked Data Models from Switchyard's API routes](!docs!/learn/fundamentals/api-routes/retrieve-custom-links) documentation.
