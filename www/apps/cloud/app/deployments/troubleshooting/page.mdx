...
In this guide, you'll find solutions to common issues that may arise when deploying your Switchyard applications on Cloud.

## Files Not Found After Deployment

Before deploying your Switchyard application on Cloud, Switchyard runs the `build` script defined in your `package.json` file, then copies only the output of the [build process](!docs!/learn/build), which is the `.medusa/server` directory.

The `.medusa/server` directory holds the compiled JavaScript files of your project, the production build of the admin dashboard, and other necessary files to run your Switchyard application in production.
...
- [Refer to Switchyard's troubleshooting guides](!resources!/troubleshooting).
