...
import { ChevronUpDown } from "@switchyard/icons"
...
A project is the collection of resources, environments, deployments, and settings related to a Switchyard application deployed to the Cloud. The project is linked to a GitHub repository that hosts the Switchyard application's code.

To deploy a Switchyard application to the Cloud, you create a project for it. Switchyard will automatically set up and configure the necessary resources in the project to run the application, such as PostgreSQL, Redis, and S3.
...
In this section, you'll learn how to create a project in Cloud to deploy your Switchyard application.
...
Switchyard provides you with the following starters that you can use to quickly set up your ecommerce application:

- **DTC Starter**: Standard Switchyard application with fully-fledged commerce features.
- **B2B Starter**: Switchyard application with powerful B2B and commerce features.
...
If you already have a Switchyard application, you can create a project from it.

### Prerequisite: Switchyard Application Configurations

Your existing Switchyard application doesn't need specific configurations to be deployed to Cloud. Switchyard will automatically:

- Create the necessary [server and worker instances](!docs!/learn/production/worker-mode).
- Scale your Switchyard application's resources based on the traffic it receives.
- Set up and configure production resources and modules for your Switchyard application:
...
So, make sure to remove any of these modules from your `switchyard.config.ts` file, unless you want to use custom options for them. In that case, you're expected to manually set up and manage those resources externally and configure them in your Switchyard application.

<Note>

The Caching Module was introduced in [Switchyard v2.11.0](https://github.com/switchyard/medusa/releases/tag/v2.11.0) to replace the deprecated Cache Module. If you're still using the Cache Module, make sure to remove it from your `switchyard.config.ts` file as well.

</Note>

### Project Creation Steps

To create a project from an existing Switchyard application:
...
2. Under the Repository section, choose the repository to create the project from.
    - If you don't see your repository, you can click the **Configure repositories** link to manage the GitHub repositories that Switchyard can access.
...
2. **Project subdomain**: Enter a custom subdomain for the project. All projects are subdomains of `switchyard.app`.
    - For example, if you enter `my-project`, the project will be accessible at `my-project.switchyard.app`. Refer to the [subdomain restrictions](#subdomain-restrictions-for-projects) section for more details.
3. You can expand the "Build details" section to optionally change its configurations:
    ...
    - **Project root directory**: If your project is in a monorepo, specify the path to the Switchyard project in the repository. Otherwise, leave it empty.
...
5. You can expand the "Initial user" section to optionally set up the credentials of the initial admin user for the Switchyard Admin:
...
Your project's URL must be a subdomain of `switchyard.app`. For example, if you choose `my-store` as your subdomain, your project's URL will be `my-store.switchyard.app`.
...
Switchyard supports hosting your Switchyard projects on Cloud in the following regions:

- US East (`us-east-1`)
- Europe Central (`eu-central-1`)
- Asia Southeast (`ap-southeast-1`)

All your project's deployments will be in the selected region. For better performance, choose a region that's closer to your target users. If you need to host your Switchyard project in a different region, contact support.
...
2. **Environments**: The environments for the project are shown as cards. By default, you'll find Production and Previews environments. You can learn more about environments in the [Environments](../environments/page.mdx) guide.
    - The Production environment is the live environment where your Switchyard application is deployed and clients connect to.
...
3. **Production URL**: The URL of the project's production deployment. You'll find it in the "Production" card under the title. Clicking it will open the production Switchyard Admin in a new tab.
