...
import { ArrowDownTray, ArrowPath, Brackets } from "@switchyard/icons"
...
Logs are output messages from your Switchyard application's build and runtime processes for a Cloud environment's deployments.
...
- **Runtime logs**: Messages logged by the Switchyard application when it starts and while it's running, such as informational startup messages, HTTP requests, errors, and more. These logs are useful for debugging the application's deployment and runtime errors.
