...
In this guide, you'll learn about the managed database service that Switchyard provides as part of your project environments on Cloud.

## Managed Database Service on Cloud

Switchyard provisions and manages databases for each of your environments on Cloud.
...
Switchyard automatically configures the database for your environments on Cloud. So, you don't need to worry about setting up the database, configuring it with your Switchyard application, or monitoring performance and availability on your end.

---

## Cloud Database Backups

Switchyard provides automatic backups for your environment databases on Cloud. These backups are retained for 14 days and allow you to restore your database to a previous state, if needed.
...
Switchyard gives you read access to your Cloud project's database, which is useful when you need to export data, perform analytics, or troubleshoot issues.
...
Switchyard allows you to export and import database dumps for any environment on Cloud. This is useful for seeding the database with initial data, migrating from other hosting platforms, or debugging issues locally.
...
By default, when Switchyard creates a [preview environment](../environments/preview/page.mdx) on Cloud, it replicates the Production database. This allows you to test changes in a safe environment that mirrors production, without affecting the live data.
...
Switchyard automatically runs database migrations for every deployment on Cloud. This ensures that your database schema is always up-to-date with the latest changes in your Switchyard application.

When you deploy changes to an environment on Cloud, Switchyard:
...
So, if you define new database migrations in your module, define new links, or add data migration scripts, Switchyard will apply those changes automatically when you deploy the changes to Cloud.