...
import { EllipsisHorizontal, ChevronUpDown } from "@switchyard/icons"
...
Production and long-lived environments are stable and scalable, making them ideal for hosting your Switchyard applications. Switchyard ensures that they're:

- **Always available with zero-downtime deployments**: Your application will always be available for users, even during deployments. Switchyard automatically handles rolling updates without interrupting your users.
- **Automatically scaled to meet peak traffic**: Switchyard automatically scales your servers to handle traffic spikes, such as during flash sales or marketing campaigns. Your application can handle sudden increases in traffic without manual intervention.
...
2. In the side window that opens:
    ...
    - Set the custom subdomain for the environment. All environments are subdomains of `switchyard.app`. So, if you set the subdomain to `staging-my-project`, the environment's URL will be `staging-my-project.switchyard.app`.
...
After creating the environment, Switchyard will automatically trigger a deployment for the environment using the associated branch (for example, `staging`). This includes:
...
After that, Switchyard will automatically deploy the environment whenever you push a new commit to the associated branch.

### Create Switchyard Admin User in Environment

After the environment is deployed, you need to set the email and password for the environment's Switchyard Admin user.
