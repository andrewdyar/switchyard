...
- Switchyard automatically scales preview environments to zero after ten minutes of inactivity. So, if no one is using the preview environment, it will automatically shut down to save resources.
...
Switchyard will automatically create a preview environment for the PR and deploy the changes to that environment. Everytime you push a new commit to the PR, Switchyard will redeploy the preview environment with the latest changes.
...
Once the preview environment has finished deploying, you can access its Switchyard Admin either through the comment in the PR or by clicking the "Preview" button in the [project's dashboard](../../projects/page.mdx#open-project-dashboard).
...
This will open the preview environment's Switchyard Admin in a new tab, where you can test the changes made in the PR.

To log into the Switchyard Admin of the preview environment, use the admin credentials of the Production environment. If you [configured the shared previews settings](#manage-shared-previews-settings) to use a different environment's database, you can log in with the admin credentials of that environment instead.

You can also [send requests to the preview deployment's API](../../deployments/page.mdx#send-requests-to-the-deployment).

### Preview URL Format

Switchyard generates the preview environment URL by:

- Taking the first `42` characters of the PR's branch name.
- Appending a dash (`-`) followed by the project's handle.
    - You can find the project's handle in the project dashboard's URL, which is of the format `cloud.switchyard.com/:org_id/projects/:project_handle`.
- Removing unsafe characters.
- Adding the `switchyard.app` subdomain.

For example, if your PR's branch name is `feat/product-review` and the project's handle is `m1234`, the URL of the deployed preview environment would be `https://featproduct-review-m1234.switchyard.app`.
