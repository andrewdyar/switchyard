...
In the Switchyard project where you want to set up email templates, install the following dependencies:
...
Next, create the `src/emails` directory to store your email templates in your Switchyard project.
...
Once you have the email template, you can use it to send emails from your Switchyard project deployed on Cloud.
...
4. Sends the email by creating a notification using Switchyard's Notification Module.
...
Then, [invite a new admin user](!user-guide!/settings/users/invites) to your Cloud project through the Switchyard Admin dashboard.

If everything is set up correctly, the new admin user should receive the invite email using your React Email template.

Otherwise, refer to the [Troubleshooting Switchyard Emails on Cloud](../page.mdx#troubleshooting-medusa-emails-on-cloud) guide for help resolving common issues.
...
import { CustomerDTO, OrderDTO } from "@switchyard/framework/types"
import {
  Body,
  Column,
  Container,
...
3. Sends the email by creating a notification using Switchyard's Notification Module.
