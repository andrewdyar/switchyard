#!/usr/bin/env python3
"""
Process the full category dataset from Supabase and update the documentation.
This script processes all level 3 and 4 categories and generates the complete markdown tree.
"""
import json
import re
...
def update_documentation(data, doc_path):
    ...
    print(f"Total level 3 categories: {sum(len(l3_dict) for l2_dict in tree.values() for l3_dict in l2_dict.values())}")
    print(f"Total level 4 categories: {sum(len(l4_list) for l2_dict in tree.values() for l3_dict in l2_dict.values() for l4_list in l3_dict.values() if isinstance(l4_list, list))}")
